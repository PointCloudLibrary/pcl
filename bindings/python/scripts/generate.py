...
class bind:
    ...
    def handle_final(self, filename):
        """
        ...
        for i in range(len(self._linelist)):
            if self._linelist[i].startswith("namespace"):
                ...
                self._linelist[i] = "".join(
                    (f"PYBIND11_MODULE({self._module_name}, m)", "{", self._linelist[i])
                )
                break
...
def generate(source, module_name):
    ...
    if header_info:
        ...
        lines_to_write = bind_object.handle_final(filename=f"{filename}")
...
def main():
    ...
    for source in args.files:
        ...
        lines_to_write = generate(source, module_name="pcl")
