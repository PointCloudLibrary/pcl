...
std::mutex depth_image_mutex, ir_image_mutex, image_mutex;
...
bool received_new_depth_data = false, received_new_ir_image = false,
...
{
  std::cout << "\n\nUsage: " << progName
            << " [options] [scene.pcd] <model.pcl> [model_2.pcl] ... [model_n.pcl]\n\n"
            << "Options:\n"
            << "-------------------------------------------\n"
            ...
            << "-s <float>      support size for the interest points (diameter of the "
               "used sphere in meters)"
            << " (default " << support_size << ")\n"
            << "-i <float>      minimum interest value (0-1) (default "
            << min_interest_value << ")"
            << "-t <int>        maximum number of threads to use (default "
            << max_no_of_threads << ")\n"
            << "-h              this help\n"
            << "\n\n";
}
...
main (int argc, char** argv)
{
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr keypoints_cloud_ptr(
  ...
  while (!viewer.wasStopped()) {
    ...
    if (received_new_depth_data && depth_image_mutex.try_lock()) {
      ...
      float focal_length_x = depth_image_ptr->getFocalLength(),
    ...
    pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ>
        color_handler_keypoints(keypoints_cloud_ptr, 0, 255, 0);
    if (!viewer.updatePointCloud(
            keypoints_cloud_ptr, color_handler_keypoints, "keypoints"))
