...
public:
  ...
  populateDatabase (const std::vector<std::string>& filenames)
  {
    ...
    for (std::size_t i = 0; i < n; ++i) {
      ...
      if (filename.compare (filename.size() - 4, 4, ".pcd") == 0) {
        ...
        models_[i].local_descriptors =
            loadPointCloud<LocalDescriptorT> (filename, "_localdesc.pcd");
        models_[i].global_descriptor =
            loadPointCloud<GlobalDescriptorT> (filename, "_globaldesc.pcd");
      }
      *descriptors_ += *(models_[i].global_descriptor);
    }
    kdtree_ = pcl::KdTreeFLANN<GlobalDescriptorT>::Ptr (
        new pcl::KdTreeFLANN<GlobalDescriptorT>);
  ...
  recognizeAndAlignPoints (const PointCloudPtr& query_cloud)
  {
    ...
    PointCloudPtr output =
        alignModelPoints (models_[best_match], query_object, params_);
    return (output);
  }
