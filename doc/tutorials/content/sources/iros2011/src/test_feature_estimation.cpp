...
main (int argc, char** argv)
{
  if (argc < 2) {
    ...
    pcl::console::print_info ("    -n radius  ...................................... "
                              "Estimate surface normals\n");
    pcl::console::print_info (
        "    -k min_scale,nr_octaves,nr_scales,min_contrast... Detect keypoints\n");
    pcl::console::print_info ("    -l radius ....................................... "
                              "Compute local descriptors\n");
    pcl::console::print_info (
        "    -s output_name (without .pcd extension).......... Save outputs\n");
    pcl::console::print_info ("Note: \n");
    pcl::console::print_info (
        "  Each of the first four options depends on the options above it.\n");
    pcl::console::print_info ("  Saving (-s) will output individual files for each "
                              "option used (-n,-k,-f,-g).\n");
    return (1);
  }
  ...
  bool estimate_surface_normals =
      pcl::console::parse_argument (argc, argv, "-n", surface_radius) > 0;
  ...
  bool detect_keypoints =
      pcl::console::parse_argument (argc, argv, "-k", params_string) > 0;
  if (detect_keypoints) {
    assert (normals);
    ...
    keypoints = detectKeypoints (
        cloud, normals, min_scale, nr_octaves, nr_scales, min_contrast);
  ...
  bool compute_local_descriptors =
      pcl::console::parse_argument (argc, argv, "-l", feature_radius) > 0;
  if (compute_local_descriptors) {
    assert (normals && keypoints);
    local_descriptors =
        computeLocalDescriptors (cloud, normals, keypoints, feature_radius);
  ...
  // Or visualize the result
    ...
    if (keypoints) {
      ...
      vis.setPointCloudRenderingProperties (
          pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "keypoints");
    }
    if (global_descriptor) {
