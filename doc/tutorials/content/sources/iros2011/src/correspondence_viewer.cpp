                              ...
                              std::vector<float>& correspondence_scores_out)
{
  ...
  for (std::size_t i = 0; i < source_descriptors->size(); ++i) {
    descriptor_kdtree.nearestKSearch (
        *source_descriptors, i, k, k_indices, k_squared_distances);
...
void
visualize_correspondences (const PointCloudPtr points1,
                           ...
                           std::size_t max_to_display)
{
  // We want to visualize two clouds side-by-side, so do to this, we'll make copies of
  // the clouds and transform them by shifting one to the left and the other to the
  // right.  Then we'll draw lines between the corresponding points
...
main (int argc, char** argv)
{
  if (argc < 3) {
    ...
    pcl::console::print_info ("    -n nr_correspondences ... The number of "
                              "correspondence to draw (Default = all)\n");
    pcl::console::print_info ("Note: The inputs (source and target) must be specified "
  ...
  find_feature_correspondences (
      source_descriptors, target_descriptors, correspondences, correspondence_scores);

  visualize_correspondences (source_points,
                             source_keypoints,
                             target_points,
                             target_keypoints,
                             correspondences,
