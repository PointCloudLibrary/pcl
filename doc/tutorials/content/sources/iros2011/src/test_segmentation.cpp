...
main (int argc, char** argv)
{
  if (argc < 2) {
    ...
    pcl::console::print_info (
        "    -p dist_threshold,max_iters  ..... Subtract the dominant plane\n");
    pcl::console::print_info (
        "    -c tolerance,min_size,max_size ... Cluster points\n");
    pcl::console::print_info (
        "    -s output.pcd .................... Save the largest cluster\n");
    return (1);
  }
  ...
  bool subtract_plane = pcl::console::parse_2x_arguments (
                            argc, argv, "-p", dist_threshold, max_iters) > 0;
  if (subtract_plane) {
  ...
  bool cluster_points = pcl::console::parse_3x_arguments (
                            argc, argv, "-c", tolerance, min_size, max_size) > 0;
  if (cluster_points) {
  ...
  // Or visualize the result
    ...
    if (cluster_points) {
      for (std::size_t i = 0; i < cluster_indices.size(); ++i) {
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr cluster_i (
        ...
        // Add the i_th cluster to the visualizer with a random color and a unique
        // identifier
