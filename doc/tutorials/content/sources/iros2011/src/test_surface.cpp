...
main (int argc, char** argv)
{
  if (argc < 2) {
    ...
    pcl::console::print_info(
        "    --surfel radius,order............... Compute surface elements\n");
    pcl::console::print_info(
        "    --convex ........................... Compute convex hull\n");
    pcl::console::print_info(
        "    --concave alpha .................... Compute concave hull\n");
    pcl::console::print_info(
        "    --greedy radius,max_nn,mu,surf_angle,min_angle,max_angle ... Compute "
        "greedy triangulation\n");
    pcl::console::print_info(
        "    -s output.pcd ...................... Save the output cloud\n");
  ...
  bool compute_surface_elements =
      pcl::console::parse_2x_arguments(
          argc, argv, "--surfel", mls_radius, polynomial_order) > 0;
  ...
  double alpha;
  bool compute_concave_hull =
      pcl::console::parse_argument(argc, argv, "--concave", alpha) > 0;
  ...
  bool perform_greedy_triangulation =
      pcl::console::parse_argument(argc, argv, "--greedy", params_string) > 0;
  if (perform_greedy_triangulation) {
    assert(surfels);

    std::vector<std::string> tokens;
    boost::split(
        tokens, params_string, boost::is_any_of(","), boost::token_compress_on);
    ...
    greedy_mesh = greedyTriangulation(surfels,
                                      radius,
  ...
  bool perform_marching_cubes =
      pcl::console::parse_2x_arguments(argc, argv, "--mc", leaf_size, iso_level) > 0;
  ...
  bool save_output =
      pcl::console::parse_argument(argc, argv, "-s", output_filename) > 0;
  if (save_output) {
    // Save the result
  ...
  // Or visualize the result
    ...
    if (compute_convex_hull) {
      vis.addPolygonMesh<PointT>(
          convex_hull->points, convex_hull->faces, "convex_hull");
    }
    if (compute_concave_hull) {
      vis.addPolygonMesh<PointT>(
          concave_hull->points, concave_hull->faces, "concave_hull");
    }
    if (perform_greedy_triangulation) {
