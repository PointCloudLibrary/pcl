...
showHelp (char* filename)
{
  ...
  std::cout
      ...
      << std::endl;
  std::cout << "                             each radius given by that value."
            << std::endl;
  ...
  std::cout << "     --cg_thresh val:        Clustering threshold (default 5)"
            << std::endl
            << std::endl;
}
...
main (int argc, char* argv[])
{
  ...
  for (std::size_t i = 0; i < scene_descriptors->size(); ++i) {
    ...
    if (found_neighs == 1 &&
        neigh_sqr_dists[0] <
            0.25f) //  add match only if the squared descriptor distance is less than
  ...
  if (show_correspondences_ || show_keypoints_) {
    ...
    pcl::visualization::PointCloudColorHandlerCustom<PointType>
        off_scene_model_color_handler (off_scene_model, 255, 255, 128);
  ...
  if (show_keypoints_) {
    pcl::visualization::PointCloudColorHandlerCustom<PointType>
        scene_keypoints_color_handler (scene_keypoints, 0, 0, 255);
    viewer.addPointCloud (
  ...
  for (std::size_t i = 0; i < rototranslations.size(); ++i) {
    ...
    if (show_correspondences_) {
      for (std::size_t j = 0; j < clustered_corrs[i].size(); ++j) {
        ...
        PointType& model_point =
            off_scene_model_keypoints->at (clustered_corrs[i][j].index_query);
        PointType& scene_point =
            scene_keypoints->at (clustered_corrs[i][j].index_match);

        //  We are drawing a line for each pair of clustered correspondences found
        //  between the model and the scene
        viewer.addLine<PointType, PointType> (
            model_point, scene_point, 0, 255, 0, ss_line.str());
      }
    }
  }
