...
{
  if (argc < 5 ||
      argc % 2 == 0) // needs at least one training cloud with class id, plus testing
                     // cloud with class id (plus name of executable)
    return (-1);
  ...
  for (unsigned int i_cloud = 0; i_cloud < number_of_training_clouds - 1; i_cloud++) {
    ...
    unsigned int tr_class =
        static_cast<unsigned int> (strtol (argv[i_cloud * 2 + 2], 0, 10));
  ...
  unsigned int testing_class =
      static_cast<unsigned int> (strtol (argv[argc - 1], 0, 10));
