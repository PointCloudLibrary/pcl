...
main (int argc, char** argv)
{
  ...
  viewer->setPointCloudRenderingProperties (
      pcl::visualization::PCL_VISUALIZER_OPACITY, 0.95, "voxel centroids");

  PointLCloudT::Ptr labeled_voxel_cloud = super.getLabeledVoxelCloud();
  viewer->addPointCloud (labeled_voxel_cloud, "labeled voxels");
  viewer->setPointCloudRenderingProperties (
      pcl::visualization::PCL_VISUALIZER_OPACITY, 0.8, "labeled voxels");

  PointNCloudT::Ptr sv_normal_cloud =
      super.makeSupervoxelNormalCloud (supervoxel_clusters);
  // We have this disabled so graph is easy to see, uncomment to see supervoxel normals
  // viewer->addPointCloudNormals<PointNormal> (sv_normal_cloud,1,0.05f,
  // "supervoxel_normals");
  ...
  for (auto label_itr = supervoxel_adjacency.cbegin();
    ...
    // Now we need to iterate through the adjacent supervoxels and make a point cloud of
    // them
    PointCloudT adjacent_supervoxel_centers;
                                  ...
                                  pcl::visualization::PCLVisualizer::Ptr& viewer)
{
  ...
  for (auto adjacent_itr = adjacent_supervoxel_centers.begin();
       adjacent_itr != adjacent_supervoxel_centers.end();
       ++adjacent_itr) {
