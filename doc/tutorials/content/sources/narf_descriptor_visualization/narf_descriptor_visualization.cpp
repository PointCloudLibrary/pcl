...
printUsage (const char* progName)
{
  std::cout << "\n\nUsage: " << progName << " [options] <scene.pcd>\n\n"
            ...
            << "-c <int>     coordinate frame of the input point cloud (default "
...
main (int argc, char** argv)
{
  ...
  if (pcl::console::parse (argc, argv, "-o", rotation_invariant) >= 0)
    std::cout << "Switching rotation invariant feature version "
  ...
  for (unsigned int y = 0; y < range_image.height; ++y) {
    for (unsigned int x = 0; x < range_image.width; ++x) {
      ...
      pcl::Narf::extractFromRangeImageAndAddToList (range_image,
                                                    x,
                                                    y,
                                                    descriptor_size,
                                                    support_size,
                                                    ...
                                                    narfs[index]);
      // std::cout << "Extracted "<<narfs[index].size ()<<" features for pixel
      // "<<x<<","<<y<<".\n";
    }
  }
  std::cout << "100%\n";
  std::cout << "Done.\n\n Now you can click on points in the image to visualize how "
               "the descriptor is "
  ...
  while (true) {
    ...
    // range_image.real2DToInt2D (clicked_pixel_x_f, clicked_pixel_y_f, clicked_pixel_x,
    // clicked_pixel_y);
    ...
    pcl::Narf narf;
    if (!narf.extractFromRangeImage (range_image,
                                     clicked_pixel_x,
                                     clicked_pixel_y,
                                     descriptor_size,
                                     support_size)) {
    ...
    surface_patch_widget.showFloatImage (narf.getSurfacePatch(),
                                         surface_patch_pixel_size,
                                         surface_patch_pixel_size,
                                         -0.5f * surface_patch_world_size,
                                         0.5f * surface_patch_world_size,
                                         true);
    ...
    for (int descriptor_value_idx=0; descriptor_value_idx<narf.getDescriptorSize ();
    ++descriptor_value_idx)
    {
      float angle = descriptor_value_idx*angle_step_size + surface_patch_rotation;
      //surface_patch_widget.markLine (patch_middle, patch_middle,
    patch_middle+line_length*sinf (angle),
                                     //patch_middle+line_length*-std::cos (angle),
    ...
    {
      //surface_patch_widget.markLine (radius-0.5, radius-0.5, radius-0.5f
    + 2.0f*radius*sinf (rotations[i]),
                                                //radius-0.5f - 2.0f*radius*std::cos
    (rotations[i]), pcl::visualization::Vector3ub (255,0,0));
    }*/

    descriptor_widget.showFloatImage (
        narf.getDescriptor(), narf.getDescriptorSize(), 1, -0.1f, 0.3f, true);
    ...
    for (unsigned int point_index = 0; point_index < range_image.size();
         ++point_index) {
      ...
      for (unsigned int i = 0; i < narfs_of_selected_point.size(); ++i) {
        for (unsigned int j = 0; j < narfs_of_current_point.size(); ++j) {
          descriptor_distance =
              (std::min) (descriptor_distance,
                          narfs_of_selected_point[i]->getDescriptorDistance (
                              *narfs_of_current_point[j]));
        }
      }
    }
    descriptor_distances_widget.showFloatImage (descriptor_distance_image,
                                                range_image.width,
                                                range_image.height,
