...
main (int argc, char** argv)
{
  ...
  for (pcl::index_t i = 0; i < static_cast<pcl::index_t>(cloud->size()); ++i) {
    if (pcl::console::find_argument(argc, argv, "-s") >= 0 ||
      ...
      if (i % 5 == 0)
        ...
        (*cloud)[i].z =
            sqrt(1 - ((*cloud)[i].x * (*cloud)[i].x) - ((*cloud)[i].y * (*cloud)[i].y));
      else
  ...
  // creates the visualization object and adds either our original cloud or all of the
  // inliers depending on the command line arguments specified.
  pcl::visualization::PCLVisualizer::Ptr viewer;
  if (pcl::console::find_argument(argc, argv, "-f") >= 0 ||
      pcl::console::find_argument(argc, argv, "-sf") >= 0)
