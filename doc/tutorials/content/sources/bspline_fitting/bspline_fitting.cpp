...
void
PointCloud2Vector3d (pcl::PointCloud<Point>::Ptr cloud,
...
main (int argc, char* argv[])
{
  ...
  if (argc < 3) {
    printf ("\nUsage: pcl_example_nurbs_fitting_surface pcd<PointXYZ>-in-file "
            "3dm-out-file\n\n");
    exit (0);
  }
  pcd_file = argv[1];
  ...
  for (unsigned i = 0; i < refinement; i++) {
    ...
    fit.assemble (params);
    fit.solve();
    pcl::on_nurbs::Triangulation::convertSurface2Vertices (
        fit.m_nurbs, mesh_cloud, mesh_vertices, mesh_resolution);
  ...
  ON_NurbsCurve curve_nurbs =
      pcl::on_nurbs::FittingCurve2dAPDM::initNurbsCurve2D (order, curve_data.interior);
                ...
                pcl::visualization::PCLVisualizer& viewer)
{
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr curve_cps (
      new pcl::PointCloud<pcl::PointXYZRGB>);
