...
protected:
   ...
   */
  void
  segmentation (typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr input,
   ...
   */
  void
  detectKeypoints (typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr input,
    ...
    typename pcl::PointCloud<FeatureType>::Ptr features)
{
  ...
  // if (pcl::dynamic_pointer_cast<typename pcl::FeatureFromNormals<pcl::PointXYZRGB,
  // pcl::Normal, FeatureType> > (feature_extractor_))
  {
...
main (int argc, char** argv)
{
  if (argc < 6) {
    pcl::console::print_info (
        "Syntax is: %s <source-pcd-file> <target-pcd-file> <keypoint-method> "
        "<descriptor-type> <surface-reconstruction-method>\n",
        argv[0]);
