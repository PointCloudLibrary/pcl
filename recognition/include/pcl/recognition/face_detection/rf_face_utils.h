...
class FeatureHandlerDepthAverage
: public pcl::FeatureHandler<FT, DataSet, ExampleIndex> {
...
public:
  ...
  /** \brief Sets the number of channels a feature has (i.e. 1 - depth, 4 - depth +
   * normals) \param[in] nf Number of channels.
   */
  void
  setNumChannels (int nf)
  {
    num_channels_ = nf;
  }
   ...
   */
  /*void createRandomFeatures(const std::size_t num_of_features, std::vector<FT> &
   features)
   {
   srand (time(NULL));
   int min_s = 10;
   ...
   f.threshold_ = -range_d + (rand () / static_cast<float> (RAND_MAX)) * (range_d
   * 2.f); features.push_back (f);
   }
   }*/
   ...
   * \param[in] examples The set of examples of the data set the feature is evaluated
   * on. \param[out] results The destination for the results of the feature evaluation.
  ...
  void
  generateCodeForEvaluation (const FT& /*feature*/,
                             ::std::ostream& /*stream*/) const override
  {}
};

/** \brief Statistics estimator for regression trees which optimizes information gain
 * and pose parameters error. */
template <class LabelDataType, class NodeType, class DataSet, class ExampleIndex>
class PoseClassRegressionVarianceStatsEstimator
...
public:
  ...
  getLabelOfNode (NodeType& node) const override
  {
    return node.value;
  }
   ...
   */
  inline unsigned int
  computeMeanAndCovarianceOffset (DataSet& data_set,
                                  std::vector<ExampleIndex>& examples,
   ...
   */
  inline unsigned int
  computeMeanAndCovarianceAngles (DataSet& data_set,
                                  std::vector<ExampleIndex>& examples,
                                  Eigen::Matrix3d& covariance_matrix,
                                  Eigen::Vector3d& centroid) const
  {
   ...
   * \param[in] examples The examples which define which parts of the data set are used
   * for evaluation. \param[in] label_data The label_data corresponding to the examples.
  ...
  // param[in] node The node for which code is generated.
  void
  generateCodeForOutput (NodeType& /*node*/, std::ostream& stream) const override
  {
    stream << "ERROR: RegressionVarianceStatsEstimator does not implement "
              "generateCodeForBranchIndex(...)";
  }
