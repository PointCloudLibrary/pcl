...
private:
  ...
  std::vector<std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>>
      head_center_votes_clustered_{};
  std::vector<std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>>
      head_center_original_votes_clustered_{};
  std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>
      angle_votes_{};
  std::vector<float> uncertainties_{};
  std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>
      head_clusters_centers_{};
  std::vector<Eigen::Vector3f, Eigen::aligned_allocator<Eigen::Vector3f>>
      head_clusters_rotation_{};
...
public:
   ...
   */
  void
  setForestFilename (std::string& ff)
  {
    forest_filename_ = ff;
  }

  void
  setUseNormals (bool use)
  {
    use_normals_ = use;
  }
  ...
  void
  setModelPath (std::string& model);

  void
  setPoseRefinement (bool do_it, int iters = 5)
  {
  ...
  void
  setLeavesFaceThreshold (float p)
  {
    thres_face_ = p;
  }

  void
  setLeavesFaceMaxVariance (float max)
  {
  ...
  void
  setFaceMinVotes (int mv)
  {
    min_votes_size_ = mv;
  }

  void
  setNumVotesUsedForPose (int n)
  {
    used_for_pose_ = n;
  }
   ...
   */
  void
  setForest (pcl::DecisionForest<NodeType>& forest)
  {
    forest_ = forest;
  }
  ...
  void
  getFaceHeatMap (pcl::PointCloud<pcl::PointXYZI>::Ptr& heat_map)
  {
  ...
  getVotes2 (pcl::PointCloud<pcl::PointXYZI>::Ptr& votes_cloud)
  {
    ...
    for (std::size_t i = 0; i < head_center_original_votes_clustered_.size(); i++) {
      for (std::size_t j = 0; j < head_center_original_votes_clustered_[i].size();
           j++, p++) {
   ...
   */
  void
  setInputCloud (pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud)
  {
    input_ = cloud;
  }

  void
  setFaceHeatMapCloud (pcl::PointCloud<pcl::PointXYZI>::Ptr& heat_map)
  {
