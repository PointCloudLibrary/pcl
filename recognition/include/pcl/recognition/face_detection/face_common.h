...
public:
  std::vector<pcl::IntegralImage2D<float, 1>::Ptr>
      iimages_; // also pointer to the respective integral image
  int row_, col_;
  int wsize_;
  int label_;
...
public:
  ...
  serialize (::std::ostream& stream) const
  {
    ...
    for (std::size_t i = 0; i < 3; i++)
      stream.write (reinterpret_cast<const char*> (&trans_mean_[i]),
                    sizeof (trans_mean_[i]));

    for (std::size_t i = 0; i < 3; i++)
      stream.write (reinterpret_cast<const char*> (&rot_mean_[i]),
                    sizeof (rot_mean_[i]));

    for (std::size_t i = 0; i < 3; i++)
      for (std::size_t j = 0; j < 3; j++)
        stream.write (reinterpret_cast<const char*> (&covariance_trans_ (i, j)),
                      sizeof (covariance_trans_ (i, j)));

    for (std::size_t i = 0; i < 3; i++)
      for (std::size_t j = 0; j < 3; j++)
        stream.write (reinterpret_cast<const char*> (&covariance_rot_ (i, j)),
  ...
  deserialize (::std::istream& stream)
  {
    ...
    stream.read (reinterpret_cast<char*> (&num_of_sub_nodes),
                 sizeof (num_of_sub_nodes));
    ...
    for (std::size_t i = 0; i < 3; i++)
      for (std::size_t j = 0; j < 3; j++)
        stream.read (reinterpret_cast<char*> (&covariance_trans_ (i, j)),
                     sizeof (covariance_trans_ (i, j)));

    for (std::size_t i = 0; i < 3; i++)
      for (std::size_t j = 0; j < 3; j++)
        stream.read (reinterpret_cast<char*> (&covariance_rot_ (i, j)),
                     sizeof (covariance_rot_ (i, j)));
