 ...
 * "A Global Hypotheses Verification Method for 3D Object Recognition", A. Aldoma and F.
 * Tombari and L. Di Stefano and Markus Vincze, ECCV 2012 \author Aitor Aldoma \ingroup
 * recognition
 */
template <typename ModelT, typename SceneT>
class PCL_EXPORTS GlobalHypothesesVerification
: public HypothesisVerification<ModelT, SceneT> {
private:
  // Helper classes
  ...
  public:
    ...
    std::vector<float>
        explained_distances_; // closest distances to the scene for point i
    std::vector<int> unexplained_in_neighborhood; // indices vector referencing
  ...
  std::vector<float>
      explained_by_RM_distance_weighted; // represents the points of scene_cloud_ that
                                         // are explained by the recognition models
  std::vector<float>
      unexplained_by_RM_neighboorhods; // represents the points of scene_cloud_ that are
                                       // not explained by the active hypotheses in the
                                       // neighboorhod of the recognition models
  ...
  void
  updateUnexplainedVector (std::vector<int>& unexplained_,
  ...
  {
    {
      ...
      for (std::size_t i = 0; i < unexplained_.size(); i++) {
        ...
        bool prev_unexplained = (unexplained_by_RM[unexplained_[i]] > 0) &&
                                (explained_by_RM[unexplained_[i]] == 0);
        ...
        else // the hypothesis is being added and unexplains unexplained_[i], so
             // increase by 1 unless its explained by another hypothesis
        {
      ...
      for (const int& i : explained) {
        if (val < 0) {
          // the hypothesis is being removed, check that there are no points that become
          // unexplained and have clutter unexplained hypotheses
          if ((explained_by_RM[i] == 0) && (unexplained_by_RM[i] > 0)) {
            add_to_unexplained += unexplained_by_RM[i]; // the points become unexplained
          }
        }
        else {
          // std::cout << "being added..." << add_to_unexplained << " " <<
          // unexplained_by_RM[explained[i]] << std::endl;
          if ((explained_by_RM[i] == 1) &&
              (unexplained_by_RM[i] > 0)) { // the only hypothesis explaining that point
            add_to_unexplained -=
                unexplained_by_RM[i]; // the points are not unexplained any longer
                                      // because this hypothesis explains them
          }
        }
      }
    ...
    for (std::size_t i = 0; i < vec.size(); i++) {
      ...
      if ((explained_[vec[i]] > 1) && prev_dup) {     // its still a duplicate, we are
        ...
        add_to_duplicity_ -= 2;
               ...
               !prev_dup) { // it was not a duplicate but it is now, add 2, we are
                            // adding a conflicting hypothesis for the point
        add_to_duplicity_ += 2;
      }
    }
  ...
  float
  getTotalExplainedInformation (std::vector<int>& explained_,
                                std::vector<float>& explained_by_RM_distance_weighted,
                                int* duplicity_)
  {
  ...
  float
  getTotalBadInformation (std::vector<RecognitionModelPtr>& recog_models)
  {
    float bad_info = 0;
  ...
  float
  getUnexplainedInformationInNeighborhood (std::vector<float>& unexplained,
                                           std::vector<int>& explained)
  {
...
public:
  ...
  void
  setResolutionOccupancyGrid (float r)
  {
