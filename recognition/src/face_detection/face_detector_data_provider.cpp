...
// much better solution would be pass a visualization function from the user code to
// make it transparent to the training process
            ...
            std::vector<std::vector<int>>& yaw_pitch_bins)
{
  ...
  for (int j = 0; j < num_pitch; j++) {
    std::cout << pcl_round (static_cast<float> (min_pitch) +
                            res_pitch * static_cast<float> (j))
              << "\t";
  }
  ...
  for (int i = 0; i < num_yaw; i++) {
    std::cout << pcl_round (static_cast<float> (min_yaw) +
                            res_yaw * static_cast<float> (i))
              << " => \t\t";
    for (int j = 0; j < num_pitch; j++) {
      // std::cout <<  std::setprecision(3) << yaw_pitch_bins[i][j] /
      // static_cast<float>(max_elems) << "\t";
        ...
        initialize (std::string& data_dir)
{
  ...
  int num_yaw =
      static_cast<int> ((std::abs (min_yaw) * 2) / static_cast<int> (res_yaw + 1.f));
  int num_pitch = static_cast<int> ((std::abs (min_pitch) * 2) /
                                    static_cast<int> (res_pitch + 1.f));
  ...
  for (const auto& filename : files) {
    ...
    if (readMatrixFromFile (pose_file, pose_mat)) {
      ...
      int y = static_cast<int> (
          pcl_round ((ea[0] + static_cast<float> (std::abs (min_yaw))) / res_yaw));
      int p = static_cast<int> (
          pcl_round ((ea[1] + static_cast<float> (std::abs (min_pitch))) / res_pitch));
  ...
  float average =
      static_cast<float> (total_elems) / (static_cast<float> (num_pitch + num_yaw));
  ...
  // reduce unbalance from dataset by capping the number of images per bin, keeping at
  // least a certain min
    ...
    for (int i = 0; i < num_yaw; i++) {
      for (int j = 0; j < num_pitch; j++) {
        if (yaw_pitch_bins[i][j] >= min_images_per_bin_) {
          std::shuffle (
              image_files_per_bin[i][j].begin(), image_files_per_bin[i][j].end(), rng);
                             ...
                             std::vector<ExampleIndex>& examples)
{
  ...
  for (std::size_t j = 0; j < files.size(); j++) {
    ...
    pcl::PointCloud<pcl::PointXYZL>::Ptr cloud_labels (
        new pcl::PointCloud<pcl::PointXYZL>);
    pcl::PointCloud<pcl::PointXYZL>::Ptr loaded_cloud_labels (
        new pcl::PointCloud<pcl::PointXYZL>);
    ...
    if (USE_NORMALS_) {
      ...
      integral_image_normal_x->setInput (data + 0,
                                         normals->width,
                                         normals->height,
      ...
      integral_image_normal_y->setInput (data + 1,
                                         normals->width,
                                         normals->height,
                                         element_stride_normal,
                                         row_stride_normal);
      ...
      integral_image_normal_z->setInput (data + 2,
                                         normals->width,
                                         normals->height,
                                         element_stride_normal,
                                         row_stride_normal);
    ...
    //************************************************
    {
      ...
      for (int col = 0; col < (static_cast<int> (cloud_labels->width) - w_size_);
           col++) {
        for (int row = 0; row < (static_cast<int> (cloud_labels->height) - w_size_);
             row++) {
          ...
          float percent = 0.5f;
          if (static_cast<float> (integral_image_depth->getFiniteElementsCount (
                  col, row, w_size_, w_size_)) <
              (percent * static_cast<float> (w_size_ * w_size_)))
            continue;
...
#if PCL_FACE_DETECTION_VIS_TRAINING_FDDP == 1
        pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_intensity2 (
            new pcl::PointCloud<pcl::PointXYZI> (*cloud_intensity));
  ...
  std::cout << training_examples.size() << " " << labels.size() << " " << total_neg
            << " " << total_pos << std::endl;
