...
void
pcl::RFFaceDetectorTrainer::trainWithDataProvider()
{
...
void
pcl::RFFaceDetectorTrainer::faceVotesClustering()
{
  ...
  int msi = 10;
  std::cout << "Number of clusters:" << clusters_mean.size()
            << " votes:" << head_center_votes_.size() << std::endl;
...
pcl::RFFaceDetectorTrainer::detectFaces()
{
        ...
        if (integral_image_depth->getFiniteElementsCount(col, row, w_size_, w_size_) >
          ...
          for (const auto& leaf : leaves) {
            if (leaf.value >= thres_face_) {
              if ((leaf.covariance_trans_.trace() + leaf.covariance_rot_.trace()) >
                  trans_max_variance_)
                continue;
  ...
  if (pose_refinement_ && (!head_clusters_centers_.empty())) {
    ...
    /*pcl::PapazovHV<pcl::PointXYZ, pcl::PointXYZ> papazov;
     ...
     std::cout << "Valid heads after HV:" << valid << " before was:" << mask_hv.size()
     << std::endl; head_clusters_centers_.resize(valid);
