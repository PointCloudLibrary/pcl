...
void showDepthFromDevice(short * buf_int, int frame_width, int frame_height, int maxZ)
{
if(needWriteToFile==1) {
	CreatePCLFileFromKinect(buf_int, frame_width, frame_height, maxZ);
	needWriteToFile=0;
	LOGI("Wrote PCL File");
usleep(1000000);
}
...
int width_ds=frame_width*scale;
	...
	int numberOfPoints=height_ds*width_ds;
...
for(int  i=0;i<height_ds;i++)
	...
	// 525 - parameter of the camera for 640*480 resolution, in case data downsampling we need reduce it proportional.	
		*(pxyz+i*width_ds*3+j*3+0)=(short)((j-width_ds/2)*depth/(525*scale));  
		*(pxyz+i*width_ds*3+j*3+1)=(short)((i-height_ds/2)*depth/(525*scale));
		*(pxyz+i*width_ds*3+j*3+2)=(short)(depth);//(-depth*255/maxZ);//(-depth*depth*255/maxZ/maxZ);


		*(col+i*width_ds*3+j*3+0)=(short)255-depth*255/maxZ;
		*(col+i*width_ds*3+j*3+1)=(short)255-depth*255/maxZ;
		*(col+i*width_ds*3+j*3+2)=(short)255;
	if (depth==0) {
 		*(pxyz+i*width_ds*3+j*3+0)=(short)0;
		*(pxyz+i*width_ds*3+j*3+1)=(short)0;
		*(pxyz+i*width_ds*3+j*3+2)=(short)0;}
...
JNIEXPORT jboolean JNICALL Java_com_itseez_icpdemo_KiwiNative_PCLPath(JNIEnv* env, jobject obj, jstring filename, int id);
  ...
  JNIEXPORT void JNICALL Java_com_itseez_icpdemo_KiwiNative_showDepthFromDevice(JNIEnv * env, jclass clazz, jobject buf,  jint maxZ, jint frame_width, jint frame_height);
...
int firsttime=0;
  JNIEXPORT void JNICALL Java_com_itseez_icpdemo_KiwiNative_showDepthFromDevice(JNIEnv * env, jclass clazz, jobject buf, jint maxZ, jint frame_width, jint frame_height)
{
	 short * buf_int = static_cast<short *>(env->GetDirectBufferAddress(buf));
...
//	showDepthFromDevice(buf_int, frame_width, frame_height,maxZ);

	CreatePointCloud(buf_int, frame_width, frame_height,maxZ);
	if(firsttime<5) {firsttime++; app->resetView();}
