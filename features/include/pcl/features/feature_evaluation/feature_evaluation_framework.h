...
#include <pcl/features/feature_evaluation/feature_test.h>
...
namespace pcl
{
  ...
  public:
    /** \brief Adds all feature descriptor test classes to list of tests.
      *
      */
    FeatureEvaluationFramework () : source_input_(), target_input_(),
                                    gt_offset_(Eigen::Vector3f::Identity()), gt_rotation_(Eigen::Quaternionf::Identity())
    {
      tests_.clear();
    ...
    /*
     * If target file is not specified, the target cloud is obtained by transforming the source cloud by the ground truth
     *
     * NOTE: The ground truth must be set before calling this function without a target file!!!
     */
    void setInputClouds (std::string source_file, std::string target_file, std::string label = "")
    {
      ...
      if (target_file != "")
        ...
        pcl::transformPointCloud (*source_input_, *target_input_, gt_offset_, gt_rotation_);
    ...
    void setGroundTruth (std::string ground_t)
    {
      ...
      float x, y, z, w;
      infile >> x >> y >> z >> w;
      gt_rotation_ = Eigen::Quaternionf(w,x,y,z);
      infile.close();
    }
    ...
    void setThreshold (float t)
    {
      ...
      delta_threshold_ = 1;                     //any non-zero value will work
    }
    ...
    void runSingleTest ()
    {
      ...
      (tests_[feature_name_])->setGroundTruth (gt_offset_, gt_rotation_);
    ...
    void clearData ()
    {
      typename std::map<std::string, FeatureCorrespondenceTestPtr, std::less<std::string>,
        Eigen::aligned_allocator< std::pair<std::string, FeatureCorrespondenceTestPtr> > >::iterator it;
  ...
  private:
    ...
    /** \brief A map from class name to the FeatureCorrespondenceTests for those tests
      */
    std::map<std::string, FeatureCorrespondenceTestPtr, std::less<std::string>,
      Eigen::aligned_allocator< std::pair<std::string, FeatureCorrespondenceTestPtr> > > tests_;
    ...
    Eigen::Vector3f gt_offset_;
    Eigen::Quaternionf gt_rotation_;
