...
int
OrganizedNeighborSearch<PointT>::radiusSearch(const PointCloudConstPtr& cloud_arg,
                                              int index_arg,
                                              double radius_arg,
...
template <typename PointT>
int
OrganizedNeighborSearch<PointT>::radiusSearch(int index_arg,
...
template <typename PointT>
int
OrganizedNeighborSearch<PointT>::radiusSearch(const PointT& p_q_arg,
                                              const double radius_arg,
                                              std::vector<int>& k_indices_arg,
                                              std::vector<float>& k_sqr_distances_arg,
                                              int max_nn_arg) const
  ...
  for (int x = leftX; (x <= rightX) && (nnn < max_nn_arg); x++)
    for (int y = leftY; (y <= rightY) && (nnn < max_nn_arg); y++) {
      ...
      double squared_distance =
          (point_dist_x * point_dist_x + point_dist_y * point_dist_y +
...
template <typename PointT>
void
OrganizedNeighborSearch<PointT>::getProjectedRadiusSearchBox(const PointT& point_arg,
                                                             ...
                                                             int& maxY_arg) const
{
  ...
  // see
  // http://www.wolframalpha.com/input/?i=solve+%5By%2Fsqrt%28f^2%2By^2%29*c-f%2Fsqrt%28f^2%2By^2%29*b%2Br%3D%3D0%2C+f%3D1%2C+y%5D
...
template <typename PointT>
int
OrganizedNeighborSearch<PointT>::nearestKSearch(int index_arg,
                                                int k_arg,
                                                std::vector<int>& k_indices_arg,
                                                std::vector<float>& k_sqr_distances_arg)
{
...
template <typename PointT>
int
OrganizedNeighborSearch<PointT>::nearestKSearch(const PointCloudConstPtr& cloud_arg,
                                                int index_arg,
                                                int k_arg,
                                                std::vector<int>& k_indices_arg,
                                                std::vector<float>& k_sqr_distances_arg)
...
template <typename PointT>
int
OrganizedNeighborSearch<PointT>::nearestKSearch(const PointT& p_q_arg,
                                                int k_arg,
                                                std::vector<int>& k_indices_arg,
                                                std::vector<float>& k_sqr_distances_arg)
{
  ...
  typename std::vector<radiusSearchLoopkupEntry>::const_iterator
  ...
  while ((radiusSearchLookup_Iterator != radiusSearchLookup_.end()) &&
    ...
    if ((x >= 0) && (y >= 0) && (x < (int)input_->width) && (y < (int)input_->height)) {
      ...
      if ((point.x == point.x) && // check for NaNs
        ...
        // calculate squared distance
        squared_distance = (point_dist_x * point_dist_x + point_dist_y * point_dist_y +
                            point_dist_z * point_dist_z);
  ...
  if ((int)nearestNeighbors.size() == k_arg) {
    ...
    // search for maximum distance between search point to window borders in 2-D search
    // window
  ...
  for (int y = 0; y < (int)input_->height; y++)
    for (int x = 0; x < (int)input_->width; x++) {
      ...
      if (((*input_)[i].x == (*input_)[i].x) && // check for NaNs
        ...
        if ((double)(x - input_->width / 2) * (double)(y - input_->height / 2) *
                point.z !=
            0) {
...
template <typename PointT>
void
OrganizedNeighborSearch<PointT>::generateRadiusLookupTable(unsigned int width,
                                                           unsigned int height)
{
  if ((this->radiusLookupTableWidth_ != (int)width) ||
...
#define PCL_INSTANTIATE_OrganizedNeighborSearch(T)                                     \
  template class pcl::OrganizedNeighborSearch<T>;
