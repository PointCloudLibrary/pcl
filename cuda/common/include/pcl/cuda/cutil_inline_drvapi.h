...
inline void
__cuCtxSync (const char* file, const int line)
{
...
inline int
_ConvertSMVer2CoresDrvApi (int major, int minor)
{
...
inline int
cutilDrvGetMaxGflopsGraphicsDeviceId ()
{
  CUdevice current_device = 0;
  CUdevice max_perf_device = 0;
  int device_count = 0;
...
inline void
__cuCheckMsg (const char* msg, const char* file, const int line)
{
  CUresult err = cuCtxSynchronize();
  if (CUDA_SUCCESS != err) {
...
inline int
cutilDeviceInitDrv (int ARGC, char** ARGV)
{}
#else
inline int
cutilDeviceInitDrv (int ARGC, char** ARGV)
...
#if __DEVICE_EMULATION__
inline CUdevice
cutilChooseCudaDeviceDrv (int argc, char** argv, int* p_devID)
#else
inline CUdevice
cutilChooseCudaDeviceDrv (int argc, char** argv, int* p_devID)
{
...
inline void
__cutilDrvQAFinish (int argc, char** argv, bool bStatus)
{
