...
inline void
__cutilCondition (int val, char* file, int line)
{
...
inline void
__cutilExit (int argc, char** argv)
{
...
// This function returns the best GPU (with maximum GFLOPS)
inline int
cutGetMaxGflopsGraphicsDeviceId ()
{
  int current_device = 0;
  int max_compute_perf = 0;
  int max_perf_device = 0;
...
// Give a little more for Windows : the console window often disappears before we can
// read the message
#ifdef _WIN32
#if 1         // ndef UNICODE
#ifdef _DEBUG // Do this only in debug mode...
inline void
VSPrintf (FILE* file, LPCSTR fmt, ...)
{
...
// when the user double clicks on the error line in the Output pane. Like any compile
// error.
...
inline void
__cutilGetLastError (const char* errorMessage, const char* file, const int line)
{
...
inline void
__cutilGetLastErrorAndSync (const char* errorMessage, const char* file, const int line)
{
  cudaError_t err = cudaGetLastError();
  if (cudaSuccess != err) {
    FPRINTF ((stderr,
...
{}
inline int
cutilChooseCudaDevice (int ARGC, char** ARGV)
{}
#else
inline int
cutilDeviceInit (int ARGC, char** ARGV)
...
inline void
cutilCudaCheckCtxLost (const char* errorMessage, const char* file, const int line)
{
  cudaError_t err = cudaGetLastError();
  if (cudaSuccess != err) {
    FPRINTF ((stderr,
...
inline void
__cutilQAFinish (int argc, char** argv, bool bStatus)
{
...
inline bool
cutilCudaCapabilities (int major_version, int minor_version, int argc, char** argv)
{
  cudaDeviceProp deviceProp;
  deviceProp.major = 0;
  deviceProp.minor = 0;
