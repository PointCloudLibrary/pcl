 ...
 *
 * This demo loads a stereo image pair and computes the disparity map and related
 * organized point cloud using the PCL AdaptiveCost-2SO Stereo Matching algorithm or the
 * PCL Block-based stereo matching algorithm (you have to comment it out, see the code)
 * Input pcds should be "stereo_left.pcd" and "stereo_right.pcd" which can be found in
 * the test subfolder within trunk. A rescaled version of the disparity map is
 * displayed, as well as the point cloud.
 *
...
main (int argc, char** argv)
{
  ...
  if (!left_cloud->isOrganized() || !right_cloud->isOrganized() ||
      left_cloud->width != right_cloud->width ||
      left_cloud->height != right_cloud->height) {
