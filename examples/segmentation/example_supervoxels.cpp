...
void
printText (pcl::visualization::PCLVisualizer::Ptr viewer);

/** \brief Removes info text in the specified PCLVisualizer
 *  \param[in] viewer_arg The PCLVisualizer to modify  */
void
removeText (pcl::visualization::PCLVisualizer::Ptr viewer);
...
main (int argc, char** argv)
{
  if (argc < 2) {
    pcl::console::print_info (
        ...
        "(disables use of single camera transform) \n -o <output-file> \n -O "
        "<refined-output-file> \n-l <output-label-file> \n -L "
        "<refined-output-label-file> \n-v <voxel resolution> \n-s <seed resolution> "
        "\n-c <color weight> \n-z <spatial weight> \n-n <normal_weight>] \n",
        argv[0]);
    return (1);
  }
  ...
  if (!pcd_file_specified) {
    ...
    vtkImageReader2* depth_reader =
        reader_factory->CreateImageReader2 (depth_path.c_str());
    ...
    if (!ignore_provided_normals) {
      if (hasField (input_pointcloud2, "normal_x")) {
        std::cout << "Using normals contained in file. Set --nonormals option to "
                     "disable this.\n";
  ...
  if (cloud->isOrganized() && !disable_transform) {
    for (const auto& point : *cloud)
      if (point.z < 0) {
        PCL_ERROR ("Points found with negative Z values, this is not compatible with "
                   "the single camera transform!\n");
        PCL_ERROR ("Set the --NT option to disable the single camera transform!\n");
        return 1;
      }
    std::cout << "You have the single camera transform enabled - this should be used "
                 "with point clouds captured from a single camera.\n";
  ...
  if (cloud->isOrganized()) {
    ...
    pcl::io::PointCloudImageExtractorFromLabelField<PointLT> pcie (
        pcl::io::PointCloudImageExtractorFromLabelField<PointLT>::COLORS_RGB_GLASBEY);
  ...
  while (!viewer->wasStopped()) {
    if (show_supervoxels) {
      if (!viewer->updatePointCloud ((show_refined) ? refined_labeled_voxel_cloud
                                                    : labeled_voxel_cloud,
                                     "colored voxels")) {
        viewer->addPointCloud ((show_refined) ? refined_labeled_voxel_cloud
                                              : labeled_voxel_cloud,
                               "colored voxels");
        viewer->setPointCloudRenderingProperties (
            pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3.0, "colored voxels");
      }
    }
    else {
    ...
    if (show_voxel_centroids) {
      if (!viewer->updatePointCloud (voxel_centroid_cloud, "voxel centroids")) {
        ...
        viewer->setPointCloudRenderingProperties (
            pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2.0, "voxel centroids");
      }
    }
    else {
    ...
    if (show_normals) {
      auto sv_itr = ((show_refined) ? refined_supervoxel_clusters.begin()
                                    : supervoxel_clusters.begin());
      auto sv_itr_end = ((show_refined) ? refined_supervoxel_clusters.end()
    ...
    if (show_graph && !graph_added) {
      poly_names.clear();
      for (auto label_itr = label_adjacency.begin();
           label_itr != label_adjacency.end();) {
        // First get the label
        std::uint32_t supervoxel_label = label_itr->first;
        // Now get the supervoxel corresponding to the label
        pcl::Supervoxel<PointT>::Ptr supervoxel =
            supervoxel_clusters.at (supervoxel_label);
        // Now we need to iterate through the adjacent supervoxels and make a point
        // cloud of them
        PointCloudT adjacent_supervoxel_centers;
        for (auto adjacent_itr = label_adjacency.equal_range (supervoxel_label).first;
             adjacent_itr != label_adjacency.equal_range (supervoxel_label).second;
             ++adjacent_itr) {
