...
- download, compile and install swig 2.0.11 from http://www.swig.org/
...
- currently if you want to use a template class type, you must instaniate it in swig files, for 
	example:
		if you want to use "pcl::PointCloud<pcl::PointXYZRGB>" then you must use that "%template(PointCloud_PointXYZRGB) pcl::PointCloud<pcl::PointXYZRGB>;" (you will find me doing that in the file [PointCloud.i] so do as you see)
		and by making that and running ./exec.sh , you will find wrapper.pcl.PointCloud_PointXYZRGB class which is your desired one
