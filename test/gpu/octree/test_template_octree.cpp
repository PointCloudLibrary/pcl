...
TEST(PCL_OctreeGPU, PointXYZ)
{
  ...
  */
  ...
  // gpu build
  pcl::gpu::OcTree<pcl::PointXYZ> octree_device;
  ...
  pcl::gpu::OcTree<pcl::PointXYZ>::ResultSqrDists dists_device;
...
TEST(PCL_OctreeGPU, PointXYZRGB)
{
  ...
  */
  ...
  const std::array<pcl::PointXYZRGB, 10> coords{
      pcl::PointXYZRGB{-1.f, -1.f, -1.f, 0, 0, 1},
      pcl::PointXYZRGB{-1.f, -1.f, 1.f, 0, 1, 0},
      pcl::PointXYZRGB{-1.f, 1.f, -1.f, 1, 0, 0},
      pcl::PointXYZRGB{-1.f, 1.f, 1.f, 1, 1 , 0},
      pcl::PointXYZRGB{1.f, -1.f, -1.f, 1, 1, 0},
      pcl::PointXYZRGB{1.f, -1.f, 1.f, 0, 0, 1},
      pcl::PointXYZRGB{1.f, 1.f, -1.f, 1, 0, 1},
      pcl::PointXYZRGB{1.f, 1.f, 1.f, 1, 0, 1},
      pcl::PointXYZRGB{-0.9f, -0.2f, -0.75f, 1, 0,1},
      pcl::PointXYZRGB{-0.4f, -0.6f, -0.75f, 1, 1, 1},
  };
  ...
  //generate custom pointcloud
  constexpr pcl::index_t point_size = 1000 * coords.size();
  auto cloud = pcl::make_shared<pcl::PointCloud<pcl::PointXYZRGB>>(point_size, 1);

  // copy chunks of 10 points at the same time
  for (auto it = cloud->begin(); it != cloud->cend(); it += coords.size())
    std::copy(coords.cbegin(), coords.cend(), it);

  const std::vector<pcl::PointXYZRGB> queries = {
      {-0.4, -0.2, -0.75, 1, 0, 1}, // should be different across CPU and GPU if different
                           // traversal methods are used
      {-0.6, -0.2, -0.75, 0, 0, 1}, // should be same across CPU and GPU
      {1.1, 1.1, 1.1, 0, 0, 1},     // out of range query
  };
  ...
  // gpu build
  pcl::gpu::OcTree<pcl::PointXYZRGB> octree_device;
  octree_device.setCloud(cloud_device);
  octree_device.build();

  // build host octree
  constexpr float host_octree_resolution = 0.05;
  pcl::octree::OctreePointCloudSearch<pcl::PointXYZRGB> octree_host(
      host_octree_resolution);
  octree_host.setInputCloud(cloud);
  octree_host.addPointsFromInputCloud();

  // upload queries
  pcl::gpu::OcTree<pcl::PointXYZRGB>::Queries queries_device;
  queries_device.upload(queries);
  ...
  std::vector<float> dists_pcl(queries.size());
  std::vector<float> dists_gpu(queries.size());
  pcl::gpu::OcTree<pcl::PointXYZRGB>::ResultSqrDists dists_device;
