...
TEST (PCL, PCDGrabber)
{
  pcl::PCDGrabber<PointT> grabber (
      pcd_files_, 10, false); // TODO add directory functionality
  EXPECT_EQ (grabber.size(), pcds_.size());
  std::vector<CloudT::ConstPtr> grabbed_clouds;
  std::function<void (const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr&)> fxn =
      [&] (const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr& input_cloud) {
        grabbed_clouds.push_back (input_cloud);
      };
...
TEST (PCL, ImageGrabberTIFF)
{
  ...
  std::function<void (const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr&)> fxn =
      [&] (const CloudT::ConstPtr& input_cloud) {
        cloud_callback (signal_received, cloud_buffer, input_cloud);
      };
  grabber.registerCallback (fxn);
  ...
  for (std::size_t i = 0; i < pcds_.size(); i++) {
    ...
    for (std::size_t j = 0; j < pcds_[i]->size(); j++) {
      ...
      // EXPECT_EQ (pcd_pt.a, tiff_pt.a);    // the alpha channel in the PCD set was
      // saved as 255 but it should have been 0
    }
  }
}
...
TEST (PCL, ImageGrabberPCLZF)
{
  ...
  for (std::size_t i = 0; i < pcds_.size(); i++) {
    ...
    for (std::size_t j = 0; j < pcds_[i]->size(); j++) {
      ...
      // EXPECT_EQ (pcd_pt.a, pclzf_pt.a);     // the alpha channel in the PCD set was
      // saved as 255 but it should have been 0
    }
  }
}
...
TEST (PCL, ImageGrabberSetIntrinsicsTIFF)
{
  ...
  // grabber.getCameraIntrinsics (fx_old, fy_old, cx_old, cy_old); Need to use old
  // intrinsics, can't trust defaults
  fx_old = 525;
  fy_old = 525;
  cx_old = 320;
  cy_old = 240;
...
{
  if (argc < 2) {
    std::cerr << "No test files were given. Please add the path to grabber_sequences "
                 "to this test."
              << std::endl;
    return (-1);
  }
  ...
  for (pcl_fs::directory_iterator itr (pcd_dir_); itr != end_itr; ++itr) {
    if (!is_directory (itr->status()) &&
        boost::algorithm::to_upper_copy (itr->path().extension().string()) == ".PCD") {
