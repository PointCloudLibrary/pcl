...
struct TestData {
  TestData (pcl::Indices const& insideIndices,
...
std::vector<TestData>
createTestDataSuite (std::function<pcl::PointXYZ()> inside_point_generator,
                     std::function<pcl::PointXYZ()> outside_point_generator)
{
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr outside_cloud (
  ...
  pcl::Indices inside_indices_for_outside_cloud; // empty indices, cause outside_cloud
                                                 // don't contains any inside point
  ...
  test_data_suite.emplace_back (std::move (inside_indices_for_inside_cloud),
                                inside_cloud);
  test_data_suite.emplace_back (std::move (inside_indices_for_outside_cloud),
                                outside_cloud);
  test_data_suite.emplace_back (std::move (inside_indices_for_mixed_cloud),
                                mixed_cloud);
  ...
  SetUp () override
  {
    ...
    for (pcl::PointXYZ const& baseOffset : baseOffsetList_) {
      ...
      auto outside_point_generator = [this, &baseOffset] () {
        ...
        std::generate (pt.begin(), pt.end(), [this] {
          float v = rg_();
      ...
      pcl::CropHull<pcl::PointXYZ> crop_hull_filter =
          createDefaultCropHull (input_cloud);
      std::vector<TestData> test_data_suite =
          createTestDataSuite (inside_point_generator, outside_point_generator);
...
public:
  RandomGenerator() : gen_ (seed), rd_ (-0.5f, 0.5f) {}
...
{
  static pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud (
      new pcl::PointCloud<pcl::PointXYZ>);
...
using CropHullTestTraits2dList =
    std::tuple<std::tuple<CropHullTestTraits2d, RandomGenerator<0>>,
               std::tuple<CropHullTestTraits2d, RandomGenerator<123>>,
               std::tuple<CropHullTestTraits2d, RandomGenerator<456>>>;
using CropHullTestTraits3dList =
    std::tuple<std::tuple<CropHullTestTraits3d, RandomGenerator<0>>,
               std::tuple<CropHullTestTraits3d, RandomGenerator<123>>,
               std::tuple<CropHullTestTraits3d, RandomGenerator<456>>>;
using CropHullTestTypes =
    ::testing::Types<std::tuple_element_t<0, CropHullTestTraits2dList>,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// since test input cloud has same distribution for all dimensions, this test also check
// problem from issue #3960 //
  ...
  for (auto& entry : this->data_) {
    ...
    for (TestData const& test_data : entry.second) {
      ...
      for (size_t i = 0; i < test_data.input_cloud_->size(); ++i) {
        pcl::PointXYZ expectedPoint =
            test_data.inside_mask_[i] ? test_data.input_cloud_->at (i) : defaultPoint;
