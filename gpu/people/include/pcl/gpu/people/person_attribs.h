...
public:
   ...
   * \return 0 when successful, -1 when an error occurred, datastructure might become
  ...
  std::vector<std::vector<float>>
      part_ideal_length_; // Ideal length between two body parts
  std::vector<std::vector<float>>
      max_length_offset_;           // Max allowed length offset between two body parts
