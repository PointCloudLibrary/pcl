...
public:
  ...
  */
  void
  SetProxy (ON_Object* proxy1, ON_Object* proxy2, bool bCountReferences);

  bool
  SetParentIRef (const ON_InstanceRef& iref, ON_UUID iref_id, int idef_geometry_index);
  ...
  */
  const ON_Object*
  ProxyObject (int proxy_object_index) const;
  ...
  */
  void
  RemapObjectId (const ON_SimpleArray<ON_UuidPair>& uuid_remap);
