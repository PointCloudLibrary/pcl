...
/*  Typecode format 4 bytes long
  ...
  | |               | |               | |  |
   ...
   |        |        |         +-- specific codes
   |        |        |
   |        |        +-- RESERVED - DO NOT USE (should be 0) (will be used to control
  CRC on/off) |        | |        +-- category:_000 0000 0000 0001  Legacy geometry
  TCODE_LEGACY_GEOMETRY |                     _000 0000 0000 0010  openNURBS object
  TCODE_OPENNURBS_OBJECT |                     _000 0000 0000 0100  -- RESERVED - DO NOT
  USE (should be 0 in any typecode) -- |                     _000 0000 0000 1000  --
  RESERVED - DO NOT USE (should be 0 in any typecode) -- |                     _000 0000
  0001 0000  Geometry           TCODE_GEOMETRY |                     _000 0000 0010 0000
  Annotation |                     _000 0000 0100 0000  Display Attributes TCODE_DISPLAY
   |                     _000 0000 1000 0000  Rendering          TCODE_RENDER
   |                     _000 0001 0000 0000
   |                     _000 0010 0000 0000  Interface          TCODE_INTERFACE
   |                     _000 0100 0000 0000  -- RESERVED - DO NOT USE (should be 0 in
  any typecode) -- |                     _000 1000 0000 0000  Tolerances TCODE_TOLERANCE
...
#define TCODE_DIMSTYLE_TABLE                                                           \
  (TCODE_TABLE | 0x0020) /* annotation dimension style table */

#define TCODE_INSTANCE_DEFINITION_TABLE                                                \
  (TCODE_TABLE | 0x0021) /* instance definition table */
...
#define TCODE_OBSOLETE_LAYERSET_TABLE                                                  \
  (TCODE_TABLE | 0x0024) /* obsolete layer set table */
...
/* added 27 June 2008 as a chunk to hold userdata on ON_Viewports saved in named view
 * list */
#define TCODE_VIEW_VIEWPORT_USERDATA (TCODE_TABLEREC | TCODE_CRC | 0x0D3B)

/* records in bitmap table */
#define TCODE_BITMAP_RECORD                                                            \
  (TCODE_TABLEREC | TCODE_CRC | 0x0090) /* bitmap table record derived from ON_Bitmap  \
                                         */

/* records in material table */
#define TCODE_MATERIAL_RECORD                                                          \
  (TCODE_TABLEREC | TCODE_CRC |                                                        \
   0x0040) /* material table record derived from ON_Material */

/* records in layer table */
#define TCODE_LAYER_RECORD                                                             \
  (TCODE_TABLEREC | TCODE_CRC | 0x0050) /* layer table record derived from ON_Layer */

/* records in light table */
#define TCODE_LIGHT_RECORD                                                             \
  (TCODE_TABLEREC | TCODE_CRC | 0x0060) /* light table record derived from ON_Light */
#define TCODE_LIGHT_RECORD_ATTRIBUTES                                                  \
  (TCODE_INTERFACE | TCODE_CRC | 0x0061) /* ON_3dmObjectAttributes chunk */
#define TCODE_LIGHT_RECORD_ATTRIBUTES_USERDATA                                         \
  (TCODE_INTERFACE | 0x0062) /* ON_3dmObjectAttributes userdata chunk */
...
/* the user record header was added in 200910190 and is inside the TCODE_USER_TABLE_UUID
 * chunk */
...
#define TCODE_OBJECT_RECORD_TYPE                                                       \
  (TCODE_INTERFACE | TCODE_SHORT | 0x0071) /* ON::object_type value */
#define TCODE_OBJECT_RECORD_ATTRIBUTES                                                 \
  (TCODE_INTERFACE | TCODE_CRC | 0x0072) /* ON_3dmObjectAttributes chunk */
#define TCODE_OBJECT_RECORD_ATTRIBUTES_USERDATA                                        \
  (TCODE_INTERFACE | 0x0073) /* ON_3dmObjectAttributes userdata chunk */
#define TCODE_OBJECT_RECORD_HISTORY                                                    \
  (TCODE_INTERFACE | TCODE_CRC | 0x0074) /* construction history */
#define TCODE_OBJECT_RECORD_HISTORY_HEADER                                             \
  (TCODE_INTERFACE | TCODE_CRC | 0x0075) /* construction history header*/
#define TCODE_OBJECT_RECORD_HISTORY_DATA                                               \
  (TCODE_INTERFACE | TCODE_CRC | 0x0076) /* construction history data */
...
//
//   TCODE_OBJECT_RECORD_TYPE required - used to quickly filter and skip unwanted
objects
...
#define TCODE_OPENNURBS_CLASS_USERDATA_HEADER                                          \
  (TCODE_OPENNURBS_OBJECT | TCODE_CRC | 0x7FF9)
...
//   16 byte UUID ( a.k.a. GUID ) openNURBS class ID - determines specific openNURBS
class
...
#define TCODE_RHINOIO_OBJECT_NURBS_CURVE                                               \
  (TCODE_OPENNURBS_OBJECT | 0x0008) /* old CRhinoNurbsCurve   */
#define TCODE_RHINOIO_OBJECT_NURBS_SURFACE                                             \
  (TCODE_OPENNURBS_OBJECT | 0x0009) /* old CRhinoNurbsSurface */
#define TCODE_RHINOIO_OBJECT_BREP                                                      \
  (TCODE_OPENNURBS_OBJECT | 0x000B) /* old CRhinoBrep         */
#define TCODE_RHINOIO_OBJECT_DATA                                                      \
  (TCODE_OPENNURBS_OBJECT |                                                            \
   0xFFFE) /* obsolete - don't confuse with TCODE_OPENNURBS_OBJECT_DATA */
#define TCODE_RHINOIO_OBJECT_END                                                       \
  (TCODE_OPENNURBS_OBJECT |                                                            \
   0xFFFF) /* obsolete - don't confuse with TCODE_OPENNURBS_OBJECT_END */

/* OpenNURBS classes the require a unique tcode */
#define TCODE_OPENNURBS_BUFFER                                                         \
  (TCODE_OPENNURBS_OBJECT | TCODE_CRC | 0x0100) /* chunk stores ON_Buffer classes */
...
#define TCODE_LAYERTABLE                                                               \
  (TCODE_SHORT | TCODE_TABLE | 0x0001) /* obsolete - do not use */
...
#define TCODE_RGBDISPLAY                                                               \
  (TCODE_SHORT | TCODE_DISPLAY | 0x0006) /* will be used for color by object */
...
/* use LAYERSTATE ( 0 = LAYER_ON, 1 = LAYER_OFF, 2 = LAYER_LOCKED ) instead of above
 * individual toggles */
...
#define TCODE_RENDERMESHPARAMS                                                         \
  (TCODE_DISPLAY | 0x0020) /* block of parameters for render meshes */
