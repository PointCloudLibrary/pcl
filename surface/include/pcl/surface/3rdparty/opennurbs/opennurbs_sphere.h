...
public:
  ...
  bool
  Create (const ON_3dPoint& center, double radius); // center radius

  ON_Circle
  LatitudeRadians (double latitude_radians) const;
  ON_Circle
  LatitudeDegrees (double latitude_degrees) const;
  ON_Circle
  LongitudeRadians (double longitude_radians) const;
  ON_Circle
  LongitudeDegrees (double longitude_degrees) const;
  ...
  ON_3dPoint
  PointAt (double longitude_radians,
           double latitude_radians)
      const; // longitude [0,2pi], latitude [-pi/2,pi/2] in radians

  ON_3dVector
  NormalAt (double longitude_radians,
            double latitude_radians)
      const; // longitude [0,2pi], latitude [-pi/2,pi/2] in radians
  ...
  // parameterization of NURBS surface does not match sphere's transcendental
  // paramaterization
  int
  ...
  */
  ON_RevSurface*
  RevSurfaceForm (bool bArcLengthParameterization, ON_RevSurface* srf = NULL) const;
  ON_DEPRECATED ON_RevSurface*
