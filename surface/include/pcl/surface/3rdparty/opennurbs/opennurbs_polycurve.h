...
public:
  ...
  */
  bool
  IsValid (bool bAllowGaps, ON_TextLog* text_log) const;
  ...
  */
  ...
  int
  Count () const; // number of segment curves
  ...
  */
  double
  SegmentCurveParameter (double polycurve_parameter) const;
  ...
  */
  double
  PolyCurveParameter (int segment_index, double segmentcurve_parameter) const;
  ...
  */
  ON_Interval
  SegmentDomain (int segment_index) const;
  ...
  */
  int
  SegmentIndex (double polycurve_parameter) const;
  ...
  */
  int
  SegmentIndex (ON_Interval sub_domain, int* segment_index0, int* segment_index1) const;

  ON_Curve*
  FirstSegmentCurve () const; // returns NULL if count = 0

  ON_Curve*
  LastSegmentCurve () const; // returns NULL if count = 0
  ...
  */
  int
  FindNextGap (int segment_index0) const;
  ...
  */
  bool
  HasGapAt (int segment_index) const;
  ...
  */
  bool
  CloseGap (int gap_index, int segments_to_modify);
  ...
  */
  ...
  void
  Reserve (int); // make sure capacity is at least the specified count

  // ON_Curve pointers added with Prepend(), Append(), PrependAndMatch(),
  // AppendANdMatch(),and Insert() are deleted by ~ON_PolyCurve(). Use ON_CurveProxy(
  // ON_Curve*) if you want the original curve segment to survive ~ON_PolyCurve().
  ON_BOOL32
  Prepend (ON_Curve*); // Prepend curve.
  ON_BOOL32
  Append (ON_Curve*); // Append curve.
  ON_BOOL32
  Insert (int, // segment_index,
          ON_Curve*);

  // PrependAndMatch() and AppendAndMatch() return false if this->IsCLosed() or
  // this->Count() > 0 and curve is closed
  ON_BOOL32
  PrependAndMatch (ON_Curve*); // Prepend and match end of curve to start of polycurve
  ON_BOOL32
  ...
  */
  bool
  HasSynchronizedSegmentDomains () const;
  ...
  */
  ...
  void
  SetSegment (int index, ON_Curve* crv);
  ...
  */
  bool
  SetParameterization (const double* t);
  ...
  /*
    ...
    Parameters:
      t    - [in]	  	parameter
      index -[out]	index into m_t such that if the function returns true then t is
    equal to, or is within tolerance of m_t[index]. if function returns false then the
    value of index is
  ...
  */
  bool
  ParameterSearch (double t, int& index, bool bEnableSnap) const;
