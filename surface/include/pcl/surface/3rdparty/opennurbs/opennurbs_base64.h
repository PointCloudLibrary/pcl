...
public:
  ...
  */
  virtual bool
  Out (void* callback_context, ON__UINT32 out_buffer_size, const char* out_buffer);
...
public:
  ...
  const char*
  Decode (const char* base64str);
  const char*
  Decode (const char* base64str, std::size_t base64str_count);
  const wchar_t*
  Decode (const wchar_t* base64str);
  const wchar_t*
  Decode (const wchar_t* base64str, std::size_t base64str_count);
...
private:
  ...
  void
  DecodeHelper1 (); // decodes "**==" quartet into 1 byte
  void
  DecodeHelper2 (); // decodes "***=" quartet into 2 bytes
};
