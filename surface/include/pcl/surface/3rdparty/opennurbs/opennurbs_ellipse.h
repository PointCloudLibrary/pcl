...
public:
  ...
  Normal () const;
  const ON_Plane&
  Plane () const; // plane containing ellipse
  ...
  */
  ...
  bool
  GetFoci (ON_3dPoint& F1, ON_3dPoint& F2) const;
  ...
  ON_3dPoint
  PointAt (double) const;
  ON_3dVector
  DerivativeAt (int,   // desired derivative ( >= 0 )
                double // parameter
  ) const;

  ON_3dVector
  TangentAt (double) const; // returns unit tangent
  ON_3dVector
  CurvatureAt (double) const; // returns curvature vector

  // returns parameters of point on ellipse that is closest to given point
  ON_BOOL32
  ClosestPointTo (const ON_3dPoint&, double*) const;
  // returns point on ellipse that is closest to given point
  ON_3dPoint
  ClosestPointTo (const ON_3dPoint&) const;

  // evaluate ellipse's implicit equation in plane
  double
  EquationAt (const ON_2dPoint&) const;
  ON_2dVector
  GradientAt (const ON_2dPoint&) const;
  ...
  // parameterization of NURBS curve does not match ellipse's transcendental
  // paramaterization
  int
  GetNurbForm (ON_NurbsCurve&) const; // returns 0=failure, 2=success
