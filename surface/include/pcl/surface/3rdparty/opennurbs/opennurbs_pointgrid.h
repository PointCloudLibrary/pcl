...
public:
  ...
  */
  ...
  int
  PointCount ( // number of points in grid direction
      int      // dir 0 = "s", 1 = "t"
  ) const;

  int
  PointCount ( // total number of points in grid
      void) const;

  ON_3dPoint&
  Point (int, int // point index ( 0 <= i <= PointCount(0), 0 <= j <= PointCount(1)
  );
  ...
  int
  PointArrayStride ( // point stride in grid direction
      int            // dir 0 = "s", 1 = "t"
  ) const;
