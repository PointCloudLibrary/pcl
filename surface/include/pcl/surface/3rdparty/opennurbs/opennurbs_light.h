...
public:
  ...
  */
  ...
  void
  SetDirection (const ON_3dVector&);
  ...
  Direction () const;
  ON_3dVector
  PerpindicularDirection () const;

  double
  Intensity () const; // 0.0 = 0%  1.0 = 100%
  void
  SetIntensity (double);
  ...
  void
  SetPowerWatts (double);
  void
  SetPowerLumens (double);
  void
  SetPowerCandela (double);
  ...
  //
  void
  SetAttenuation (double, double, double);
  void
  SetAttenuation (const ON_3dVector&);
  ON_3dVector
  Attenuation () const;
  double
  Attenuation (double) const; // computes 1/(a[0] + d*a[1] + d^2*a[2]) where d =
                              // argument returns 0 if a[0] + d*a[1] + d^2*a[2] <= 0
  ...
  //
  void
  SetSpotAngleDegrees (double);
  double
  SpotAngleDegrees () const;

  void
  SetSpotAngleRadians (double);
  double
  SpotAngleRadians () const;
  ...
  bool
  GetSpotLightRadii (double* inner_radius, double* outer_radius) const;
  ...
  //
  void
  SetLength (const ON_3dVector&);
  ON_3dVector
  Length () const;

  void
  SetWidth (const ON_3dVector&);
  ON_3dVector
  Width () const;
  ...
  //
  void
  SetLightName (const char*);
  void
  SetLightName (const wchar_t*);
  const ON_wString&
  LightName () const;
...
public:
  ...
  ON_3dVector
      m_width; // only for rectangular lights
