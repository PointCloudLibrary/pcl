...
public:
  ...
  int
  IsVisible (const ON_Xform& bbox2c) const;

  void
  Destroy (); // invalidates bounding box
  ...
  Max () const;
  ON_3dVector
  Diagonal () const; // max corner - min corner
  ON_3dPoint
  Center () const;
  ON_3dPoint
  Corner ( // 8 corners of box
      int, // x_index   0 = Min().x, 1 = Max().x
      int, // y_index   0 = Min().y, 1 = Max().y
      int  // z_index   0 = Min().z, 1 = Max().z
  ) const;
  bool
  GetCorners (ON_3dPointArray& box_corners // returns list of 8 corner points
  ) const;
  bool
  GetCorners (ON_3dPoint box_corners[8] // returns list of 8 corner points
  ) const;

  bool
  IsValid () const; // empty boxes are not valid
  ...
  */
  ...
  double
  Tolerance () const; // rough guess at a tolerance to use for comparing
                      // objects in this bounding box

  // All of these Set() functions set or expand a box to enclose the points in the
  ...
  bool
  Set (const ON_3dPoint& point, int bGrowBox = false);

  bool
  Set (const ON_SimpleArray<ON_4dPoint>& point_array, int bGrowBox = false);

  bool
  Set (const ON_SimpleArray<ON_3dPoint>& point_array, int bGrowBox = false);

  bool
  Set (const ON_SimpleArray<ON_2dPoint>& point_array, int bGrowBox = false);

  bool
  IsPointIn (const ON_3dPoint& test_point, // point to test
  ...
  ON_3dPoint
  ClosestPoint (const ON_3dPoint& test_point) const;
  ...
  */
  double
  MinimumDistanceTo (const ON_BoundingBox& other) const;
  ...
  */
  double
  MaximumDistanceTo (const ON_BoundingBox& other) const;
  ...
  */
  double
  MinimumDistanceTo (const ON_Line& line) const;
  ...
  */
  double
  MinimumDistanceTo (const ON_Plane& plane) const;
  double
  ...
  */
  double
  MaximumDistanceTo (const ON_Line& line) const;
  ...
  */
  double
  MaximumDistanceTo (const ON_Plane& plane) const;
  double
  MaximumDistanceTo (const ON_PlaneEquation& plane_equation) const;
  ...
  */
  bool
  IsFartherThan (double d, const ON_Line& line) const;
  ...
  */
  bool
  IsFartherThan (double d, const ON_Plane& plane) const;
  ...
  */
  bool
  IsFartherThan (double d, const ON_BoundingBox& other) const;
  ...
  int
  GetClosestPoint (const ON_Line&, // line
  ...
  bool
  GetClosestPoint (const ON_BoundingBox&, // "other" bounding box
  ...
  ON_3dPoint
  FarPoint (const ON_3dPoint& // test_point
  ) const;

  //////////
  // Get points on bounding boxes that are farthest from each other.
  bool
  GetFarPoint (const ON_BoundingBox&, // "other" bounding box
  ...
  */
  bool
  Intersection (const ON_BoundingBox& other_bbox);
  ...
  */
  bool
  Includes (const ON_BoundingBox& other, bool bProperSubSet = false) const;
  ...
  */
  bool
  IsDisjoint (const ON_BoundingBox& other_bbox) const;
...
ON_DECL
double
ON_BoundingBoxTolerance (int dim, const double* bboxmin, const double* bboxmax);
...
ON_DECL
bool
ON_BeyondSinglePrecision (const ON_BoundingBox& bbox, ON_Xform* xform);
