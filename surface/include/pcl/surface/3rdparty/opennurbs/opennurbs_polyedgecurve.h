...
public:
  ...
  */
  bool
  Create (const ON_BrepTrim* trim, const ON_UUID& object_id);
  ...
  */
  bool
  Create (const ON_Curve* curve, const ON_UUID& object_id);
  ...
  ON_BOOL32
  Append (ON_PolyEdgeSegment* new_segment); // Append curve.
  ON_BOOL32
  Insert (int segment_index, ON_PolyEdgeSegment* new_segment);

  // if the segment is an edge, the following
  // return non-NULL pointers.
  const ON_BrepEdge*
  EdgeAt (double t) const;
  const ON_BrepTrim*
  TrimAt (double t) const;
  const ON_Brep*
  BrepAt (double t) const;
  const ON_BrepFace*
  FaceAt (double t) const;
  const ON_Surface*
  SurfaceAt (double t) const;
  ON_Surface::ISO
  IsoType (double t) const;

  double
  EdgeParameter (double t) const;

  // Test if there are any surface edges in the polyedge
  bool
  ContainsAnyEdges () const;
  // Test if all segments of the polyedge are surface edges
  bool
  ContainsAllEdges () const;
  ...
  */
  int
  FindEdge (const ON_BrepEdge* edge) const;
  ...
  */
  int
  FindTrim (const ON_BrepTrim* trim) const;
  ...
  */
  int
  FindCurve (const ON_Curve* curve) const;
  ...
  ON_BOOL32
  PrependAndMatch (ON_Curve*);
  ON_BOOL32
  AppendAndMatch (ON_Curve*);
