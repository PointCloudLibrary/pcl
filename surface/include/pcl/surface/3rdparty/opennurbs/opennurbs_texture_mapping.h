...
public:
  ...
  */
  bool
  SetSurfaceParameterMapping (void);
  ...
  */
  bool
  SetCylinderMapping (const ON_Cylinder& cylinder, bool bIsCapped);
  ...
  */
  bool
  SetSphereMapping (const ON_Sphere& sphere);
  ...
  */
  bool
  GetMappingCylinder (ON_Cylinder& cylinder) const;
  ...
  */
  bool
  GetMappingSphere (ON_Sphere& sphere) const;
  ...
  */
  bool
  ReverseTextureCoordinate (int dir);
  ...
  */
  bool
  SwapTextureCoordinate (int i, int j);
  ...
  */
  bool
  TileTextureCoordinate (int dir, double count, double offset);
  ...
  */
  ...
  int
  EvaluateBoxMapping (const ON_3dPoint& P, const ON_3dVector& N, ON_3dPoint* T) const;
  ...
  */
  bool
  HasMatchingTextureCoordinates (const ON_Mesh& mesh,
                                 const ON_Xform* object_xform = 0) const;
  bool
  HasMatchingTextureCoordinates (const class ON_MappingTag& tag,
                                 const ON_Xform* object_xform = 0) const;
...
public:
  ...
  // If m_bCapped is false, the cylinder or box is "infinite", if m_bCapped is true,
  // they are finite. In planar mappings, m_bCapped=false means "the Z texture
  // coordinate will always be 0.0" this is now the default behaviour in Rhino 5.0 -
  // it's what users expect apparently.
  bool m_bCapped;
  ...
  TypeFromInt (int i);
  static PROJECTION
  ProjectionFromInt (int i);
  static TEXTURE_SPACE
