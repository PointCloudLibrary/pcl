...
ON_CurveProxy::ON_CurveProxy(const ON_Curve* c) : m_real_curve(c), m_bReversed(0)
{
  if (m_real_curve)
...
: m_real_curve(c), m_bReversed(0), m_real_curve_domain(domain), m_this_domain(domain)
{}

unsigned int
ON_CurveProxy::SizeOf() const
{
...
ON__UINT32
ON_CurveProxy::DataCRC(ON__UINT32 current_remainder) const
{
  if (m_real_curve)
  ...
  current_remainder =
      ON_CRC32(current_remainder, sizeof(m_real_curve_domain), &m_real_curve_domain);
  current_remainder =
      ON_CRC32(current_remainder, sizeof(m_this_domain), &m_this_domain);
  return current_remainder;
}

ON_CurveProxy&
ON_CurveProxy::operator=(const ON_CurveProxy& src)
{
  if (this != &src) {
...
ON_CurveProxy::~ON_CurveProxy() { m_real_curve = 0; }

double
ON_CurveProxy::RealCurveParameter(double t) const
{
...
double
ON_CurveProxy::ThisCurveParameter(double real_curve_parameter) const
{
...
ON_Interval
ON_CurveProxy::RealCurveInterval(const ON_Interval* sub_domain) const
{
  if (!sub_domain)
...
bool
ON_CurveProxy::ProxyCurveIsReversed() const
{
  return m_bReversed;
}

void
ON_CurveProxy::SetProxyCurve(const ON_Curve* real_curve)
{
...
const ON_Curve*
ON_CurveProxy::ProxyCurve() const
{
  return m_real_curve;
}

bool
ON_CurveProxy::SetProxyCurveDomain(ON_Interval proxy_curve_subdomain)
{
  DestroyCurveTree();
...
ON_Interval
ON_CurveProxy::ProxyCurveDomain() const
{
  return m_real_curve_domain;
}

ON_Curve*
ON_CurveProxy::DuplicateCurve() const
{
...
ON_CurveProxy::IsValid(ON_TextLog* text_log) const
{
  ...
  if (rc && !m_real_curve->Domain().Includes(m_real_curve_domain)) {
    rc = false;
    if (text_log)
      text_log->Print("ON_CurveProxy.m_real_curve_domain is not included "
                      "m_real_curve->Domain().\n");
  }
...
ON_BOOL32
ON_CurveProxy::Write(ON_BinaryArchive& // open binary file
) const
{
  return false;
}
...
ON_BOOL32
ON_CurveProxy::Read(ON_BinaryArchive& // open binary file
)
{
  return false;
}
...
ON_Interval
ON_CurveProxy::Domain() const
{
  return m_this_domain;
}

ON_BOOL32
ON_CurveProxy::SetDomain(double t0, double t1)
{
  ON_BOOL32 rc = false;
  if (t0 < t1) {
    DestroyCurveTree();
...
bool
ON_CurveProxy::SetDomain(ON_Interval domain)
{
  return SetDomain(domain[0], domain[1]) ? true : false;
}

// Do not change SpanCount() without making sure it gives the correct result for use in
// GetSpanVector()
int
ON_CurveProxy::SpanCount() const
{
  if (!m_real_curve)
    return 0;
  ...
  while (i <= rsc && rsv[i] <= m_real_curve_domain[0])
    i++;
...
ON_BOOL32
ON_CurveProxy::GetSpanVector(double* d) const
{
  ...
  if (rsc < 1)
    return false;
  double* rsv = (double*)onmalloc((rsc + 1) * sizeof(double));
  if (!rsv || !m_real_curve->GetSpanVector(rsv))
    return false;
  ON_Interval domain = m_real_curve->Domain();
...
int
ON_CurveProxy::Degree() const
{
  return m_real_curve ? m_real_curve->Degree() : 0;
}

ON_BOOL32
ON_CurveProxy::GetParameterTolerance(double t,       // t = parameter in domain
...
int
ON_CurveProxy::IsPolyline(ON_SimpleArray<ON_3dPoint>* pline_points,
...
ON_BOOL32
ON_CurveProxy::IsPlanar(ON_Plane* plane, // if not NULL and true is returned, then plane
...
ON_BOOL32
ON_CurveProxy::IsInPlane(const ON_Plane& plane, // plane to test
                         double tolerance // tolerance to use when checking linearity
) const
{
...
ON_BOOL32
ON_CurveProxy::Trim(const ON_Interval& domain)
{
  bool rc = false;
...
ON_BOOL32
ON_CurveProxy::Split(double t, ON_Curve*& left_side, ON_Curve*& right_side) const
{
  ON_BOOL32 rc = false;
  if (m_this_domain.IsIncreasing() && m_real_curve_domain.IsIncreasing() &&
      m_this_domain.Includes(t, true)) {
    ...
    if (m_real_curve_domain.Includes(crv_t, true)) {
      ...
      if (left_real_dom.IsIncreasing() && right_real_dom.IsIncreasing() &&
          left_this_dom.IsIncreasing() && right_this_dom.IsIncreasing()) {
        ...
        if (!left_side)
          left_side = left_proxy;
        if (!right_side)
          right_side = right_proxy;
...
ON_BOOL32
ON_CurveProxy::GetCurveParameterFromNurbFormParameter(double nurbs_t,
                                                      double* curve_t) const
{
  ...
  if (m_real_curve) {
    ...
    rc = real_crv->GetCurveParameterFromNurbFormParameter(RealCurveParameter(nurbs_t),
                                                          curve_t);
    if (rc)
...
ON_BOOL32
ON_CurveProxy::GetNurbFormParameterFromCurveParameter(double curve_t,
                                                      double* nurbs_t) const
{
  ...
  if (m_real_curve) {
    ...
    rc = real_crv->GetNurbFormParameterFromCurveParameter(RealCurveParameter(curve_t),
                                                          nurbs_t);
    if (rc)
