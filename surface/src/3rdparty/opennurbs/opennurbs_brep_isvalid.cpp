    ...
    if (fli == 0) {
      ...
      if (loop.m_type != ON_BrepLoop::inner) {
        if (text_log) {
          ...
          text_log->PushIndent();
          text_log->Print (
              "brep.m_L[face.m_li[%d]=%d].m_type is not inner.\n", fli, li);
          text_log->PopIndent();
        }
        return false;
      }
...
ON_Brep::IsValidEdgeTopology (int edge_index, ON_TextLog* text_log) const
{
  ...
  if (0 == m_C3[c3i]) {
    if (text_log)
      text_log->Print (
          "ON_Brep.m_E[%d].m_c3i = %d, but m_C3[%d] is NULL.\n", edge_index, c3i, c3i);
    return false;
  }
  ...
  if (0 == edge.ProxyCurve()) {
    if (text_log)
      text_log->Print (
          "brep.m_E[%d].m_c3i = %d, but edge.ProxyCurve() is NULL.\n", edge_index, c3i);
    return false;
  }
  ...
  if (!c3_dom.Includes (proxy_sub_dom)) {
    if (text_log) {
      text_log->Print ("brep.m_E[%d] edge is not valid.\n", edge_index);
      text_log->PushIndent();
      text_log->Print ("m_C3[%d].Domain() = (%g,%g) does not inlclude "
                       "m_E[%d].ProxyCurveDomain() = (%g,%g) is not increasing\n",
                       c3i,
                       c3_dom[0],
                       c3_dom[1],
                       edge_index,
...
ON_Brep::IsValidLoopTopology (int loop_index, ON_TextLog* text_log) const
{
  ...
  for (lti = 0; lti < loop.m_ti.Count(); lti++) {
    ...
    if (trim.m_li != loop_index) {
      if (text_log) {
        text_log->Print (
            "brep loop m_L[%d] or trim m_T[%d] is not valid.\n", loop_index, ti);
        text_log->PushIndent();
...
ON_Brep::IsValidTrimTopology (int trim_index, ON_TextLog* text_log) const
{
  ...
  if (!c2_dom.Includes (proxy_sub_dom)) {
    if (text_log) {
      text_log->Print ("brep.m_T[%d] trim is not valid.\n", trim_index);
      text_log->PushIndent();
      text_log->Print ("m_C2[%d].Domain() = (%g,%g) does not include "
                       "m_T[%d].ProxyCurveDomain() = (%g,%g) is not increasing\n",
                       trim.m_c2i,
                       c2_dom[0],
                       c2_dom[1],
                       trim_index,
  ...
  if (!bFoundTrim) {
    if (text_log) {
      text_log->Print ("brep.m_T[%d] trim or brep.m_L[%d] loop is not valid.\n",
                       ...
                       trim.m_li);
      text_log->PushIndent();
      text_log->Print ("trim.m_li = %d but loop.m_ti[] does not contain %d (should "
                       "appear once in).\n",
                       trim.m_li,
                       trim_index);
      text_log->PopIndent();
  ...
  if (trim.m_type == ON_BrepTrim::singular) {
    ...
    if (trim.m_bRev3d) {
      if (text_log)
        text_log->Print (
            "ON_Brep.m_T[%d].m_type = singular, but m_bRev3d = %d (should be 0).\n",
            trim_index,
            trim.m_bRev3d);
      return false;
    }
    if (trim.m_vi[0] != trim.m_vi[1]) {
      if (text_log)
        text_log->Print (
            "ON_Brep.m_T[%d].m_type = singular, but m_vi = (%d,%d) (should "
            "be same vertex index).\n",
            trim_index,
            trim.m_vi[0],
            trim.m_vi[1]);
      return false;
