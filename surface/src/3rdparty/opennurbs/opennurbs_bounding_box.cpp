...
ON_3dPoint
ON_BoundingBox::Min() const
{
  return m_min;
}

ON_3dPoint
ON_BoundingBox::Max() const
{
  return m_max;
}

ON_3dVector
ON_BoundingBox::Diagonal() const
{
  return m_max - m_min;
}

ON_3dPoint
ON_BoundingBox::Center() const
{
...
double
ON_ClippingRegion::ClipPlaneTolerance() const
{
    ...
    *cv++ = w;
    ...
    if (some_out && !all_out) {
      ...
      while (i--) {
        ...
        y = xform[4] * cv[0] + xform[5] * cv[1] + xform[6] * cv[2] + xform[7] * cv[3];
        z = xform[8] * cv[0] + xform[9] * cv[1] + xform[10] * cv[2] + xform[11] * cv[3];
        w = xform[12] * cv[0] + xform[13] * cv[1] + xform[14] * cv[2] +
            xform[15] * cv[3];
        *cv++ = x;
        *cv++ = y;
        *cv++ = z;
...
ON_BoundingBox::IsPointIn (const ON_3dPoint& p, int bStrictlyIn) const
{
  bool bIn = false;
  if (bStrictlyIn) {
    bIn = m_min.x < p.x && p.x < m_max.x && m_min.y < p.y && p.y < m_max.y &&
...
static bool
Intersect (ON_Interval A, ON_Interval B, ON_Interval& AB);
...
ON_BoundingBox::IsDisjoint (const ON_BoundingBox& other_bbox) const
{
  ...
  if (m_min.y > m_max.y || other_bbox.m_min.y > other_bbox.m_max.y ||
      m_min.y > other_bbox.m_max.y || m_max.y < other_bbox.m_min.y) {
    return true;
  }
  if (m_min.z > m_max.z || other_bbox.m_min.z > other_bbox.m_max.z ||
      m_min.z > other_bbox.m_max.z || m_max.z < other_bbox.m_min.z) {
    return true;
  }
  return false;
}
...
double
ON_BoundingBox::Area() const
{
  double dx = m_max.x - m_min.x;
  double dy = m_max.y - m_min.y;
  double dz = m_max.z - m_min.z;
  return (dx >= 0.0 && dy >= 0.0 && dz >= 0.0) ? 2.0 * (dx * dy + dy * dz + dz * dx)
                                               : 0.0;
}
      ...
      if (is_rat) {
        ...
        if (xform) {
          for (/*empty*/; count--; points += stride) {
            ...
            if (bbox.m_min.y > P.y)
              bbox.m_min.y = P.y;
            else if (bbox.m_max.y < P.y)
              bbox.m_max.y = P.y;
            if (bbox.m_min.z > P.z)
              bbox.m_min.z = P.z;
            else if (bbox.m_max.z < P.z)
              bbox.m_max.z = P.z;
          }
          if (dim < 3) {
            for (i = dim; i < 3; i++) {
...
ON_BeyondSinglePrecision (const ON_BoundingBox& bbox, ON_Xform* xform)
{
  ...
  if (bbox.IsValid()) {
    ...
    bool bTooFar = (bbox.m_min.x >= too_far || bbox.m_min.y >= too_far ||
                    bbox.m_min.x >= too_far || bbox.m_max.x <= -too_far ||
                    bbox.m_max.y <= -too_far || bbox.m_max.x <= -too_far);
    bool bTooBig = (bbox.m_min.x <= -too_big || bbox.m_min.y <= -too_big ||
                    bbox.m_min.x <= -too_big || bbox.m_max.x >= too_big ||
                    bbox.m_max.y >= too_big || bbox.m_max.x >= too_big);
    if (bTooFar || bTooBig) {
      rc = true;
      if (0 != xform) {
...
static double
ON_BBoxMinimumDistanceToHelper (const ON_BoundingBox& bbox, ON_Line line)
{
  // 8 Feb 2005 - new function - not tested yet
