...
ON_BOOL32
ON_Torus::IsValid (ON_TextLog* text_log) const
{
  bool rc = false;
  if (minor_radius <= 0.0) {
    if (text_log)
...
ON_BOOL32
ON_Torus::Create (const ON_Circle& major__circle, double minor__radius)
{
...
ON_3dPoint
ON_Torus::PointAt (double major_angle_radians, double minor_angle_radians) const
{
  EVAL_SETUP_MINOR;
  EVAL_SETUP_MAJOR;
  return ((major_radius + cos_ma * minor_radius) * raxis +
          sin_ma * minor_radius * plane.zaxis + plane.origin);
}

ON_3dVector
ON_Torus::NormalAt (double major_angle_radians, double minor_angle_radians) const
{
  EVAL_SETUP_MINOR;
  EVAL_SETUP_MAJOR;
...
ON_Circle
ON_Torus::MajorCircleRadians (double minor_angle_radians) const
{
  EVAL_SETUP_MINOR;
...
ON_Circle
ON_Torus::MajorCircleDegrees (double minor_angle_degrees) const
{
  return MajorCircleRadians (minor_angle_degrees * ON_PI / 180.0);
}

ON_Circle
ON_Torus::MinorCircleRadians (double major_angle_radians) const
{
  EVAL_SETUP_MAJOR;
  ON_Circle c;
  c.plane.xaxis = raxis;
...
ON_Circle
ON_Torus::MinorCircleDegrees (double minor_angle_degrees) const
{
  return MinorCircleRadians (minor_angle_degrees * ON_PI / 180.0);
}

ON_3dPoint
ON_Torus::Center() const
{
  return plane.origin;
}
...
double
ON_Torus::MajorRadius() const
{
  return major_radius;
}

double
ON_Torus::MinorRadius() const
{
  return minor_radius;
}
...
ON_3dPoint
ON_Torus::ClosestPointTo (ON_3dPoint test_point) const
{
  const ON_Circle major_circle (plane, major_radius);
...
ON_BOOL32
ON_Torus::Rotate (double angle,           // angle in radians
                  const ON_3dVector& axis // axis of rotation
)
{
...
int
ON_Torus::GetNurbForm (ON_NurbsSurface& s) const
{
  int rc = 0;
...
ON_RevSurface*
ON_Torus::RevSurfaceForm (ON_RevSurface* srf) const
{
  if (srf)
    srf->Destroy();
