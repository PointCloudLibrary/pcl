...
ON_TextExtra*
ON_TextExtra::TextExtension(ON_TextEntity2* pText, bool bCreate)
{
  ON_TextExtra* pExtra = 0;
  if (pText) {
    pExtra = ON_TextExtra::Cast(
        pText->GetUserData(ON_TextExtra::m_ON_TextExtra_class_id.Uuid()));
    if (pExtra == 0 && bCreate) {
...
const ON_TextExtra*
ON_TextExtra::TextExtension(const ON_TextEntity2* pText, bool bCreate)
{
  return TextExtension((ON_TextEntity2*)pText, bCreate);
}

void
ON_TextExtra::SetDefaults()
{
...
void
ON_TextExtra::Dump(ON_TextLog&) const
{
  // do nothing
}

unsigned int
ON_TextExtra::SizeOf() const
{
...
ON_BOOL32
ON_TextExtra::Write(ON_BinaryArchive& archive) const
{
  bool rc = archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK, 1, 0);

  if (rc)
    rc = archive.WriteUuid(m_parent_uuid);
  if (rc)
    rc = archive.WriteBool(m_bDrawMask);
  if (rc)
    rc = archive.WriteInt(m_color_source);
  if (rc)
    rc = archive.WriteColor(m_mask_color);
  if (rc)
    rc = archive.WriteDouble(m_border_offset);
...
ON_BOOL32
ON_TextExtra::Read(ON_BinaryArchive& archive)
{
  int major_version = 1;
  int minor_version = 0;
  bool rc =
  ...
  if (rc)
    rc = archive.ReadUuid(m_parent_uuid);
  if (rc)
    rc = archive.ReadBool(&m_bDrawMask);
  if (rc)
    rc = archive.ReadInt(&m_color_source);
  if (rc)
    rc = archive.ReadColor(m_mask_color);
  if (rc)
    rc = archive.ReadDouble(&m_border_offset);
...
ON_BOOL32
ON_TextExtra::GetDescription(ON_wString& description)
{
...
bool
ON_TextExtra::DrawTextMask() const
{
  return m_bDrawMask;
}

void
ON_TextExtra::SetDrawTextMask(bool bDraw)
{
  m_bDrawMask = bDraw;
}

int
ON_TextExtra::MaskColorSource() const
{
  return m_color_source;
}

void
ON_TextExtra::SetMaskColorSource(int source)
{
  if (source == 1)
    m_color_source = 1;
  else
...
ON_Color
ON_TextExtra::MaskColor() const
{
  return m_mask_color;
}

void
ON_TextExtra::SetMaskColor(ON_Color color)
{
  m_mask_color = color;
}

double
ON_TextExtra::MaskOffsetFactor() const
{
  return m_border_offset;
}

void
ON_TextExtra::SetMaskOffsetFactor(double offset)
{
...
AnnotationExtension (ON_Annotation2* pDim, bool bCreate)
{
  ON_DimensionExtra* pExtra = 0;
  if (pDim) {
    pExtra = ON_DimensionExtra::Cast(
        pDim->GetUserData(ON_DimensionExtra::m_ON_DimensionExtra_class_id.Uuid()));
    if (pExtra == 0 && bCreate) {
...
ON_DimensionExtra*
ON_DimensionExtra::DimensionExtension(ON_LinearDimension2* pDim, bool bCreate)
{
  return AnnotationExtension((ON_Annotation2*)pDim, bCreate);
}

const ON_DimensionExtra*
ON_DimensionExtra::DimensionExtension(const ON_LinearDimension2* pDim, bool bCreate)
{
  return DimensionExtension((ON_LinearDimension2*)pDim, bCreate);
}

ON_DimensionExtra*
ON_DimensionExtra::DimensionExtension(ON_RadialDimension2* pDim, bool bCreate)
{
  return AnnotationExtension((ON_Annotation2*)pDim, bCreate);
}

const ON_DimensionExtra*
ON_DimensionExtra::DimensionExtension(const ON_RadialDimension2* pDim, bool bCreate)
{
  return DimensionExtension((ON_RadialDimension2*)pDim, bCreate);
}

ON_DimensionExtra*
ON_DimensionExtra::DimensionExtension(ON_OrdinateDimension2* pDim, bool bCreate)
{
  return AnnotationExtension((ON_Annotation2*)pDim, bCreate);
}

const ON_DimensionExtra*
ON_DimensionExtra::DimensionExtension(const ON_OrdinateDimension2* pDim, bool bCreate)
{
  return DimensionExtension((ON_OrdinateDimension2*)pDim, bCreate);
}

void
ON_DimensionExtra::SetDefaults()
{
...
void
ON_DimensionExtra::Dump(ON_TextLog&) const
{
  // do nothing
}

unsigned int
ON_DimensionExtra::SizeOf() const
{
  unsigned int sz = ON_UserData::SizeOf();
  sz += sizeof(*this) - sizeof(ON_UserData);
  return sz;
...
ON_BOOL32
ON_DimensionExtra::Write(ON_BinaryArchive& archive) const
{
  int major_version = 1;
  int minor_version = 1;
  bool rc =
      archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK, major_version, minor_version);

  if (rc)
    rc = archive.WriteUuid(m_partent_uuid);
  if (rc)
    rc = archive.WriteInt(m_arrow_position);
  if (rc) {
    if (m_text_rects) {
  ...
  if (rc)
    rc = archive.WriteDouble(m_distance_scale);
...
ON_BOOL32
ON_DimensionExtra::Read(ON_BinaryArchive& archive)
{
  int major_version = 1;
  int minor_version = 0;
  bool rc =
  ...
  if (rc)
    rc = archive.ReadUuid(m_partent_uuid);
  if (rc)
    rc = archive.ReadInt(&m_arrow_position);

  int rect_count = 0;
  if (rc)
    rc = archive.ReadInt(&rect_count);
  if (rc && rect_count)
  ...
  if (minor_version > 0) {
    if (rc)
      rc = archive.ReadDouble(&m_distance_scale);
  }
...
ON_BOOL32
ON_DimensionExtra::GetDescription(ON_wString& description)
{
...
ON_BOOL32
ON_DimensionExtra::Archive() const
{
  // true to write to file
  return true;
}
...
int
ON_DimensionExtra::ArrowPosition() const
{
  return m_arrow_position;
}

void
ON_DimensionExtra::SetArrowPosition(int position)
{
  if (position > 0)
    m_arrow_position = 1;
  else if (position < 0)
...
double
ON_DimensionExtra::DistanceScale() const
{
  return m_distance_scale;
}

void
ON_DimensionExtra::SetDistanceScale(double s)
{
  m_distance_scale = s;
}

void
ON_DimensionExtra::SetModelSpaceBasePoint(ON_3dPoint basepoint)
{
  m_modelspace_basepoint = basepoint;
}

ON_3dPoint
ON_DimensionExtra::ModelSpaceBasePoint() const
{
...
int
ON_Annotation2::Index() const
{
  return m_index;
}

void
ON_Annotation2::SetIndex(int index)
{
  m_index = index;
}

void
ON_Annotation2::Create()
{
...
void
ON_Annotation2::EmergencyDestroy()
{
  m_points.EmergencyDestroy();
  m_usertext.EmergencyDestroy();
}

ON_Annotation2::ON_Annotation2() { Create(); }

ON_Annotation2::~ON_Annotation2() { Destroy(); }

bool
ON_Annotation2::EvaluatePoint(const ON_ObjRef& objref, ON_3dPoint& P) const
{
  bool rc = false;
...
ON_Annotation2&
ON_Annotation2::operator=(const ON_Annotation& src)
{
...
ON_BOOL32
ON_Annotation2::IsValid(ON_TextLog* text_log) const
{
  if (!m_plane.IsValid()) {
    if (text_log) {
...
*/
static bool
WriteAnnotation2UserText_V4 (ON_BinaryArchive& file, const ON_wString& s)
{
  bool rc;
  ON_wString s4;
  int len = s.Length();
...
static bool
WriteAnnotation2UserText_V5 (ON_BinaryArchive& file, const ON_wString& s)
{
  bool rc;
...
ON_BOOL32
ON_Annotation2::Write(ON_BinaryArchive& file) const
{
  int i;
  bool rc = false;
  ...
  while (rc) {
    ...
    // 16 Nov, 2011 - Lowell - Change text to bottom left justified for pre-v5 files
    // rr94270 This stuff is moved to CRhinoDoc::Write3DMHelper() so it will help with
...
ON_BOOL32
ON_Annotation2::Read(ON_BinaryArchive& file)
{
  ...
  bool bInChunk =
      (file.Archive3dmVersion() >= 5 && file.ArchiveOpenNURBSVersion() >= 200710180);
...
ON::object_type
ON_Annotation2::ObjectType() const
{
  return ON::annotation_object;
}

int
ON_Annotation2::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_Annotation2::Transform(const ON_Xform& xform)
{
...
int
ON_Plane_Repair (ON_Plane& plane)
{
  int rc;
  if (plane.IsValid()) {
    rc = 1;
...
ON_BOOL32
ON_RadialDimension2::Transform(const ON_Xform& xform)
{
...
ON_BOOL32
ON_Leader2::Transform(const ON_Xform& xform)
{
...
ON_BOOL32
ON_AngularDimension2::Transform(const ON_Xform& xform)
{
      ...
      if (rc) {
        if (bUpdatePoints) {
          ...
          if (rc) {
            ...
            m_points[end_pt_index].Set(m_radius * cos(m_angle),
                                       m_radius * sin(m_angle));
            m_points[arc_pt_index].Set(m_radius * cos(arc_pt_angle),
                                       m_radius * sin(arc_pt_angle));
            if (m_userpositionedtext) {
              ...
              m_points[userpositionedtext_pt_index].Set(m_radius * cos(0.5 * m_angle),
                                                        m_radius * sin(0.5 * m_angle));
            }
          }
        }
        else {
...
bool
ON_Annotation2::IsText() const
{
  return (ON::dtTextBlock == m_type);
}

bool
ON_Annotation2::IsLeader() const
{
  return (ON::dtLeader == m_type);
}

bool
ON_Annotation2::IsDimension() const
{
  return (ON::dtTextBlock != m_type && ON::dtLeader != m_type);
}

double
ON_Annotation2::NumericValue() const
{
  return ON_UNSET_VALUE;
}

void
ON_Annotation2::SetHeight(double ht)
{
  if (ht > ON_SQRT_EPSILON)
    m_textheight = ht;
}

double
ON_Annotation2::Height() const
{
  return m_textheight;
}

void
ON_Annotation2::SetType(ON::eAnnotationType type)
{
  m_type = type;
...
ON::eAnnotationType
ON_Annotation2::Type() const
{
  return m_type;
}

void
ON_Annotation2::SetPlane(const ON_Plane& plane)
{
  m_plane = plane;
}

const ON_Plane&
ON_Annotation2::Plane() const
{
  return m_plane;
}

void
ON_Annotation2::SetPointCount(int count)
{
...
int
ON_Annotation2::PointCount() const
{
  return m_points.Count();
}

void
ON_Annotation2::SetPoints(const ON_2dPointArray& points)
{
  m_points = points;
}

const ON_2dPointArray&
ON_Annotation2::Points() const
{
  return m_points;
}

void
ON_Annotation2::SetPoint(int idx, const ON_2dPoint& point)
{
  if (idx >= 0) {
...
ON_2dPoint
ON_Annotation2::Point(int idx) const
{
  return (idx >= 0 && idx < m_points.Count()) ? m_points[idx] : ON_2dPoint(0.0, 0.0);
}

void
ON_Annotation2::SetUserText(const wchar_t* text_value)
...
void
ON_Annotation2::SetUserPositionedText(int bMoved)
{
  m_userpositionedtext = bMoved ? true : false;
}
bool
ON_Annotation2::UserPositionedText() const
{
...
void
ON_Annotation2::ReservePoints(int count)
{
...
const wchar_t*
ON_Annotation2::DefaultText()
{
  return L"";
}

void
ON_Annotation2::SetTextDisplayMode(ON::eTextDisplayMode mode)
{
  m_textdisplaymode = mode;
}

ON::eTextDisplayMode
ON_Annotation2::TextDisplayMode() const
{
  return m_textdisplaymode;
}

void
ON_Annotation2::ConvertBack(ON_Annotation& target)
{
  target.SetType(Type());
...
void
ON_Annotation2::SetJustification(unsigned int justification)
{
...
ON_BOOL32
ON_LinearDimension2::IsValid(ON_TextLog* text_log) const
{
...
ON_BOOL32
ON_LinearDimension2::Write(ON_BinaryArchive& archive) const
{
  // 18 October 2007 Dale Lear
...
ON_BOOL32
ON_LinearDimension2::Read(ON_BinaryArchive& archive)
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
  //    add IO support for information specific to ON_LinearDimension2
...
ON_BOOL32
ON_LinearDimension2::GetBBox(double* boxmax, double* boxmin, ON_BOOL32 bGrowBox) const
{
  ON_BoundingBox bbox;
  if (bGrowBox) {
...
ON_LinearDimension2::Repair()
{
  ...
  const int userpositionedtext_pt_index =
      ON_LinearDimension2::userpositionedtext_pt_index;
  const int dim_pt_count = ON_LinearDimension2::dim_pt_count;

  int rc = 0;
  if (m_points.Count() >= dim_pt_count && m_points[ext0_pt_index].IsValid() &&
    ...
    if (ON::dtDimAligned == m_type &&
        (m_points[ext1_pt_index].x < 0.0 || m_points[ext1_pt_index].y != 0.0)) {
      rc = 2;
    ...
    if (!ON_IsValid(m_points[arrow0_pt_index].y)) {
      ...
      if (!ON_IsValid(m_points[arrow1_pt_index].y))
        m_points[arrow1_pt_index].y =
            0.5 * (m_points[ext0_pt_index].y + m_points[ext1_pt_index].y);
    ...
    if (!m_userpositionedtext) {
      if (m_points[userpositionedtext_pt_index].y != m_points[arrow0_pt_index].y ||
          m_points[userpositionedtext_pt_index].x !=
              0.5 * (m_points[arrow0_pt_index].x + m_points[arrow1_pt_index].x)) {
        rc = 2;
...
ON_BOOL32
ON_LinearDimension2::Transform(const ON_Xform& xform)
{
  // Dale Lear - this override fixes RR 11114 by correctly
  //             handling non uniform scaling.
  bool rc = xform.IsIdentity();
...
double
ON_LinearDimension2::NumericValue() const
{
...
int
ON_LinearDimension2::StyleIndex() const
{
  return ON_Annotation2::Index();
}

void
ON_LinearDimension2::SetStyleIndex(int i)
{
...
ON_2dPoint
ON_LinearDimension2::Dim2dPoint(int point_index) const
{
  ON_2dPoint p2;
  if (m_points.Count() < dim_pt_count) {
...
ON_3dPoint
ON_LinearDimension2::Dim3dPoint(int point_index) const
{
...
void
ON_LinearDimension2::GetV2Form(ON_LinearDimension& dim)
{
  ON_Annotation2::ConvertBack(dim);
}

bool
ON_LinearDimension2::CreateFromV2(const ON_Annotation& v2_ann,
...
int
ON_LinearDimension2::GetDimensionLineSegments(ON_RECT gdi_text_rect,
                                              int gdi_height_of_I,
                                              ...
                                              bool& bInside) const
{
  ...
  int i = (m_points[ext0_pt_index].x <= m_points[ext1_pt_index].x) ? ext0_pt_index
                                                                   : ext1_pt_index;
  ...
  if (ForceArrows == -1 || // force outside
      (dimwidth < mindimwidth &&
       ForceArrows != 1)) // arrowheads have to be "outside" - they won't fit inside
                          // even without text
  {
  ...
  // Horizontal text or Userpositioned text
    ...
    ON_3dVector vp_zaxis =
        (ON::dtHorizontal == textdisplay && vp) ? vp->CameraZ() : m_plane.zaxis;
    ...
    ray.from = gdi_to_world * R;
    ray.to = ray.from + vp_zaxis;
    ...
    for (i = 0; i < 4; i++) {
      ...
      y1 = corners[(i + 1) % 4].y -
           m_points[1].y; // vertical dist from next corner to dimension line
    ...
    // Nov 4 2009 - Lowell - Added test for no intersection of text with dimension line
    // rr33003 Important for user positioned text that's moved out of the way of the
    // dimension line
...
const wchar_t*
ON_LinearDimension2::DefaultText()
{
  return L"<>";
}
...
ON_BOOL32
ON_RadialDimension2::IsValid(ON_TextLog* text_log) const
{
  ...
  if (4 != m_points.Count()) {
    if (text_log) {
      text_log->Print(
          "ON_RadialDimension2 - m_points.Count() = %d (should be 4 or 5)\n",
          m_points.Count());
    }
    return false;
  }
...
ON_BOOL32
ON_RadialDimension2::Write(ON_BinaryArchive& archive) const
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
...
ON_BOOL32
ON_RadialDimension2::Read(ON_BinaryArchive& archive)
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
  //    add IO support for information specific to ON_RadialDimension2
...
ON_BOOL32
ON_RadialDimension2::GetBBox(double* boxmax, double* boxmin, ON_BOOL32 bGrowBox) const
{
  ON_BoundingBox bbox;
  if (bGrowBox) {
    bbox.m_min.x = boxmin[0];
...
ON_2dPoint
ON_RadialDimension2::Dim2dPoint(int point_index) const
{
  ON_2dPoint p2;
...
ON_3dPoint
ON_RadialDimension2::Dim3dPoint(int point_index) const
{
  ON_2dPoint p2 = Dim2dPoint(point_index);
  return (ON_UNSET_VALUE == p2.x) ? ON_UNSET_POINT : m_plane.PointAt(p2.x, p2.y);
}
                                      ...
                                      double offset_distance)
{
  ...
  bool rc = false;
  if (center.IsValid() && arrowtip.IsValid() && normal.IsValid() && !normal.IsZero() &&
      xaxis.IsValid() && !xaxis.IsZero()) {
...
double
ON_RadialDimension2::NumericValue() const
{
  double d = 0.0;
  if (m_points.Count() >= dim_pt_count) {
...
int
ON_RadialDimension2::StyleIndex() const
{
  return ON_Annotation2::Index();
}

void
ON_RadialDimension2::SetStyleIndex(int i)
{
  ON_Annotation2::SetIndex(i);
}

const wchar_t*
ON_RadialDimension2::DefaultRadiusText()
{
...
const wchar_t*
ON_RadialDimension2::DefaultDiameterText()
{
...
// 6-23-03 lw Added v2 file writing of annotation
void
ON_RadialDimension2::GetV2Form(ON_RadialDimension& dim)
{
  ON_Annotation2::ConvertBack(dim);
}

bool
ON_RadialDimension2::CreateFromV2(const ON_Annotation& v2_ann,
                                  const ON_3dmAnnotationSettings& settings,
                                  int dimstyle_index)
{
  bool rc = false;
  if (ON::dtDimRadius == v2_ann.m_type || ON::dtDimDiameter == v2_ann.m_type) {
    ...
    if (points.Count() >= 4) {
      ...
      m_userpositionedtext = false;
      m_type = v2_ann.Type();
      m_textdisplaymode = (2 == settings.m_textalign) ? ON::dtHorizontal : ON::dtInLine;
      m_index = dimstyle_index;
      ON_2dVector v = m_points[0];
      if (!v.IsZero()) {
        m_plane.origin = m_plane.PointAt(v.x, v.y);
...
public:
  static ON_AngularDimension2Extra*
  AngularDimensionExtra (ON_AngularDimension2* pDim /*, bool bCreate*/);
  static const ON_AngularDimension2Extra*
  AngularDimensionExtra (const ON_AngularDimension2* pDim /*, bool bCreate*/);
...
ON_AngularDimension2Extra*
ON_AngularDimension2Extra::AngularDimensionExtra(ON_AngularDimension2* pDim)
{
  ON_AngularDimension2Extra* pExtra = 0;
  if (pDim) {
    pExtra = ON_AngularDimension2Extra::Cast(pDim->GetUserData(
        ON_AngularDimension2Extra::m_ON_AngularDimension2Extra_class_id.Uuid()));
    if (pExtra == 0) {
...
const ON_AngularDimension2Extra*
ON_AngularDimension2Extra::AngularDimensionExtra(const ON_AngularDimension2* pDim)
{
...
void
ON_AngularDimension2Extra::Dump(ON_TextLog&) const
{
  // do nothing
}

unsigned int
ON_AngularDimension2Extra::SizeOf() const
{
  unsigned int sz = ON_UserData::SizeOf();
  sz += sizeof(*this) - sizeof(ON_UserData);
  return sz;
...
ON_BOOL32
ON_AngularDimension2Extra::Write(ON_BinaryArchive& archive) const
{
  int major_version = 1;
  int minor_version = 0;
  bool rc =
  ...
  if (rc)
    rc = archive.WriteDouble(m_dimpoint_offset[0]);
  if (rc)
    rc = archive.WriteDouble(m_dimpoint_offset[1]);
...
ON_BOOL32
ON_AngularDimension2Extra::Read(ON_BinaryArchive& archive)
{
  int major_version = 1;
  int minor_version = 0;
  bool rc =
  ...
  if (rc)
    rc = archive.ReadDouble(&m_dimpoint_offset[0]);
  if (rc)
    rc = archive.ReadDouble(&m_dimpoint_offset[1]);
...
ON_BOOL32
ON_AngularDimension2Extra::GetDescription(ON_wString& description)
{
...
ON_BOOL32
ON_AngularDimension2Extra::Archive() const
{
  // true to write to file
  return true;
}
...
void
ON_AngularDimension2Extra::Scale(double scale)
{
...
double
ON_AngularDimension2Extra::DimpointOffset(int index) const
{
  if (index == 0)
...
void
ON_AngularDimension2Extra::SetDimpointOffset(int index, double offset)
{
  if (index >= 0 && index <= 1)
...
ON_AngularDimension2::ON_AngularDimension2() : m_angle(0.0), m_radius(1.0)
{
  ...
  //  ON_AngularDimension2Extra::AngularDimensionExtra(this, true); if(pDE)
  //  {
...
ON_BOOL32
ON_AngularDimension2::IsValid(ON_TextLog* text_log) const
{
  ...
  if (0.0 == m_points[1].x && 0.0 == m_points[1].y) {
    if (text_log) {
      text_log->Print("ON_AngularDimension2 - angle dim m_points[1] = center (should "
                      "be on start ray).\n");
    }
    return false;
  }
  ...
  if (0.0 == m_points[2].x && 0.0 == m_points[2].y) {
    if (text_log) {
      text_log->Print("ON_AngularDimension2 - angle dim m_points[2] = center (should "
                      "be on end ray).\n");
    }
    return false;
  }
  ...
  if (0.0 == m_points[3].x && 0.0 == m_points[3].y) {
    if (text_log) {
      text_log->Print("ON_AngularDimension2 - angle dim m_points[3] = center (should "
                      "be on interior of arc).\n");
    }
    return false;
  }
  ...
  while (
      a3 <
      a1) // Oct 23 2009 LW changed from a3 <= a1 to allow point at end of arc rr53634
    a3 += 2.0 * ON_PI;
  ...
  if (a3 >
      a2) // Oct 23 2009 LW changed from a3 >= a2 to allow point at end of arc rr53634
  {
    if (text_log) {
      text_log->Print(
...
ON_BOOL32
ON_AngularDimension2::GetBBox(double* boxmax, double* boxmin, ON_BOOL32 bGrowBox) const
{
  ON_BoundingBox bbox;
  if (bGrowBox) {
    bbox.m_min.x = boxmin[0];
...
ON_2dPoint
ON_AngularDimension2::Dim2dPoint(int point_index) const
{
  ON_2dPoint p2;
  if (m_points.Count() < dim_pt_count || point_index < 0) {
    p2.x = p2.y = ON_UNSET_VALUE;
    ...
    if (text_pivot_pt == point_index) {
      point_index = m_userpositionedtext ? userpositionedtext_pt_index : arcmid_pt;
    }
...
ON_3dPoint
ON_AngularDimension2::Dim3dPoint(int point_index) const
{
  ON_2dPoint p2 = Dim2dPoint(point_index);
  return (ON_UNSET_VALUE == p2.x) ? ON_UNSET_POINT : m_plane.PointAt(p2.x, p2.y);
}
...
ON_BOOL32
ON_AngularDimension2::Write(ON_BinaryArchive& file) const
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
  //    add IO support for information specific to ON_LinearDimension2
...
ON_BOOL32
ON_AngularDimension2::Read(ON_BinaryArchive& file)
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
...
static ON_BOOL32
VectorAngle (const ON_2dVector& v, double& angle)
{
  if (v.IsTiny())
    return false;
...
bool
ON_AngularDimension2::CreateFromV2(const ON_Annotation& v2_ann,
                                   const ON_3dmAnnotationSettings& settings,
                                   int dimstyle_index)
{
...
bool
ON_AngularDimension2::CreateFromArc(const ON_Arc& arc)
{
...
bool
ON_AngularDimension2::GetArc(ON_Arc& arc) const
{
  bool rc = false;
  // Jan 25, 2007 - changed min radius from >0 to >ON_SQRT_EPSILON
  // to avoid domain problems trying to use very small arcs later
  if (ON_IsValid(m_radius) && m_radius > ON_SQRT_EPSILON && ON_IsValid(m_angle) &&
      m_angle > 0.0 && m_angle <= 2.0 * ON_PI && m_plane.origin.IsValid() &&
      m_plane.xaxis.IsValid() && m_plane.yaxis.IsValid() && m_plane.zaxis.IsValid() &&
      fabs(m_plane.zaxis.Length() - 1.0) <= ON_SQRT_EPSILON && 4 == m_points.Count()) {
    ON_3dVector X =
        m_plane.PointAt(m_points[start_pt_index].x, m_points[start_pt_index].y) -
        m_plane.origin;
...
bool
ON_AngularDimension2::GetExtensionLines(ON_Line extensions[2]) const
{
  bool rc = false;
  ...
  if (ON_IsValid(m_radius) && m_radius > ON_SQRT_EPSILON && ON_IsValid(m_angle) &&
      ...
      m_plane.xaxis.IsValid() && m_plane.yaxis.IsValid() && m_plane.zaxis.IsValid() &&
      fabs(m_plane.zaxis.Length() - 1.0) <= ON_SQRT_EPSILON && 4 == m_points.Count()) {
    const ON_AngularDimension2Extra* pDE =
        ON_AngularDimension2Extra::AngularDimensionExtra(this);
    if (pDE != 0) {
...
void
ON_AngularDimension2::SetAngle(double angle)
{
  m_angle = angle;
}

double
ON_AngularDimension2::Angle() const
{
  return m_angle;
}

void
ON_AngularDimension2::SetRadius(double radius)
{
  m_radius = radius;
}

double
ON_AngularDimension2::Radius() const
{
  return m_radius;
}

double
ON_AngularDimension2::NumericValue() const
{
...
int
ON_AngularDimension2::StyleIndex() const
{
  return ON_Annotation2::Index();
}

void
ON_AngularDimension2::SetStyleIndex(int i)
{
  ON_Annotation2::SetIndex(i);
}

const wchar_t*
ON_AngularDimension2::DefaultText()
{
  return L"<>"; // Aug 31, 2009 - Lowell
}

void
ON_AngularDimension2::ConvertBack(
    ON_AngularDimension2& // target - formal parameter intentionally ignored in this
                          // virtual function
)
{}
...
// 6-23-03 lw Added v2 file writing of annotation
void
ON_AngularDimension2::GetV2Form(ON_AngularDimension& dim)
{
  ON_Annotation2::ConvertBack(dim);
...
static void
OrientRectHelper (ON_2dVector corners[4])
{
...
int
ON_AngularDimension2::GetDimensionArcSegments(ON_RECT gdi_text_rect,
                                              int gdi_height_of_I,
                                              ON_Xform gdi_to_world,
                                              const ON_DimStyle& dimstyle,
                                              double dimscale,
                                              ...
                                              bool& bInside) const
{
  ...
  if (0 == gdi_height_of_I) {
    // Default to height of Ariel 'I' (this code still works if
    // ON_Font::normal_font_height != 256)
    gdi_height_of_I = (165 * ON_Font::normal_font_height) / 256;
  }
  ...
  if (m_radius <= arrowwidth + tailwidth ||
      m_radius * (a1 - a0) < 2.0 * (arrowwidth) + tailwidth) {
  ...
  if ((ON::dtHorizontal == textdisplay && vp) || m_userpositionedtext) {
    ...
    ray.to.x = corners[3].x / m_radius;
    ray.to.y = corners[3].y / m_radius;
    ray.to.z = ray.from.z = 0.0;
    for (i = 0; i < 4; i++) {
      r0 = r1;
      r1 = corners[i].Length() - m_radius;
      ray.from = ray.to;
      ray.to.x = corners[i].x / m_radius;
      ray.to.y = corners[i].y / m_radius;
      if (r0 < 0.0 && r1 < 0.0) {
    ...
    if (ON_UNSET_VALUE != aa0 && ON_UNSET_VALUE != aa1 && a0 <= aa0 && aa0 < aa1 &&
        aa1 <= a1) {
      t = arrow_angle;
...
double
ON_AngularDimension2::DimpointOffset(int index) const
{
  const ON_AngularDimension2Extra* pDE =
      ON_AngularDimension2Extra::AngularDimensionExtra(this);
  if (pDE != 0)
...
void
ON_AngularDimension2::SetDimpointOffset(int index, double offset)
{
  ON_AngularDimension2Extra* pDE =
      ON_AngularDimension2Extra::AngularDimensionExtra(this);
  if (pDE != 0)
...
ON_BOOL32
ON_OrdinateDimension2::Transform(const ON_Xform& xform)
{
  bool rc = xform.IsIdentity();
  if (!rc) {
    return ON_Annotation2::Transform(xform);
...
ON_2dPoint
ON_OrdinateDimension2::Dim2dPoint(int point_index, double default_offset) const
{
  ...
  if (point_index >= 0 && point_index < dim_pt_count &&
      m_points.Count() == dim_pt_count) {
    p2 = m_points[point_index];
  }
...
ON_3dPoint
ON_OrdinateDimension2::Dim3dPoint(int point_index, double default_offset) const
{
...
ON_BOOL32
ON_OrdinateDimension2::IsValid(ON_TextLog* text_log) const
{
...
ON_BOOL32
ON_OrdinateDimension2::Write(ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_OrdinateDimension2::Read(ON_BinaryArchive& file)
{
...
double
ON_OrdinateDimension2::NumericValue() const
{
  if (m_direction == 0)
...
int
ON_OrdinateDimension2::StyleIndex() const
{
  return ON_Annotation2::Index();
}

void
ON_OrdinateDimension2::SetStyleIndex(int i)
{
  ON_Annotation2::SetIndex(i);
}

ON_BOOL32
ON_OrdinateDimension2::GetBBox(double* boxmin, double* boxmax, ON_BOOL32 bGrowBox) const
{
  ON_BoundingBox bbox;
  if (bGrowBox) {
    bbox.m_min.x = boxmin[0];
...
int
ON_OrdinateDimension2::ImpliedDirection() const
{
  int direction = -1;
...
int
ON_OrdinateDimension2::Direction() const
{
  return m_direction;
}

void
ON_OrdinateDimension2::SetDirection(int direction)
{
  m_direction = direction;
}

const wchar_t*
ON_OrdinateDimension2::DefaultText()
{
  return L"<>";
}

double
ON_OrdinateDimension2::KinkOffset(int index) const
{
  if (index == 0)
...
void
ON_OrdinateDimension2::SetKinkOffset(int index, double offset)
{
  if (index == 0)
...
ON_BOOL32
ON_TextEntity2::IsValid(ON_TextLog* text_log) const
{
...
ON_BOOL32
ON_TextEntity2::Write(ON_BinaryArchive& archive) const
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
...
ON_BOOL32
ON_TextEntity2::Read(ON_BinaryArchive& archive)
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
  //    add IO support for information specific to ON_TextEntity2
...
ON_BOOL32
ON_TextEntity2::GetBBox(double* boxmax, double* boxmin, ON_BOOL32 bGrowBox) const
{
  ON_BoundingBox bbox;
  if (bGrowBox) {
    bbox.m_min.x = boxmin[0];
...
int
ON_TextEntity2::FontIndex() const
{
  return m_index;
}

void
ON_TextEntity2::SetFontIndex(int i)
{
  m_index = i;
}

ON_BOOL32
ON_TextEntity2::Transform(const ON_Xform& xform)
{
  // Dale Lear - this override fixes RR 11114 by correctly
  //             handling non uniform scaling.
  bool rc = xform.IsIdentity();
...
// 6-23-03 lw Added v2 file writing of annotation
void
ON_TextEntity2::GetV2Form(ON_TextEntity& text)
{
...
void
ON_TextEntity2::SetJustification(unsigned int justification)
{
  m_justification = justification;
}

unsigned int
ON_TextEntity2::Justification()
{
  return m_justification;
}

bool
ON_TextEntity2::DrawTextMask() const
{
...
void
ON_TextEntity2::SetDrawTextMask(bool bDraw)
{
...
int
ON_TextEntity2::MaskColorSource() const
{
  const ON_TextExtra* pTE = ON_TextExtra::TextExtension(this, false);
  if (pTE)
...
void
ON_TextEntity2::SetMaskColorSource(int source)
{
  ON_TextExtra* pTE = ON_TextExtra::TextExtension(this, true);
  if (pTE)
...
ON_Color
ON_TextEntity2::MaskColor() const
{
  const ON_TextExtra* pTE = ON_TextExtra::TextExtension(this, false);
  if (pTE)
...
void
ON_TextEntity2::SetMaskColor(ON_Color color)
{
  ON_TextExtra* pTE = ON_TextExtra::TextExtension(this, true);
  if (pTE)
...
double
ON_TextEntity2::MaskOffsetFactor() const
{
  const ON_TextExtra* pTE = ON_TextExtra::TextExtension(this, false);
  if (pTE)
...
void
ON_TextEntity2::SetMaskOffsetFactor(double offset)
{
  ON_TextExtra* pTE = ON_TextExtra::TextExtension(this, true);
  if (pTE)
...
bool
ON_TextEntity2::AnnotativeScaling() const
{
  return m_annotative_scale;
}

void
ON_TextEntity2::SetAnnotativeScaling(bool b)
{
...
ON_BOOL32
ON_Leader2::IsValid(ON_TextLog* text_log) const
{
...
ON_BOOL32
ON_Leader2::Write(ON_BinaryArchive& archive) const
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
...
ON_BOOL32
ON_Leader2::Read(ON_BinaryArchive& archive)
{
  // 18 October 2007 Dale Lear
  //    I added the chunk wrapping so V5 and future versions can
  //    add IO support for information specific to ON_Leader2
...
ON_BOOL32
ON_Leader2::GetBBox(double* boxmax, double* boxmin, ON_BOOL32 bGrowBox) const
{
  ON_BoundingBox bbox;
  if (bGrowBox) {
    bbox.m_min.x = boxmin[0];
...
ON_2dPoint
ON_Leader2::Dim2dPoint(int point_index) const
{
  ON_2dPoint p2;
...
ON_3dPoint
ON_Leader2::Dim3dPoint(int point_index) const
{
  ON_2dPoint p2 = Dim2dPoint(point_index);
  return (ON_UNSET_VALUE == p2.x) ? ON_UNSET_POINT : m_plane.PointAt(p2.x, p2.y);
}
...
void
ON_Leader2::AddPoint(const ON_2dPoint& point)
{
  m_points.Append(point);
}

bool
ON_Leader2::RemovePoint(int idx)
{
  bool rc = true;
...
// April 22, 2010 Lowell - Added to support right justified text on left pointing leader
// tails rr64292
bool
ON_Leader2::GetTextDirection(ON_2dVector& text_dir) const
{
  bool rc = false;
  const int point_count = m_points.Count();
  if (point_count < 2) {
...
bool
ON_Leader2::GetArrowHeadDirection(ON_2dVector& arrowhead_dir) const
{
  bool rc = false;
  const int point_count = m_points.Count();
  if (point_count < 2) {
...
bool
ON_Leader2::GetArrowHeadTip(ON_2dPoint& arrowhead_tip) const
{
  bool rc = false;
...
bool
ON_RadialDimension2::GetArrowHeadDirection(ON_2dVector& arrowhead_dir) const
{
  bool rc = false;
  if (m_points.Count() < 4) {
    arrowhead_dir.Set(-1.0, 0.0);
...
bool
ON_RadialDimension2::GetArrowHeadTip(ON_2dPoint& arrowhead_tip) const
{
  bool rc = false;
  if (m_points.Count() >= 2) {
...
// 6-23-03 lw Added v2 file writing of annotation
void
ON_Leader2::GetV2Form(ON_Leader& leader)
{
...
ON_BOOL32
ON_TextDot::IsValid(ON_TextLog* text_log) const
{
...
void
ON_TextDot::Dump(ON_TextLog& log) const
{
...
ON_BOOL32
ON_TextDot::Write(ON_BinaryArchive& file) const
{
  ON_BOOL32 rc = file.Write3dmChunkVersion(1, 0);
  if (rc)
    rc = file.WritePoint(m_point);
  if (rc)
    rc = file.WriteInt(m_height);
  if (rc)
    rc = file.WriteString(m_text);
  if (rc)
    rc = file.WriteString(m_fontface);
  if (rc)
    rc = file.WriteInt(m_display);

  return rc;
}

ON_BOOL32
ON_TextDot::Read(ON_BinaryArchive& file)
{
  m_text.Empty();
  int major_version = 0;
  int minor_version = 0;
  ...
  if (major_version == 1) {
    if (rc)
      rc = file.ReadPoint(m_point);
    if (rc)
      rc = file.ReadInt(&m_height);
    if (rc)
      rc = file.ReadString(m_text);
    if (rc)
      rc = file.ReadString(m_fontface);
    if (rc)
      rc = file.ReadInt(&m_display);
  }
  else {
    rc = false;
  }
...
ON::object_type
ON_TextDot::ObjectType() const
{
  return ON::text_dot;
}

int
ON_TextDot::Dimension() const
{
  return 3;
}
...
ON_BOOL32
ON_TextDot::Transform(const ON_Xform& xform)
{
...
bool
ON_TextDot::IsDeformable() const
{
  return true;
}

bool
ON_TextDot::MakeDeformable()
{
  return true;
}

const ON_3dPoint&
ON_TextDot::Point() const
{
  return m_point;
}

void
ON_TextDot::SetPoint(const ON_3dPoint& point)
{
  m_point = point;
}

int
ON_TextDot::Height() const
{
  return m_height;
}

void
ON_TextDot::SetHeight(int height)
{
  if (height > 2)
    m_height = height;
}
...
void
ON_TextDot::SetTextString(const wchar_t* string)
{
  m_text.Empty();
  if (string) {
...
void
ON_TextDot::SetFontFace(const wchar_t* face)
{
  if (face)
    m_fontface = face;
  else
...
void
ON_TextDot::SetAlwaysOnTop(bool bTop)
{
  if (bTop)
    m_display |= 1;
  else
...
bool
ON_TextDot::AlwaysOnTop() const
{
  return (m_display & 1) == 1;
}

void
ON_TextDot::SetTransparent(bool bTransparent)
{
  if (bTransparent)
    m_display |= 2;
  else
...
bool
ON_TextDot::Transparent() const
{
  return (m_display & 2) == 2;
}

void
ON_TextDot::SetBold(bool bBold)
{
  if (bBold)
    m_display |= 4;
  else
...
bool
ON_TextDot::Bold() const
{
  return (m_display & 4) == 4;
}

void
ON_TextDot::SetItalic(bool bItalic)
{
  if (bItalic)
    m_display |= 8;
  else
...
bool
ON_TextDot::Italic() const
{
  return (m_display & 8) == 8;
}

ON_Annotation2Text::ON_Annotation2Text() { memset(&m_rect, 0, sizeof(m_rect)); }

ON_Annotation2Text::~ON_Annotation2Text() {}

ON_Annotation2Text&
ON_Annotation2Text::operator=(const char* s)
{
  SetText(s);
  return *this;
}
...
ON_Annotation2Text&
ON_Annotation2Text::operator=(const wchar_t* s)
{
  SetText(s);
  return *this;
}
...
void
ON_Annotation2Text::SetText(const char* s)
{
...
void
ON_Annotation2Text::SetText(const wchar_t* s)
{
  ON_wString::operator=(s);
  memset(&m_rect, 0, sizeof(m_rect));
}
                             ...
                             ON_Xform& xform) const
{
  ...
  const ON::eTextDisplayMode textalignment =
      dimstyle ? ON::TextDisplayMode(dimstyle->TextAlignment()) : ON::dtNormal;
...
static bool
GetLeaderEndAndDirection (const ON_Annotation2* pAnn, ON_2dPoint& E, ON_2dVector& R)
{
  bool rc = false;
  ...
  if (ann_m_points.Count() >= 4 &&
      (ON::dtDimDiameter == ann_type || ON::dtDimRadius == ann_type)) {
                             ...
                             ON_Xform& xform) const
{
  ...
  double textheight =
      (ON::dtTextBlock == ann_type) ? m_textheight * dimscale : dimstyle_textheight;
  if (0.0 == textheight)
    textheight = 1.0;
  ...
  const double text_line_width =
      gdi_to_plane_scale * (gdi_text_rect.right - gdi_text_rect.left);
  ...
  if (ON::dtHorizontal != dimstyle_textalignment) {
    if (ON::dtDimLinear == ann_type || ON::dtDimAligned == ann_type) {
    ...
    else if (ON::dtDimDiameter == ann_type || ON::dtDimRadius == ann_type ||
             ON::dtLeader == ann_type || ON::dtDimOrdinate == ann_type) {
      ...
      text_centering_translation =
          (dimstyle_textgap + 0.5 * text_line_width) * text_centering_rotation;

      ON_3dVector V = text_centering_rotation.x * m_plane.xaxis +
                      text_centering_rotation.y * m_plane.yaxis;
      x = V * cameraX;
      y = V * cameraY;
  ...
  ON_2dVector text_offset_translation(0.0,
                                      0.0); // CRhinoText::Offset() = text->Offset()
  switch (ann_type) {
  case ON::dtDimLinear:
  case ON::dtDimAligned:
    if (m_points.Count() >= ON_LinearDimension2::dim_pt_count) {
      ...
      if (linear_dim) {
        text_offset_translation =
            linear_dim->Dim2dPoint(ON_LinearDimension2::text_pivot_pt);
      }
    }
    break;
  ...
  case ON::dtDimAngular:
    if (m_points.Count() >= ON_AngularDimension2::dim_pt_count) {
      ...
      if (angular_dim) {
        text_offset_translation =
            angular_dim->Dim2dPoint(ON_AngularDimension2::text_pivot_pt);
      }
    }
    break;
  ...
  if (ON::dtHorizontal == dimstyle_textalignment) {
    ...
    if (2 == position_style) {
      ...
      ON_3dVector V = R.x * m_plane.xaxis + R.y * m_plane.yaxis;
      x = V * cameraX;
      y = (x > -ON_SQRT_EPSILON) ? dimstyle_textgap
  ...
  gdi_to_world = horizonal_xform * plane_to_world * plane_translation *
                 text_centering_xform * gdi_to_plane;
...
bool
ON_Annotation2::GetTextPoint(ON_2dPoint& text_2d_point) const
{
  bool rc = false;
  switch (m_type) {
  case ON::dtTextBlock:
...
//
class /*NEVER PUT THIS CLASS IN THE SDK*/ ON_AnnotationTextFormula
: public ON_UserData {
...
public:
  ...
  ~ON_AnnotationTextFormula();
  // NO! - do not add IO support to this userdata! // ON_BOOL32 Write(ON_BinaryArchive&)
  // const; NO! - do not add IO support to this userdata! // ON_BOOL32
  // Read(ON_BinaryArchive&);
  ON_BOOL32
  GetDescription (ON_wString&);
  // NO! - do not add IO support to this userdata! // ON_BOOL32 Archive() const;
  static ON_AnnotationTextFormula*
  Get (const ON_Annotation2*);
  static void
  Set (ON_Annotation2*, const wchar_t* text_formula);
...
{
  m_userdata_uuid =
      ON_AnnotationTextFormula::m_ON_AnnotationTextFormula_class_id.Uuid();
  m_application_uuid = ON_opennurbs5_id;
  m_userdata_copycount = 1;
}

ON_BOOL32
ON_AnnotationTextFormula::GetDescription(ON_wString& description)
{
...
ON_AnnotationTextFormula*
ON_AnnotationTextFormula::Get(const ON_Annotation2* p)
{
  return (0 != p) ? ON_AnnotationTextFormula::Cast(p->GetUserData(
                        ON_AnnotationTextFormula::m_ON_AnnotationTextFormula_class_id
                            .Uuid()))
                  : 0;
}

void
ON_AnnotationTextFormula::Set(ON_Annotation2* p, const wchar_t* text_formula)
{
  if (0 != p) {
...
void
ON_Annotation2::SetTextValue(const wchar_t* text_value)
{
  m_usertext = text_value;
}

const wchar_t*
ON_Annotation2::TextValue() const
{
  return ((const wchar_t*)m_usertext);
}

void
ON_Annotation2::SetTextFormula(const wchar_t* text_formula)
{
  ON_AnnotationTextFormula::Set(this, text_formula);
}

const wchar_t*
ON_Annotation2::TextFormula() const
{
