...
bool
ON_BezierCage::Read(ON_BinaryArchive& archive)
{
  Destroy();
...
bool
ON_BezierCage::Write(ON_BinaryArchive& archive) const
{
...
ON_BezierCage::ON_BezierCage() : m_dim(0), m_is_rat(0), m_cv_capacity(0), m_cv(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_order[2] = 0;
...
ON_BezierCage::~ON_BezierCage() { Destroy(); }
...
void
ON_BezierCage::Dump(ON_TextLog& dump) const
{
...
int
ON_BezierCage::Dimension() const
{
  return m_dim;
}

bool
ON_BezierCage::Create(const ON_BoundingBox& bbox, int order0, int order1, int order2)
{
  /*
            7______________6
            |\             |\
...
bool
ON_BezierCage::Create(int dim, bool is_rat, int order0, int order1, int order2)
{
  if (m_cv_capacity < 1)
    m_cv = 0;
  m_dim = (dim > 0) ? dim : 0;
...
bool
ON_BezierCage::Create(const ON_3dPoint* box_corners, int order0, int order1, int order2)
{
  int i, j, k;
  double r, s, t;
...
void
ON_BezierCage::EmergencyDestroy()
{
  m_cv_capacity = 0;
  m_cv_stride[0] = 0;
  m_cv_stride[1] = 0;
...
bool
ON_BezierCage::GetBBox( // returns true if successful
...
bool
ON_BezierCage::Transform(const ON_Xform& xform)
{
  int i, j;
...
bool
ON_BezierCage::Rotate(double sin_angle,        // sin(angle)
...
bool
ON_BezierCage::Rotate(double angle,            // angle in radians
                      const ON_3dVector& axis, // axis of rotation
                      const ON_3dPoint& center // center of rotation
)
{
...
bool
ON_BezierCage::Translate(const ON_3dVector& delta)
{
  ON_Xform tr;
...
) const
{
  ...
  i = cvdim * sizeof(*vtmp);
  vtmp = m_is_rat ? (i < ((int)(10 * 4 * sizeof(vtmparray[0]))) ? vtmparray
                                                                : (double*)alloca(i))
                  : v;
  memset(vtmp, 0, i);
...
ON_3dPoint
ON_BezierCage::PointAt(double r, double s, double t) const
{
  ON_3dPoint pt;
  if (m_dim <= 3) {
    pt.x = 0.0;
...
ON_3dPoint
ON_BezierCage::PointAt(ON_3dPoint rst) const
{
  ON_3dPoint pt;
  if (m_dim <= 3) {
    pt.x = 0.0;
...
bool
ON_BezierCage::IsRational() const
{
  return m_is_rat ? true : false;
}

int
ON_BezierCage::CVSize() const
{
  return (m_is_rat && m_dim > 0) ? m_dim + 1 : m_dim;
}

int
ON_BezierCage::Order(int dir) const
{
  return (dir >= 0 && dir <= 2) ? m_order[dir] : 0;
}

int
ON_BezierCage::Degree(int dir) const
{
  int order = Order(dir);
...
double*
ON_BezierCage::CV(int i, int j, int k) const
{
#if defined(ON_DEBUG)
  if (0 == m_cv) {
  ...
  if (i < 0 || i >= m_order[0] || j < 0 || j >= m_order[1] || k < 0 ||
      k >= m_order[2]) {
...
ON::point_style
ON_BezierCage::CVStyle() const
{
  return m_is_rat ? ON::homogeneous_rational : ON::not_rational;
}

double
ON_BezierCage::Weight(int i, int j, int k) const
{
...
bool
ON_BezierCage::SetWeight(int i, int j, int k, double w)
{
  bool rc = false;
  if (m_is_rat) {
    double* cv = CV(i, j, k);
...
bool
ON_BezierCage::SetCV(int i, int j, int k, ON::point_style style, const double* Point)
{
  bool rc = true;
  int n;
  double w;
...
bool
ON_BezierCage::SetCV(int i, int j, int k, const ON_3dPoint& point)
{
  bool rc = false;
  double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_BezierCage::SetCV(int i, int j, int k, const ON_4dPoint& point)
{
  bool rc = false;
  double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_BezierCage::GetCV(int i, int j, int k, ON::point_style style, double* Point) const
{
  const double* cv = CV(i, j, k);
  if (!cv)
    return false;
...
bool
ON_BezierCage::GetCV(int i, int j, int k, ON_3dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_BezierCage::GetCV(int i, int j, int k, ON_4dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_BezierCage::ReserveCVCapacity(int capacity // number of doubles to reserve
)
{
...
bool
ON_BezierCage::IsSingular( // true if surface side is collapsed to a point
...
const ON_Xform&
ON_BezierCageMorph::WorldToUnitCube() const
{
...
const ON_BezierCage&
ON_BezierCageMorph::BezierCage() const
{
...
bool
ON_BezierCageMorph::Transform(const ON_Xform& xform)
{
...
bool
ON_BezierCageMorph::Read(ON_BinaryArchive& archive)
{
  int major_version = 0;
  int minor_version = 0;
  bool rc =
...
bool
ON_BezierCageMorph::Write(ON_BinaryArchive& archive) const
{
  bool rc = archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK, 1, 0);
  if (rc) {
    if (rc)
...
bool
ON_BezierCageMorph::SetXform(ON_Xform world2unitcube)
{
...
bool
ON_BezierCageMorph::SetBezierCage(ON_BezierCage& unitcube2world)
{
