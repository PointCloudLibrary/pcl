...
ON_Interval::ON_Interval() { m_t[0] = m_t[1] = ON_UNSET_VALUE; }

ON_Interval::ON_Interval (double t0, double t1) { Set (t0, t1); }
...
void
ON_Interval::Set (double t0, double t1)
{
  m_t[0] = t0;
  m_t[1] = t1;
}
...
double
ON_Interval::ParameterAt (double x) const
{
  return (ON_IS_VALID (x) ? ((1.0 - x) * m_t[0] + x * m_t[1]) : ON_UNSET_VALUE);
}

ON_Interval
ON_Interval::ParameterAt (ON_Interval x) const
{
  return ON_Interval (ParameterAt (x[0]), ParameterAt (x[1]));
}

double
ON_Interval::NormalizedParameterAt ( // returns x so that min*(1.0-x) + max*x = input
    double t) const
{
  if (!ON_IS_VALID (t))
...
ON_Interval::NormalizedParameterAt ( // returns x so that min*(1.0-x) + max*x = input
    ON_Interval t) const
{
  return ON_Interval (NormalizedParameterAt (t[0]), NormalizedParameterAt (t[1]));
}
...
{
  return (m_t[0] < m_t[1] && ON_IS_VALID (m_t[0]) && ON_IS_VALID (m_t[1])) ? true
                                                                           : false;
}
...
bool
ON_Interval::Intersection ( // this = this intersect arg
...
bool
ON_Interval::Union ( // this = this union arg
    const ON_Interval& other)
{
  bool rc = false;
  if (other.IsEmptySet()) {
...
bool
ON_Interval::Union (int count, const double* t)
{
  bool rc = false;
  double a, mn, mx;
...
bool
ON_3dVector::Decompose ( // Computes a, b, c such that this vector = a*X + b*Y + c*Z
                         //
...
bool
ON_3fVector::PerpendicularTo (const ON_3fVector& v)
{
  ON_3dVector V (*this);
...
bool
ON_3dVector::PerpendicularTo (const ON_3dVector& v)
{
  // bool rc = false;
  int i, j, k;
  double a, b;
                              ...
                              const ON_3dPoint& P2)
{
  ...
  const double e0 = s0 * fabs (ON_DotProduct (N0, V0)) +
                    s1 * fabs (ON_DotProduct (N0, V1)) +
                    s2 * fabs (ON_DotProduct (N0, V2));
  const double e1 = s0 * fabs (ON_DotProduct (N1, V0)) +
                    s1 * fabs (ON_DotProduct (N1, V1)) +
                    s2 * fabs (ON_DotProduct (N1, V2));
  const double e2 = s0 * fabs (ON_DotProduct (N2, V0)) +
                    s1 * fabs (ON_DotProduct (N2, V1)) +
                    s2 * fabs (ON_DotProduct (N2, V2));
...
void
ON_2dPoint::Transform (const ON_Xform& xform)
{
  double xx, yy, ww;
  if (xform.m_xform) {
...
void
ON_3dPoint::Transform (const ON_Xform& xform)
{
  double xx, yy, zz, ww;
  if (xform.m_xform) {
    ww = xform.m_xform[3][0] * x + xform.m_xform[3][1] * y + xform.m_xform[3][2] * z +
         xform.m_xform[3][3];
    if (ww != 0.0)
      ww = 1.0 / ww;
    xx = ww * (xform.m_xform[0][0] * x + xform.m_xform[0][1] * y +
               xform.m_xform[0][2] * z + xform.m_xform[0][3]);
    yy = ww * (xform.m_xform[1][0] * x + xform.m_xform[1][1] * y +
               xform.m_xform[1][2] * z + xform.m_xform[1][3]);
    zz = ww * (xform.m_xform[2][0] * x + xform.m_xform[2][1] * y +
               xform.m_xform[2][2] * z + xform.m_xform[2][3]);
    x = xx;
    y = yy;
    z = zz;
  }
...
void
ON_4dPoint::Transform (const ON_Xform& xform)
{
  double xx, yy, zz, ww;
  if (xform.m_xform) {
...
void
ON_2fPoint::Transform (const ON_Xform& xform)
{
  double xx, yy, ww;
  if (xform.m_xform) {
    ww = xform.m_xform[3][0] * x + xform.m_xform[3][1] * y + xform.m_xform[3][3];
...
void
ON_2fPoint::Rotate (double angle,            // angle in radians
...
void
ON_2fPoint::Rotate (double sin_angle,        // sin(angle)
...
void
ON_3fPoint::Rotate (double angle,            // angle in radians
...
void
ON_3fPoint::Rotate (double sin_angle,        // sin(angle)
                    double cos_angle,        // cos(angle)
                    const ON_3fVector& axis, // axis of rotation
                    const ON_3fPoint& center // center of rotation
)
...
void
ON_3fPoint::Transform (const ON_Xform& xform)
{
  double xx, yy, zz, ww;
  if (xform.m_xform) {
    ww = xform.m_xform[3][0] * x + xform.m_xform[3][1] * y + xform.m_xform[3][2] * z +
...
void
ON_4fPoint::Transform (const ON_Xform& xform)
{
  double xx, yy, zz, ww;
  if (xform.m_xform) {
    xx = xform.m_xform[0][0] * x + xform.m_xform[0][1] * y + xform.m_xform[0][2] * z +
...
double
ON_3fPoint::Fuzz (double absolute_tolerance // (default =  ON_ZERO_TOLERANCE)
) const
{
...
ON_4dPoint::Normalize()
{
  ...
  if (c > 0.0) {
    ...
    const double s =
        1.0 / (c * sqrt (f[0] * f[0] + f[1] * f[1] + f[2] * f[2] + f[3] * f[3]));
    x *= s;
    y *= s;
    z *= s;
    w *= s;
...
bool
ON_2fVector::Decompose ( // Computes a, b such that this vector = a*X + b*Y
                         //
...
bool
ON_2dVector::Decompose ( // Computes a, b such that this vector = a*X + b*Y
                         //
                         // If X,Y is known to be an orthonormal frame,
                         // then a = V*X, b = V*Y will compute
                         // the same result more quickly.
...
void
ON_2dVector::Transform (const ON_Xform& xform)
{
  double xx, yy;
...
void
ON_3fVector::Transform (const ON_Xform& xform)
{
  const double dx = x;
  const double dy = y;
  const double dz = z;
  double xx =
      xform.m_xform[0][0] * dx + xform.m_xform[0][1] * dy + xform.m_xform[0][2] * dz;
  double yy =
      xform.m_xform[1][0] * dx + xform.m_xform[1][1] * dy + xform.m_xform[1][2] * dz;
  double zz =
      xform.m_xform[2][0] * dx + xform.m_xform[2][1] * dy + xform.m_xform[2][2] * dz;
  x = (float)xx;
  y = (float)yy;
  z = (float)zz;
}
...
void
ON_3dVector::Transform (const ON_Xform& xform)
{
  double xx, yy, zz;
  xx = xform.m_xform[0][0] * x + xform.m_xform[0][1] * y + xform.m_xform[0][2] * z;
  yy = xform.m_xform[1][0] * x + xform.m_xform[1][1] * y + xform.m_xform[1][2] * z;
...
void
ON_3dPoint::Rotate (double angle,            // angle in radians
...
void
ON_3dPoint::Rotate (double sin_angle,        // sin(angle)
                    double cos_angle,        // cos(angle)
                    const ON_3dVector& axis, // axis of rotation
                    const ON_3dPoint& center // center of rotation
)
...
void
ON_2dPoint::Rotate (double angle,            // angle in radians
...
void
ON_2dPoint::Rotate (double sin_angle,        // sin(angle)
                    double cos_angle,        // cos(angle)
                    const ON_2dPoint& center // center of rotation
)
{
...
void
ON_2dVector::Rotate (double angle // angle in radians
)
{
...
void
ON_2dVector::Rotate (double sin_angle, // sin(angle)
                     double cos_angle  // cos(angle)
)
{
  ON_Xform rot;
...
bool
ON_IsOrthogonalFrame (const ON_2dVector& X, const ON_2dVector& Y)
{
...
bool
ON_IsOrthonormalFrame (const ON_2dVector& X, const ON_2dVector& Y)
{
...
bool
ON_IsRightHandFrame (const ON_2dVector& X, const ON_2dVector& Y)
{
...
void
ON_3fVector::Rotate (double angle,           // angle in radians
                     const ON_3fVector& axis // axis of rotation
)
{
...
void
ON_3fVector::Rotate (double sin_angle,       // sin(angle)
                     double cos_angle,       // cos(angle)
                     const ON_3fVector& axis // axis of rotation
)
{
...
void
ON_3dVector::Rotate (double angle,           // angle in radians
                     const ON_3dVector& axis // axis of rotation
)
{
  Rotate (sin (angle), cos (angle), axis);
...
void
ON_3dVector::Rotate (double sin_angle,       // sin(angle)
                     double cos_angle,       // cos(angle)
                     const ON_3dVector& axis // axis of rotation
)
{
...
bool
ON_IsOrthogonalFrame (const ON_3dVector& X, const ON_3dVector& Y, const ON_3dVector& Z)
{
  // returns true if X, Y, Z is an orthogonal frame
  ...
  if (fabs (xy) > ON_SQRT_EPSILON || fabs (yz) > ON_SQRT_EPSILON ||
...
bool
ON_IsOrthonormalFrame (const ON_3dVector& X, const ON_3dVector& Y, const ON_3dVector& Z)
{
  // returns true if X, Y, Z is an orthonormal frame
...
bool
ON_IsRightHandFrame (const ON_3dVector& X, const ON_3dVector& Y, const ON_3dVector& Z)
{
  // returns true if X, Y, Z is an orthonormal right hand frame
...
ON_2dPoint
ON_2dPoint::operator* (const ON_Xform& xform) const
{
...
ON_3dPoint
ON_3dPoint::operator* (const ON_Xform& xform) const
{
...
double
ON_3dPoint::Fuzz (double absolute_tolerance // (default =  ON_ZERO_TOLERANCE)
) const
{
  double t = MaximumCoordinate() * ON_RELATIVE_TOLERANCE;
  return (t > absolute_tolerance) ? t : absolute_tolerance;
...
ON_4dPoint
ON_4dPoint::operator* (const ON_Xform& xform) const
{
...
ON_2dVector
ON_2dVector::operator* (const ON_Xform& xform) const
{
...
ON_3dVector
ON_3dVector::operator* (const ON_Xform& xform) const
{
...
double
ON_3fVector::Fuzz (double absolute_tolerance // (default =  ON_ZERO_TOLERANCE)
) const
{
  double t = MaximumCoordinate() * ON_RELATIVE_TOLERANCE;
  return (t > absolute_tolerance) ? t : absolute_tolerance;
...
double
ON_3dVector::Fuzz (double absolute_tolerance // (default =  ON_ZERO_TOLERANCE)
) const
{
  double t = MaximumCoordinate() * ON_RELATIVE_TOLERANCE;
  return (t > absolute_tolerance) ? t : absolute_tolerance;
...
ON_2fPoint::operator float*() { return &x; }

ON_2fPoint::operator const float*() const { return &x; }

ON_2fPoint&
ON_2fPoint::operator= (const double* p)
{
  if (p) {
    x = (float)p[0];
    y = (float)p[1];
...
ON_2fPoint&
ON_2fPoint::operator= (const float* p)
{
  if (p) {
    x = p[0];
    y = p[1];
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_4fPoint& h)
{
  const float w = (h.w != 1.0f && h.w != 0.0f) ? 1.0f / h.w : 1.0f;
  x = w * h.x;
  y = w * h.y;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_2dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_3dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_4dPoint& h)
{
  const double w = (h.w != 1.0 && h.w != 0.0) ? 1.0 / h.w : 1.0;
  x = (float)(w * h.x);
  y = (float)(w * h.y);
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_2dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator= (const ON_3dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  return *this;
...
ON_2fPoint&
ON_2fPoint::operator+= (const ON_2fPoint& p)
{
  x += p.x;
  y += p.y;
  return *this;
...
ON_2fPoint
ON_2fPoint::operator* (int d) const
{
  return ON_2fPoint (x * d, y * d);
}

ON_2fPoint
ON_2fPoint::operator* (float d) const
{
  return ON_2fPoint (x * d, y * d);
}

ON_2dPoint
ON_2fPoint::operator* (double d) const
{
  return ON_2dPoint (x * d, y * d);
}

ON_2fPoint
ON_2fPoint::operator/ (int i) const
{
...
ON_2fPoint
ON_2fPoint::operator+ (const ON_2fPoint& p) const
{
  return ON_2fPoint (x + p.x, y + p.y);
}

ON_2fPoint
ON_2fPoint::operator+ (const ON_2fVector& v) const
{
...
ON_3fPoint
ON_2fPoint::operator+ (const ON_3fPoint& p) const
{
  return ON_3fPoint (x + p.x, y + p.y, p.z);
}

ON_3fPoint
ON_2fPoint::operator+ (const ON_3fVector& v) const
{
...
ON_2dPoint
ON_2fPoint::operator+ (const ON_2dPoint& p) const
{
  return ON_2dPoint (x + p.x, y + p.y);
}

ON_2dPoint
ON_2fPoint::operator+ (const ON_2dVector& v) const
{
...
ON_3dPoint
ON_2fPoint::operator+ (const ON_3dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, p.z);
}

ON_3dPoint
ON_2fPoint::operator+ (const ON_3dVector& v) const
{
...
float
ON_2fPoint::operator* (const ON_2fPoint& h) const
{
  return x * h.x + y * h.y;
}

float
ON_2fPoint::operator* (const ON_2fVector& h) const
{
  return x * h.x + y * h.y;
}

float
ON_2fPoint::operator* (const ON_4fPoint& h) const
{
...
float
ON_2fPoint::operator[] (unsigned int i) const
{
  return (i <= 0) ? x : y;
}

float&
ON_2fPoint::operator[] (unsigned int i)
{
  float* pd = (i <= 0) ? &x : &y;
  return *pd;
}
...
double
ON_2fPoint::DistanceTo (const ON_2fPoint& p) const
{
  return ON_Length2d (p.x - x, p.y - y);
}

int
ON_2fPoint::MaximumCoordinateIndex() const
{
  return (fabs (y) > fabs (x)) ? 1 : 0;
}

double
ON_2fPoint::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
ON_2fPoint
operator* (int d, const ON_2fPoint& p)
{
  return ON_2fPoint (d * p.x, d * p.y);
}

ON_2fPoint
operator* (float d, const ON_2fPoint& p)
{
  return ON_2fPoint (d * p.x, d * p.y);
}

ON_2dPoint
operator* (double d, const ON_2fPoint& p)
{
...
ON_3fPoint::operator float*() { return &x; }

ON_3fPoint::operator const float*() const { return &x; }

ON_3fPoint&
ON_3fPoint::operator= (const double* p)
{
  if (p) {
    x = (float)p[0];
    y = (float)p[1];
...
ON_3fPoint&
ON_3fPoint::operator= (const float* p)
{
  if (p) {
    x = p[0];
    y = p[1];
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0;
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_4fPoint& p)
{
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0f;
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  z = v.z;
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_2dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = 0.0f;
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_3dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = (float)p.z;
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_4dPoint& p)
{
  const double w = (p.w != 1.0 && p.w != 0.0) ? 1.0 / p.w : 1.0;
  x = (float)(w * p.x);
  y = (float)(w * p.y);
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_2dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  z = 0.0f;
...
ON_3fPoint&
ON_3fPoint::operator= (const ON_3dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  z = (float)v.z;
...
ON_3fPoint&
ON_3fPoint::operator+= (const ON_3fPoint& p)
{
  x += p.x;
  y += p.y;
  z += p.z;
...
ON_3fPoint
ON_3fPoint::operator* (int d) const
{
  return ON_3fPoint (x * d, y * d, z * d);
}

ON_3fPoint
ON_3fPoint::operator* (float d) const
{
  return ON_3fPoint (x * d, y * d, z * d);
}

ON_3dPoint
ON_3fPoint::operator* (double d) const
{
...
ON_3fPoint
ON_3fPoint::operator+ (const ON_3fPoint& p) const
{
  return ON_3fPoint (x + p.x, y + p.y, z + p.z);
}

ON_3fPoint
ON_3fPoint::operator+ (const ON_3fVector& v) const
{
...
ON_3fPoint
ON_3fPoint::operator+ (const ON_2fPoint& p) const
{
  return ON_3fPoint (x + p.x, y + p.y, z);
}

ON_3fPoint
ON_3fPoint::operator+ (const ON_2fVector& v) const
{
...
ON_3dPoint
ON_3fPoint::operator+ (const ON_3dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z + p.z);
}

ON_3dPoint
ON_3fPoint::operator+ (const ON_3dVector& v) const
{
...
ON_3dPoint
ON_3fPoint::operator+ (const ON_2dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z);
}

ON_3dPoint
ON_3fPoint::operator+ (const ON_2dVector& v) const
{
...
float
ON_3fPoint::operator* (const ON_3fPoint& h) const
{
  return x * h.x + y * h.y + z * h.z;
}

float
ON_3fPoint::operator* (const ON_3fVector& h) const
{
  return x * h.x + y * h.y + z * h.z;
}

float
ON_3fPoint::operator* (const ON_4fPoint& h) const
{
...
float
ON_3fPoint::operator[] (int i) const
{
  return ((i <= 0) ? x : ((i >= 2) ? z : y));
}

float&
ON_3fPoint::operator[] (int i)
{
...
float
ON_3fPoint::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : ((i >= 2) ? z : y));
}

float&
ON_3fPoint::operator[] (unsigned int i)
{
  float* pd = (i <= 0) ? &x : ((i >= 2) ? &z : &y);
  return *pd;
}
...
double
ON_3fPoint::DistanceTo (const ON_3fPoint& p) const
{
  return ON_Length3d (p.x - x, p.y - y, p.z - z);
}

int
ON_3fPoint::MaximumCoordinateIndex() const
{
  return (fabs (y) > fabs (x)) ? ((fabs (z) > fabs (y)) ? 2 : 1)
                               : ((fabs (z) > fabs (x)) ? 2 : 0);
}

double
ON_3fPoint::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
ON_3fPoint
operator* (int d, const ON_3fPoint& p)
{
  return ON_3fPoint (d * p.x, d * p.y, d * p.z);
}

ON_3fPoint
operator* (float d, const ON_3fPoint& p)
{
  return ON_3fPoint (d * p.x, d * p.y, d * p.z);
}

ON_3dPoint
operator* (double d, const ON_3fPoint& p)
{
...
ON_4fPoint::operator float*() { return &x; }

ON_4fPoint::operator const float*() const { return &x; }

ON_4fPoint&
ON_4fPoint::operator= (const double* p)
{
  if (p) {
    x = (float)p[0];
    y = (float)p[1];
...
ON_4fPoint&
ON_4fPoint::operator= (const float* p)
{
  if (p) {
    x = p[0];
    y = p[1];
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  z = p.z;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  z = v.z;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_2dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = 0.0;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_3dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = (float)p.z;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_4dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = (float)p.z;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_2dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  z = 0.0;
...
ON_4fPoint&
ON_4fPoint::operator= (const ON_3dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  z = (float)v.z;
...
ON_4fPoint&
ON_4fPoint::operator+= (const ON_4fPoint& p)
{
  // sum w = sqrt(w1*w2)
  if (p.w == w) {
    x += p.x;
...
ON_4fPoint
ON_4fPoint::operator+ (const ON_4fPoint& p) const
{
...
ON_4fPoint
ON_4fPoint::operator* (float d) const
{
...
float
ON_4fPoint::operator* (const ON_4fPoint& h) const
{
  return x * h.x + y * h.y + z * h.z + w * h.w;
}

bool
ON_4fPoint::operator== (ON_4fPoint p) const
{
  ON_4fPoint a = *this;
  a.Normalize();
  p.Normalize();
  if (fabs (a.x - p.x) > ON_SQRT_EPSILON)
    return false;
  if (fabs (a.y - p.y) > ON_SQRT_EPSILON)
    return false;
  if (fabs (a.z - p.z) > ON_SQRT_EPSILON)
    return false;
  if (fabs (a.w - p.w) > ON_SQRT_EPSILON)
    return false;
  return true;
}
...
float
ON_4fPoint::operator[] (int i) const
{
  return ((i <= 0) ? x : ((i >= 3) ? w : ((i == 1) ? y : z)));
}

float&
ON_4fPoint::operator[] (int i)
{
...
float
ON_4fPoint::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : ((i >= 3) ? w : ((i == 1) ? y : z)));
}

float&
ON_4fPoint::operator[] (unsigned int i)
{
  float* pd = (i <= 0) ? &x : ((i >= 3) ? &w : (i == 1) ? &y : &z);
  return *pd;
}
...
int
ON_4fPoint::MaximumCoordinateIndex() const
{
  const float* a = &x;
...
double
ON_4fPoint::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
ON_4fPoint
operator* (float d, const ON_4fPoint& p)
{
  return ON_4fPoint (d * p.x, d * p.y, d * p.z, d * p.w);
}

ON_4dPoint
operator* (double d, const ON_4fPoint& p)
{
...
// static
const ON_2fVector&
ON_2fVector::UnitVector (int index)
{
...
ON_2fVector::operator float*() { return &x; }

ON_2fVector::operator const float*() const { return &x; }

ON_2fVector&
ON_2fVector::operator= (const double* v)
{
  if (v) {
    x = (float)v[0];
    y = (float)v[1];
...
ON_2fVector&
ON_2fVector::operator= (const float* v)
{
  if (v) {
    x = v[0];
    y = v[1];
...
ON_2fVector&
ON_2fVector::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator= (const ON_2dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator= (const ON_3dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator= (const ON_2dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator= (const ON_3dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  return *this;
...
ON_2fVector&
ON_2fVector::operator+= (const ON_2fVector& v)
{
  x += v.x;
  y += v.y;
  return *this;
...
ON_2fVector
ON_2fVector::operator* (int d) const
{
  return ON_2fVector (x * d, y * d);
}

ON_2fVector
ON_2fVector::operator* (float d) const
{
  return ON_2fVector (x * d, y * d);
}

ON_2dVector
ON_2fVector::operator* (double d) const
{
...
float
ON_2fVector::operator* (const ON_2fPoint& v) const
{
  return (x * v.x + y * v.y);
}

double
ON_2fVector::operator* (const ON_2dVector& v) const
{
  return (x * v.x + y * v.y);
}
...
ON_2fPoint
ON_2fVector::operator+ (const ON_2fPoint& p) const
{
  return ON_2fPoint (x + p.x, y + p.y);
}
...
ON_3fVector
ON_2fVector::operator+ (const ON_3fVector& v) const
{
  return ON_3fVector (x + v.x, y + v.y, v.z);
}

ON_3fPoint
ON_2fVector::operator+ (const ON_3fPoint& p) const
{
  return ON_3fPoint (x + p.x, y + p.y, p.z);
}
...
ON_3fPoint
ON_2fVector::operator- (const ON_3fPoint& v) const
{
  return ON_3fPoint (x - v.x, y - v.y, -v.z);
}
...
ON_2dPoint
ON_2fVector::operator+ (const ON_2dPoint& p) const
{
  return ON_2dPoint (x + p.x, y + p.y);
}
...
ON_2dPoint
ON_2fVector::operator- (const ON_2dPoint& v) const
{
  return ON_2dPoint (x - v.x, y - v.y);
}
...
ON_3dPoint
ON_2fVector::operator+ (const ON_3dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, p.z);
}
...
ON_3dPoint
ON_2fVector::operator- (const ON_3dPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, -v.z);
}

float
ON_2fVector::operator* (const ON_4fPoint& h) const
{
  return x * h.x + y * h.y;
}
...
float
ON_2fVector::operator[] (int i) const
{
  return ((i <= 0) ? x : y);
}

float&
ON_2fVector::operator[] (int i)
{
  float* pd = (i <= 0) ? &x : &y;
  return *pd;
}
...
float
ON_2fVector::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : y);
}

float&
ON_2fVector::operator[] (unsigned int i)
{
  float* pd = (i <= 0) ? &x : &y;
  return *pd;
}
...
int
ON_2fVector::MaximumCoordinateIndex() const
{
  return ((fabs (y) > fabs (x)) ? 1 : 0);
}

double
ON_2fVector::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
double
ON_2fVector::LengthSquared() const
{
  return (x * x + y * y);
}

double
ON_2fVector::Length() const
{
...
bool
ON_2fVector::IsUnitVector() const
{
  return (x != ON_UNSET_FLOAT && y != ON_UNSET_FLOAT &&
          fabs (Length() - 1.0) <= 1.0e-5);
}

bool
ON_3fVector::IsUnitVector() const
{
  return (x != ON_UNSET_FLOAT && y != ON_UNSET_FLOAT && z != ON_UNSET_FLOAT &&
          fabs (Length() - 1.0) <= 1.0e-5);
}

bool
ON_2fVector::IsTiny (double tiny_tol) const
{
  return (fabs (x) <= tiny_tol && fabs (y) <= tiny_tol);
}

bool
ON_2fVector::IsZero() const
{
...
bool
ON_2fVector::PerpendicularTo ( // Result is not unitized.
...
bool
ON_2fVector::PerpendicularTo (const ON_2fPoint& p, const ON_2fPoint& q)
{
  return PerpendicularTo (q - p);
}

ON_2fVector
operator* (int d, const ON_2fVector& v)
{
  return ON_2fVector (d * v.x, d * v.y);
}

ON_2fVector
operator* (float d, const ON_2fVector& v)
{
  return ON_2fVector (d * v.x, d * v.y);
}

ON_2dVector
operator* (double d, const ON_2fVector& v)
{
  return ON_2dVector (d * v.x, d * v.y);
}

float
ON_DotProduct (const ON_2fVector& a, const ON_2fVector& b)
{
...
ON_3fVector
ON_CrossProduct (const ON_2fVector& a, const ON_2fVector& b)
{
...
// static
const ON_3fVector&
ON_3fVector::UnitVector (int index)
{
...
ON_3fVector::operator float*() { return &x; }

ON_3fVector::operator const float*() const { return &x; }

ON_3fVector&
ON_3fVector::operator= (const double* v)
{
  if (v) {
    x = (float)v[0];
    y = (float)v[1];
...
ON_3fVector&
ON_3fVector::operator= (const float* v)
{
  if (v) {
    x = v[0];
    y = v[1];
...
ON_3fVector&
ON_3fVector::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_3fVector&
ON_3fVector::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0f;
...
ON_3fVector&
ON_3fVector::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  z = p.z;
...
ON_3fVector&
ON_3fVector::operator= (const ON_2dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  z = 0.0f;
...
ON_3fVector&
ON_3fVector::operator= (const ON_3dVector& v)
{
  x = (float)v.x;
  y = (float)v.y;
  z = (float)v.z;
...
ON_3fVector&
ON_3fVector::operator= (const ON_2dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = 0.0f;
...
ON_3fVector&
ON_3fVector::operator= (const ON_3dPoint& p)
{
  x = (float)p.x;
  y = (float)p.y;
  z = (float)p.z;
...
ON_3fVector&
ON_3fVector::operator+= (const ON_3fVector& v)
{
  x += v.x;
  y += v.y;
  z += v.z;
...
ON_3fVector
ON_3fVector::operator* (int d) const
{
  return ON_3fVector (x * d, y * d, z * d);
}

ON_3fVector
ON_3fVector::operator* (float d) const
{
  return ON_3fVector (x * d, y * d, z * d);
}

ON_3dVector
ON_3fVector::operator* (double d) const
{
  return ON_3dVector (x * d, y * d, z * d);
}

float
ON_3fVector::operator* (const ON_3fVector& v) const
{
  return (x * v.x + y * v.y + z * v.z);
}

float
ON_3fVector::operator* (const ON_3fPoint& v) const
{
  return (x * v.x + y * v.y + z * v.z);
}
...
ON_3fPoint
ON_3fVector::operator+ (const ON_3fPoint& p) const
{
  return ON_3fPoint (x + p.x, y + p.y, z + p.z);
}
...
ON_3fVector
ON_3fVector::operator+ (const ON_2fVector& v) const
{
  return ON_3fVector (x + v.x, y + v.y, z);
}

ON_3fPoint
ON_3fVector::operator+ (const ON_2fPoint& p) const
{
  return ON_3fPoint (x + p.x, y + p.y, z);
}
...
ON_3fPoint
ON_3fVector::operator- (const ON_2fPoint& v) const
{
  return ON_3fPoint (x - v.x, y - v.y, z);
}
...
ON_3dPoint
ON_3fVector::operator+ (const ON_3dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z + p.z);
}
...
ON_3dPoint
ON_3fVector::operator- (const ON_3dPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, z - v.z);
}
...
ON_3dPoint
ON_3fVector::operator+ (const ON_2dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z);
}
...
ON_3dPoint
ON_3fVector::operator- (const ON_2dPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, z);
}

float
ON_3fVector::operator* (const ON_4fPoint& h) const
{
  return x * h.x + y * h.y + z * h.z;
}
...
float
ON_3fVector::operator[] (int i) const
{
  return ((i <= 0) ? x : ((i >= 2) ? z : y));
}

float&
ON_3fVector::operator[] (int i)
{
  float* pd = (i <= 0) ? &x : ((i >= 2) ? &z : &y);
  return *pd;
}
...
float
ON_3fVector::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : ((i >= 2) ? z : y));
}

float&
ON_3fVector::operator[] (unsigned int i)
{
  float* pd = (i <= 0) ? &x : ((i >= 2) ? &z : &y);
  return *pd;
}
...
int
ON_3fVector::MaximumCoordinateIndex() const
{
  return (fabs (y) > fabs (x)) ? ((fabs (z) > fabs (y)) ? 2 : 1)
                               : ((fabs (z) > fabs (x)) ? 2 : 0);
}
...
double
ON_3fVector::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
double
ON_3fVector::LengthSquared() const
{
  return (x * x + y * y + z * z);
}

double
ON_3fVector::Length() const
{
...
bool
ON_3fVector::IsTiny (double tiny_tol) const
{
  return (fabs (x) <= tiny_tol && fabs (y) <= tiny_tol && fabs (z) <= tiny_tol);
}

bool
ON_3fVector::IsZero() const
{
  return (x == 0.0f && y == 0.0f && z == 0.0f);
}

ON_3fVector
operator* (int d, const ON_3fVector& v)
{
  return ON_3fVector (d * v.x, d * v.y, d * v.z);
}

ON_3fVector
operator* (float d, const ON_3fVector& v)
{
  return ON_3fVector (d * v.x, d * v.y, d * v.z);
}

ON_3dVector
operator* (double d, const ON_3fVector& v)
{
  return ON_3dVector (d * v.x, d * v.y, d * v.z);
}

float
ON_DotProduct (const ON_3fVector& a, const ON_3fVector& b)
{
  // inner (dot) product between 3d vectors
...
float
ON_TripleProduct (const ON_3fVector& a, const ON_3fVector& b, const ON_3fVector& c)
{
  // = a o (b x c )
...
float
ON_TripleProduct (const float* a, const float* b, const float* c)
{
  // = a o (b x c )
...
ON_2dPoint::operator double*() { return &x; }

ON_2dPoint::operator const double*() const { return &x; }

ON_2dPoint&
ON_2dPoint::operator= (const float* p)
{
  if (p) {
    x = (double)p[0];
    y = (double)p[1];
...
ON_2dPoint&
ON_2dPoint::operator= (const double* p)
{
  if (p) {
    x = p[0];
    y = p[1];
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_3dPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_4dPoint& h)
{
  const double w = (h.w != 1.0 && h.w != 0.0) ? 1.0 / h.w : 1.0;
  x = w * h.x;
  y = w * h.y;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_2dVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_3dVector& v)
{
  x = v.x;
  y = v.y;
  y = v.z;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_4fPoint& h)
{
  const double w = (h.w != 1.0f && h.w != 0.0f) ? 1.0 / ((double)h.w) : 1.0;
  x = w * h.x;
  y = w * h.y;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2dPoint&
ON_2dPoint::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  y = v.z;
...
ON_2dPoint&
ON_2dPoint::operator+= (const ON_2dPoint& p)
{
  x += p.x;
  y += p.y;
  return *this;
...
ON_2dPoint
ON_2dPoint::operator* (int i) const
{
  double d = i;
  return ON_2dPoint (x * d, y * d);
}
...
ON_2dPoint
ON_2dPoint::operator* (float f) const
{
  double d = f;
  return ON_2dPoint (x * d, y * d);
}
...
ON_2dPoint
ON_2dPoint::operator* (double d) const
{
  return ON_2dPoint (x * d, y * d);
}
...
ON_2dPoint
ON_2dPoint::operator+ (const ON_2dPoint& p) const
{
  return ON_2dPoint (x + p.x, y + p.y);
}

ON_2dPoint
ON_2dPoint::operator+ (const ON_2dVector& v) const
{
  return ON_2dPoint (x + v.x, y + v.y);
}
...
ON_3dPoint
ON_2dPoint::operator+ (const ON_3dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, p.z);
}

ON_3dPoint
ON_2dPoint::operator+ (const ON_3dVector& v) const
{
  return ON_3dPoint (x + v.x, y + v.y, v.z);
}
...
ON_2dPoint
ON_2dPoint::operator+ (const ON_2fPoint& p) const
{
  return ON_2dPoint (x + p.x, y + p.y);
}

ON_2dPoint
ON_2dPoint::operator+ (const ON_2fVector& v) const
{
  return ON_2dPoint (x + v.x, y + v.y);
}
...
ON_3dPoint
ON_2dPoint::operator+ (const ON_3fPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, p.z);
}

ON_3dPoint
ON_2dPoint::operator+ (const ON_3fVector& v) const
{
  return ON_3dPoint (x + v.x, y + v.y, v.z);
}
...
double
ON_2dPoint::operator* (const ON_2dPoint& h) const
{
  return x * h.x + y * h.y;
}

double
ON_2dPoint::operator* (const ON_2dVector& h) const
{
  return x * h.x + y * h.y;
}

double
ON_2dPoint::operator* (const ON_4dPoint& h) const
{
  return x * h.x + y * h.y + h.w;
}
...
double
ON_2dPoint::operator[] (unsigned int i) const
{
  return (i <= 0) ? x : y;
}

double&
ON_2dPoint::operator[] (unsigned int i)
{
  double* pd = (i <= 0) ? &x : &y;
  return *pd;
}
...
double
ON_2dPoint::DistanceTo (const ON_2dPoint& p) const
{
  return ON_Length2d (p.x - x, p.y - y);
}

int
ON_2dPoint::MaximumCoordinateIndex() const
{
  return (fabs (y) > fabs (x)) ? 1 : 0;
}

double
ON_2dPoint::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
int
ON_2dPoint::MinimumCoordinateIndex() const
{
  return (fabs (y) < fabs (x)) ? 1 : 0;
}

double
ON_2dPoint::MinimumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) < c)
    c = fabs (y);
...
ON_2dPoint
operator* (int i, const ON_2dPoint& p)
{
  double d = i;
  return ON_2dPoint (d * p.x, d * p.y);
}
...
ON_2dPoint
operator* (float f, const ON_2dPoint& p)
{
  double d = f;
  return ON_2dPoint (d * p.x, d * p.y);
}
...
ON_2dPoint
operator* (double d, const ON_2dPoint& p)
{
  return ON_2dPoint (d * p.x, d * p.y);
}
...
ON_3dPoint::operator double*() { return &x; }

ON_3dPoint::operator const double*() const { return &x; }

ON_3dPoint&
ON_3dPoint::operator= (const float* p)
{
  if (p) {
    x = (double)p[0];
    y = (double)p[1];
...
ON_3dPoint&
ON_3dPoint::operator= (const double* p)
{
  if (p) {
    x = p[0];
    y = p[1];
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_2dPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_4dPoint& p)
{
  const double w = (p.w != 1.0 && p.w != 0.0) ? 1.0 / p.w : 1.0;
  x = w * p.x;
  y = w * p.y;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_2dVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_3dVector& v)
{
  x = v.x;
  y = v.y;
  z = v.z;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_2fPoint& p)
{
  x = (double)p.x;
  y = (double)p.y;
  z = (double)0.0;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_3fPoint& p)
{
  x = (double)p.x;
  y = (double)p.y;
  z = (double)p.z;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_4fPoint& p)
{
  const double w = (p.w != 1.0f && p.w != 0.0f) ? 1.0 / ((double)p.w) : 1.0;
  x = w * ((double)p.x);
  y = w * ((double)p.y);
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_3dPoint&
ON_3dPoint::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  z = v.z;
...
ON_3dPoint&
ON_3dPoint::operator+= (const ON_3dPoint& p)
{
  x += p.x;
  y += p.y;
  z += p.z;
...
ON_3dPoint
ON_3dPoint::operator* (int i) const
{
  double d = i;
  return ON_3dPoint (x * d, y * d, z * d);
}
...
ON_3dPoint
ON_3dPoint::operator* (float f) const
{
  double d = f;
  return ON_3dPoint (x * d, y * d, z * d);
}
...
ON_3dPoint
ON_3dPoint::operator+ (const ON_2dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z);
}

ON_3dPoint
ON_3dPoint::operator+ (const ON_2dVector& v) const
{
  return ON_3dPoint (x + v.x, y + v.y, z);
}
...
ON_3dPoint
ON_3dPoint::operator+ (const ON_3fPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z + p.z);
}

ON_3dPoint
ON_3dPoint::operator+ (const ON_3fVector& v) const
{
  return ON_3dPoint (x + v.x, y + v.y, z + v.z);
}
...
ON_3dPoint
ON_3dPoint::operator+ (const ON_2fPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z);
}

ON_3dPoint
ON_3dPoint::operator+ (const ON_2fVector& v) const
{
  return ON_3dPoint (x + v.x, y + v.y, z);
}
...
double
ON_3dPoint::operator* (const ON_3dPoint& h) const
{
  return x * h.x + y * h.y + z * h.z;
}

double
ON_3dPoint::operator* (const ON_3dVector& h) const
{
  return x * h.x + y * h.y + z * h.z;
}

double
ON_3dPoint::operator* (const ON_4dPoint& h) const
{
  return x * h.x + y * h.y + z * h.z + h.w;
}
...
double
ON_3dPoint::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : ((i >= 2) ? z : y));
}

double&
ON_3dPoint::operator[] (unsigned int i)
{
  double* pd = (i <= 0) ? &x : ((i >= 2) ? &z : &y);
  return *pd;
}
...
double
ON_3dPoint::DistanceTo (const ON_3dPoint& p) const
{
  return ON_Length3d (p.x - x, p.y - y, p.z - z);
}

int
ON_3dPoint::MaximumCoordinateIndex() const
{
  return (fabs (y) > fabs (x)) ? ((fabs (z) > fabs (y)) ? 2 : 1)
                               : ((fabs (z) > fabs (x)) ? 2 : 0);
}
...
double
ON_3dPoint::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
int
ON_3dPoint::MinimumCoordinateIndex() const
{
  return (fabs (y) < fabs (x)) ? ((fabs (z) < fabs (y)) ? 2 : 1)
                               : ((fabs (z) < fabs (x)) ? 2 : 0);
}
...
double
ON_3dPoint::MinimumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) < c)
    c = fabs (y);
...
ON_3dPoint
operator* (int i, const ON_3dPoint& p)
{
  double d = i;
  return ON_3dPoint (d * p.x, d * p.y, d * p.z);
}
...
ON_3dPoint
operator* (float f, const ON_3dPoint& p)
{
  double d = f;
  return ON_3dPoint (d * p.x, d * p.y, d * p.z);
}
...
ON_3dPoint
operator* (double d, const ON_3dPoint& p)
{
  return ON_3dPoint (d * p.x, d * p.y, d * p.z);
}
...
ON_4dPoint::operator double*() { return &x; }

ON_4dPoint::operator const double*() const { return &x; }

ON_4dPoint&
ON_4dPoint::operator= (const float* p)
{
  if (p) {
    x = (double)p[0];
    y = (double)p[1];
...
ON_4dPoint&
ON_4dPoint::operator= (const double* p)
{
  if (p) {
    x = p[0];
    y = p[1];
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_2dPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_3dPoint& p)
{
  x = p.x;
  y = p.y;
  z = p.z;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_2dVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_3dVector& v)
{
  x = v.x;
  y = v.y;
  z = v.z;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_2fPoint& p)
{
  x = (double)p.x;
  y = (double)p.y;
  z = 0.0;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_3fPoint& p)
{
  x = (double)p.x;
  y = (double)p.y;
  z = (double)p.z;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_4fPoint& p)
{
  x = (double)p.x;
  y = (double)p.y;
  z = (double)p.z;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_2fVector& v)
{
  x = (double)v.x;
  y = (double)v.y;
  z = 0.0;
...
ON_4dPoint&
ON_4dPoint::operator= (const ON_3fVector& v)
{
  x = (double)v.x;
  y = (double)v.y;
  z = (double)v.z;
...
ON_4dPoint&
ON_4dPoint::operator+= (const ON_4dPoint& p)
{
  // sum w = sqrt(w1*w2)
  if (p.w == w) {
    x += p.x;
...
ON_4dPoint
ON_4dPoint::operator+ (const ON_4dPoint& p) const
{
...
ON_4dPoint
ON_4dPoint::operator* (double d) const
{
...
double
ON_4dPoint::operator* (const ON_4dPoint& h) const
{
  return x * h.x + y * h.y + z * h.z + w * h.w;
}

bool
ON_4dPoint::operator== (ON_4dPoint p) const
{
  ON_4dPoint a = *this;
  a.Normalize();
  p.Normalize();
...
double
ON_4dPoint::operator[] (int i) const
{
  return ((i <= 0) ? x : ((i >= 3) ? w : ((i == 1) ? y : z)));
}

double&
ON_4dPoint::operator[] (int i)
{
...
double
ON_4dPoint::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : ((i >= 3) ? w : ((i == 1) ? y : z)));
}

double&
ON_4dPoint::operator[] (unsigned int i)
{
  double* pd = (i <= 0) ? &x : ((i >= 3) ? &w : (i == 1) ? &y : &z);
  return *pd;
}
...
int
ON_4dPoint::MaximumCoordinateIndex() const
{
  const double* a = &x;
  int i = (fabs (y) > fabs (x)) ? 1 : 0;
  if (fabs (z) > fabs (a[i]))
...
double
ON_4dPoint::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
int
ON_4dPoint::MinimumCoordinateIndex() const
{
  const double* a = &x;
  int i = (fabs (y) < fabs (x)) ? 1 : 0;
  if (fabs (z) < fabs (a[i]))
...
double
ON_4dPoint::MinimumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) < c)
    c = fabs (y);
...
ON_4dPoint
operator* (double d, const ON_4dPoint& p)
{
  return ON_4dPoint (d * p.x, d * p.y, d * p.z, d * p.w);
}
...
// static
const ON_2dVector&
ON_2dVector::UnitVector (int index)
{
...
ON_2dVector::operator double*() { return &x; }

ON_2dVector::operator const double*() const { return &x; }

ON_2dVector&
ON_2dVector::operator= (const float* v)
{
  if (v) {
    x = (double)v[0];
    y = (double)v[1];
...
ON_2dVector&
ON_2dVector::operator= (const double* v)
{
  if (v) {
    x = v[0];
    y = v[1];
...
ON_2dVector&
ON_2dVector::operator= (const ON_3dVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator= (const ON_2dPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator= (const ON_3dPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  return *this;
...
ON_2dVector&
ON_2dVector::operator+= (const ON_2dVector& v)
{
  x += v.x;
  y += v.y;
  return *this;
...
ON_2dVector
ON_2dVector::operator* (int i) const
{
  double d = i;
  return ON_2dVector (x * d, y * d);
}
...
ON_2dVector
ON_2dVector::operator* (float f) const
{
  double d = f;
  return ON_2dVector (x * d, y * d);
}
...
ON_2dVector
ON_2dVector::operator* (double d) const
{
  return ON_2dVector (x * d, y * d);
}

double
ON_2dVector::operator* (const ON_2dVector& v) const
{
  return (x * v.x + y * v.y);
}

double
ON_2dVector::operator* (const ON_2dPoint& v) const
{
  return (x * v.x + y * v.y);
}

double
ON_2dVector::operator* (const ON_2fVector& v) const
{
  return (x * v.x + y * v.y);
}
...
ON_2dPoint
ON_2dVector::operator+ (const ON_2dPoint& p) const
{
  return ON_2dPoint (x + p.x, y + p.y);
}
...
ON_3dPoint
ON_2dVector::operator+ (const ON_3dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, p.z);
}
...
ON_3dPoint
ON_2dVector::operator- (const ON_3dPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, -v.z);
}
...
ON_2dPoint
ON_2dVector::operator+ (const ON_2fPoint& p) const
{
  return ON_2dPoint (x + p.x, y + p.y);
}
...
ON_2dPoint
ON_2dVector::operator- (const ON_2fPoint& v) const
{
  return ON_2dPoint (x - v.x, y - v.y);
}
...
ON_3dPoint
ON_2dVector::operator+ (const ON_3fPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, p.z);
}
...
ON_3dPoint
ON_2dVector::operator- (const ON_3fPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, -v.z);
}

double
ON_2dVector::operator* (const ON_4dPoint& h) const
{
  return x * h.x + y * h.y;
}
...
double
ON_2dVector::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : y);
}

double&
ON_2dVector::operator[] (unsigned int i)
{
  double* pd = (i <= 0) ? &x : &y;
  return *pd;
}
...
int
ON_2dVector::MaximumCoordinateIndex() const
{
  return ((fabs (y) > fabs (x)) ? 1 : 0);
}

double
ON_2dVector::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
int
ON_2dVector::MinimumCoordinateIndex() const
{
  return (fabs (y) < fabs (x)) ? 1 : 0;
}

double
ON_2dVector::MinimumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) < c)
    c = fabs (y);
...
double
ON_2dVector::LengthSquared() const
{
  return (x * x + y * y);
}
...
double
ON_2dVector::Length() const
{
  return ON_Length2d (x, y);
}

double
ON_2dVector::WedgeProduct (const ON_2dVector& B) const
{
  return x * B.y - y * B.x;
}
...
bool
ON_2dVector::IsTiny (double tiny_tol) const
{
  return (fabs (x) <= tiny_tol && fabs (y) <= tiny_tol);
}

bool
ON_2dVector::IsZero() const
{
  return (x == 0.0 && y == 0.0);
}

bool
ON_2dVector::IsUnitVector() const
{
  return (x != ON_UNSET_VALUE && y != ON_UNSET_VALUE &&
          fabs (Length() - 1.0) <= ON_SQRT_EPSILON);
}

// set this vector to be perpendicular to another vector
bool
ON_2dVector::PerpendicularTo ( // Result is not unitized.
                               // returns false if input vector is zero
    const ON_2dVector& v)
{
  y = v.x;
...
// set this vector to be perpendicular to a line defined by 2 points
bool
ON_2dVector::PerpendicularTo (const ON_2dPoint& p, const ON_2dPoint& q)
{
  return PerpendicularTo (q - p);
}

ON_2dVector
operator* (int i, const ON_2dVector& v)
{
  double d = i;
  return ON_2dVector (d * v.x, d * v.y);
}
...
ON_2dVector
operator* (float f, const ON_2dVector& v)
{
  double d = f;
  return ON_2dVector (d * v.x, d * v.y);
}
...
ON_2dVector
operator* (double d, const ON_2dVector& v)
{
  return ON_2dVector (d * v.x, d * v.y);
}

double
ON_DotProduct (const ON_2dVector& a, const ON_2dVector& b)
{
  // inner (dot) product between 3d vectors
  return (a.x * b.x + a.y * b.y);
}
...
double
ON_WedgeProduct (const ON_2dVector& a, const ON_2dVector& b)
{
...
ON_3dVector
ON_CrossProduct (const ON_2dVector& a, const ON_2dVector& b)
{
...
// static
const ON_3dVector&
ON_3dVector::UnitVector (int index)
{
...
ON_3dVector::operator double*() { return &x; }

ON_3dVector::operator const double*() const { return &x; }

ON_3dVector&
ON_3dVector::operator= (const float* v)
{
  if (v) {
    x = (double)v[0];
    y = (double)v[1];
...
ON_3dVector&
ON_3dVector::operator= (const double* v)
{
  if (v) {
    x = v[0];
    y = v[1];
...
ON_3dVector&
ON_3dVector::operator= (const ON_2dVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_3dVector&
ON_3dVector::operator= (const ON_2dPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0;
...
ON_3dVector&
ON_3dVector::operator= (const ON_3dPoint& p)
{
  x = p.x;
  y = p.y;
  z = p.z;
...
ON_3dVector&
ON_3dVector::operator= (const ON_2fVector& v)
{
  x = v.x;
  y = v.y;
  z = 0.0;
...
ON_3dVector&
ON_3dVector::operator= (const ON_3fVector& v)
{
  x = v.x;
  y = v.y;
  z = v.z;
...
ON_3dVector&
ON_3dVector::operator= (const ON_2fPoint& p)
{
  x = p.x;
  y = p.y;
  z = 0.0;
...
ON_3dVector&
ON_3dVector::operator= (const ON_3fPoint& p)
{
  x = p.x;
  y = p.y;
  z = p.z;
...
ON_3dVector&
ON_3dVector::operator+= (const ON_3dVector& v)
{
  x += v.x;
  y += v.y;
  z += v.z;
...
ON_3dVector
ON_3dVector::operator* (int i) const
{
  double d = i;
  return ON_3dVector (x * d, y * d, z * d);
}
...
ON_3dVector
ON_3dVector::operator* (float f) const
{
  double d = f;
  return ON_3dVector (x * d, y * d, z * d);
}
...
ON_3dVector
ON_3dVector::operator* (double d) const
{
  return ON_3dVector (x * d, y * d, z * d);
}
...
double
ON_3dVector::operator* (const ON_3dPoint& v) const
{
  return (x * v.x + y * v.y + z * v.z);
}
...
ON_3dPoint
ON_3dVector::operator+ (const ON_2dPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z);
}
...
ON_3dPoint
ON_3dVector::operator- (const ON_2dPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, z);
}
...
ON_3dPoint
ON_3dVector::operator+ (const ON_3fPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z + p.z);
}
...
ON_3dPoint
ON_3dVector::operator- (const ON_3fPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, z - v.z);
}
...
ON_3dPoint
ON_3dVector::operator+ (const ON_2fPoint& p) const
{
  return ON_3dPoint (x + p.x, y + p.y, z);
}
...
ON_3dPoint
ON_3dVector::operator- (const ON_2fPoint& v) const
{
  return ON_3dPoint (x - v.x, y - v.y, z);
}

double
ON_3dVector::operator* (const ON_4dPoint& h) const
{
  return x * h.x + y * h.y + z * h.z;
}
...
double
ON_3dVector::operator[] (unsigned int i) const
{
  return ((i <= 0) ? x : ((i >= 2) ? z : y));
}

double&
ON_3dVector::operator[] (unsigned int i)
{
  double* pd = (i <= 0) ? &x : ((i >= 2) ? &z : &y);
  return *pd;
}
...
int
ON_3dVector::MaximumCoordinateIndex() const
{
  return (fabs (y) > fabs (x)) ? ((fabs (z) > fabs (y)) ? 2 : 1)
                               : ((fabs (z) > fabs (x)) ? 2 : 0);
}
...
double
ON_3dVector::MaximumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) > c)
    c = fabs (y);
...
int
ON_3dVector::MinimumCoordinateIndex() const
{
  return (fabs (y) < fabs (x)) ? ((fabs (z) < fabs (y)) ? 2 : 1)
                               : ((fabs (z) < fabs (x)) ? 2 : 0);
}
...
double
ON_3dVector::MinimumCoordinate() const
{
  double c = fabs (x);
  if (fabs (y) < c)
    c = fabs (y);
...
double
ON_3dVector::LengthSquared() const
{
  return (x * x + y * y + z * z);
}
...
bool
ON_3dVector::IsTiny (double tiny_tol) const
{
  return (fabs (x) <= tiny_tol && fabs (y) <= tiny_tol && fabs (z) <= tiny_tol);
}

bool
ON_3dVector::IsZero() const
{
  return (x == 0.0 && y == 0.0 && z == 0.0);
}

bool
ON_3dVector::IsUnitVector() const
{
  return (x != ON_UNSET_VALUE && y != ON_UNSET_VALUE && z != ON_UNSET_VALUE &&
          fabs (Length() - 1.0) <= ON_SQRT_EPSILON);
}

ON_3dVector
operator* (int i, const ON_3dVector& v)
{
  double d = i;
  return ON_3dVector (d * v.x, d * v.y, d * v.z);
}
...
ON_3dVector
operator* (float f, const ON_3dVector& v)
{
  double d = f;
  return ON_3dVector (d * v.x, d * v.y, d * v.z);
}
...
ON_3dVector
operator* (double d, const ON_3dVector& v)
{
  return ON_3dVector (d * v.x, d * v.y, d * v.z);
}

double
ON_DotProduct (const ON_3dVector& a, const ON_3dVector& b)
{
  // inner (dot) product between 3d vectors
  return (a.x * b.x + a.y * b.y + a.z * b.z);
}
...
double
ON_TripleProduct (const ON_3dVector& a, const ON_3dVector& b, const ON_3dVector& c)
{
  // = a o (b x c )
  return (a.x * (b.y * c.z - b.z * c.y) + a.y * (b.z * c.x - b.x * c.z) +
          a.z * (b.x * c.y - b.y * c.x));
...
double
ON_TripleProduct (const double* a, const double* b, const double* c)
{
  // = a o (b x c )
  return (a[0] * (b[1] * c[2] - b[2] * c[1]) + a[1] * (b[2] * c[0] - b[0] * c[2]) +
          a[2] * (b[0] * c[1] - b[1] * c[0]));
...
bool
ON_2dVector::IsValid() const
{
  return (ON_IS_VALID (x) && ON_IS_VALID (y)) ? true : false;
}

bool
ON_2dVector::IsUnsetVector() const
{
  return (ON_UNSET_VALUE == x && ON_UNSET_VALUE == y);
}

bool
ON_3dVector::IsValid() const
{
  return (ON_IS_VALID (x) && ON_IS_VALID (y) && ON_IS_VALID (z)) ? true : false;
}

bool
ON_3dVector::IsUnsetVector() const
{
  return (ON_UNSET_VALUE == x && ON_UNSET_VALUE == y && ON_UNSET_VALUE == z);
}

bool
ON_2dPoint::IsValid() const
{
  return (ON_IS_VALID (x) && ON_IS_VALID (y)) ? true : false;
}

bool
ON_2dPoint::IsUnsetPoint() const
{
  return (ON_UNSET_VALUE == x && ON_UNSET_VALUE == y);
}

bool
ON_3dPoint::IsValid() const
{
  return (ON_IS_VALID (x) && ON_IS_VALID (y) && ON_IS_VALID (z)) ? true : false;
}

bool
ON_3dPoint::IsUnsetPoint() const
{
  return (ON_UNSET_VALUE == x && ON_UNSET_VALUE == y && ON_UNSET_VALUE == z);
}

bool
ON_4dPoint::IsValid() const
{
  return (ON_IS_VALID (x) && ON_IS_VALID (y) && ON_IS_VALID (z) && ON_IS_VALID (w))
             ? true
             : false;
}

bool
ON_4dPoint::IsUnsetPoint() const
{
  return (ON_UNSET_VALUE == x && ON_UNSET_VALUE == y && ON_UNSET_VALUE == z &&
          ON_UNSET_VALUE == w);
}
...
void
ON_2dPoint::Set (double xx, double yy)
{
  x = xx;
  y = yy;
}
...
void
ON_3dPoint::Set (double xx, double yy, double zz)
{
  x = xx;
  y = yy;
  z = zz;
...
void
ON_4dPoint::Set (double xx, double yy, double zz, double ww)
{
  x = xx;
  y = yy;
  z = zz;
...
void
ON_2dVector::Set (double xx, double yy)
{
  x = xx;
  y = yy;
}
...
void
ON_3dVector::Set (double xx, double yy, double zz)
{
  x = xx;
  y = yy;
  z = zz;
...
void
ON_2fPoint::Set (float xx, float yy)
{
  x = xx;
  y = yy;
}
...
void
ON_3fPoint::Set (float xx, float yy, float zz)
{
  x = xx;
  y = yy;
  z = zz;
...
void
ON_4fPoint::Set (float xx, float yy, float zz, float ww)
{
  x = xx;
  y = yy;
  z = zz;
...
void
ON_2fVector::Set (float xx, float yy)
{
  x = xx;
  y = yy;
}
...
void
ON_3fVector::Set (float xx, float yy, float zz)
{
  x = xx;
  y = yy;
  z = zz;
...
bool
ON_PlaneEquation::Create (ON_3dPoint P, ON_3dVector N)
{
  bool rc = false;
...
ON_PlaneEquation::ON_PlaneEquation() : ON_3dVector (0.0, 0.0, 0.0), d (0.0) {}

ON_PlaneEquation::ON_PlaneEquation (double xx, double yy, double zz, double dd)
: ON_3dVector (xx, yy, zz), d (dd)
{}

const ON_PlaneEquation ON_PlaneEquation::UnsetPlaneEquation (ON_UNSET_VALUE,
                                                             ON_UNSET_VALUE,
                                                             ON_UNSET_VALUE,
                                                             ON_UNSET_VALUE);
...
double
ON_PlaneEquation::ZeroTolerance() const
{
...
bool
ON_PlaneEquation::Transform (const ON_Xform& xform)
{
  bool rc = IsValid();
  if (rc) {
    ...
    if (rc) {
      ...
      const double zz = z;
      const double dd = d;
      x = T.m_xform[0][0] * xx + T.m_xform[1][0] * yy + T.m_xform[2][0] * zz +
          T.m_xform[3][0] * dd;
      y = T.m_xform[0][1] * xx + T.m_xform[1][1] * yy + T.m_xform[2][1] * zz +
          T.m_xform[3][1] * dd;
      z = T.m_xform[0][2] * xx + T.m_xform[1][2] * yy + T.m_xform[2][2] * zz +
          T.m_xform[3][2] * dd;
      d = T.m_xform[0][3] * xx + T.m_xform[1][3] * yy + T.m_xform[2][3] * zz +
          T.m_xform[3][3] * dd;
    }
  }
  return rc;
...
double
ON_PlaneEquation::ValueAt (ON_3dPoint P) const
{
  return (x * P.x + y * P.y + z * P.z + d);
}

double
ON_PlaneEquation::ValueAt (ON_4dPoint P) const
{
  return (x * P.x + y * P.y + z * P.z + d * P.w);
}

double
ON_PlaneEquation::ValueAt (ON_3dVector P) const
{
  return (x * P.x + y * P.y + z * P.z + d);
}

double
ON_PlaneEquation::ValueAt (double xx, double yy, double zz) const
{
                           ...
                           double value_range[2]) const
{
  ...
  if (0 != value_range) {
    value[0] = s = e[0] * ((double)P[0].x) + e[1] * ((double)P[0].y) +
                   e[2] * ((double)P[0].z) + e[3];
    value_range[0] = s;
    value_range[1] = s;
    for (i = 1; i < Pcount; i++) {
      value[i] = s = e[0] * ((double)P[i].x) + e[1] * ((double)P[i].y) +
                     e[2] * ((double)P[i].z) + e[3];
      if (s < value_range[0])
        value_range[0] = s;
    ...
    for (i = 0; i < Pcount; i++) {
      value[i] = e[0] * ((double)P[i].x) + e[1] * ((double)P[i].y) +
                 e[2] * ((double)P[i].z) + e[3];
    }
  }
...
ON_3dPoint
ON_PlaneEquation::ClosestPointTo (ON_3dPoint P) const
{
...
double
ON_PlaneEquation::MinimumValueAt (const ON_BoundingBox& bbox) const
{
  double xx, yy, zz, s;

  s = x * bbox.m_min.x;
  if (s < (xx = x * bbox.m_max.x))
    xx = s;

  s = y * bbox.m_min.y;
  if (s < (yy = y * bbox.m_max.y))
    yy = s;

  s = z * bbox.m_min.z;
  if (s < (zz = z * bbox.m_max.z))
    zz = s;
...
double
ON_PlaneEquation::MaximumValueAt (const ON_BoundingBox& bbox) const
{
  double xx, yy, zz, s;
                                          ...
                                          double stop_value) const
{
  ...
  if (point_count < 1 || point_stride < (bRational ? 4 : 3) || 0 == points) {
  ...
  if (ON_IsValid (stop_value)) {
    if (bRational) {
      ...
      w = (0.0 != w) ? 1.0 / w : 1.0;
      max_value =
          fabs (x * w * points[0] + y * w * points[1] + z * w * points[2] + points[3]);
      ...
      for (point_count--; point_count--; /*empty iterator*/) {
        ...
        w = points[3];
        w = (0.0 != w) ? 1.0 / w : 1.0;
        value = fabs (x * w * points[0] + y * w * points[1] + z * w * points[2] +
                      points[3]);
        if (value > max_value) {
...
bool
ON_PlaneEquation::operator== (const ON_PlaneEquation& eq) const
{
...
bool
ON_BoundingBox::IsValid() const
{
  return (m_min.x <= m_max.x && ON_IS_VALID (m_min.x) && ON_IS_VALID (m_max.x) &&
          m_min.y <= m_max.y && ON_IS_VALID (m_min.y) && ON_IS_VALID (m_max.y) &&
          m_min.z <= m_max.z && ON_IS_VALID (m_min.z) && ON_IS_VALID (m_max.z));
};

void
ON_BoundingBox::Dump (ON_TextLog& text_log) const
{
...
bool
ON_IsDegenrateConicHelper (double A, double B, double C, double D, double E)
{
  //
                                   ...
                                   double* zero_pivot)
{
  ...
  for (i = 1; i < 6; i++) {
    ...
    if (pts[i].x < bboxmin.x)
      bboxmin.x = pts[i].x;
    else if (pts[i].x > bboxmax.x)
      bboxmax.x = pts[i].x;
    if (pts[i].y < bboxmin.y)
      bboxmin.y = pts[i].y;
    else if (pts[i].y > bboxmax.y)
      bboxmax.y = pts[i].y;
  }
                             ...
                             double* minor_radius)
{
  ...
  if (!ON_IsValid (conic[0]) || !ON_IsValid (conic[1]) || !ON_IsValid (conic[2]) ||
      !ON_IsValid (conic[3]) || !ON_IsValid (conic[4]) || !ON_IsValid (conic[5])) {
    return false;
  }
    ...
    double a, double b, double x0, double y0, double alpha, double conic[6])
{
  ...
  if (!(a > 0.0 && b > 0.0 && ON_IsValid (x0) && ON_IsValid (y0) &&
        ON_IsValid (alpha))) {
    return false;
  }
