...
ON_MeshNgonList::~ON_MeshNgonList() { Destroy(); }

void
ON_MeshNgonList::Destroy()
{
  m_ngons_count = 0;
  m_ngons_capacity = 0;
  if (0 != m_ngons) {
...
int
ON_MeshNgonList::NgonCount() const
{
...
unsigned int
ON_MeshNgonUserData::SizeOf() const
{
...
ON_BOOL32
ON_MeshNgonUserData::Archive() const
{
  return (0 != m_ngon_list && m_ngon_list->NgonCount() > 0);
}

static bool
ON_ValidateNgon (const ON_MeshNgon* ngon, int mesh_V_count, int mesh_F_count)
{
  unsigned int j;
  if (0 == ngon || ngon->N < 0)
    return false;
...
static bool
ON_ValidateMeshNgonUserData (ON_MeshNgonUserData* ngud, const ON_Mesh& mesh)
{
  int i;
