...
ON_Sum::ON_Sum() { Begin(0.0); }

int
ON_Sum::SummandCount() const
{
  return m_pos_count + m_neg_count + m_zero_count;
}

void
ON_Sum::Begin(double starting_value)
{
  m_sum_err = 0.0;
  m_pos_sum = 0.0;
  m_neg_sum = 0.0;
...
double
ON_Sum::SortAndSum(int count, double* a)
{
...
double
ON_Sum::Total(double* error_estimate)
{
  double x;
