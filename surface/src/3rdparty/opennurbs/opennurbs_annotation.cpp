...
const ON_3dmAnnotationSettings&
...
ON_Annotation::~ON_Annotation() { Destroy(); }

bool
ON_Annotation::IsText() const
{
  return Type() == ON::dtTextBlock;
}
bool
ON_Annotation::IsLeader() const
{
  return Type() == ON::dtLeader;
}
bool
ON_Annotation::IsDimension() const
{
  if (IsText() || IsLeader())
    return false;
  return true;
...
// virtual
double
ON_Annotation::NumericValue() const
{
  return 0.0;
}
// virtual
...
ON::eTextDisplayMode
ON_Annotation::TextDisplayMode() const
{
...
ON_Plane
ON_Annotation::Plane() const
{
  return m_plane;
}
int
ON_Annotation::PointCount() const
{
...
const ON_SimpleArray<ON_2dPoint>&
ON_Annotation::Points() const
{
  return m_points;
}
void
...
const ON_wString&
ON_Annotation::DefaultText() const
{
...
bool
ON_Annotation::UserPositionedText() const
{
...
ON::object_type
ON_Annotation::ObjectType() const
{
  return ON::annotation_object;
}

int
ON_Annotation::Dimension() const
{
  return 3;
}
...
ON::object_type
ON_AnnotationArrow::ObjectType() const
{
  return ON::annotation_object;
}

int
ON_AnnotationArrow::Dimension() const
{
  return 3;
}
...
ON_3dVector
ON_AnnotationArrow::Vector() const
{
  return (m_head - m_tail);
}

ON_3dPoint
ON_AnnotationArrow::Head() const
{
  return m_head;
}

ON_3dPoint
ON_AnnotationArrow::Tail() const
{
  return m_tail;
}
