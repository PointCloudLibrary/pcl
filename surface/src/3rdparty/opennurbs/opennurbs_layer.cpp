...
ON__LayerPerViewSettings::DataCRC (ON__UINT32 current_remainder) const
{
  ...
  if (0 != settings_mask) {
    ...
    if (0 != (settings_mask & ON_Layer::per_viewport_persistent_visibility))
      current_remainder = ON_CRC32 (current_remainder,
                                    sizeof (m_persistent_visibility),
...
bool
ON__LayerPerViewSettings::Write (const ON_Layer&,
...
// virtual ON_Object override
ON_BOOL32
ON__LayerExtensions::IsValid (ON_TextLog*) const { return true; }
...
// virtual ON_UserData override
ON_BOOL32
ON__LayerExtensions::Archive() const { return !IsEmpty(); }
  ...
  if (!ON_UuidIsNil (viewport_id)) {
    ...
    if (ud) {
      ...
      if (bCreate) {
        ...
        {
          if (0 ==
              memcmp (&viewport_id, &vp_settings[i].m_viewport_id, sizeof (ON_UUID)))
            return (vp_settings + i);
        }
      }
    }
                                   ...
                                   unsigned int settings_mask)
{
  ...
  if (0 != settings_mask && this != &source_layer) {
    if (ON_UuidIsNil (viewport_id)) {
      ...
      const ON__LayerExtensions* soruce_layer_ud =
...
// virtual ON_Object override
ON_BOOL32
ON__LayerSettingsUserData::IsValid (ON_TextLog*) const { return true; }
