...
ON_Layer::ON_Layer() : m_extension_bits(0) { Default(); }
...
ON_Layer::~ON_Layer() { m_name.Destroy(); }

static void
SetExtensionBit (unsigned char* layer_m_extension_bits, unsigned char mask)
{
  *layer_m_extension_bits |= mask;
}

static void
ClearExtensionBit (unsigned char* layer_m_extension_bits, unsigned char mask)
{
...
static bool
ExtensionBit (unsigned char layer_m_extension_bits, unsigned char mask)
{
  return (0 != (layer_m_extension_bits & mask));
}

ON_BOOL32
ON_Layer::IsValid(ON_TextLog* text_log) const
{
  if (m_name.IsEmpty()) {
    if (text_log) {
...
const wchar_t*
ON::IsNameReferenceDelimiter(const wchar_t* s)
{
...
LayerFullName (const wchar_t* s0)
{
  if (0 == s0 || 0 == s0[0])
    return 0;
  const wchar_t* t;
...
bool
ON_Layer::GetLeafName(const wchar_t* layer_name, ON_wString& leaf_name)
{
...
bool
ON_Layer::GetParentName(const wchar_t* layer_name, ON_wString& parent_path_name)
{
  const wchar_t* s0 = LayerFullName(layer_name);
  const wchar_t* s1 = LayerLeafName(s0);
  if (0 != s1 && 0 != *s1 && s0 < s1) {
...
bool
ON_Layer::RemoveReferenceName(const wchar_t* layer_name, ON_wString& layer_path_name)
{
...
bool
ON_Layer::GetReferenceName(const wchar_t* layer_name, ON_wString& reference_name)
{
...
void
ON_Layer::Dump(ON_TextLog& dump) const
{
  ...
  dump.Print("display color rgb = ");
  dump.PrintRGB(m_color);
  dump.Print("\n");
  dump.Print("plot color rgb = ");
  dump.PrintRGB(m_plot_color);
  dump.Print("\n");
...
) const
{
  ...
  while (rc) {
...

    {
      ...
      short s = 0;
      if (rc)
        rc = file.WriteShort(s); // default pattern
      if (rc)
        rc = file.WriteShort(s); // default pattern index
      if (rc)
        rc = file.WriteDouble(0.0); // default thickness
      if (rc)
        rc = file.WriteDouble(1.0); // default scale
    }
    if (!rc)
      break;
...
)
{
  ...
  if (rc && major_version == 1) {
    ...
    if (rc)
      rc = file.ReadInt(&m_iges_level);
    if (rc)
      rc = file.ReadInt(&m_material_index);
    if (rc)
      rc = file.ReadInt(&obsolete_value1);
    if (rc)
...
bool
ON_Layer::SetLayerName(const char* s)
{
  m_name = s;
...
bool
ON_Layer::SetLayerName(const wchar_t* s)
{
  m_name = s;
  return IsValid() ? true : false;
}
...
const ON_wString&
ON_Layer::LayerName() const
{
  return m_name;
}

void
ON_Layer::SetColor(ON_Color c)
{
  m_color = c;
}

void
ON_Layer::SetPlotColor(ON_Color c)
{
  m_plot_color = c;
}

ON_Color
ON_Layer::Color() const
{
  return m_color;
}

ON_Color
ON_Layer::PlotColor() const
{
  return ((m_plot_color == ON_UNSET_COLOR) ? m_color : m_plot_color);
}

bool
ON_Layer::SetLinetypeIndex(int index)
{
  if (index >= -1) {
...
int
ON_Layer::LinetypeIndex() const
{
  return m_linetype_index;
}

bool
ON_Layer::IsVisible() const
{
  return m_bVisible;
}

void
ON_Layer::SetVisible(bool bVisible)
{
...
void
ON_Layer::SetLocked(bool bLocked)
{
...
bool
ON_Layer::IsLocked() const
{
  return m_bLocked;
}

bool
ON_Layer::IsVisibleAndNotLocked() const
{
  return (m_bVisible && !m_bLocked);
}

bool
ON_Layer::IsVisibleAndLocked() const
{
  return (m_bVisible && m_bLocked);
}

bool
ON_Layer::SetRenderMaterialIndex(int i)
{
  m_material_index = i;
  return true;
}
...
int
ON_Layer::RenderMaterialIndex() const
{
  return m_material_index;
}

bool
ON_Layer::SetLayerIndex(int i)
{
  m_layer_index = i;
  return true;
}
...
int
ON_Layer::LayerIndex() const
{
  return m_layer_index;
}

bool
ON_Layer::SetIgesLevel(int level)
{
  m_iges_level = level;
  return true;
}
...
int
ON_Layer::IgesLevel() const
{
  return m_iges_level;
}

double
ON_Layer::PlotWeight() const
{
  return m_plot_weight_mm;
}

void
ON_Layer::SetPlotWeight(double plot_weight_mm)
{
  m_plot_weight_mm =
      (ON_IsValid(plot_weight_mm) && (plot_weight_mm > 0.0 || -1.0 == plot_weight_mm))
          ? plot_weight_mm
          : 0.0;
}
...
public:
  ...
  SetDefaultValues ();
  bool
  Write (const ON_Layer& layer, ON_BinaryArchive& binary_archive) const;
  bool
  Read (const ON_Layer& layer, ON_BinaryArchive& binary_archive);
  ...
  static int
  Compare (const ON__LayerPerViewSettings* a, const ON__LayerPerViewSettings* b);

  static int
  CompareViewportId (const ON__LayerPerViewSettings* a,
  ...
  */
  void
  CopySettings (const ON__LayerPerViewSettings* src, unsigned int settings_mask);
};

ON__UINT32
ON__LayerPerViewSettings::DataCRC(ON__UINT32 current_remainder) const
{
  const unsigned int settings_mask = SettingsMask();
  if (0 != settings_mask) {
    if (0 != (settings_mask & ON_Layer::per_viewport_id))
      current_remainder =
          ON_CRC32(current_remainder, sizeof(m_viewport_id), &m_viewport_id);
    ...
    if (0 != (settings_mask & ON_Layer::per_viewport_plot_color))
      current_remainder =
          ON_CRC32(current_remainder, sizeof(m_plot_color), &m_plot_color);
    if (0 != (settings_mask & ON_Layer::per_viewport_plot_weight))
      current_remainder =
          ON_CRC32(current_remainder, sizeof(m_plot_weight_mm), &m_plot_weight_mm);
    ...
    if (0 != (settings_mask & ON_Layer::per_viewport_persistent_visibility))
      current_remainder = ON_CRC32(
          current_remainder, sizeof(m_persistent_visibility), &m_persistent_visibility);
  }
  return current_remainder;
}

void
ON__LayerPerViewSettings::CopySettings(const ON__LayerPerViewSettings* src,
...
int
ON__LayerPerViewSettings::Compare(const ON__LayerPerViewSettings* a,
                                  const ON__LayerPerViewSettings* b)
{
...
int
ON__LayerPerViewSettings::CompareViewportId(const ON__LayerPerViewSettings* a,
                                            const ON__LayerPerViewSettings* b)
{
...
unsigned int
ON__LayerPerViewSettings::SettingsMask() const
{
  ...
  if (!ON_UuidIsNil(m_viewport_id)) {
    ...
    if ((m_plot_weight_mm >= 0.0 || -1.0 == m_plot_weight_mm) &&
        ON_IsValid(m_plot_weight_mm))
...
ON__LayerPerViewSettings::ON__LayerPerViewSettings() { SetDefaultValues(); }

void
ON__LayerPerViewSettings::SetDefaultValues()
{
...
bool
ON__LayerPerViewSettings::Write(const ON_Layer&, ON_BinaryArchive& binary_archive) const
{
  ...
  for (;;) {
    ...
    if (0 != (ON_Layer::per_viewport_visible & bits)) {
      ...
      if (!binary_archive.WriteChar(
              m_visible)) // (makes old a file old rhinos can read)
        break;
    }
...
bool
ON__LayerPerViewSettings::Read(const ON_Layer& layer, ON_BinaryArchive& binary_archive)
{
  SetDefaultValues();
  ...
  for (;;) {
    ...
    if (0 != (ON_Layer::per_viewport_visible & bits)) {
      ...
      if (minor_version >= 1) {
        ...
        // Yes, writing m_visible and reading m_persistent_visibility is done on
        // purpose.
...
public:
  ...
  static ON__LayerPerViewSettings*
  ViewportSettings (const ON_Layer& layer,
                    const unsigned char* layer_m_extension_bits,
                    ON_UUID viewport_id,
                    bool bCreate);

  static void
  DeleteViewportSettings (const ON_Layer& layer,
                          const unsigned char* layer_m_extension_bits,
                          const ON__LayerPerViewSettings* vp_settings_to_delete);

  static ON__LayerExtensions*
  LayerExtensions (const ON_Layer& layer,
                   const unsigned char* layer_m_extension_bits,
                   bool bCreate);
...
ON__LayerExtensions*
ON__LayerExtensions::LayerExtensions(const ON_Layer& layer,
                                     const unsigned char* layer_m_extension_bits,
                                     bool bCreate)
{
  ON__LayerExtensions* ud = ON__LayerExtensions::Cast(
      layer.GetUserData(ON__LayerExtensions::m_ON__LayerExtensions_class_id.Uuid()));
...
// virtual ON_Object override
ON_BOOL32
ON__LayerExtensions::IsValid(ON_TextLog*) const
{
  return true;
}
...
// virtual ON_Object override
unsigned int
ON__LayerExtensions::SizeOf() const
{
...
// virtual ON_Object override
ON__UINT32
ON__LayerExtensions::DataCRC(ON__UINT32) const
{
  ON__UINT32 crc = 0;
...
// virtual ON_Object override
ON_BOOL32
ON__LayerExtensions::Write(ON_BinaryArchive& binary_archive) const
{
...
// virtual ON_Object override
ON_BOOL32
ON__LayerExtensions::Read(ON_BinaryArchive& binary_archive)
{
...
// virtual ON_UserData override
ON_BOOL32
ON__LayerExtensions::Archive() const
{
  return !IsEmpty();
}
...
// virtual ON_UserData override
ON_BOOL32
ON__LayerExtensions::GetDescription(ON_wString& description)
{
...
ON__LayerPerViewSettings*
ON__LayerExtensions::ViewportSettings(const ON_Layer& layer,
                                      const unsigned char* layer_m_extension_bits,
                                      ON_UUID viewport_id,
                                      bool bCreate)
{
  if (!ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions* ud =
        ON__LayerExtensions::LayerExtensions(layer, layer_m_extension_bits, bCreate);
    if (ud) {
      int i;
      ...
      if (bCreate) {
        ...
        for (i = 0; i <= vp_settings_count;
             i++) // "i <= ..." is correct because of the .AppendNew()
        {
          if (0 == memcmp(&viewport_id, &vp_settings[i].m_viewport_id, sizeof(ON_UUID)))
            return (vp_settings + i);
        }
...
{
  ON__LayerExtensions* ud =
      ON__LayerExtensions::LayerExtensions(layer, layer_m_extension_bits, false);
  if (ud) {
...
bool
ON__LayerExtensions::IsEmpty() const
{
...
void
ON_Layer::SetPerViewportColor(ON_UUID viewport_id, ON_Color layer_color)
{
  if (ON_UuidIsNil(viewport_id)) {
    ...
    ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, bSet);
    if (vp_settings) {
      vp_settings->m_color = layer_color;
      if (!bSet && 0 == vp_settings->SettingsMask())
        ON__LayerExtensions::DeleteViewportSettings(
            *this, &m_extension_bits, vp_settings);
    }
  }
}

void
ON_Layer::SetColor(ON_Color layer_color, const ON_UUID& viewport_id)
{
  SetPerViewportColor(viewport_id, layer_color);
}

ON_Color
ON_Layer::PerViewportColor(ON_UUID viewport_id) const
{
  if (!ExtensionBit(m_extension_bits, 0x01)) {
    const ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, false);
...
ON_Color
ON_Layer::Color(const ON_UUID& viewport_id) const
{
  return PerViewportColor(viewport_id);
}

void
ON_Layer::SetPerViewportPlotColor(ON_UUID viewport_id, ON_Color plot_color)
{
  if (ON_UuidIsNil(viewport_id)) {
...
void
ON_Layer::SetPlotColor(ON_Color plot_color, const ON_UUID& viewport_id)
{
  return SetPerViewportPlotColor(viewport_id, plot_color);
}

ON_Color
ON_Layer::PerViewportPlotColor(ON_UUID viewport_id) const
{
  if (!ExtensionBit(m_extension_bits, 0x01)) {
    const ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, false);
...
void
ON_Layer::SetPerViewportPlotWeight(ON_UUID viewport_id, double plot_weight_mm)
{
  if (ON_UuidIsNil(viewport_id)) {
    ...
    bool bSet = (ON_IsValid(plot_weight_mm) &&
                 (plot_weight_mm >= 0.0 || -1.0 == plot_weight_mm));
    ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, bSet);
    if (vp_settings) {
...
void
ON_Layer::SetPlotWeight(double plot_weight_mm, const ON_UUID& viewport_id)
{
  SetPerViewportPlotWeight(viewport_id, plot_weight_mm);
}

double
ON_Layer::PerViewportPlotWeight(ON_UUID viewport_id) const
{
  if (!ExtensionBit(m_extension_bits, 0x01)) {
    const ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, false);
    if (0 != vp_settings &&
        (vp_settings->m_plot_weight_mm >= 0.0 || -1.0 == vp_settings->m_plot_weight_mm))
...
double
ON_Layer::PlotWeight(const ON_UUID& viewport_id) const
{
  return PerViewportPlotWeight(viewport_id);
}

bool
ON_Layer::PerViewportIsVisible(ON_UUID viewport_id) const
{
  if (!ExtensionBit(m_extension_bits, 0x01)) {
    if (ON_UuidIsNil(viewport_id)) {
      ...
      if (!m_bVisible) {
        ...
        const ON__LayerExtensions* ud =
            ON__LayerExtensions::LayerExtensions(*this, &m_extension_bits, false);
        if (0 != ud) {
      ...
      const ON__LayerPerViewSettings* vp_settings =
...
bool
ON_Layer::IsVisible(const ON_UUID& viewport_id) const
{
  return PerViewportIsVisible(viewport_id);
}

void
ON_Layer::SetVisible(bool bVisible, const ON_UUID& viewport_id)
{
  SetPerViewportVisible(viewport_id, bVisible);
}

void
ON_Layer::SetPerViewportVisible(ON_UUID viewport_id, bool bVisible)
{
  if (ON_UuidIsNil(viewport_id)) {
    ...
    ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, true);
    if (vp_settings) {
      vp_settings->m_visible = (bVisible) ? 1  // layer is on in this viewport
...
bool
ON_Layer::PerViewportPersistentVisibility(ON_UUID viewport_id) const
{
...
void
ON_Layer::SetPerViewportPersistentVisibility(ON_UUID viewport_id, bool bVisibleChild)
{
  // added to fix bug 82587
  if (ON_UuidIsNotNil(viewport_id)) {
    ...
    ON__LayerPerViewSettings* vp_settings = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, viewport_id, bCreate);
    if (vp_settings)
...
void
ON_Layer::UnsetPerViewportPersistentVisibility(ON_UUID viewport_id)
{
  // added to fix bug 82587
  if (ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions* ud =
...
void
ON_Layer::DeletePerViewportColor(const ON_UUID& viewport_id)
{
  if (ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions* ud =
        ON__LayerExtensions::LayerExtensions(*this, &m_extension_bits, false);
...
void
ON_Layer::DeletePerViewportPlotColor(const ON_UUID& viewport_id)
{
  if (ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions* ud =
        ON__LayerExtensions::LayerExtensions(*this, &m_extension_bits, false);
...
int
ON_Layer::UpdateViewportIds(const ON_UuidPairList& viewport_id_map)
{
  ...
  for (int i = 0; i < ud->m_vp_settings.Count(); i++) {
    ...
    if (viewport_id_map.FindId1(s.m_viewport_id, &new_id) &&
...
void
ON_Layer::DeletePerViewportPlotWeight(const ON_UUID& viewport_id)
{
  if (ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions* ud =
        ON__LayerExtensions::LayerExtensions(*this, &m_extension_bits, false);
...
void
ON_Layer::DeletePerViewportVisible(const ON_UUID& viewport_id)
{
  if (ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions* ud =
        ON__LayerExtensions::LayerExtensions(*this, &m_extension_bits, false);
...
ON_Layer::GetPerViewportVisibilityViewportIds(
    ON_SimpleArray<ON_UUID>& viewport_id_list) const
{
...
bool
ON_Layer::HasPerViewportSettings(const ON_UUID& viewport_id) const
{
  return HasPerViewportSettings(viewport_id, 0xFFFFFFFF);
}

bool
ON_Layer::HasPerViewportSettings(ON_UUID viewport_id, unsigned int settings_mask) const
{
  ...
  if (0 != settings_mask) {
    if (ON_UuidIsNil(viewport_id)) {
      ...
      const ON__LayerPerViewSettings* pvs = ON__LayerExtensions::ViewportSettings(
          *this, &m_extension_bits, viewport_id, false);
...
bool
ON_Layer::CopyPerViewportSettings(ON_UUID source_viewport_id,
                                  ON_UUID destination_viewport_id)
{
  bool rc = false;
  if (ON_UuidIsNotNil(source_viewport_id) && ON_UuidIsNotNil(destination_viewport_id) &&
      0 != ON_UuidCompare(source_viewport_id, destination_viewport_id)) {
    const ON__LayerPerViewSettings* src = ON__LayerExtensions::ViewportSettings(
        *this, &m_extension_bits, source_viewport_id, false);
    if (0 != src) {
      ...
      ON__LayerPerViewSettings* dst = ON__LayerExtensions::ViewportSettings(
          *this, &m_extension_bits, destination_viewport_id, true);
      if (0 != dst) {
        *dst = local_src;
...
bool
ON_Layer::CopyPerViewportSettings(const ON_Layer& source_layer,
                                  ON_UUID viewport_id,
                                  unsigned int settings_mask)
{
  bool rc = false;
  if (0 != settings_mask && this != &source_layer) {
    if (ON_UuidIsNil(viewport_id)) {
      ...
      const ON__LayerExtensions* soruce_layer_ud = ON__LayerExtensions::LayerExtensions(
      ...
      if (0 != soruce_layer_ud) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          ON__LayerPerViewSettings* dst = ON__LayerExtensions::ViewportSettings(
              *this, &m_extension_bits, src.m_viewport_id, true);
          if (0 != dst) {
      ...
      const ON__LayerPerViewSettings* src = ON__LayerExtensions::ViewportSettings(
          source_layer, &source_layer.m_extension_bits, viewport_id, false);
      if (0 != src) {
        ON__LayerPerViewSettings* dst = ON__LayerExtensions::ViewportSettings(
            *this, &m_extension_bits, viewport_id, true);
        if (0 != dst) {
...
void
ON_Layer::DeletePerViewportSettings(const ON_UUID& viewport_id) const
{
  if (ON_UuidIsNil(viewport_id)) {
    ON__LayerExtensions::DeleteViewportSettings(*this, &m_extension_bits, 0);
  }
...
void
ON_Layer::CullPerViewportSettings(int viewport_id_count,
...
ON__UINT32
ON_Layer::PerViewportSettingsCRC() const
{
  ON__UINT32 crc = 0;
  if (!ExtensionBit(m_extension_bits, 0x01)) {
    ON__LayerExtensions* ud =
...
unsigned int
ON_Layer::Differences(const ON_Layer& layer0, const ON_Layer& layer1)
{
  /*
  enum
  {
...
void
ON_Layer::Set(unsigned int settings, const ON_Layer& settings_values)
{
...
class /*NEVER EXPORT THIS CLASS DEFINITION*/ ON__LayerSettingsUserData
: public ON_UserData {
...
public:
  ...
  static ON__LayerSettingsUserData*
  LayerSettings (const ON_Layer& layer, bool bCreate);
...
public:
  enum {
    valid_settings = (ON_Layer::color_settings | ON_Layer::plot_color_settings |
                      ON_Layer::visible_settings | ON_Layer::locked_settings |
  ...
  HaveSettings () const
  {
    return 0 != (m_settings & ON__LayerSettingsUserData::valid_settings);
  }

  bool
  HaveColor () const
  {
    return 0 != (m_settings & ON_Layer::color_settings);
  }
  bool
  HavePlotColor () const
  {
    return 0 != (m_settings & ON_Layer::plot_color_settings);
  }
  bool
  HaveVisible () const
  {
    return 0 != (m_settings & ON_Layer::visible_settings);
  }
  bool
  HaveLocked () const
  {
    return 0 != (m_settings & ON_Layer::locked_settings);
  }
  bool
  HavePlotWeight () const
  {
    return 0 != (m_settings & ON_Layer::plot_weight_settings);
  }
...
ON__LayerSettingsUserData*
ON__LayerSettingsUserData::LayerSettings(const ON_Layer& layer, bool bCreate)
{
  ON__LayerSettingsUserData* ud = ON__LayerSettingsUserData::Cast(layer.GetUserData(
      ON__LayerSettingsUserData::m_ON__LayerSettingsUserData_class_id.Uuid()));
  if (!ud && bCreate) {
...
// virtual ON_Object override
ON_BOOL32
ON__LayerSettingsUserData::IsValid(ON_TextLog*) const
{
  return true;
}
...
// virtual ON_Object override
unsigned int
ON__LayerSettingsUserData::SizeOf() const
{
...
// virtual ON_Object override
ON__UINT32
ON__LayerSettingsUserData::DataCRC(ON__UINT32 current_remainder) const
{
  ON__UINT32 crc = current_remainder;
  crc = ON_CRC32(crc, sizeof(m_settings), &m_settings);
  if (HaveColor())
    crc = ON_CRC32(crc, sizeof(m_color), &m_color);
  if (HavePlotColor())
    crc = ON_CRC32(crc, sizeof(m_plot_color), &m_plot_color);
  if (HaveVisible())
    crc = ON_CRC32(crc, sizeof(m_bVisible), &m_bVisible);
  if (HaveLocked())
    crc = ON_CRC32(crc, sizeof(m_bLocked), &m_bLocked);
  if (HavePlotWeight())
    crc = ON_CRC32(crc, sizeof(m_plot_weight_mm), &m_plot_weight_mm);
  return crc;
}

// virtual ON_Object override
ON_BOOL32
ON__LayerSettingsUserData::Write(ON_BinaryArchive& binary_archive) const
{
  bool rc = binary_archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK, 1, 0);
  if (!rc)
    return false;
...
// virtual ON_Object override
ON_BOOL32
ON__LayerSettingsUserData::Read(ON_BinaryArchive& binary_archive)
{
  Defaults();
...
// virtual ON_UserData override
ON_BOOL32
ON__LayerSettingsUserData::Archive() const
{
...
// virtual ON_UserData override
ON_BOOL32
ON__LayerSettingsUserData::GetDescription(ON_wString& description)
{
...
void
ON_Layer::SaveSettings(unsigned int settings, bool bUpdate)
{
...
unsigned int
ON_Layer::SavedSettings() const
{
  const ON__LayerSettingsUserData* ud =
      ON__LayerSettingsUserData::LayerSettings(*this, false);
  return (0 != ud ? ud->m_settings : 0);
}

bool
ON_Layer::GetSavedSettings(ON_Layer& layer, unsigned int&) const
{
  const ON__LayerSettingsUserData* ud =
      ON__LayerSettingsUserData::LayerSettings(*this, false);
  if (0 == ud)
...
bool
ON_Layer::PersistentVisibility() const
{
...
void
ON_Layer::SetPersistentVisibility(bool bVisibleChild)
{
  const unsigned char and_mask = 0xF9;
  const unsigned char or_bit =
      ON_UuidIsNotNil(m_parent_layer_id) ? (bVisibleChild ? 0x02 : 0x04) : 0x00;
...
bool
ON_Layer::PersistentLocking() const
{
...
void
ON_Layer::SetPersistentLocking(bool bLockedChild)
{
  const unsigned char and_mask = 0xE7;
  const unsigned char or_bit =
      ON_UuidIsNotNil(m_parent_layer_id) ? (bLockedChild ? 0x08 : 0x10) : 0x00;
  m_extension_bits &= and_mask;
  m_extension_bits |= or_bit;
}
