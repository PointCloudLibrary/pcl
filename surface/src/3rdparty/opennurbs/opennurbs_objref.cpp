...
: m_type(ON_COMPONENT_INDEX::invalid_type), m_index(-1)
{}

ON_COMPONENT_INDEX::ON_COMPONENT_INDEX(ON_COMPONENT_INDEX::TYPE type, int index)
: m_type(type), m_index(index)
{}

ON_COMPONENT_INDEX::TYPE
ON_COMPONENT_INDEX::Type(int i)
{
  TYPE t = invalid_type;
  ...
  case extrusion_bottom_profile:
    t = extrusion_bottom_profile;
    break;
  case extrusion_top_profile:
    t = extrusion_top_profile;
    break;
  case extrusion_wall_edge:
    t = extrusion_wall_edge;
    break;
  case extrusion_wall_surface:
    t = extrusion_wall_surface;
    break;
  case extrusion_cap_surface:
    t = extrusion_cap_surface;
    break;
  case extrusion_path:
    t = extrusion_path;
    break;
...
void
ON_COMPONENT_INDEX::Set(ON_COMPONENT_INDEX::TYPE type, int index)
{
  m_type = type;
  m_index = index;
}
...
bool
ON_COMPONENT_INDEX::IsMeshComponentIndex() const
{
  bool rc = false;
  switch (m_type) {
...
bool
ON_COMPONENT_INDEX::IsAnnotationComponentIndex() const
{
  bool rc = false;
  switch (m_type) {
...
bool
ON_COMPONENT_INDEX::IsBrepComponentIndex() const
{
  bool rc = false;
  switch (m_type) {
...
bool
ON_COMPONENT_INDEX::IsIDefComponentIndex() const
{
  return (ON_COMPONENT_INDEX::idef_part == m_type && m_index >= 0);
}

bool
ON_COMPONENT_INDEX::IsPolyCurveComponentIndex() const
{
  return (ON_COMPONENT_INDEX::polycurve_segment == m_type && m_index >= 0);
}

bool
ON_COMPONENT_INDEX::IsGroupMemberComponentIndex() const
{
  return (ON_COMPONENT_INDEX::group_member == m_type && m_index >= 0);
}

bool
ON_COMPONENT_INDEX::IsExtrusionProfileComponentIndex() const
{
...
bool
ON_COMPONENT_INDEX::IsExtrusionPathComponentIndex() const
{
  return (ON_COMPONENT_INDEX::extrusion_path == m_type && m_index >= -1 &&
          m_index <= 1);
}

bool
ON_COMPONENT_INDEX::IsExtrusionWallEdgeComponentIndex() const
{
  return (ON_COMPONENT_INDEX::extrusion_wall_edge == m_type && m_index >= 0);
}

bool
ON_COMPONENT_INDEX::IsExtrusionWallSurfaceComponentIndex() const
{
  return (ON_COMPONENT_INDEX::extrusion_wall_surface == m_type && m_index >= 0);
}

bool
ON_COMPONENT_INDEX::IsExtrusionWallComponentIndex() const
{
...
bool
ON_COMPONENT_INDEX::IsExtrusionComponentIndex() const
{
  return ((ON_COMPONENT_INDEX::extrusion_bottom_profile == m_type ||
           ON_COMPONENT_INDEX::extrusion_top_profile == m_type ||
...
bool
ON_COMPONENT_INDEX::IsPointCloudComponentIndex() const
{
...
int
ON_COMPONENT_INDEX::Compare(const ON_COMPONENT_INDEX* a, const ON_COMPONENT_INDEX* b)
{
...
bool
ON_COMPONENT_INDEX::operator==(const ON_COMPONENT_INDEX& other) const
{
...
void
ON_ObjRefEvaluationParameter::Default()
{
...
bool
ON_ObjRefEvaluationParameter::Write(ON_BinaryArchive& archive) const
{
...
bool
ON_ObjRefEvaluationParameter::Read(ON_BinaryArchive& archive)
{
  Default();
...
bool
ON_ObjRef_IRefID::Write(ON_BinaryArchive& archive) const
{
...
bool
ON_ObjRef_IRefID::Read(ON_BinaryArchive& archive)
{
  Default();
...
bool
ON_ObjRef::Write(ON_BinaryArchive& archive) const
{
...
bool
ON_ObjRef::Read(ON_BinaryArchive& archive)
{
  int major_version = 0;
  int minor_version = 0;
  bool rc =
...
ON_ObjRef::~ON_ObjRef() { DecrementProxyReferenceCount(); }

void
ON_ObjRef::RemapObjectId(const ON_SimpleArray<ON_UuidPair>& id_remap)
{
  ...
  int i =
      id_remap.BinarySearch((const ON_UuidPair*)&m_uuid, ON_UuidPair::CompareFirstUuid);
  if (i >= 0)
    m_uuid = id_remap[i].m_uuid[1];
}

int
ON_ObjRef::ProxyReferenceCount() const
{
  return m__proxy_ref_count ? *m__proxy_ref_count : 0;
}

const ON_Brep*
ON_BrepParent (const ON_Geometry* geo)
{
...
const ON_Mesh*
ON_MeshParent (const ON_Geometry* geo)
{
                         ...
                         int idef_geometry_index)
{
  ...
  if (m__iref.Count() > 0) {
    // nested irefs
    if (0 == m__proxy2 || 0 == m__proxy_ref_count || *m__proxy_ref_count <= 0) {
      return false;
    }
    ...
    while (m__proxy1 || m__proxy2 || m__proxy_ref_count) {
      ...
      if (0 != m__proxy1 && 0 == m__proxy2 && 0 != m__proxy_ref_count &&
          1 == *m__proxy_ref_count && m__proxy1 != m_geometry &&
...
const ON_Object*
ON_ObjRef::ProxyObject(int proxy_object_index) const
{
  return ((1 == proxy_object_index) ? m__proxy1
                                    : ((2 == proxy_object_index) ? m__proxy2 : 0));
}

void
ON_ObjRef::SetProxy(ON_Object* proxy1, ON_Object* proxy2, bool bCountReferences)
{
  ...
  if (bCountReferences && (m__proxy1 || m__proxy2)) {
    m__proxy_ref_count =
        (int*)onmalloc_from_pool(ON_MainMemoryPool(), sizeof(*m__proxy_ref_count));
    *m__proxy_ref_count = 1;
  }
}

void
ON_ObjRef::DecrementProxyReferenceCount()
{
