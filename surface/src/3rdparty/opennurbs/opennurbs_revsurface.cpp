...
void
ON_RevSurface::DestroyRuntimeCache (bool bDelete)
{
...
ON_RevSurface*
ON_RevSurface::New (const ON_RevSurface& rev_surface)
{
...
ON_RevSurface::~ON_RevSurface() { Destroy(); }

void
ON_RevSurface::Destroy()
{
  DestroySurfaceTree();
  if (m_curve) {
    delete m_curve;
...
ON__UINT32
ON_RevSurface::DataCRC (ON__UINT32 current_remainder) const
{
  if (m_curve)
...
ON_RevSurface&
ON_RevSurface::operator= (const ON_RevSurface& src)
{
  if (this != &src) {
    Destroy();
...
ON_BOOL32
ON_RevSurface::SetAngleRadians (double start_angle_radians, double end_angle_radians)
{
  bool rc = false;
...
{
  return SetAngleRadians (start_angle_degrees * ON_PI / 180.0,
                          end_angle_degrees * ON_PI / 180.0);
}

ON_BOOL32
ON_RevSurface::IsValid (ON_TextLog* text_log) const
{
  if (!m_curve) {
    if (text_log)
...
void
ON_RevSurface::Dump (ON_TextLog& dump) const
{
...
ON_BOOL32
ON_RevSurface::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_RevSurface::Read (ON_BinaryArchive& file)
{
  int major_version = 0;
  int minor_version = 0;
  char bHaveCurve = 0;
...
int
ON_RevSurface::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_RevSurface::Transform (const ON_Xform& xform)
{
  DestroyRuntimeCache();
...
ON_BOOL32
ON_RevSurface::Evaluate ( // returns false if unable to evaluate
    double s,             // angle
...
int
ON_RevolutionTensor::DimensionA() const
{
  return 2;
}

int
ON_RevolutionTensor::DimensionB() const
{
  return 3;
}

int
ON_RevolutionTensor::DimensionC() const
{
  return 3;
}
                               ...
                               double* SrfPoint)
{
  ...
  x = (ShapePoint[0] - O.x) * X.x + (ShapePoint[1] - O.y) * X.y +
      (ShapePoint[2] - O.z) * X.z;
  y = (ShapePoint[0] - O.x) * Y.x + (ShapePoint[1] - O.y) * Y.y +
      (ShapePoint[2] - O.z) * Y.z;
  z = (ShapePoint[0] - O.x) * Z.x + (ShapePoint[1] - O.y) * Z.y +
      (ShapePoint[2] - O.z) * Z.z;
...
int
ON_RevSurface::GetNurbForm (class ON_NurbsSurface& srf, double tolerance) const
{
  int rc = 0;
  if (0 != m_curve) {
    ON_NurbsCurve a, c;
...
int
ON_RevSurface::HasNurbForm() const
...
bool
ON_RevSurface::GetNurbFormParameterFromSurfaceParameter (double surface_s,
                                                         double surface_t,
                                                         double* nurbs_s,
                                                         double* nurbs_t) const
{
  ...
  if (m_bTransposed) {
    double temp = surface_s;
    surface_s = surface_t;
    surface_t = temp;
    double* pTemp = nurbs_s;
    nurbs_s = nurbs_t;
    nurbs_t = pTemp;
  }
...
ON_Curve*
ON_RevSurface::IsoCurve (int dir, double c) const
{
...
ON_BOOL32
ON_RevSurface::Trim (int dir, const ON_Interval& domain)
{
  ON_BOOL32 rc = false;
  if (dir != 0 && dir != 1)
    return false;
...
bool
ON_RevSurface::Extend (int dir, const ON_Interval& domain)
{
  if (dir != 0 && dir != 1)
    return false;
  if (IsClosed (dir))
  ...
  if (dir == 0) {
    ...
    a[0] = m_angle.ParameterAt (t0);
    a[1] = m_angle.ParameterAt (t1);
    if (a.Length() > 2.0 * ON_PI + ON_ZERO_TOLERANCE)
      a[1] = a[0] + 2.0 * ON_PI;
    m_angle = a;
    m_t = dom;
    rc = true;
...
ON_BOOL32
ON_RevSurface::IsSingular (int side) const // 0 = south, 1 = east, 2 = north, 3 = west
{
  bool rc = false;
  ON_3dPoint P, Q;
  // double d, tol;
  ...
  if (0 == side || 2 == side) {
    P = (0 == side) ? m_curve->PointAtStart() : m_curve->PointAtEnd();
...
ON_BOOL32
ON_RevSurface::IsPeriodic (int dir) const // dir  0 = "s", 1 = "t"
{
  ON_BOOL32 rc = false;
  if (m_bTransposed)
    dir = dir ? 0 : 1;
...
ON_BOOL32
ON_RevSurface::IsClosed (int dir) const // dir  0 = "s", 1 = "t"
{
  ON_BOOL32 rc = false;
  if (m_bTransposed)
    dir = dir ? 0 : 1;
...
ON_BOOL32
ON_RevSurface::GetParameterTolerance ( // returns tminus < tplus: parameters tminus <= s
                                       // <= tplus
...
ON_Interval
ON_RevSurface::Domain (int dir) const
{
  ON_Interval d;
  if (m_bTransposed)
    dir = 1 - dir;
...
ON_BOOL32
ON_RevSurface::GetSurfaceSize (double* width, double* height) const
{
  ON_BOOL32 rc = false;
  if (m_bTransposed) {
    double* ptr = width;
...
int
ON_RevSurface::SpanCount (int dir) const
{
  int span_count = 0;
  if (m_bTransposed)
    dir = 1 - dir;
...
ON_BOOL32
ON_RevSurface::GetSpanVector (int dir, double* s) const
{
  ON_BOOL32 rc = false;
  if (m_bTransposed)
    dir = 1 - dir;
...
int
ON_RevSurface::Degree (int dir) const
{
  int span_degree = 0;
  if (m_bTransposed)
    dir = 1 - dir;
...
void
ON_RevSurface::ClearBoundingBox()
{
  m_bbox.Destroy();
}

ON_BOOL32
ON_RevSurface::GetBBox ( // returns true if successful
...
) const
{
  ...
  if (!rc) {
    ...
    if (rc) {
      // Dec 16 2010 - Chuck - if the angle range does not include 0, m_curve is not
      // part of the surface. bbox = cbox;
  ...
  if (rc) {
    if (boxmin) {
      if (bGrowBox) {
        if (m_bbox.m_min.x < boxmin[0])
          boxmin[0] = m_bbox.m_min.x;
        if (m_bbox.m_min.y < boxmin[1])
          boxmin[1] = m_bbox.m_min.y;
        if (m_bbox.m_min.z < boxmin[2])
    ...
    if (boxmax) {
      if (bGrowBox) {
        if (m_bbox.m_max.x > boxmax[0])
          boxmax[0] = m_bbox.m_max.x;
        if (m_bbox.m_max.y > boxmax[1])
          boxmax[1] = m_bbox.m_max.y;
        if (m_bbox.m_max.z > boxmax[2])
      ...
      if (m_curve->IsArc (&plane, &arc, tolerance)) {
        ...
        if (P.DistanceTo (arc.Center()) <= tolerance) {
          ...
          if (sphere) {
            ...
            sphere->plane.xaxis =
                ON_CrossProduct (sphere->plane.zaxis, sphere->plane.yaxis);
...
bool
ON_Surface::IsTorus (ON_Torus* torus, double tolerance) const
{
  if (!ON_IsValid (tolerance) || tolerance <= 0.0)
    tolerance = ON_ZERO_TOLERANCE;
...
ON_BOOL32
ON_RevSurface::IsCylindrical (ON_Cylinder* cylinder, double tolerance) const
{
  ON_Cylinder c;
  ON_Line line;
  double r[2] = {0.0, 0.0};
...
ON_BOOL32
ON_RevSurface::IsConical (ON_Cone* cone, double tolerance) const
{
  ON_Cone c;
  ON_Line line;
  double r[2] = {0.0, 0.0};
