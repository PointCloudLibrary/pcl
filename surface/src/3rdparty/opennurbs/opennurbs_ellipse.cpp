...
ON_BOOL32
ON_Ellipse::IsValid() const
{
  return (plane.IsValid() && radius[0] > ON_ZERO_TOLERANCE &&
          radius[1] > ON_ZERO_TOLERANCE)
             ? true
             : false;
}
...
const ON_3dVector&
ON_Ellipse::Normal() const
{
  return plane.zaxis;
}

const ON_Plane&
ON_Ellipse::Plane() const
{
  return plane;
}
...
static int
distSqToEllipse (void* p, double t, double* f, double* df)
{
