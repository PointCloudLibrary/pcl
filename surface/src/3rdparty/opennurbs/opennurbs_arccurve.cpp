...
ON_ArcCurve::ON_ArcCurve (const ON_Circle& circle) { ON_ArcCurve::operator= (circle); }
...
ON__UINT32
ON_ArcCurve::DataCRC (ON__UINT32 current_remainder) const
{
...
ON_ArcCurve&
ON_ArcCurve::operator= (const ON_ArcCurve& src)
{
  if (this != &src) {
...
ON_ArcCurve&
ON_ArcCurve::operator= (const ON_Arc& A)
{
  m_arc = A;
  m_t.m_t[0] = 0.0;
  m_t.m_t[1] = A.Length();
...
ON_BOOL32
ON_ArcCurve::IsValid (ON_TextLog* text_log) const
{
  if (!m_t.IsIncreasing()) {
    if (0 != text_log)
      text_log->Print (
...
void
ON_ArcCurve::Dump (ON_TextLog& dump) const
{
...
ON_BOOL32
ON_ArcCurve::Write (ON_BinaryArchive& file // open binary file
) const
{
...
ON_BOOL32
ON_ArcCurve::Read (ON_BinaryArchive& file // open binary file
)
{
  int major_version = 0;
  int minor_version = 0;
...
ON_BOOL32
ON_ArcCurve::SetDomain (double t0, double t1)
{
  ON_BOOL32 rc = false;
  if (t0 < t1) {
    m_t.Set (t0, t1);
...
bool
ON_ArcCurve::ChangeDimension (int desired_dimension)
{
...
ON_Interval
ON_ArcCurve::Domain() const
{
  return m_t;
}

ON_BOOL32
ON_ArcCurve::ChangeClosedCurveSeam (double t)
{
  bool rc = false;
  if (IsCircle()) {
...
int
ON_ArcCurve::SpanCount() const
{
  return 1;
}

ON_BOOL32
ON_ArcCurve::GetSpanVector (double* s) const
{
  s[0] = m_t[0];
  s[1] = m_t[1];
...
ON_ArcCurve::IsLinear ( // true if curve locus is a line segment
    double // tolerance - formal parameter intentionally ignored in this virtual
           // function
) const
{
  return false;
...
ON_BOOL32
ON_ArcCurve::IsPlanar (ON_Plane* plane, // if not NULL and true is returned, then plane
...
ON_BOOL32
ON_ArcCurve::IsInPlane (const ON_Plane& plane, // plane to test
                        double tolerance // tolerance to use when checking linearity
) const
{
...
ON_BOOL32
ON_ArcCurve::IsClosed() const { return m_arc.IsCircle(); }

ON_BOOL32
ON_ArcCurve::IsPeriodic() const { return m_arc.IsCircle(); }
...
ON_BOOL32
ON_ArcCurve::SetStartPoint (ON_3dPoint start_point)
{
  if (IsCircle())
    return false;
  ON_BOOL32 rc = false;
...
ON_BOOL32
ON_ArcCurve::SetEndPoint (ON_3dPoint end_point)
{
  if (IsCircle())
    return false;
  ON_BOOL32 rc = false;
...
ON_BOOL32
ON_ArcCurve::Evaluate ( // returns false if unable to evaluate
...
ON_BOOL32
ON_ArcCurve::Trim (const ON_Interval& in)
{
  ...
  if (rc) {
    ...
    if (a1 - a0 > ON_ZERO_TOLERANCE &&
        m_arc.SetAngleIntervalRadians (ON_Interval (a0, a1))) {
      m_t = in;
    }
    else {
      rc = false;
...
bool
ON_ArcCurve::Extend (const ON_Interval& domain)
...
ON_BOOL32
ON_ArcCurve::Split (double t, ON_Curve*& left_side, ON_Curve*& right_side) const
{
...
static ON_BOOL32
NurbsCurveArc (const ON_Arc& arc, int dim, ON_NurbsCurve& nurb)
{
  if (!arc.IsValid())
    return false;
...
int
ON_Arc::GetNurbForm (ON_NurbsCurve& nurbscurve) const
...
bool
ON_Arc::GetRadianFromNurbFormParameter (double NurbParameter,
                                        double* RadianParameter) const
{
  //  TRR#53994.
  // 16-Sept-09  Replaced this code so we dont use LocalClosestPoint.
  // In addition to being slower than neccessary the old method suffered from getting
  // the wrong answer at the seam of a full circle,  This probably only happened with
  // large coordinates where many digits of precision get lost.
...
bool
ON_Arc::GetNurbFormParameterFromRadian (double RadianParameter,
...
int
ON_ArcCurve::GetNurbForm ( // returns 0: unable to create NURBS representation
...
ON_BOOL32
ON_ArcCurve::GetCurveParameterFromNurbFormParameter (double nurbs_t,
                                                     double* curve_t) const
{
  double radians;

  double arcnurb_t =
      m_arc.DomainRadians().ParameterAt (m_t.NormalizedParameterAt (nurbs_t));
...
ON_BOOL32
ON_ArcCurve::GetNurbFormParameterFromCurveParameter (double curve_t,
                                                     double* nurbs_t) const
{
  double radians =
      m_arc.DomainRadians().ParameterAt (m_t.NormalizedParameterAt (curve_t));
  double arcnurb_t;
  ...
  if (rc) // Oct 29, 2009 - Dale Lear added condition to set *nurbs_t = curve_t
    *nurbs_t =
        m_t.ParameterAt (m_arc.DomainRadians().NormalizedParameterAt (arcnurb_t));
  else
    *nurbs_t = curve_t;
  return rc;
}
...
bool
ON_ArcCurve::IsCircle() const
{
  return m_arc.IsCircle() ? true : false;
}

double
ON_ArcCurve::Radius() const
{
  return m_arc.Radius();
}

double
ON_ArcCurve::AngleRadians() const
{
  return m_arc.AngleRadians();
}

double
ON_ArcCurve::AngleDegrees() const
{
...
const ON_ClassId*
ON__OBSOLETE__CircleCurve::ClassId() const
{
...
ON_BOOL32
ON__OBSOLETE__CircleCurve::Read (ON_BinaryArchive& file // open binary file
)
{
  int major_version = 0;
  int minor_version = 0;
