...
int
ON_Matrix::RowCount() const
{
  return m_row_count;
}

int
ON_Matrix::ColCount() const
{
  return m_col_count;
}

int
ON_Matrix::MinCount() const
{
  return (m_row_count <= m_col_count) ? m_row_count : m_col_count;
}

int
ON_Matrix::MaxCount() const
{
  return (m_row_count >= m_col_count) ? m_row_count : m_col_count;
}
...
)
{
  ...
  if (ri1 > ri0 && ci1 > ci0) {
    ...
    if (b) {
      ...
      m_col_offset = ci0; // this is the only line of code where m_col_offset should be
                          // set to a non-zero value
      if (ci0 != 0) {
        int i;
...
ON_Matrix::SwapRows (int row0, int row1)
{
  ...
  if (this_m && 0 <= row0 && row0 < m_row_count && 0 <= row1 && row1 < m_row_count) {
    if (row0 != row1) {
      double* tmp = this_m[row0];
      this_m[row0] = this_m[row1];
      this_m[row1] = tmp;
    }
    b = true;
  }
...
bool
ON_Matrix::IsRowOrthoNormal() const
{
  double d;
  int i, j;
...
bool
ON_Matrix::IsColOrthoganal() const
{
  double d0, d1, d;
  int i, j0, j1;
...
bool
ON_Matrix::IsColOrthoNormal() const
{
  double d;
  int i, j;
