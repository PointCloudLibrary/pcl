...
const ON_3dPoint&
ON_Plane::Origin() const
{
  return origin;
}

const ON_3dVector&
ON_Plane::Xaxis() const
{
  return xaxis;
}

const ON_3dVector&
ON_Plane::Yaxis() const
{
  return yaxis;
}

const ON_3dVector&
ON_Plane::Normal() const
{
  return zaxis;
}
...
ON_Plane::Transform(const ON_Xform& xform)
{
  ...
  bool bUseVectorXform = (0.0 == xform.m_xform[3][0] && 0.0 == xform.m_xform[3][1] &&
                          0.0 == xform.m_xform[3][2] && 1.0 == xform.m_xform[3][3]);

  ON_3dVector xaxis_vec =
      bUseVectorXform ? (xform * xaxis) : ((xform * (origin + xaxis)) - origin_pt);
