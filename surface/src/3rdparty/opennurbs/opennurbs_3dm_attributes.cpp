...
void
ON_3dmObjectAttributes::Default()
{
  PurgeUserData();
  m_uuid = ON_nil_uuid;
  m_name.Destroy();
...
ON_3dmObjectAttributes::Dump(ON_TextLog& dump) const
{
  ...
  case ON::material_from_layer:
    sMaterialSource = "layer material";
    break;
...
bool
ON_3dmObjectAttributes::IsInstanceDefinitionObject() const
{
  return (ON::idef_object == Mode());
}

bool
ON_3dmObjectAttributes::IsVisible() const
{
  return m_bVisible;
}
...
int
ON_3dmObjectAttributes::GroupCount() const
{
...
const int*
ON_3dmObjectAttributes::GroupList() const
{
...
int
ON_3dmObjectAttributes::TopGroup() const
{
...
void
ON_3dmObjectAttributes::RemoveFromTopGroup()
{
  int c = m_group.Count();
  if (c > 0) {
    c--;
...
void
ON_3dmObjectAttributes::RemoveFromAllGroups()
{
  m_group.Destroy();
}
  ...
  if (i > 0) {
    ...
    if (search_material.m_viewport_id != ON_nil_uuid) {
      if (search_material.m_display_material_id != ON_nil_uuid) {
        while (i--) {
          if ((m_dmref[i].m_display_material_id ==
               search_material.m_display_material_id) &&
...
void
ON_3dmObjectAttributes::RemoveAllDisplayMaterialRefs()
{
  m_dmref.Destroy();
}
...
int
ON_3dmObjectAttributes::DisplayMaterialRefCount() const
{
