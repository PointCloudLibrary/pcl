...
ON_PointCloud::~ON_PointCloud() { Destroy(); }

void
ON_PointCloud::Destroy()
{
  m_H.Destroy();
  m_C.Destroy();
  m_N.Destroy();
...
void
ON_PointCloud::EmergencyDestroy()
{
...
ON::object_type
ON_PointCloud::ObjectType() const
{
  return ON::pointset_object;
}

int
ON_PointCloud::Dimension() const
{
  return 3;
}
...
bool
ON_PointCloud::IsDeformable() const
{
  return true;
}

bool
ON_PointCloud::MakeDeformable()
{
  return true;
}
...
int
ON_PointCloud::PointCount() const
{
  return m_P.Count();
}
...
void
ON_PointCloud::InvalidateBoundingBox()
{
  m_bbox.Destroy();
}
...
bool
ON_PointCloud::IsOrdered() const
{
  return (m_flags & 1) ? true : false;
}

bool
ON_PointCloud::HasPlane() const
{
...
bool
ON_PointCloud::HasPointColors() const
{
  const int point_count = m_P.Count();
...
bool
ON_PointCloud::HasPointNormals() const
{
  const int point_count = m_P.Count();
...
int
ON_PointCloud::HiddenPointCount() const
{
  int point_count;
  return (m_hidden_count > 0 && (point_count = m_P.Count()) > 0 &&
          m_hidden_count < point_count && m_H.Count() == point_count)
             ? m_hidden_count
             : 0;
}

void
ON_PointCloud::DestroyHiddenPointArray()
{
  m_hidden_count = 0;
  m_H.Destroy();
}
...
const bool*
ON_PointCloud::HiddenPointArray() const
{
  return (m_hidden_count > 0 && m_H.Count() == m_P.Count()) ? m_H.Array() : 0;
}
