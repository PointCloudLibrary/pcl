...
bool
ON_PolyEdgeSegment::Create(const ON_BrepTrim* trim, const ON_UUID& object_id)
{
  Init();
  if (!trim)
    return false;
  ...
  ON_CurveProxy::SetProxyCurve(c3, edge->ProxyCurveDomain());
...
bool
ON_PolyEdgeSegment::ReversedEdgeDir() const
{
  bool rc = false;
  if (m_edge) {
...
bool
ON_PolyEdgeSegment::ReversedTrimDir() const
{
  bool rc = false;
  if (m_trim && m_edge) {
    rc = ReversedEdgeDir();
...
bool
ON_PolyEdgeSegment::Create(const ON_Curve* curve, const ON_UUID& object_id)
{
  // bool rc = false;
  Init();
  if (!curve)
...
const ON_BrepEdge*
ON_PolyEdgeSegment::Edge() const
{
  return m_edge;
}

const ON_BrepTrim*
ON_PolyEdgeSegment::Trim() const
{
  return m_trim;
}

const ON_Brep*
ON_PolyEdgeSegment::Brep() const
{
  return m_brep;
}

const ON_BrepFace*
ON_PolyEdgeSegment::Face() const
{
  return m_face;
}

const ON_Surface*
ON_PolyEdgeSegment::Surface() const
{
  return m_surface;
}

ON_Surface::ISO
ON_PolyEdgeSegment::IsoType() const
{
  return m_trim ? m_trim->m_iso : ON_Surface::not_iso;
}

ON_Interval
ON_PolyEdgeSegment::EdgeDomain() const
{
  return m_edge_domain;
}

ON_Interval
ON_PolyEdgeSegment::TrimDomain() const
{
  return m_trim_domain;
}

void
ON_PolyEdgeSegment::ClearEvalCacheHelper()
{
  m_t = ON_UNSET_VALUE;
...
double
ON_PolyEdgeSegment::EdgeParameter(double t) const
{
...
ON_BOOL32
ON_PolyEdgeCurve::SetStartPoint(ON_3dPoint)
{
  return false; // cannot change edges
}

ON_BOOL32
ON_PolyEdgeCurve::SetEndPoint(ON_3dPoint)
{
  return false; // cannot change edges
}

ON_BOOL32
ON_PolyEdgeCurve::ChangeClosedCurveSeam(double t)
{
...
ON_BOOL32
ON_PolyEdgeCurve::PrependAndMatch(ON_Curve*)
{
  return false; // cannot change edges
}

ON_BOOL32
ON_PolyEdgeCurve::AppendAndMatch(ON_Curve*)
{
  return false; // cannot change edges
}

ON_Curve*
ON_PolyEdgeSegment::DuplicateCurve() const
{
...
ON_Curve*
ON_PolyEdgeCurve::DuplicateCurve() const
{
...
ON_BOOL32
ON_PolyEdgeSegment::IsClosed(void) const
{
  ON_BOOL32 rc = ON_CurveProxy::IsClosed();
  if (!rc && m_edge && m_edge->m_vi[0] == m_edge->m_vi[1] &&
      m_edge->ProxyCurve() == ProxyCurve() &&
      m_edge->ProxyCurveDomain() == ProxyCurveDomain() && 0 != ProxyCurve() &&
...
ON_BOOL32
ON_PolyEdgeCurve::Remove(int segment_index)
{
  ...
  if (!rc && SegmentCount() > 1) {
    ...
    if (edge0 && edge1 && edge0->Brep() == edge1->Brep()) {
      ...
      if (et0 != ON_UNSET_VALUE && et1 != ON_UNSET_VALUE) {
        ...
        else if (edge0 == edge1 && fabs(et0 - et1) <= ON_ZERO_TOLERANCE &&
                 edom0.Includes(et0, true) && edom1.Includes(et1, true)) {
...
bool
ON_PolyEdgeCurve::Create(const ON_BrepTrim* trim, const ON_UUID& object_id)
{
  bool rc = false;
  Destroy();
...
bool
ON_PolyEdgeCurve::Create(const ON_Curve* curve, const ON_UUID& object_id)
{
  bool rc = false;
  Destroy();
  // m_is_closed_helper = 0;
...
int
ON_PolyEdgeCurve::SegmentCount() const
{
  return ON_PolyCurve::Count();
}

ON_PolyEdgeSegment*
ON_PolyEdgeCurve::SegmentCurve(int segment_index) const
{
  return ON_PolyEdgeSegment::Cast(ON_PolyCurve::SegmentCurve(segment_index));
}

ON_PolyEdgeSegment*
ON_PolyEdgeCurve::operator[](int segment_index) const
{
  return SegmentCurve(segment_index);
}

void
ON_PolyEdgeCurve::DestroyRuntimeCache(bool bDelete)
{
  // m_is_closed_helper = 0;
...
void
ON_PolyEdgeSegment::DestroyRuntimeCache(bool bDelete)
{
  ClearEvalCacheHelper();
...
ON_BOOL32
ON_PolyEdgeCurve::Prepend(ON_PolyEdgeSegment* new_segment)
{
  DestroyRuntimeCache();
  ON_BOOL32 rc = false;
  if (new_segment) {
...
const ON_BrepEdge*
ON_PolyEdgeCurve::EdgeAt(double t) const
{
...
const ON_BrepTrim*
ON_PolyEdgeCurve::TrimAt(double t) const
{
  ON_PolyEdgeSegment* seg = SegmentCurve(SegmentIndex(t));
...
const ON_Brep*
ON_PolyEdgeCurve::BrepAt(double t) const
{
  ON_PolyEdgeSegment* seg = SegmentCurve(SegmentIndex(t));
...
const ON_BrepFace*
ON_PolyEdgeCurve::FaceAt(double t) const
{
  ON_PolyEdgeSegment* seg = SegmentCurve(SegmentIndex(t));
...
const ON_Surface*
ON_PolyEdgeCurve::SurfaceAt(double t) const
{
  ON_PolyEdgeSegment* seg = SegmentCurve(SegmentIndex(t));
...
ON_Surface::ISO
ON_PolyEdgeCurve::IsoType(double t) const
{
  ON_PolyEdgeSegment* seg = SegmentCurve(SegmentIndex(t));
...
double
ON_PolyEdgeCurve::EdgeParameter(double t) const
{
  double edge_t = ON_UNSET_VALUE;
...
bool
ON_PolyEdgeCurve::ContainsAnyEdges() const
{
...
bool
ON_PolyEdgeCurve::ContainsAllEdges() const
{
  int i, count = SegmentCount();
  for (i = 0; i < count; i++) {
    ON_PolyEdgeSegment* segment = SegmentCurve(i);
...
int
ON_PolyEdgeCurve::FindEdge(const ON_BrepEdge* edge) const
{
  int rc = -1;
  if (0 != edge) {
    int i, count = SegmentCount();
...
int
ON_PolyEdgeCurve::FindTrim(const ON_BrepTrim* trim) const
{
  int rc = -1;
  if (0 != trim) {
    int i, count = SegmentCount();
...
int
ON_PolyEdgeCurve::FindCurve(const ON_Curve* curve) const
{
  int rc = -1;
  if (0 != curve) {
    int i, count = SegmentCount();
    for (i = 0; i < count; i++) {
      ON_PolyEdgeSegment* segment = SegmentCurve(i);
      if (0 != segment && (curve == segment || curve == segment->ProxyCurve() ||
                           curve == segment->Edge())) {
        rc = i;
        break;
      }
...
ON_BOOL32
ON_PolyEdgeSegment::Write(ON_BinaryArchive& archive) const
{
...
ON_BOOL32
ON_PolyEdgeSegment::Read(ON_BinaryArchive& archive)
{
  Init();
  int major_version = 0;
  int minor_version = 0;
