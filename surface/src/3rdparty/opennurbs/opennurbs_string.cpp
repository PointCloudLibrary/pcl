...
void
ON_String::EmergencyDestroy()
{
  Create();
}

void
ON_String::EnableReferenceCounting (bool)
{
  // TODO fill this in;
}

bool
ON_String::IsReferenceCounted() const
{
  return true;
}

ON_aStringHeader*
ON_String::Header() const
{
...
void
ON_String::CreateArray (int capacity)
{
  Destroy();
  if (capacity > 0) {
    ON_aStringHeader* p = (ON_aStringHeader*)onmalloc (sizeof (ON_aStringHeader) +
...
void
ON_String::ReserveArray (std::size_t array_capacity)
{
  ON_aStringHeader* p = Header();
...
void
ON_String::CopyToArray (const ON_String& s)
{
  CopyToArray (s.Length(), s.Array());
}

void
ON_String::CopyToArray (int size, const char* s)
{
  if (size > 0 && s && s[0]) {
    ReserveArray (size);
...
void
ON_String::CopyToArray (int size, const unsigned char* s)
{
  CopyToArray (size, ((char*)s));
}

void
ON_String::AppendToArray (const ON_String& s)
{
  AppendToArray (s.Length(), s.Array());
}

void
ON_String::AppendToArray (int size, const char* s)
{
  if (size > 0 && s && s[0]) {
...
void
ON_String::AppendToArray (int size, const unsigned char* s)
{
  AppendToArray (size, ((char*)s));
}

int
ON_String::Length (const char* s)
{
...
int
ON_String::Length (const unsigned char* s)
{
...
ON_String::ON_String() { Create(); }

ON_String::~ON_String() { Destroy(); }
...
bool
ON_String::LoadResourceString (HINSTANCE instance, UINT id)
{
...
char
ON_String::operator[] (int i) const
{
  return m_s[i];
}

bool
ON_String::IsEmpty() const
{
  return (Header()->string_length <= 0) ? true : false;
}

ON_String&
ON_String::operator= (const ON_String& src)
{
  if (m_s != src.m_s) {
    if (src.IsEmpty()) {
      Destroy();
...
ON_String&
ON_String::operator= (const char* s)
{
  if ((void*)s != (void*)m_s)
...
ON_String&
ON_String::operator= (unsigned char c)
{
  CopyToArray (1, &c);
  return *this;
}
...
ON_String&
ON_String::operator= (const unsigned char* s)
{
  if ((void*)s != (void*)m_s)
    CopyToArray (Length (s), s);
  return *this;
...
ON_String&
ON_String::operator= (const wchar_t* w)
{
...
ON_String&
ON_String::operator= (const ON_wString& w)
{
  *this = w.Array();
  return *this;
}
...
ON_String
ON_String::operator+ (const ON_String& s2) const
{
  ON_String s (*this);
  s.AppendToArray (s2);
  return s;
...
ON_String
ON_String::operator+ (char s2) const
{
  ON_String s (*this);
...
ON_String
ON_String::operator+ (unsigned char s2) const
{
  ON_String s (*this);
  s.AppendToArray (1, &s2);
  return s;
...
ON_String
ON_String::operator+ (const char* s2) const
{
  ON_String s (*this);
...
ON_String
ON_String::operator+ (const unsigned char* s2) const
{
  ON_String s (*this);
  s.AppendToArray (ON_String::Length (s2), s2);
  return s;
...
void
ON_String::Append (const char* s, int count)
{
...
void
ON_String::Append (const unsigned char* s, int count)
{
  // append specified number of characters
  if (s && count > 0)
    AppendToArray (count, s);
...
const ON_String&
ON_String::operator+= (const ON_String& s)
{
  AppendToArray (s);
  return *this;
}
...
const ON_String&
ON_String::operator+= (unsigned char s)
{
  AppendToArray (1, &s);
  return *this;
}
...
void
ON_String::SetLength (std::size_t string_length)
{
...
ON__UINT32
ON_String::DataCRC (ON__UINT32 current_remainder) const
{
...
int
ON_String::Compare (const char* s) const
{
  int rc = 0;
  if (s && s[0]) {
    if (IsEmpty()) {
...
int
ON_String::Compare (const unsigned char* s) const
{
  return ON_String::Compare ((const char*)s);
}

int
ON_String::CompareNoCase (const char* s) const
{
  int rc = 0;
  if (s && s[0]) {
    if (IsEmpty()) {
...
int
ON_String::CompareNoCase (const unsigned char* s) const
{
  return ON_String::CompareNoCase ((const char*)s);
}

ON_String::operator const char*() const { return (m_s == pEmptyaString) ? NULL : m_s; }

bool
ON_WildCardMatch (const char* s, const char* pattern)
{
...
bool
ON_WildCardMatchNoCase (const char* s, const char* pattern)
{
  if (!pattern || !pattern[0]) {
    return (!s || !s[0]) ? true : false;
  }
...
bool
ON_String::WildCardMatch (const char* pattern) const
{
  return ON_WildCardMatch (m_s, pattern);
}

bool
ON_String::WildCardMatch (const unsigned char* pattern) const
{
  return ON_WildCardMatch (m_s, (const char*)pattern);
}

bool
ON_String::WildCardMatchNoCase (const char* pattern) const
{
  return ON_WildCardMatchNoCase (m_s, pattern);
}

bool
ON_String::WildCardMatchNoCase (const unsigned char* pattern) const
{
  return ON_WildCardMatchNoCase (m_s, (const char*)pattern);
}

int
ON_String::Replace (const char* token1, const char* token2)
{
  int count = 0;
...
int
ON_String::Replace (const unsigned char* token1, const unsigned char* token2)
{
  return Replace ((const char*)token1, (const char*)token2);
}

int
ON_String::Replace (char token1, char token2)
{
  int count = 0;
  int i = Length();
  while (i--) {
...
int
ON_String::Replace (unsigned char token1, unsigned char token2)
{
...
int
ON_String::Find (unsigned char c) const
{
  return Find ((char)c);
}

int
ON_String::ReverseFind (char c) const
{
  // find first single character
  if (IsEmpty())
    return -1;
...
int
ON_String::ReverseFind (unsigned char c) const
{
  return ReverseFind ((char)c);
}

int
ON_String::Find (const char* s) const
{
  int rc = -1;
  if (s && s[0] && !IsEmpty()) {
    const char* p;
...
int
ON_String::Find (const unsigned char* s) const
{
...
void
ON_String::TrimLeft (const char* s)
{
  char c;
  const char* sc;
  char* dc;
...
void
ON_String::TrimRight (const char* s)
{
  char c;
  const char* sc;
...
void
ON_String::TrimLeftAndRight (const char* s)
{
  TrimRight (s);
  TrimLeft (s);
}
...
int
ON_String::Remove (const char chRemove)
{
  CopyArray();
  ...
  *pstrDest = 0;
  int nCount =
      (int)(pstrSource - pstrDest); // the (int) is for 64 bit std::size_t conversion
...
void
ON_String::SetAt (int i, unsigned char c)
{
  SetAt (i, (char)c);
}

ON_String
ON_String::Mid (int i, int count) const
{
  ON_String s;
...
ON_String
ON_String::Mid (int i) const
{
  return Mid (i, Length() - i);
}

ON_String
ON_String::Left (int count) const
{
  ON_String s;
  if (count > Length())
    count = Length();
...
ON_String
ON_String::Right (int count) const
{
  ON_String s;
  if (count > Length())
    count = Length();
...
void ON_MSC_CDECL
ON_String::Format (const char* sFormat, ...)
{
...
void ON_MSC_CDECL
ON_String::Format (const unsigned char* sFormat, ...)
{
#define MAX_MSG_LENGTH 2048
  char sMessage[MAX_MSG_LENGTH];
  va_list args;
...
bool
ON_CheckSum::IsSet() const
{
  return (0 != m_size || 0 != m_time || 0 != m_crc[0] || 0 != m_crc[1] ||
          0 != m_crc[2] || 0 != m_crc[3] || 0 != m_crc[4] || 0 != m_crc[5] ||
          0 != m_crc[6] || 0 != m_crc[7]);
}

bool
ON_CheckSum::SetBufferCheckSum (std::size_t size, const void* buffer, time_t time)
{
  bool rc = false;
  Zero();
  if (size != 0 && buffer != 0) {
...
bool
ON::IsDirectory (const wchar_t* pathname)
{
  bool rc = false;
...
bool
ON::IsDirectory (const char* utf8pathname)
{
  bool rc = false;
...
int
ON::IsOpenNURBSFile (const wchar_t* pathname)
{
  int version = 0;
  if (0 != pathname && 0 != pathname[0]) {
...
int
ON::IsOpenNURBSFile (const char* utf8pathname)
{
  int version = 0;
  if (0 != utf8pathname && 0 != utf8pathname[0]) {
...
bool
ON_CheckSum::SetFileCheckSum (FILE* fp)
{
  bool rc = false;
  Zero();
  if (fp) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      while (1024 == count && m_size < sz0) {
        count = (int)fread (
            buffer, 1, 1024, fp); // the (int) is for 64 bit std::size_t conversion
        if (count > 0) {
          m_size += count;
...
bool
ON_CheckSum::Write (ON_BinaryArchive& archive) const
{
  bool rc = false;
...
bool
ON_CheckSum::Read (ON_BinaryArchive& archive)
{
  bool rc;
...
bool
ON_CheckSum::SetFileCheckSum (const wchar_t* filename)
{
  bool rc = false;
  Zero();
...
bool
ON_CheckSum::CheckBuffer (std::size_t size, const void* buffer) const
{
  if (m_size != size)
    return false;
  if (0 == size)
...
bool
ON_CheckSum::CheckFile (FILE* fp, bool bSkipTimeCheck) const
{
  if (!fp)
    return false;
...
bool
ON_CheckSum::CheckFile (const wchar_t* filename, bool bSkipTimeCheck) const
{
  bool rc = false;
  if (filename && filename[0]) {
    FILE* fp = ON::OpenFile (filename, L"rb");
...
void
ON_CheckSum::Dump (ON_TextLog& text_log) const
{
