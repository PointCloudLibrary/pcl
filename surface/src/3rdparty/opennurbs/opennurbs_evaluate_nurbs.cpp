...
double
ON_EvaluateBernsteinBasis (int degree, int i, double t)
/*****************************************************************************
...
bool
ON_RemoveBezierSingAt0 (int dim, int order, int cv_stride, double* cv)
{
...
bool
ON_RemoveBezierSingAt1 (int dim, int order, int cv_stride, double* cv)
{
  const int cvdim = dim + 1;
  int i, k, ord0, CVlen;
  ord0 = order;
...
bool
ON_EvaluateNurbsBasis (int order, const double* knot, double t, double* N)
{
  /*****************************************************************************
...
static bool
ON_EvaluateNurbsRationalSpan (int dim,            // dimension
                              int order,          // order
                              const double* knot, // knot[] array of (2*order-2) doubles
                              int cv_stride,      // cv_stride >= (is_rat)?dim+1:dim
                              const double* cv,   // cv[order*cv_stride] array
...
bool
ON_EvaluateNurbsSpan (int dim,            // dimension
                      ON_BOOL32 is_rat,   // true if NURBS is rational
                      int order,          // order
                      const double* knot, // knot[] array of (2*order-2) doubles
                      int cv_stride,      // cv_stride >= (is_rat)?dim+1:dim
      ...
      if (der_count > 2) {
        ...
        for (d = 3; d <= der_count; d++) {
          ...
          for (j0 = 0; j0 < order0; j0++) {
            cv = cv0 + j0 * cv_stride0;
            for (j1 = 0; j1 < order1; j1++) {
              for (d0 = d, d1 = 0; d0 > der_count0 && d1 <= d1max; d0--, d1++) {
 ...
 */
{
  double workarray[21], alpha0, alpha1, t0, t1, dt, *delta_t, *free_delta_t, *cv0, *cv1;
  const double *k0, *k1;
  int degree, i, j, k;
  ...
  for (i = 0; i < cvdim; i++) { // do one coordinate at a time
    ...
    for (j = 1; j < order; j++) {
      for (k = j; k < order; k++) { // djk
        space[k - j] = (knot[degree + k - j] - t[j - 1]) /
                           (knot[degree + k - j] - knot[k - 1]) * space[k - j] +
                       (t[j - 1] - knot[k - 1]) / (knot[degree + k - j] - knot[k - 1]) *
                           space[k - j + 1];
      }
    }
