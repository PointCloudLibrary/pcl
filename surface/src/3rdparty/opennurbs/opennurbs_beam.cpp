...
ON_Extrusion::GetBBox (double* boxmin, double* boxmax, int bGrowBox) const
{
  ...
  if (m_path.IsValid() && m_profile) {
    ...
    if (m_profile->GetTightBoundingBox (bbox) &&
        GetBoundingBoxHelper (*this, bbox, 0)) {
      rc = true;
      if (bGrowBox) {
...
Profile2dTransform (ON_Extrusion& e, const ON_Xform& profile_xform, bool bNeedReverse)
{
  ...
  if (0 != polyprofile) {
    ...
    if (bNeedDeformable && !e.m_profile->IsDeformable()) {
      ...
      rc = e.m_profile->Transform (profile_xform) ? true
                                                  : ON_Extrusion_TransformFailed();
    }
    if (rc && bNeedReverse) {
      double t0, t1;
...
ON_Extrusion::Transform (const ON_Xform& xform)
{
  ...
  if ((bSamePathDir && bSameUpDir) ||
      fabs (fabs (QUxQToUxT) - 1.0) <= ON_SQRT_EPSILON) {
    if (QUxQToUxT < 0.0)
      QUxQT = -UxT;
    else
      QUxQT = UxT;
  ...
  for (int profile_index = 0; profile_index < profile_count; profile_index++) {
    ...
    profile_fi.Init();
    {
      ...
      profile_fi.m_profile_orientation =
          (profile_fi.m_bClosedProfile) ? ON_ClosedCurveOrientation (*newprofile, 0)
                                        : 0;
...
) const
{
  ...
  if (m_bTransposed && num_der > 0) {
    ...
    void* tmp =
        (sz <= sizeof (xform0)) ? ((void*)&xform0.m_xform[0][0]) : onmalloc (sz);
    for (i = 1; i <= num_der; i++) {
