...
static bool
ON_ExtrusionPolyCurveProfileIsNotValid ()
{
  return false; // good place for a breakpoint
}

bool
ON_Extrusion::IsValidPolyCurveProfile(const ON_PolyCurve& polycurve,
                                      ON_TextLog* text_log)
{
  const bool bAllowGaps = true;
  bool rc = polycurve.IsValid(bAllowGaps, text_log)
                ? true
                : ON_ExtrusionPolyCurveProfileIsNotValid();
  if (!rc)
...
bool
ON_Extrusion::CleanupPolyCurveProfile(ON_PolyCurve& polycurve)
{
...
static void
ON_ExtrusionInitializeHelper (ON_Extrusion& extrusion)
{
...
static void
ON_ExtrusionCopyHelper (const ON_Extrusion& src, ON_Extrusion& dst)
{
  if (&src != &dst) {
    if (dst.m_profile) {
    ...
    dst.m_profile = src.m_profile ? src.m_profile->DuplicateCurve() : 0;
...
bool
ON_Extrusion::SetPath(ON_3dPoint A, ON_3dPoint B)
{
  double distAB = 0.0;
  bool rc =
      A.IsValid() && B.IsValid() && (distAB = A.DistanceTo(B)) > ON_ZERO_TOLERANCE;
  if (rc) {
    m_path.from = A;
    m_path.to = B;
    m_t.Set(0.0, 1.0);
...
bool
ON_Extrusion::SetPathAndUp(ON_3dPoint A, ON_3dPoint B, ON_3dVector up)
{
  double distAB = 0.0;

  bool rc = up.IsValid() && up.Length() > ON_ZERO_TOLERANCE && A.IsValid() &&
            B.IsValid() && (distAB = A.DistanceTo(B)) > ON_ZERO_TOLERANCE;
...
int
ON_Extrusion::PathParameter() const
{
  return m_bTransposed ? 0 : 1;
}

int
ON_Extrusion::ProfileParameter() const
{
...
ON_3dVector
ON_Extrusion::PathTangent() const
{
...
bool
ON_Extrusion::SetMiterPlaneNormal(ON_3dVector N, int end)
{
  bool rc = false;
  if (end >= 0 && end <= 1) {
    if (N.IsValid() && N.z > ON_Extrusion::m_Nz_min &&
...
void
ON_Extrusion::GetMiterPlaneNormal(int end, ON_3dVector& N) const
{
...
int
ON_Extrusion::IsMitered() const
{
  int rc = 0;
  if (m_bHaveN[0] && m_N[0].IsUnitVector() && m_N[0].z > m_Nz_min &&
      (m_N[0].x != 0.0 || m_N[0].y != 0.0))
    rc += 1;
  if (m_bHaveN[1] && m_N[1].IsUnitVector() && m_N[1].z > m_Nz_min &&
      (m_N[1].x != 0.0 || m_N[1].y != 0.0))
    rc += 2;
  return rc;
}

int
ON_Extrusion::CapCount() const
{
...
bool
ON_Extrusion::GetPathPlane(double s, ON_Plane& plane) const
{
  ON_Plane p;
...
bool
ON_Extrusion::GetProfilePlane(double s, ON_Plane& plane) const
{
  ON_Plane p;
  p.origin = ON_3dPoint::Origin;
  p.zaxis = PathTangent();
...
ON_Extrusion::GetProfileTransformation(double s, ON_Xform& xform) const
{
  ...
  if (!ON_GetEndCapTransformation(
          m_path.PointAt(m_t.m_t[0]), T, m_up, m_bHaveN[0] ? &m_N[0] : 0, xform0, 0, 0))
    return false;
  if (!ON_GetEndCapTransformation(
          m_path.PointAt(m_t.m_t[1]), T, m_up, m_bHaveN[1] ? &m_N[1] : 0, xform1, 0, 0))
    return false;
...
static bool
CleanProfileSegment (ON_Curve* curve)
{
...
static bool
ProfileHelper (int desired_orientation, ON_Curve* profile)
{
  ...
  if (fabs(bbox.m_min.z) > ON_ZERO_TOLERANCE ||
      fabs(bbox.m_max.z) > ON_ZERO_TOLERANCE) {
    ON_ERROR("ON_Extrusion::Set/Add Profile - profile->BoundingBox() is not in the "
             "world xy plane.");
    return false;
  }
  ...
  if (profile->IsClosed()) {
    ...
    default: {
      ON_ERROR(
          "ON_Extrusion::Set/Add Profile - invalid desired_orientation parameter.");
      return false;
    } break;
    }
  }
  ...
  if (0 != polycurve) {
    ...
    if (polycurve->SegmentCurves().Count() + 1 !=
        polycurve->SegmentParameters().Count()) {
      ON_ERROR("ON_Extrusion::Set/Add Profile - ON_PolyCurve segment and parameter "
               "counts do not agree.");
      return false;
    }
...
bool
ON_Extrusion::SetOuterProfile(ON_Curve* outer_profile, bool bCap)
{
  if (0 != m_profile) {
    ON_ERROR(
        "ON_Extrusion::SetOuterProfile() called when m_profile is already not null.");
    return false;
  }
...
bool
ON_Extrusion::AddInnerProfile(ON_Curve* inner_profile)
{
  ...
  if (1 == m_profile_count && !m_profile->IsClosed()) {
    ON_ERROR(
        "ON_Extrusion::AddInnerProfile() called when outer profile is not closed.");
    return false;
  }

  ON_PolyCurve* polycurve = ON_PolyCurve::Cast(m_profile);
  if (m_profile_count > 1 && 0 == polycurve) {
    ON_ERROR("ON_Extrusion::AddInnerProfile() called when  m_profile_count > 1 but "
             "m_profile is not an ON_PolyCurve.");
    return false;
  }
  if (m_profile_count > 1 && m_profile_count != polycurve->Count()) {
    ON_ERROR("ON_Extrusion::AddInnerProfile() called when  m_profile_count > 1 but "
             "m_profile_count != m_profile->Count().");
    return false;
  }
...
const ON_PolyCurve*
ON_Extrusion::PolyProfile() const
{
  if (m_profile_count <= 1)
    return 0;
  const ON_PolyCurve* poly_profile = ON_PolyCurve::Cast(m_profile);
  return (0 != poly_profile && m_profile_count == poly_profile->Count()) ? poly_profile
                                                                         : 0;
}

const ON_Curve*
ON_Extrusion::Profile(int profile_index) const
{
...
ON_Curve*
ON_Extrusion::Profile3d(ON_COMPONENT_INDEX ci) const
{
...
ON_LineCurve*
ON_Extrusion::PathLineCurve(ON_LineCurve* line_curve) const
{
  if (!m_path.IsValid())
    return 0;
...
ON_Curve*
ON_Extrusion::WallEdge(ON_COMPONENT_INDEX ci) const
{
  ...
  ON_3dPoint profileP =
      profile_end ? profile2d->PointAtEnd() : profile2d->PointAtStart();
  if (!profileP.IsValid())
    return 0;
  profileP.z = 0.0;
...
ON_Surface*
ON_Extrusion::WallSurface(ON_COMPONENT_INDEX ci) const
{
...
ON_Curve*
ON_Extrusion::Profile3d(int profile_index, double s) const
{
  ...
  if (!profile3d->ChangeDimension(3) || !profile3d->Transform(xform)) {
    delete profile3d;
    return 0;
  }
...
int
ON_Extrusion::ProfileIndex(double profile_parameter) const
{
  if (!m_profile)
    return -1;
...
int
ON_Extrusion::ProfileCount() const
{
  if (!m_profile)
    return 0;
...
int
ON_Extrusion::GetProfileCurves(ON_SimpleArray<const ON_Curve*>& profile_curves) const
{
  if (!m_profile)
    return 0;
...
ON_Extrusion::ON_Extrusion() { ON_ExtrusionInitializeHelper(*this); }
...
ON_Extrusion&
ON_Extrusion::operator=(const ON_Extrusion& src)
{
  if (this != &src) {
    Destroy();
...
static void
ON_Extrusion_IsNotValidMessage (ON_TextLog* text_log, const char* msg)
{
...
static bool
ON_ExtrusionIsNotValid ()
{
  return ON_IsNotValid(); // good place for a breakpoint
}

ON_BOOL32
ON_Extrusion::IsValid(ON_TextLog* text_log) const
{
  // check m_profile
  if (m_profile_count < 1) {
  ...
  if (m_profile_count > 1) {
    ...
    if (0 == c) {
      ON_Extrusion_IsNotValidMessage(
          text_log, "m_profile_count > 1 but m_profile is not an ON_PolyCurve.");
      return ON_ExtrusionIsNotValid();
    }
    if (m_profile_count != c->Count()) {
      ON_Extrusion_IsNotValidMessage(
          text_log,
          "m_profile_count > 1 but m_profile_count != m_profile->SegmentCount().");
      return ON_ExtrusionIsNotValid();
    }

    if (!ON_Extrusion::IsValidPolyCurveProfile(*c, text_log)) {
      ON_Extrusion_IsNotValidMessage(text_log,
                                     "m_profile is not a valid ON_PolyCurve.");
      return ON_ExtrusionIsNotValid();
    }
    for (int i = 0; i < m_profile_count; i++) {
      ...
      if (0 == segment) {
        ON_Extrusion_IsNotValidMessage(
            text_log,
            "m_profile_count > 1 but a m_profile_count->SegmentCurve() is null.");
        return ON_ExtrusionIsNotValid();
      }
      if (!segment->IsClosed()) {
        ON_Extrusion_IsNotValidMessage(
            text_log,
            "m_profile_count > 1 but a m_profile_count->SegmentCurve() is not closed.");
        return ON_ExtrusionIsNotValid();
      }
    }
  }
  ...
  if (!(0.0 <= m_t.m_t[0] && m_t.m_t[0] < m_t.m_t[1] && m_t.m_t[1] <= 1.0)) {
    ON_Extrusion_IsNotValidMessage(text_log,
                                   "m_t does not satisfy 0<=m_t[0]<m_t[1]<=1");
    return ON_ExtrusionIsNotValid();
  }
  ...
  if (m_bHaveN[0]) {
    ...
    if (!(m_N[0].z > ON_Extrusion::m_Nz_min)) {
      ON_Extrusion_IsNotValidMessage(
          text_log, "m_N[0].z is too small (<=ON_Extrusion::m_Nz_min) or negative");
      return ON_ExtrusionIsNotValid();
    }
  }
  if (m_bHaveN[1]) {
    ...
    if (!(m_N[1].z > ON_Extrusion::m_Nz_min)) {
      ON_Extrusion_IsNotValidMessage(
          text_log, "m_N[1].z is too small (<=ON_Extrusion::m_Nz_min) or negative");
      return ON_ExtrusionIsNotValid();
    }
  }
...
void
ON_Extrusion::Dump(ON_TextLog& text_log) const
{
...
ON__UINT32
ON_Extrusion::DataCRC(ON__UINT32 current_remainder) const
{
  if (m_profile)
  ...
  current_remainder =
      ON_CRC32(current_remainder, sizeof(m_path_domain), &m_path_domain);
  current_remainder =
      ON_CRC32(current_remainder, sizeof(m_bTransposed), &m_bTransposed);
  current_remainder =
      ON_CRC32(current_remainder, sizeof(m_profile_count), &m_profile_count);
...
ON_BOOL32
ON_Extrusion::Write(ON_BinaryArchive& binary_archive) const
{
...
ON_BOOL32
ON_Extrusion::Read(ON_BinaryArchive& binary_archive)
{
  Destroy();
  int major_version = 0;
  int minor_version = 0;
...
ON::object_type
ON_Extrusion::ObjectType() const
{
  return ON::extrusion_object;
}

int
ON_Extrusion::Dimension() const
{
  return 3;
}
...
ON_BOOL32
ON_Extrusion::GetBBox(double* boxmin, double* boxmax, int bGrowBox) const
{
  bool rc = false;
  if (m_path.IsValid() && m_profile) {
    ...
    if (m_profile->GetTightBoundingBox(bbox) && GetBoundingBoxHelper(*this, bbox, 0)) {
      rc = true;
      if (bGrowBox) {
        bGrowBox = (boxmin[0] <= boxmax[0] && boxmin[1] <= boxmax[1] &&
                    boxmin[2] <= boxmax[2] && ON_IsValid(boxmax[0]) &&
                    ON_IsValid(boxmax[1]) && ON_IsValid(boxmax[2]));
      }
      if (bGrowBox) {
        if (boxmin[0] > bbox.m_min.x)
          boxmin[0] = bbox.m_min.x;
        if (boxmin[1] > bbox.m_min.y)
          boxmin[1] = bbox.m_min.y;
        if (boxmin[2] > bbox.m_min.z)
          boxmin[2] = bbox.m_min.z;
        if (boxmax[0] < bbox.m_max.x)
          boxmax[0] = bbox.m_max.x;
        if (boxmax[1] < bbox.m_max.y)
          boxmax[1] = bbox.m_max.y;
        if (boxmax[2] < bbox.m_max.z)
                                  ...
                                  const ON_Xform* xform) const
{
  ...
  if (m_path.IsValid() && m_profile) {
    ON_BoundingBox bbox;
    if (m_profile->GetTightBoundingBox(bbox) &&
        GetBoundingBoxHelper(*this, bbox, xform)) {
      if (bGrowBox)
...
static bool
Profile2dTransform (ON_Extrusion& e, const ON_Xform& profile_xform, bool bNeedReverse)
{
  ...
  bool rc = false;
  bool bNeedDeformable =
      (fabs(profile_xform.m_xform[0][0]) != fabs(profile_xform.m_xform[1][1]) ||
  ...
  if (0 != polyprofile) {
    ...
    if (bNeedDeformable && !e.m_profile->IsDeformable()) {
      ...
      rc =
          e.m_profile->Transform(profile_xform) ? true : ON_Extrusion_TransformFailed();
    }
    if (rc && bNeedReverse) {
      double t0, t1;
...
ON_BOOL32
ON_Extrusion::Transform(const ON_Xform& xform)
{
  ...
  const bool bUseVectorXform =
      (0.0 == xform[3][0] && 0.0 == xform[3][1] && 0.0 == xform[3][2]);
  ...
  if (bHavePathShear) {
    ...
    if (fabs(path_shear.y) <= ON_SQRT_EPSILON)
      path_shear.y = 0.0;
    bHavePathShear =
        path_shear.IsUnitVector() && (path_shear.x != 0.0 || path_shear.y != 0.0);
  }
  ...
  for (int i = 0; i < 2; i++) {
    if (m_bHaveN[i]) {
      ...
      if (fabs(QN[i].y) <= ON_SQRT_EPSILON)
        QN[i].y = 0.0;
      if (QN[i].z < ON_Extrusion::m_Nz_min || QN[i].z >= 1.0 - ON_SQRT_EPSILON ||
  ...
  if (!bHavePathShear) {
    ...
    const double profile_xform_tol =
        profile_x_scale_tol *
        (fabs(profile_xform.m_xform[0][0]) + profile_xform.m_xform[1][1]);
    if (fabs(fabs(profile_xform.m_xform[0][0]) - profile_xform.m_xform[1][1]) <=
        profile_xform_tol) {
      profile_xform.m_xform[0][0] = ((profile_xform.m_xform[0][0] < 0.0) ? -1.0 : 1.0) *
                                    profile_xform.m_xform[1][1];
    }
    ...
    profile_xform.m_xform[2][0] = 0.0;
    profile_xform.m_xform[2][1] = 0.0;
    profile_xform.m_xform[2][2] = 1.0;
    profile_xform.m_xform[2][3] = 0.0;
    profile_xform.m_xform[3][0] = 0.0;
    profile_xform.m_xform[3][1] = 0.0;
    profile_xform.m_xform[3][2] = 0.0;
    profile_xform.m_xform[3][3] = 1.0;
    ...
    if (profile_y_scale > xform_tol && fabs(profile_y_scale - 1.0) > xform_tol) {
      ...
      if (fabs(profile_xform.m_xform[0][0] - profile_y_scale) < profile_scale_tol) {
        ...
        if (fabs(profile_xform.m_xform[1][1] - profile_y_scale) < profile_scale_tol)
          profile_xform.m_xform[1][1] = profile_y_scale;
        else if (fabs(profile_xform.m_xform[1][1] + profile_y_scale) <
                 profile_scale_tol)
          profile_xform.m_xform[1][1] = -profile_y_scale;
      }
    }
  ...
  double profile_det = profile_xform[0][0] * profile_xform[1][1] -
                       profile_xform[1][0] * profile_xform[0][1];
...
public:
  void
  SetIsSolid (int is_solid)
  {
    m_is_solid = is_solid;
  }
  void
  SetBBox (const ON_Extrusion& extrusion)
  {
...
ON_Extrusion_BrepForm_FaceInfo::ON_Extrusion_BrepForm_FaceInfo() { Init(); }
...
bool
ON_Extrusion_BrepForm_FaceInfo::HaveBrepFaceFace() const
{
  if (m_face_index < 0)
    return false;
  return true;
...
void
ON_Extrusion_BrepForm_FaceInfo::Init()
{
  memset(this, 0, sizeof(*this));
...
ON_Brep*
ON_Extrusion::BrepForm(ON_Brep* brep) const
{
  return BrepForm(brep, true);
}

static ON_PlaneSurface*
MakeCapPlaneHelper (ON_ClassArray<ON_Extrusion_BrepForm_FaceInfo>& finfo,
                    ...
                    const ON_Xform& rot)
{
  ...
  if (outer_loop_iso_trim_count < outer_loop_trim_count || !u.IsIncreasing() ||
      !v.IsIncreasing()) {
...
bool
ON_Extrusion::ProfileIsKinked(int profile_index) const
{
...
int
ON_Extrusion::ProfileSmoothSegmentCount(int profile_index) const
{
...
ON_Extrusion::GetProfileKinkParameters(
    int profile_index, ON_SimpleArray<double>& profile_kink_parameters) const
{
  const ON_Curve* profile2d = Profile(profile_index);
  if (0 == profile2d)
    return 0;
...
ON_Brep*
ON_Extrusion::BrepForm(ON_Brep* brep, bool bSmoothFaces) const
{
  if (brep)
    brep->Destroy();
  ...
  for (int profile_index = 0; profile_index < profile_count; profile_index++) {
    ...
    profile_fi.Init();
    {
      ...
      profile_fi.m_profile_orientation =
          (profile_fi.m_bClosedProfile) ? ON_ClosedCurveOrientation(*newprofile, 0) : 0;

      if (is_capped &&
          profile_fi.m_profile_orientation != ((0 == profile_index) ? 1 : -1)) {
      ...
      if (bSmoothFaces) {
        ...
        double t0 = ON_UNSET_VALUE;
        double t1 = ON_UNSET_VALUE;
        if (!profile_segment->GetDomain(&t0, &t1) || !ON_IsValid(t0) ||
            !ON_IsValid(t1) || !(t0 < t1)) {
          delete newprofile;
          if (newbrep != brep)
            delete newbrep;
        ...
        while (GetNextProfileSegmentDiscontinuity(profile_segment, t0, t1, &t)) {
          if (t0 < t && t < t1) {
            ...
            if (newprofile->Split(t, left_side, right_side) && 0 != left_side &&
                0 != right_side) {
    ...
    for (int finfo_index = fi0; finfo_index < fi1; finfo_index++) {
      ...
      if (profile_fi.m_bClosedProfile && finfo_index > fi0 && finfo_index == fi1 - 1) {
      ...
      const ON_BrepFace* face =
          newbrep->NewFace(face_srf, fi.m_vid, fi.m_eid, fi.m_bRev3d);
      if (0 == face) {
  ...
  while (is_capped > 0 && cap_count == ((3 == is_capped) ? 2 : 1)) {
    // while(...) not a loop
    // - break's are used for flow control and there is a break at the bottom of this
    // scope.
    ...
    for (cap_index = 0; cap_index < 2; cap_index++) {
      ...
      fi0 = 0;
      int profile_index;
      for (profile_index = 0; profile_index < profile_count && fi0 < finfo.Count();
           profile_index++) {
...
bool
ON_Extrusion::GetBrepFormComponentIndex(ON_COMPONENT_INDEX extrusion_ci,
                      ...
                      ON_Interval* profile_segment_domain)
{
  ...
  if (ON_UNSET_VALUE == extrusion_profile_parameter) {
    if (extrusion_profile_index != profile_count) {
      ...
      if (0 == profile || !profile->GetDomain(&t0, &t1) || !ON_IsValid(t0) ||
          !ON_IsValid(t1) || !(t0 < t1)) {
        return false;
      }
    }
    ...
    for (int i = 0; i < profile_count; i++) {
      ...
      if (bCountProfileDiscontinuities) {
        while (GetNextProfileSegmentDiscontinuity(profile, t0, t1, &t)) {
          if (t0 < t && t < t1) {
            if (i == extrusion_profile_index && t0 <= extrusion_profile_parameter &&
                                        ...
                                        ON_COMPONENT_INDEX& brep_ci) const
{
  ...
  const int cap_count =
      (0 == is_capped || !bClosedProfile) ? 0 : ((3 == is_capped) ? 2 : 1);
...
{
  const int path_dir = PathParameter();
  return (path_dir == dir) ? m_path_domain
                           : ((1 - path_dir == dir && m_profile) ? m_profile->Domain()
                                                                 : ON_Interval());
}

ON_BOOL32
ON_Extrusion::GetSurfaceSize(double* width, double* height) const
{
  bool rc = true;
...
ON_BOOL32
ON_Extrusion::GetSpanVector( // span "knots"
    int dir, // 0 gets first parameter's domain, 1 gets second parameter's domain
...
int
ON_Extrusion::Degree( // returns maximum algebraic degree of any span
...
ON_BOOL32
ON_Extrusion::GetParameterTolerance( // returns tminus < tplus: parameters tminus <= s
                                     // <= tplus
...
ON_Surface::ISO
ON_Extrusion::IsIsoparametric(const ON_Curve& curve,
                              const ON_Interval* curve_domain) const
{
  return ON_Surface::IsIsoparametric(curve, curve_domain);
}

ON_BOOL32
ON_Extrusion::IsPlanar(ON_Plane* plane, double tolerance) const
{
...
ON_BOOL32
ON_Extrusion::IsClosed(int dir) const
{
  const int path_dir = PathParameter();
  if (1 - path_dir == dir && m_profile)
...
ON_BOOL32
ON_Extrusion::IsPeriodic(int dir) const
{
  const int path_dir = PathParameter();
  if (1 - path_dir == dir && m_profile)
...
ON_Surface::ISO
ON_Extrusion::IsIsoparametric(const ON_BoundingBox& bbox) const
{
  return ON_Surface::IsIsoparametric(bbox);
}

ON_BOOL32
ON_Extrusion::Reverse(int dir)
{
  if (0 == m_profile)
    return false;
...
ON_BOOL32
ON_Extrusion::Transpose() // transpose surface parameterization (swap "s" and "t")
{
...
) const
{
  ...
  if (m_bTransposed && num_der > 0) {
    ...
    void* tmp = (sz <= sizeof(xform0)) ? ((void*)&xform0.m_xform[0][0]) : onmalloc(sz);
    for (i = 1; i <= num_der; i++) {
...
ON_Curve*
ON_Extrusion::IsoCurve(int dir, double c) const
{
...
ON_BOOL32
ON_Extrusion::Trim(int dir, const ON_Interval& domain)
{
  bool rc = false;
...
bool
ON_Extrusion::Extend(int dir, const ON_Interval& domain)
{
  bool rc = false;
  if (1 == dir) {
...
int
ON_Extrusion::GetNurbForm(ON_NurbsSurface& nurbs_surface, double tolerance) const
{
  if (!m_profile)
    return 0;
...
int
ON_Extrusion::HasNurbForm() const
{
                                                       ...
                                                       double* surface_t) const
{
  ...
  if (surface_s) {
    rc = m_profile
             ? (m_profile->GetCurveParameterFromNurbFormParameter(nurbs_s, surface_s)
                    ? true
                    : false)
             : false;
  }
...
bool
ON_Extrusion::GetNurbFormParameterFromSurfaceParameter(double surface_s,
                                                       double surface_t,
                                                       double* nurbs_s,
                                                       double* nurbs_t) const
{
  ...
  if (nurbs_s) {
    rc = m_profile
             ? (m_profile->GetNurbFormParameterFromCurveParameter(surface_s, nurbs_s)
                    ? true
                    : false)
             : false;
  }
...
ON_SumSurface*
ON_Extrusion::SumSurfaceForm(ON_SumSurface* sum_surface) const
{
  int i;
  if (0 != sum_surface) {
    for (i = 0; i < 2; i++) {
                   ...
                   ON_Extrusion* extrusion)
{
  ...
  double inner_radius =
      (other_radius < cylinder.circle.radius) ? other_radius : cylinder.circle.radius;
  double outer_radius =
      (other_radius < cylinder.circle.radius) ? cylinder.circle.radius : other_radius;
  if (!ON_IsValid(inner_radius) || !ON_IsValid(outer_radius) ||
  ...
  ON_Extrusion* extrusion_pipe =
      ON_Extrusion::Cylinder(outer_cylinder, bCapBottom, bCapTop, extrusion);
  if (0 == extrusion_pipe) {
    delete inner_profile;
    return 0;
  }
