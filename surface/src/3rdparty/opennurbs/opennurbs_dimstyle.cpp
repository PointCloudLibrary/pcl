...
public:
  ...
  static ON_DimStyleExtra*
  DimStyleExtensionGet (ON_DimStyle* pDimStyle, bool bCreateIfNoneExists);
  ...
  static const ON_DimStyleExtra*
  DimStyleExtensionGet (const ON_DimStyle* pDimStyle);
  ...
  */
  ...
  bool
  IsFieldOverride (int field_id) const;
  ...
  //  4: Basic
  void
  SetToleranceStyle (int style);
  int
  ToleranceStyle () const;

  void
  SetToleranceResolution (int resolution);
  int
  ToleranceResolution () const;

  void
  SetToleranceUpperValue (double upper_value);
  double
  ToleranceUpperValue () const;

  void
  SetToleranceLowerValue (double lower_value);
  double
  ToleranceLowerValue () const;

  void
  SetToleranceHeightScale (double scale);
  double
  ToleranceHeightScale () const;

  void
  SetBaselineSpacing (double);
  double
  BaselineSpacing () const;
  ...
  void
  SetDimScale (double scale);
  double
  DimScale () const;
  void
  SetDimScaleSource (int source);
  int
  DimScaleSource () const;
...
bool
ON_DimStyleExtra::IsDefault() const
{
  if (m_tolerance_style != ON_DimStyle::DefaultToleranceStyle())
    return false;
  if (m_tolerance_resolution != ON_DimStyle::DefaultToleranceResolution())
    return false;
  if (m_tolerance_upper_value != ON_DimStyle::DefaultToleranceUpperValue())
    return false;
  if (m_tolerance_lower_value != ON_DimStyle::DefaultToleranceLowerValue())
    return false;
  if (m_tolerance_height_scale != ON_DimStyle::DefaultToleranceHeightScale())
    return false;
  if (m_baseline_spacing != ON_DimStyle::DefaultBaselineSpacing())
    return false;
  if (m_bDrawMask != ON_DimStyle::DefaultDrawTextMask())
    return false;
  if (m_mask_color_source != ON_DimStyle::DefaultMaskColorSource())
    return false;
  if (m_mask_color != ON_DimStyle::DefaultMaskColor())
    return false;
  if (m_dimscale != ON_DimStyle::DefaultDimScale())
    return false;
  if (m_dimscale_source != ON_DimStyle::DefaultDimScaleSource())
    return false;

  // The m_valid_fields[] settings only matter when
  // m_parent_dimstyle is not zero.
  if (!(m_parent_dimstyle == ON_nil_uuid)) {
    for (int i = 0; i < m_valid_fields.Count() && i < ON_DimStyleExtra::eFieldCount;
         i++) {
      if (!m_valid_fields[i])
        return false;
    }
...
ON_BOOL32
ON_DimStyleExtra::Archive() const
{
...
int
ON_DimStyleExtra::ToleranceStyle() const
{
...
int
ON_DimStyleExtra::ToleranceResolution() const
{
...
double
ON_DimStyleExtra::ToleranceUpperValue() const
{
...
double
ON_DimStyleExtra::ToleranceLowerValue() const
{
...
double
ON_DimStyleExtra::ToleranceHeightScale() const
{
...
double
ON_DimStyleExtra::BaselineSpacing() const
{
  return m_baseline_spacing;
}

bool
ON_DimStyleExtra::DrawTextMask() const
{
  return m_bDrawMask;
}
...
int
ON_DimStyleExtra::MaskColorSource() const
{
...
ON_Color
ON_DimStyleExtra::MaskColor() const
{
  return m_mask_color;
}
...
double
ON_DimStyleExtra::DimScale() const
{
  return m_dimscale;
}
...
int
ON_DimStyleExtra::DimScaleSource() const
{
...
ON_DimStyleExtra::CompareFields (const ON_DimStyleExtra* pOther) const
{
  ...
  if ((m_parent_dimstyle != pOther->m_parent_dimstyle) ||
      ...
      (m_mask_color != pOther->m_mask_color) || (m_dimscale != pOther->m_dimscale) ||
...
{
  // 26 Oct 2010
  // Can't create this here because reading files won't attach what's read from the file
...
int
ON_DimStyle::Index() const
{
  return m_dimstyle_index;
}

double
ON_DimStyle::ExtExtension() const
{
...
double
ON_DimStyle::LeaderArrowSize() const
{
...
int
ON_DimStyle::TextAlignment() const
{
  return m_textalign;
}
...
int
ON_DimStyle::LeaderArrowType() const
{
...
int
ON_DimStyle::AngularUnits() const
{
...
int
ON_DimStyle::LengthFormat() const
{
...
int
ON_DimStyle::LengthResolution() const
{
...
int
ON_DimStyle::AngleResolution() const
{
...
double
ON_DimStyle::LengthFactor() const
{
...
double
ON_DimStyle::AlternateLengthFactor() const
{
...
int
ON_DimStyle::AlternateLengthFormat() const
{
...
int
ON_DimStyle::AlternateLengthResolution() const
{
...
int
ON_DimStyle::AlternateAngleFormat() const
{
...
int
ON_DimStyle::AlternateAngleResolution() const
{
...
const wchar_t*
ON_DimStyle::AlternatePrefix() const
{
...
const wchar_t*
ON_DimStyle::AlternateSuffix() const
{
...
bool
ON_DimStyle::SuppressExtension1() const
{
...
bool
ON_DimStyle::SuppressExtension2() const
{
...
ON_DimStyle::Composite (const ON_DimStyle& /*OverRide*/)
{
  /*
    ...
    if( OverRide.IsFieldValid( fn_name))                          { ValidateField(
    fn_name); m_dimstyle_name = OverRide.m_dimstyle_name; } if( OverRide.IsFieldValid(
    fn_index))                         { ValidateField( fn_index); m_dimstyle_index =
    OverRide.m_dimstyle_index; } if( OverRide.IsFieldValid( fn_extextension)) {
    ValidateField( fn_extextension); m_extextension = OverRide.m_extextension; } if(
    OverRide.IsFieldValid( fn_extoffset))                     { ValidateField(
    fn_extoffset); m_extoffset = OverRide.m_extoffset; } if( OverRide.IsFieldValid(
    fn_arrowsize))                     { ValidateField( fn_arrowsize); m_arrowsize =
    OverRide.m_arrowsize; } if( OverRide.IsFieldValid( fn_leaderarrowsize)) {
    ValidateField( fn_leaderarrowsize); m_leaderarrowsize = OverRide.m_leaderarrowsize;
    } if( OverRide.IsFieldValid( fn_centermark))                    { ValidateField(
    fn_centermark); m_centermark = OverRide.m_centermark; } if( OverRide.IsFieldValid(
    fn_textgap))                       { ValidateField( fn_textgap); m_textgap =
    OverRide.m_textgap; } if( OverRide.IsFieldValid( fn_textheight)) { ValidateField(
    fn_textheight); m_textheight = OverRide.m_textheight; } if( OverRide.IsFieldValid(
    fn_textalign))                     { ValidateField( fn_textalign); m_textalign =
    OverRide.m_textalign; } if( OverRide.IsFieldValid( fn_arrowtype)) { ValidateField(
    fn_arrowtype); m_arrowtype = OverRide.m_arrowtype; } if( OverRide.IsFieldValid(
    fn_leaderarrowtype))               { ValidateField( fn_leaderarrowtype);
    m_leaderarrowtype = OverRide.m_leaderarrowtype; } if( OverRide.IsFieldValid(
    fn_angularunits))                  { ValidateField( fn_angularunits); m_angularunits
    = OverRide.m_angularunits; } if( OverRide.IsFieldValid( fn_lengthformat)) {
    ValidateField( fn_lengthformat); m_lengthformat = OverRide.m_lengthformat; } if(
    OverRide.IsFieldValid( fn_angleformat))                   { ValidateField(
    fn_angleformat); m_angleformat = OverRide.m_angleformat; } if(
    OverRide.IsFieldValid( fn_angleresolution))               { ValidateField(
    fn_angleresolution); m_angleresolution = OverRide.m_angleresolution; } if(
    OverRide.IsFieldValid( fn_lengthresolution))              { ValidateField(
    fn_lengthresolution); m_lengthresolution = OverRide.m_lengthresolution; } if(
    OverRide.IsFieldValid( fn_fontindex))                     { ValidateField(
    fn_fontindex); m_fontindex = OverRide.m_fontindex; } if( OverRide.IsFieldValid(
    fn_lengthfactor))                  { ValidateField( fn_lengthfactor); m_lengthfactor
    = OverRide.m_lengthfactor; } if( OverRide.IsFieldValid( fn_bAlternate)) {
    ValidateField( fn_bAlternate); m_bAlternate = OverRide.m_bAlternate; } if(
    OverRide.IsFieldValid( fn_alternate_lengthfactor))        { ValidateField(
    fn_alternate_lengthfactor); m_alternate_lengthfactor =
    OverRide.m_alternate_lengthfactor; } if( OverRide.IsFieldValid(
    fn_alternate_lengthformat))        { ValidateField( fn_alternate_lengthformat);
    m_alternate_lengthformat = OverRide.m_alternate_lengthformat; } if(
    OverRide.IsFieldValid( fn_alternate_lengthresolution))    { ValidateField(
    fn_alternate_lengthresolution); m_alternate_lengthresolution =
    OverRide.m_alternate_lengthresolution; } if( OverRide.IsFieldValid( fn_prefix)) {
    ValidateField( fn_prefix); m_prefix = OverRide.m_prefix; } if(
    OverRide.IsFieldValid( fn_suffix))                        { ValidateField(
    fn_suffix); m_suffix = OverRide.m_suffix; } if( OverRide.IsFieldValid(
    fn_alternate_prefix))              { ValidateField( fn_alternate_prefix);
    m_alternate_prefix = OverRide.m_alternate_prefix; } if( OverRide.IsFieldValid(
    fn_alternate_suffix))              { ValidateField( fn_alternate_suffix);
    m_alternate_suffix = OverRide.m_alternate_suffix; } if( OverRide.IsFieldValid(
    fn_dimextension))                  { ValidateField( fn_dimextension); m_dimextension
    = OverRide.m_dimextension; } if( OverRide.IsFieldValid( fn_suppressextension1)) {
    ValidateField( fn_suppressextension1); m_bSuppressExtension1 =
    OverRide.m_bSuppressExtension1; } if( OverRide.IsFieldValid( fn_suppressextension2))
    { ValidateField( fn_suppressextension2); m_bSuppressExtension2 =
    OverRide.m_bSuppressExtension2; }
  */
}
...
// This function deprecated 5/01/07 LW
void
ON_DimStyle::InvalidateAllFields()
{
  m_valid = 0;
}
// This function deprecated 5/01/07 LW
...
// ver 1.4, Dec 28, 05
double
ON_DimStyle::DimExtension() const
{
...
ON_DimStyle::OverrideFields (const ON_DimStyle& src, const ON_DimStyle& parent)
{
  ...
  for (int i = 0; i < ON_DimStyleExtra::eFieldCount; i++) {
    bool bValidSrcField =
        (0 != pDEsrc && i < src_valid_fields_count) ? pDEsrc->m_valid_fields[i] : false;
...
bool
ON_DimStyle::IsChildDimstyle() const
{
...
//  4: Basic
int
ON_DimStyle::ToleranceStyle() const
{
  // 2 November 2011 Dale Lear
...
int
ON_DimStyle::ToleranceResolution() const
{
  // 2 November 2011 Dale Lear
  //   Changed to NOT create ON_DimStyleExtra user data
  //   if it did not already exist.
...
double
ON_DimStyle::ToleranceUpperValue() const
{
  // 2 November 2011 Dale Lear
  //   Changed to NOT create ON_DimStyleExtra user data
  //   if it did not already exist.
...
double
ON_DimStyle::ToleranceLowerValue() const
{
  // 2 November 2011 Dale Lear
  //   Changed to NOT create ON_DimStyleExtra user data
  //   if it did not already exist.
...
double
ON_DimStyle::ToleranceHeightScale() const
{
  // 2 November 2011 Dale Lear
  //   Changed to NOT create ON_DimStyleExtra user data
  //   if it did not already exist.
...
double
ON_DimStyle::BaselineSpacing() const
{
  // 2 November 2011 Dale Lear
  //   Changed to NOT create ON_DimStyleExtra user data
  //   if it did not already exist.
...
// static
int
ON_DimStyle::DefaultToleranceStyle()
{
  return 0;
}
...
// static
int
ON_DimStyle::DefaultToleranceResolution()
{
  return 4;
}
...
// static
double
ON_DimStyle::DefaultToleranceUpperValue()
{
  return 0.0;
}
...
// static
double
ON_DimStyle::DefaultToleranceLowerValue()
{
  return 0.0;
}
...
// static
double
ON_DimStyle::DefaultToleranceHeightScale()
{
  return 1.0;
}
...
// static
double
ON_DimStyle::DefaultBaselineSpacing()
{
  return 1.0;
}
...
// static
bool
ON_DimStyle::DefaultDrawTextMask()
{
  return false;
}
...
// static
int
ON_DimStyle::DefaultMaskColorSource()
{
  return 0;
}
...
// static
int
ON_DimStyle::DefaultDimScaleSource()
{
  return 0;
}
...
bool
ON_DimStyle::DrawTextMask() const
{
  // 2 November 2011 Dale Lear
...
int
ON_DimStyle::MaskColorSource() const
{
  // 2 November 2011 Dale Lear
  //   Do not create user data if it does not already exist.
  const ON_DimStyleExtra* pDE = DimStyleExtension();
...
double
ON_DimStyle::MaskOffsetFactor() const
{
  if (m_textheight != 0.0)
...
int
ON_DimStyle::DimScaleSource() const
{
  // 2 November 2011 Dale Lear
  //   Do not create user data if it does not already exist.
  const ON_DimStyleExtra* pDE = DimStyleExtension();
...
ON_UUID
ON_DimStyle::SourceDimstyle() const
{
  // 2 November 2011 Dale Lear
  //   Do not create user data if it does not already exist.
  const ON_DimStyleExtra* pDE = DimStyleExtension();
...
class ON_DimStyleExtra*
ON_DimStyle::DimStyleExtension()
{
...
const class ON_DimStyleExtra*
ON_DimStyle::DimStyleExtension() const
{
  // This function gets an extensions class if one exists.
  // It must not create an extensions class if one does not exist.
  ON_DimStyleExtra* pExtra = ON_DimStyleExtra::Cast (
...
ON_DimStyle::CompareFields (const ON_DimStyle& other) const
{
  if ((m_extextension != other.m_extextension) || (m_extoffset != other.m_extoffset) ||
      ...
      (m_textgap != other.m_textgap) || (m_textheight != other.m_textheight) ||
      (m_textalign != other.m_textalign) || (m_arrowtype != other.m_arrowtype) ||
      ...
      (m_fontindex != other.m_fontindex) || (m_lengthfactor != other.m_lengthfactor) ||
      ...
      (m_prefix != other.m_prefix) || (m_suffix != other.m_suffix) ||
