...
: ON_Curve(src), m_c2(0), m_c3(0), m_s(0)
{
  *this = src;
}

unsigned int
ON_CurveOnSurface::SizeOf() const
{
...
ON_CurveOnSurface&
ON_CurveOnSurface::operator=(const ON_CurveOnSurface& src)
{
  if (this != &src) {
...
ON_BOOL32
ON_CurveOnSurface::Write(ON_BinaryArchive& file // open binary file
) const
{
...
ON_BOOL32
ON_CurveOnSurface::Read(ON_BinaryArchive& file // open binary file
)
{
  delete m_c2;
  delete m_c3;
...
ON_Interval
ON_CurveOnSurface::Domain() const
{
  ON_Interval d;
  if (m_c2)
    d = m_c2->Domain();
...
int
ON_CurveOnSurface::SpanCount() const
{
  return m_c2 ? m_c2->SpanCount() : 0;
}

ON_BOOL32
ON_CurveOnSurface::GetSpanVector( // span "knots"
...
int
ON_CurveOnSurface::Degree() const
{
  return m_c2 ? m_c2->Degree() : 0;
}

ON_BOOL32
ON_CurveOnSurface::GetParameterTolerance(double t,       // t = parameter in domain
...
{
  ON_BOOL32 rc = (m_c2 && ON_PlaneSurface::Cast(m_s))
                     ? (ON_PlaneSurface::Cast(m_s) && m_c2->IsLinear(tolerance))
                     : false;
  if (rc) {
...
{
  return (m_c2 && ON_PlaneSurface::Cast(m_s)) ? m_c2->IsArc(plane, arc, tolerance)
                                              : false;
}

ON_BOOL32
ON_CurveOnSurface::IsPlanar(ON_Plane*, // plane if not NULL and true is returned, then
...
ON_BOOL32
ON_CurveOnSurface::IsInPlane(const ON_Plane&, // plane to test
                             double // tolerance to use when checking linearity
) const
{
  return false;
  ...
  if (rc) {
    ...
    if (rc) {
      ...
      if (rc) {
        if (hint) {
          *hint =
              (chint & 0xFFFF) | ((shint[0] & 0xFF) << 16) | ((shint[1] & 0xFF) << 24);
        }
        ...
        if (der_count >= 1) {
          ...
          if (der_count >= 2) {
            ...
            d = ddu * s[1] + ddv * s[2] + du * du * s[3] + 2.0 * du * dv * s[4] +
                dv * dv * s[5];
            v[0] = d.x;
            if (dim > 1)
              v[1] = d.y;
            ...
            if (der_count >= 3) {
              ...
              d = dddu * s[1] + dddv * s[2] + 3.0 * du * ddu * s[3] +
                  3.0 * (ddu * dv + du * ddv) * s[4] + 3.0 * dv * ddv * s[5] +
                  du * du * du * s[6] + 3.0 * du * du * dv * s[7] +
                  3.0 * du * dv * dv * s[8] + dv * dv * dv * s[9];
              v[0] = d.x;
              if (dim > 1)
                v[1] = d.y;
              if (dim > 2)
