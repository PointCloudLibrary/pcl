...
public:
  static ON_HatchExtra*
  HatchExtension (ON_Hatch* pHatch, bool bCreate);
  static const ON_HatchExtra*
  HatchExtension (const ON_Hatch* pHatch, bool bCreate);
...
ON_HatchExtra*
ON_HatchExtra::HatchExtension(ON_Hatch* pHatch, bool bCreate)
{
  ON_HatchExtra* pExtra = 0;
  if (pHatch) {
    pExtra = ON_HatchExtra::Cast(
        pHatch->GetUserData(ON_HatchExtra::m_ON_HatchExtra_class_id.Uuid()));
    if (pExtra == 0 && bCreate) {
...
const ON_HatchExtra*
ON_HatchExtra::HatchExtension(const ON_Hatch* pHatch, bool bCreate)
{
...
void
ON_HatchExtra::Dump(ON_TextLog&) const
{}

unsigned int
ON_HatchExtra::SizeOf() const
{
...
ON_BOOL32
ON_HatchExtra::Write(ON_BinaryArchive& archive) const
{
  bool rc = archive.BeginWrite3dmChunk(TCODE_ANONYMOUS_CHUNK, 1, 0);

  if (rc)
    rc = archive.WriteUuid(m_parent_hatch);
  if (rc)
    rc = archive.WritePoint(m_basepoint);
...
ON_BOOL32
ON_HatchExtra::Read(ON_BinaryArchive& archive)
{
  int major_version = 0;
  int minor_version = 0;
  bool rc =
  ...
  m_basepoint.Set(0.0, 0.0);
  if (rc)
    rc = archive.ReadUuid(m_parent_hatch);
  if (rc)
    rc = archive.ReadPoint(m_basepoint);
...
ON_BOOL32
ON_HatchExtra::GetDescription(ON_wString& description)
{
...
bool
ON_HatchLine::operator==(const ON_HatchLine& src) const
{
  return (m_angle == src.m_angle && m_base == src.m_base && m_offset == src.m_offset &&
...
ON_BOOL32
ON_HatchLine::IsValid(ON_TextLog* text_log) const
{
  bool rc = m_angle >= 0.0;
  if (!rc) {
    if (text_log)
...
ON_BOOL32
ON_HatchLine::Write(ON_BinaryArchive& ar) const
{
  ON_BOOL32 rc = ar.Write3dmChunkVersion(1, 1);

  if (rc)
    rc = ar.WriteDouble(m_angle);
  if (rc)
    rc = ar.WritePoint(m_base);
  if (rc)
    rc = ar.WriteVector(m_offset);
  if (rc)
    rc = ar.WriteArray(m_dashes);

  return rc;
}

ON_BOOL32
ON_HatchLine::Read(ON_BinaryArchive& ar)
{
  m_angle = 0.0;
  m_base.Set(0.0, 0.0);
  ...
  if (major_version == 1) {
    if (rc)
      rc = ar.ReadDouble(&m_angle);
    if (rc)
      rc = ar.ReadPoint(m_base);
    if (rc)
      rc = ar.ReadVector(m_offset);
    if (rc)
      rc = ar.ReadArray(m_dashes);
  }
  return rc;
}
...
double
ON_HatchLine::Angle() const
{
  return m_angle;
}

void
ON_HatchLine::SetAngle(double angle)
{
  m_angle = angle;
...
ON_2dPoint
ON_HatchLine::Base() const
{
  return m_base;
}

void
ON_HatchLine::SetBase(const ON_2dPoint& base)
{
  m_base = base;
}

ON_2dVector
ON_HatchLine::Offset() const
{
  return m_offset;
}

void
ON_HatchLine::SetOffset(const ON_2dVector& offset)
{
  m_offset = offset;
}

int
ON_HatchLine::DashCount() const
{
  return m_dashes.Count();
}

double
ON_HatchLine::Dash(int index) const
{
...
void
ON_HatchLine::AppendDash(double dash)
{
...
void
ON_HatchLine::SetPattern(const ON_SimpleArray<double>& dashes)
{
  m_dashes = dashes;
}
...
double
ON_HatchLine::GetPatternLength() const
{
  int i;
  double length = 0.0;
...
ON_BOOL32
ON_HatchPattern::IsValid(ON_TextLog* text_log) const
{
...
void
ON_HatchPattern::Dump(ON_TextLog& dump) const
{
...
ON_BOOL32
ON_HatchPattern::Write(ON_BinaryArchive& ar) const
{
  ON_BOOL32 rc = ar.Write3dmChunkVersion(1, 2);

  if (rc)
    rc = ar.WriteInt(m_hatchpattern_index);
  if (rc)
    rc = ar.WriteInt(m_type);
  if (rc)
    rc = ar.WriteString(m_hatchpattern_name);
  if (rc)
    rc = ar.WriteString(m_description);
  if (rc) {
    if (m_type == ftLines) {
  ...
  // version 1.2 field
  if (rc)
    rc = ar.WriteUuid(m_hatchpattern_id);

  return rc;
}

ON_BOOL32
ON_HatchPattern::Read(ON_BinaryArchive& ar)
{
  m_hatchpattern_index = -1;
  ...
  if (major_version == 1) {
    if (rc)
      rc = ar.ReadInt(&m_hatchpattern_index);
    i = 0;
    if (rc)
      rc = ar.ReadInt(&i);
    if (rc) {
    ...
    if (rc)
      rc = ar.ReadString(m_hatchpattern_name);
    if (rc)
      rc = ar.ReadString(m_description);
    if (rc) {
      if (m_type == ftLines) {
        m_lines.Empty();
        int count = 0;
...
ON_HatchPattern::eFillType
ON_HatchPattern::FillType() const
{
...
void
ON_HatchPattern::SetFillType(eFillType type)
{
  m_type = type;
}

void
ON_HatchPattern::SetName(const wchar_t* pName)
{
...
void
ON_HatchPattern::SetName(const char* pName)
{
  m_hatchpattern_name = pName;
  m_hatchpattern_name.TrimLeftAndRight();
}
...
void
ON_HatchPattern::GetName(ON_wString& string) const
{
  string = m_hatchpattern_name;
}

const wchar_t*
ON_HatchPattern::Name() const
{
  return m_hatchpattern_name;
}

void
ON_HatchPattern::SetDescription(const wchar_t* pDescription)
{
  m_description = pDescription;
}

void
ON_HatchPattern::SetDescription(const char* pDescription)
{
  m_description = pDescription;
}

void
ON_HatchPattern::GetDescription(ON_wString& string) const
{
  string = m_description;
}

const wchar_t*
ON_HatchPattern::Description() const
{
  return m_description;
}

void
ON_HatchPattern::SetIndex(int i)
{
  m_hatchpattern_index = i;
}

int
ON_HatchPattern::Index() const
{
...
int
ON_HatchPattern::HatchLineCount() const
{
  return m_lines.Count();
}

int
ON_HatchPattern::AddHatchLine(const ON_HatchLine& line)
{
  m_lines.Append(line);
...
const ON_HatchLine*
ON_HatchPattern::HatchLine(int index) const
{
...
bool
ON_HatchPattern::RemoveHatchLine(int index)
{
  if (index >= 0 && index < m_lines.Count()) {
...
void
ON_HatchPattern::RemoveAllHatchLines()
{
  m_lines.Empty();
}

int
ON_HatchPattern::SetHatchLines(const ON_ClassArray<ON_HatchLine> lines)
{
  m_lines = lines;
  return m_lines.Count();
}
...
void*
ON_HatchLoop::operator new(std::size_t sz)
{
  // ON_HatchLoop new
  return onmalloc(sz);
}
...
void
ON_HatchLoop::operator delete(void* p)
{
  // ON_HatchLoop delete
  onfree(p);
}
...
void*
ON_HatchLoop::operator new(std::size_t, void* p)
{
...
void
ON_HatchLoop::operator delete(void*, void*)
{
...
ON_HatchLoop::ON_HatchLoop() : m_type(ON_HatchLoop::ltOuter), m_p2dCurve(NULL) {}
...
ON_HatchLoop::~ON_HatchLoop() { delete m_p2dCurve; }
...
ON_BOOL32
ON_HatchLoop::IsValid(ON_TextLog* text_log) const
{
...
void
ON_HatchLoop::Dump(ON_TextLog& dump) const
{
  if (m_type == ltOuter)
...
ON_BOOL32
ON_HatchLoop::Write(ON_BinaryArchive& ar) const
{
  ON_BOOL32 rc = ar.Write3dmChunkVersion(1, 1);
  if (rc)
    rc = ar.WriteInt(m_type);
  if (rc)
    rc = ar.WriteObject(m_p2dCurve);
  return rc;
}

ON_BOOL32
ON_HatchLoop::Read(ON_BinaryArchive& ar)
{
  m_type = ltOuter;
  delete m_p2dCurve;
  m_p2dCurve = NULL;
...
const ON_Curve*
ON_HatchLoop::Curve() const
{
  return m_p2dCurve;
}

bool
ON_HatchLoop::SetCurve(const ON_Curve& curve)
{
...
ON_HatchLoop::Type() const
{
  return m_type;
}

void
ON_HatchLoop::SetType(eLoopType type)
{
  m_type = type;
}
...
ON_Hatch::ON_Hatch()
: m_pattern_scale(1.0), m_pattern_rotation(0.0), m_pattern_index(-1)
{}
...
ON_Hatch&
ON_Hatch::operator=(const ON_Hatch& src)
{
  if (this != &src) {
...
ON_Hatch*
ON_Hatch::DuplicateHatch() const
{
  return Duplicate();
}

ON_BOOL32
ON_Hatch::IsValid(ON_TextLog* text_log) const
{
...
void
ON_Hatch::Dump(ON_TextLog& dump) const
{
...
ON_BOOL32
ON_Hatch::Write(ON_BinaryArchive& ar) const
{
  ON_BOOL32 rc = ar.Write3dmChunkVersion(1, 1);
  if (rc)
    rc = ar.WritePlane(m_plane);
  if (rc)
    rc = ar.WriteDouble(m_pattern_scale);
  if (rc)
    rc = ar.WriteDouble(m_pattern_rotation);
  if (rc)
    rc = ar.WriteInt(m_pattern_index);
  if (rc) {
...
ON_BOOL32
ON_Hatch::Read(ON_BinaryArchive& ar)
{
  ...
  if (major_version == 1) {
    if (rc)
      rc = ar.ReadPlane(m_plane);
    if (rc)
      rc = ar.ReadDouble(&m_pattern_scale);
    if (rc)
      rc = ar.ReadDouble(&m_pattern_rotation);
    if (rc)
      rc = ar.ReadInt(&m_pattern_index);
    if (rc) {
      m_loops.Empty();
      int i, count = 0;
      rc = ar.ReadInt(&count);
...
ON::object_type
ON_Hatch::ObjectType() const
{
  return ON::hatch_object;
}

int
ON_Hatch::Dimension() const
{
  return 3;
}
...
ON_Curve*
ON_Hatch::LoopCurve3d(int index) const
{
  int count = m_loops.Count();
  ON_Curve* pC = NULL;
...
int
ON_Hatch::PatternIndex() const
{
  return m_pattern_index;
}

void
ON_Hatch::SetPatternIndex(int index)
{
  m_pattern_index = index;
}

ON_BOOL32
ON_Hatch::GetBBox(double* bmin, double* bmax, ON_BOOL32 bGrowBox) const
{
  int i;
  int count = m_loops.Count();
  ON_BOOL32 rc = true;
...
static double
Angle3d (const ON_3dVector& axis, ON_3dVector& from, const ON_3dVector& to)
{
...
void
arbaxis (const ON_3dVector& givenaxis, ON_3dVector& newaxis)
{
...
double
arbaxisRotation (const ON_Plane& plane)
{
...
UnrotateHatch (ON_Hatch* hatch)
{
  ...
  if (fabs(P.x) > ON_ZERO_TOLERANCE || fabs(P.y) > ON_ZERO_TOLERANCE ||
      fabs(P.z) > ON_ZERO_TOLERANCE) {
...
ON_BOOL32
ON_Hatch::Transform(const ON_Xform& xform)
{
...
const ON_Plane&
ON_Hatch::Plane() const
{
  return m_plane;
}

void
ON_Hatch::SetPlane(const ON_Plane& plane)
{
  m_plane = plane;
}

double
ON_Hatch::PatternRotation() const
{
  return m_pattern_rotation;
}

void
ON_Hatch::SetPatternRotation(double rotation)
{
  m_pattern_rotation = rotation;
}

double
ON_Hatch::PatternScale() const
{
  return m_pattern_scale;
}

void
ON_Hatch::SetPatternScale(double scale)
{
...
int
ON_Hatch::LoopCount() const
{
  return m_loops.Count();
}

void
ON_Hatch::AddLoop(ON_HatchLoop* pLoop)
{
  m_loops.Append(pLoop);
}

bool
ON_Hatch::InsertLoop(int index, ON_HatchLoop* loop)
{
  if (index >= 0 &&
      index <= m_loops.Count()) // 26 June 2012 - Lowell - Changed ndex < to ndex <=
  {
...
bool
ON_Hatch::ReplaceLoops(ON_SimpleArray<const ON_Curve*> loop_curves)
{
  ...
  for (int i = 0; i < loop_curves.Count(); i++) {
    ...
    if (p2d->Dimension() == 3) {
      ...
      if (!p2d->Transform(xf) || !p2d->ChangeDimension(2)) {
        delete p2d;
        rc = false;
        break;
      }
...
void
ON_Hatch::SetBasePoint(ON_2dPoint basepoint)
{
...
void
ON_Hatch::SetBasePoint(ON_3dPoint point)
{
  ON_HatchExtra* pE = ON_HatchExtra::HatchExtension(this, true);
  if (pE) {
    ON_2dPoint base;
...
class ON_HatchExtra*
ON_Hatch::HatchExtension()
{
  ON_HatchExtra* pExtra =
      ON_HatchExtra::Cast(GetUserData(ON_HatchExtra::m_ON_HatchExtra_class_id.Uuid()));
  return pExtra;
}
