...
bool
ON_SpaceMorph::QuickPreview() const
{
...
bool
ON_SpaceMorph::PreserveStructure() const
{
  ...
  case ON_COMPONENT_INDEX::mesh_face:
    if (4 == objref.m_evp.m_t_type &&
      ...
      if (ci.m_index >= 0 && ci.m_index < m_F.Count()) {
        ...
        V[2] = m_V[fvi[2]];
        V[3] = m_V[fvi[3]];
        P = objref.m_evp.m_t[0] * V[0] + objref.m_evp.m_t[1] * V[1] +
            objref.m_evp.m_t[2] * V[2] + objref.m_evp.m_t[3] * V[3];
      }
    }
    break;
