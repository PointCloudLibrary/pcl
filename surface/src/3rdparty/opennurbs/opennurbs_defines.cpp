...
// depending on OS.
int
on_stricmp (const char* s1, const char* s2)
{
...
static int
on__hack__wcsicmp (const wchar_t* s1, const wchar_t* s2)
{
...
int
on_wcsicmp (const wchar_t* s1, const wchar_t* s2)
{
  // handle NULL strings consistently and without crashing.
  if (!s1) {
    return s2 ? -1 : 0;
...
int
on_vsnprintf (char* buffer, std::size_t count, const char* format, va_list argptr)
{
#if defined(ON_OS_WINDOWS)
...
const char*
ON::DocumentationRevision()
{
  return OPENNURBS_DOC_SVN_REVISION;
}

const char*
ON::DocumentationBranch()
{
...
ON::UnitSystem (int i)
{
  ...
  case parsecs:
    us = parsecs;
    break;
  case custom_unit_system:
    us = custom_unit_system;
    break;
  default:
    us = no_unit_system;
    break;
...
static bool
IsEnglishUnit (ON::unit_system us)
{
  return (ON::microinches == us || ON::mils == us || ON::inches == us ||
          ON::feet == us || ON::yards == us || ON::miles == us ||
          ON::printer_point == us || ON::printer_pica == us);
}
...
)
{
  ...
  if (u0 != u1 && u1 != ON::custom_unit_system && ((int)u1) > 0 && ((int)u1) < 26
    ...
    case ON::meters:
      ...
      case ON::astronomical:
        scale = 1.0 / 1.4959787e+11;
        break;
      case ON::lightyears:
        scale = 1.0 / 9.4607304725808e+15;
        break;
      case ON::parsecs:
        scale = 1.0 / 3.08567758e+16;
        break;
...
ON::PointStyle (int i)
{
  ...
  case homogeneous_rational:
    ps = homogeneous_rational;
    break;
  case euclidean_rational:
    ps = euclidean_rational;
    break;
  default:
    ps = unknown_point_style;
    break;
  }
  return ps;
}
...
ON::KnotStyle (int i)
{
  ...
  case quasi_uniform_knots:
    ks = quasi_uniform_knots;
    break;
  case piecewise_bezier_knots:
    ks = piecewise_bezier_knots;
    break;
  case clamped_end_knots:
    ks = clamped_end_knots;
    break;
  case non_uniform_knots:
    ks = non_uniform_knots;
    break;
  default:
    ks = unknown_knot_style;
    break;
  }
  return ks;
}
...
ON::Continuity (int i)
{
  ...
  case C1_locus_continuous:
    c = C1_locus_continuous;
    break;
  case C2_locus_continuous:
    c = C2_locus_continuous;
    break;
  case G1_locus_continuous:
    c = G1_locus_continuous;
    break;
  case G2_locus_continuous:
    c = G2_locus_continuous;
    break;

  case Cinfinity_continuous:
    c = Cinfinity_continuous;
    break;

  case Gsmooth_continuous:
    c = Gsmooth_continuous;
    break;
  };
...
ON::CurveStyle (int i)
{
  ...
  case hyperbola:
    cs = hyperbola;
    break;
  case planar_polyline:
    cs = planar_polyline;
    break;
  case polyline:
    cs = polyline;
    break;
  case planar_freeform_curve:
    cs = planar_freeform_curve;
    break;
  case freeform_curve:
    cs = freeform_curve;
    break;
  default:
    cs = unknown_curve_style;
    break;
  }
  return cs;
}
...
ON::SurfaceStyle (int i)
{
  ...
  case circular_cylinder:
    ss = circular_cylinder;
    break;
  case elliptical_cylinder:
    ss = elliptical_cylinder;
    break;
  case circular_cone:
    ss = circular_cone;
    break;
...
ON::ExceptionType (int i)
{
  ...
  case unable_to_write_archive:
    e = unable_to_write_archive;
    break;
  case unable_to_read_archive:
    e = unable_to_read_archive;
    break;
  case unable_to_seek_archive:
    e = unable_to_seek_archive;
    break;
  case unexpected_end_of_archive:
    e = unexpected_end_of_archive;
    break;
  case unexpected_value_in_archive:
    e = unexpected_value_in_archive;
    break;
  };
  return e;
}
...
ON::PlotWeightSource (int pw)
{
  ...
  case plot_weight_from_layer:
    ...
    break;
  case plot_weight_from_object:
    return plot_weight_from_object;
    break;
  case plot_weight_from_parent:
    return plot_weight_from_parent;
    break;
  }
  return plot_weight_from_layer;
}
...
ON::LightStyle (int i)
{
  ...
  switch (i) {
  case unknown_light_style:
    ls = unknown_light_style;
    break;
  ...
  case camera_directional_light:
    ls = camera_directional_light;
    break;
  case camera_point_light:
    ls = camera_point_light;
    break;
  case camera_spot_light:
    ls = camera_spot_light;
    break;
  case world_directional_light:
    ls = world_directional_light;
    break;
  case world_point_light:
    ls = world_point_light;
    break;
  ...
  case world_linear_light:
    ls = world_linear_light;
    break;
  case world_rectangular_light:
    ls = world_rectangular_light;
    break;
  }
  return ls;
}
...
/*enum view_type // This is already in the header. I commented it out to see if it would
compile and it does. John Croudy.
{
  model_view_type     = 0,
...
ON::ObjectType (int i)
{
  ...
  case annotation_object:
    ot = annotation_object;
    break;
  case userdata_object:
    ot = userdata_object;
    break;
  case instance_definition:
    ot = instance_definition;
    break;
  case instance_reference:
    ot = instance_reference;
    break;
  case text_dot:
    ot = text_dot;
    break;
  ...
  case hatch_object:
    ot = hatch_object;
    break;
  case morph_control_object:
    ot = morph_control_object;
    break;
  case loop_object:
    ot = loop_object;
    break;
  ...
  case edge_filter:
    ot = edge_filter;
    break;
  case polyedge_filter:
    ot = polyedge_filter;
    break;
  case meshvertex_object:
    ot = meshvertex_object;
    break;
  case meshedge_object:
    ot = meshedge_object;
    break;
  case meshface_object:
    ot = meshface_object;
    break;
  case cage_object:
    ot = cage_object;
...
ON::BitmapType (int i)
{
  ...
  switch (i) {
  case unknown_bitmap_type:
    bt = unknown_bitmap_type;
    break;
  case windows_bitmap:
    bt = windows_bitmap;
    break;
...
ON::ObjectDecoration (int i)
{
  ...
  switch (i) {
  case no_object_decoration:
    d = no_object_decoration;
    break;
...
ON::OSnapMode (int i)
{
  ...
  case os_midpoint:
    osm = os_midpoint;
    break;
  case os_intersection:
    osm = os_intersection;
    break;
  case os_end:
    osm = os_end;
    break;
  case os_perpendicular:
    osm = os_perpendicular;
    break;
  case os_tangent:
    osm = os_tangent;
    break;
