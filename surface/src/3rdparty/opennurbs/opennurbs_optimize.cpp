 ...
 *   rel_stepsize_tol, abs_stepsize_tol  (0 < rel_stepsize_tol < 1 and 0 <
 * abs_stepsize_tol) rel_stepsize_tol is a fractional tolerance and abs_stepsize_tol is
 * an absolute tolerance that determine the minimum step size for a given iteration.
 ...
 *          where not satisfied.  Try increasing max_it, rel_stepsize_tol and/or
 * abs_stepsize_tol or use the value of (*t_addr) with extreme caution.
 */
{
  // See Numerical Recipes in C's dbrent() for a description of the basic algorithm
  int rc, ok1, ok2;
  double a, b, d, d1, d2, du, dv, dw, dx, e, fu, fv, fw, fx, olde, tol1, tol2, u, u1,
      u2, v, w, x, xm;
