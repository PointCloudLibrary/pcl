...
ON_NurbsSurface*
ON_NurbsSurface::New (const ON_NurbsSurface& nurbs_surface)
{
...
{
  // static function replaces new ON_NurbsSurface(const ON_BezierSurface&
  // bezier_surface);
  return new ON_NurbsSurface (bezier_surface);
}

ON_NurbsSurface*
ON_NurbsSurface::New (int dimension,
                      ...
                      int cv_count1)
{
  ...
  return new ON_NurbsSurface (
      dimension, bIsRational, order0, order1, cv_count0, cv_count1);
}
...
ON_NurbsSurface::ON_NurbsSurface (int dim,          // dimension (>= 1)
...
ON_NurbsSurface::~ON_NurbsSurface() { Destroy(); }

unsigned int
ON_NurbsSurface::SizeOf() const
{
...
ON__UINT32
ON_NurbsSurface::DataCRC (ON__UINT32 current_remainder) const
{
  ...
  current_remainder =
      ON_CRC32 (current_remainder, 2 * sizeof (m_order[0]), &m_order[0]);
  current_remainder =
      ON_CRC32 (current_remainder, 2 * sizeof (m_cv_count[0]), &m_cv_count[0]);
  if (m_cv_count[0] > 0 && m_cv_count[1] > 0 && m_cv_stride[0] > 0 &&
      m_cv_stride[1] > 0 && m_cv) {
  ...
  current_remainder =
      ON_CRC32 (current_remainder, KnotCount (0) * sizeof (m_knot[0][0]), m_knot[0]);
  current_remainder =
      ON_CRC32 (current_remainder, KnotCount (1) * sizeof (m_knot[1][0]), m_knot[1]);

  return current_remainder;
}

ON_BOOL32
ON_NurbsSurface::SetDomain (
...
int
ON_NurbsSurface::Dimension() const
{
  return m_dim;
}

bool
ON_NurbsSurface::IsRational() const
{
  return m_is_rat ? true : false;
}

int
ON_NurbsSurface::CVSize() const
{
  return (m_is_rat) ? m_dim + 1 : m_dim;
}

int
ON_NurbsSurface::Order (int dir) const
{
  return m_order[dir ? 1 : 0];
}

int
ON_NurbsSurface::Degree (int dir) const
{
  return (m_order[dir ? 1 : 0] >= 2) ? m_order[dir ? 1 : 0] - 1 : 0;
}

int
ON_NurbsSurface::CVCount (int dir) const
{
  return m_cv_count[dir ? 1 : 0];
}

int
ON_NurbsSurface::CVCount (void) const
{
  return m_cv_count[0] * m_cv_count[1];
}

int
ON_NurbsSurface::KnotCount (int dir) const
{
  return ON_KnotCount (m_order[dir ? 1 : 0], m_cv_count[dir ? 1 : 0]);
}

double*
ON_NurbsSurface::CV (int i, int j) const
{
  return (m_cv) ? (m_cv + i * m_cv_stride[0] + j * m_cv_stride[1]) : NULL;
}

ON::point_style
ON_NurbsSurface::CVStyle() const
{
  return m_is_rat ? ON::homogeneous_rational : ON::not_rational;
}

double
ON_NurbsSurface::Weight (int i, int j) const
{
...
double
ON_NurbsSurface::Knot (int dir, int i) const
{
  return (m_knot[dir ? 1 : 0]) ? m_knot[dir ? 1 : 0][i] : 0.0;
}

int
ON_NurbsSurface::KnotMultiplicity (int dir, int i) const
{
  dir = dir ? 1 : 0;
...
const double*
ON_NurbsSurface::Knot (int dir) const
{
  return m_knot[dir ? 1 : 0];
}

bool
ON_NurbsSurface::MakeClampedUniformKnotVector (int dir, double delta)
{
  if (dir < 0 || dir > 1)
    return false;
  DestroySurfaceTree();
  ReserveKnotCapacity (dir, ON_KnotCount (m_order[dir], m_cv_count[dir]));
  return ON_MakeClampedUniformKnotVector (
      m_order[dir], m_cv_count[dir], m_knot[dir], delta);
}

bool
ON_NurbsSurface::MakePeriodicUniformKnotVector (int dir, double delta)
{
  if (dir < 0 || dir > 1)
    return false;
  DestroySurfaceTree();
  ReserveKnotCapacity (dir, ON_KnotCount (m_order[dir], m_cv_count[dir]));
  return ON_MakePeriodicUniformKnotVector (
      m_order[dir], m_cv_count[dir], m_knot[dir], delta);
}

double
ON_NurbsSurface::SuperfluousKnot (int dir, int end) const
{
  return (m_knot[dir ? 1 : 0]) ? ON_SuperfluousKnot (m_order[dir ? 1 : 0],
                                                     m_cv_count[dir ? 1 : 0],
                                                     m_knot[dir ? 1 : 0],
                                                     end)
                               : 0.0;
...
ON_BOOL32
ON_NurbsSurface::Create (int dim,          // dimension (>= 1)
                         ON_BOOL32 is_rat, // true to make a rational NURBS
                         int order0,       // order (>= 2)
                         int order1,       // order (>= 2)
                         int cv_count0,    // cv count0 (>= order0)
...
void
ON_NurbsSurface::EmergencyDestroy()
{
  Initialize();
}

void
ON_NurbsSurface::Initialize()
{
  m_dim = 0;
  m_is_rat = 0;
  m_order[0] = 0;
...
ON_BOOL32
ON_NurbsSurface::ReserveKnotCapacity (int dir, int capacity)
{
  if (dir)
    dir = 1;
  if (m_knot_capacity[dir] < capacity) {
    if (m_knot[dir]) {
      if (m_knot_capacity[dir]) {
        m_knot[dir] =
            (double*)onrealloc (m_knot[dir], capacity * sizeof (*m_knot[dir]));
...
ON_BOOL32
ON_NurbsSurface::ReserveCVCapacity (int capacity)
{
...
static void
ON_NurbsSurfaceCopyHelper (const ON_NurbsSurface& src, ON_NurbsSurface& dest)
{
  ...
  if (src.m_knot[0]) {
    ...
    memcpy (
        dest.m_knot[0], src.m_knot[0], dest.KnotCount (0) * sizeof (*dest.m_knot[0]));
  }
  if (src.m_knot[1]) {
    // copy knot array
    dest.ReserveKnotCapacity (1, dest.KnotCount (1));
    memcpy (
        dest.m_knot[1], src.m_knot[1], dest.KnotCount (1) * sizeof (*dest.m_knot[1]));
  }
  if (src.m_cv) {
    // copy cv array
    dest.ReserveCVCapacity (dest.m_cv_count[0] * dest.m_cv_count[1] *
                            dest.m_cv_stride[1]);
    ...
    if (src_stride[0] == dest.m_cv_stride[0] && src_stride[1] == dest.m_cv_stride[1]) {
      memcpy (dest.m_cv,
              src.m_cv,
              dest.m_cv_count[0] * dest.m_cv_count[1] * dest.m_cv_stride[1] *
                  sizeof (*dest.m_cv));
    }
    else {
      const double* src_cv;
...
ON_NurbsSurface&
ON_NurbsSurface::operator= (const ON_NurbsSurface& src)
{
  if (this != &src) {
...
void
ON_NurbsSurface::Dump (ON_TextLog& dump) const
{
  ...
  if (!m_cv) {
    ...
    for (i = 0; i < m_cv_count[0]; i++) {
      ...
      dump.PrintPointList (
          m_dim, m_is_rat, m_cv_count[1], m_cv_stride[1], CV (i, 0), sPreamble);
    }
  }
}

ON_BOOL32
ON_NurbsSurface::IsValid (ON_TextLog* text_log) const
{
  ON_BOOL32 rc = false;
...
ON_BOOL32
ON_NurbsSurface::GetBBox ( // returns true if successful
    double* boxmin,        // minimum
    double* boxmax,        // maximum
    ON_BOOL32 bGrowBox     // true means grow box
) const
{
  return ON_GetPointGridBoundingBox (m_dim,
                                     m_is_rat,
                                     m_cv_count[0],
                                     m_cv_count[1],
                                     m_cv_stride[0],
...
ON_BOOL32
ON_NurbsSurface::Transform (const ON_Xform& xform)
{
  DestroySurfaceTree();
  ...
  return ON_TransformPointGrid (m_dim,
                                m_is_rat,
                                m_cv_count[0],
                                m_cv_count[1],
                                m_cv_stride[0],
...
bool
ON_NurbsSurface::IsDeformable() const
{
  return true;
}

bool
ON_NurbsSurface::MakeDeformable()
{
  return true;
}

ON_BOOL32
ON_NurbsSurface::Write (ON_BinaryArchive& file // open binary file
) const
{
  ...
  if (rc) {
    ...
    if (rc)
      rc = file.WriteInt (m_order[0]);
    if (rc)
      rc = file.WriteInt (m_order[1]);
    if (rc)
      rc = file.WriteInt (m_cv_count[0]);
    if (rc)
      rc = file.WriteInt (m_cv_count[1]);

    if (rc)
      rc = file.WriteInt (0); // reserved1
    if (rc)
      rc = file.WriteInt (0); // reserved2
    ...
    if (rc)
      rc = file.WriteDouble (count, m_knot[0]);

    count = m_knot[1] ? KnotCount (1) : 0;
    if (rc)
      rc = file.WriteInt (count);
    if (rc)
      rc = file.WriteDouble (count, m_knot[1]);

    const int cv_size = CVSize();
    count = (m_cv && cv_size > 0 && m_cv_count[0] > 0 && m_cv_count[1] > 0 &&
...
ON_BOOL32
ON_NurbsSurface::Read (ON_BinaryArchive& file // open binary file
)
{
  DestroySurfaceTree();
  // NOTE - check legacy I/O code if changed
  ...
  if (rc && major_version == 1) {
    ...
    if (rc)
      rc = file.ReadInt (&order0);
    if (rc)
      rc = file.ReadInt (&order1);
    if (rc)
      rc = file.ReadInt (&cv_count0);
    if (rc)
      rc = file.ReadInt (&cv_count1);

    if (rc)
      rc = file.ReadInt (&reserved1);
    if (rc)
      rc = file.ReadInt (&reserved2);
    ...
    if (rc)
      rc = ReserveKnotCapacity (0, count);
    if (rc)
      rc = file.ReadDouble (count, m_knot[0]);

    count = 0;
    if (rc)
      rc = file.ReadInt (&count);
    if (rc)
      rc = ReserveKnotCapacity (1, count);
    if (rc)
      rc = file.ReadDouble (count, m_knot[1]);
...
ON_Interval
ON_NurbsSurface::Domain (int dir) const
{
  ON_Interval d;
  if (dir)
    dir = 1;
...
double
ON_NurbsSurface::ControlPolygonLength (int dir) const
{
  double max_length = 0.0;
  if (dir >= 0 && dir <= 1 && m_cv_count[0] >= 2 && m_cv_count[1] >= 2 &&
      m_cv != NULL) {
    double length;
    const double* p;
    int i;
    for (i = 0; i < m_cv_count[1 - dir]; i++) {
      ...
      ON_GetPolylineLength (
          m_dim, m_is_rat, m_cv_count[dir], m_cv_stride[dir], p, &length);
...
ON_BOOL32
ON_NurbsSurface::GetSurfaceSize (double* width, double* height) const
{
...
int
ON_NurbsSurface::SpanCount (int dir) const
{
  if (dir)
    dir = 1;
...
ON_BOOL32
ON_NurbsSurface::GetSpanVector (int dir, double* s) const
{
  if (dir)
    dir = 1;
...
ON_BOOL32
ON_NurbsSurface::GetParameterTolerance ( // returns tminus < tplus: parameters tminus <=
                                         // s <= tplus
    int dir,
...
) const
{
  ON_BOOL32 rc = false;
  int span_index[2];
  span_index[0] = ON_NurbsSpanIndex (m_order[0],
                                     m_cv_count[0],
                                     m_knot[0],
                                     s,
                                     (side == 2 || side == 3) ? -1 : 1,
                                     (hint) ? hint[0] : 0);
  span_index[1] = ON_NurbsSpanIndex (m_order[1],
                                     m_cv_count[1],
                                     m_knot[1],
                                     t,
                                     (side == 3 || side == 4) ? -1 : 1,
...
) const
{
  ...
  if ((dir == 0 || dir == 1) && IsValid()) {
    ...
    ON_NurbsCurve* nurbscrv =
        new ON_NurbsCurve (m_dim, m_is_rat, m_order[dir], m_cv_count[dir]);
    memcpy (nurbscrv->m_knot,
            m_knot[dir],
            nurbscrv->KnotCount() * sizeof (*nurbscrv->m_knot));
    span_index = ON_NurbsSpanIndex (
        m_order[1 - dir], m_cv_count[1 - dir], m_knot[1 - dir], c, 1, 0);
    if (span_index < 0)
      span_index = 0;
    else if (span_index > m_cv_count[1 - dir] - m_order[1 - dir])
      span_index = m_cv_count[1 - dir] - m_order[1 - dir];
    ON_NurbsCurve N (
        Scvsize * nurbscrv->CVCount(), 0, m_order[1 - dir], m_order[1 - dir]);
...
ToCurve (const ON_NurbsSurface& srf, int dir, ON_NurbsCurve* crv)
{
  double* tmp_cv = NULL;
  if (dir < 0 || dir > 1)
    return NULL;
  ...
  if (!crv->Create (srf_cv_size * srf.m_cv_count[1 - dir], // dim
                    false,                                 // is_rat
                    srf.m_order[dir],
                    srf.m_cv_count[dir]))
    return NULL;
...
static ON_BOOL32
FromCurve (ON_NurbsCurve& crv, ON_NurbsSurface& srf, int dir)
{
  ...
  crv.m_cv_capacity = 0;
  crv.m_cv = 0;
  if (srf.m_knot_capacity[dir] > 0 && srf.m_knot[dir] &&
...
ON_BOOL32
ON_NurbsSurface::Trim (int dir, const ON_Interval& domain)
{
  ON_BOOL32 rc = false;
  if (dir < 0 || dir > 1)
    return false;
...
bool
ON_NurbsSurface::Extend (int dir, const ON_Interval& domain)
{
                        ...
                        ON_Surface*& east_or_north_side) const
{
  ...
  if (left_srf != this) {
    ...
    memcpy (left_srf->m_knot[1 - dir],
            m_knot[1 - dir],
            KnotCount (1 - dir) * sizeof (m_knot[1 - dir][0]));
  }
...
ON_Surface*
ON_NurbsSurface::Offset (double, double, double*) const
{
...
ON_BOOL32
ON_NurbsSurface::IsPlanar (ON_Plane* plane, double tolerance) const
{
  ON_Plane pln;
  double d;
...
ON_BOOL32
ON_NurbsSurface::ChangeSurfaceSeam (int dir, double t)
{
  bool rc = true;
...
bool
ON_NurbsSurface::GetNextDiscontinuity (int dir,
                                       ON::continuity c,
                                       double t0,
                                       double t1,
                                       double* t,
                                       ...
                                       double curvature_tolerance) const
{
  ...
  const ON::continuity input_c =
      c; // saved so we can tell if "locus" needs to be dealt with
  c = ON::ParametricContinuity (c); // strips "locus" from c

  bool bEv2ndDer =
      (c == ON::C2_continuous || c == ON::G2_continuous || c == ON::Gsmooth_continuous);
  ...
  double segtol = (fabs (m_knot[dir][ki]) + fabs (m_knot[dir][ki + 1]) +
                   fabs (m_knot[dir][ki + 1] - m_knot[dir][ki])) *
                  ON_SQRT_EPSILON;

  if (t0 < t1) {
    int ii = ki + m_order[dir] - 2;
    if (t0 < m_knot[dir][ii + 1] && t1 > m_knot[dir][ii + 1] &&
        (m_knot[dir][ii + 1] - t0) <= segtol && ii + 2 < m_cv_count[dir]) {
      t0 = m_knot[dir][ii + 1];
      ki = ON_NurbsSpanIndex (m_order[dir], m_cv_count[dir], m_knot[dir], t0, 1, *hint);
    }
    *hint = ki;
    ...
    if (ki >= m_cv_count[dir] - 1) {
      if (input_c != c && t0 < m_knot[dir][m_cv_count[dir] - 1] &&
          t1 >= m_knot[dir][m_cv_count[dir] - 1]) {
        // have to do locus end test
        return ON_Surface::GetNextDiscontinuity (dir,
                                                 input_c,
                                                 t0,
                                                 t1,
                                                 t,
    ...
    int ii = ki + m_order[dir] - 2;
    if (t0 > m_knot[dir][ii] && t1 < m_knot[dir][ii] &&
        (t0 - m_knot[dir][ii]) <= segtol && ii > m_order[dir] - 2) {
      t0 = m_knot[dir][ii];
      ki = ON_NurbsSpanIndex (m_order[dir], m_cv_count[dir], m_knot[dir], t0, 1, *hint);
    }
    ...
    *hint = ki;
    ...
    while (ki < m_order[dir] - 2 && m_knot[dir][ki] >= t0)
      ki--;
    if (ki <= m_order[dir] - 2) {
      if (input_c != c && t0 > m_knot[dir][m_order[dir] - 2] &&
          t1 < m_knot[dir][m_order[dir] - 2]) {
        // have to do locus end test
        return ON_Surface::GetNextDiscontinuity (dir,
                                                 input_c,
                                                 t0,
  ...
  while (m_knot[dir][ki] < t1) {
    ...
    if (m_knot[dir][ki] == m_knot[dir][ki + delta]) {
      ...
      int j, j0 = 0, otherki;
      for (otherki = m_order[1 - dir] - 2; otherki < m_cv_count[1 - dir] - 1;
           otherki++) {
  ...
  if (input_c != c) {
    ...
    rc = ON_Surface::GetNextDiscontinuity (
        dir, input_c, t0, t1, t, hint, dtype, cos_angle_tolerance, curvature_tolerance);
  }
...
) const
{
  ...
  return ON_Surface::IsContinuous (desired_continuity,
                                   s,
                                   t,
                                   hint,
                                   point_tolerance,
...
ON_NurbsSurface::Reverse (int dir)
{
  ...
  ON_BOOL32 rc1 = ON_ReversePointGrid (3,
                                       m_is_rat,
                                       m_cv_count[0],
                                       m_cv_count[1],
                                       m_cv_stride[0],
...
ON_NurbsSurface::Transpose()
{
  ...
  // swap knot vectors
  i = m_knot_capacity[0];
  m_knot_capacity[0] = m_knot_capacity[1];
  m_knot_capacity[1] = i;
  double* k = m_knot[0];
  m_knot[0] = m_knot[1];
  m_knot[1] = k;
...
ON_BOOL32
ON_NurbsSurface::SetCVRow (int row_index, const ON_3dPoint& point)
{
  DestroySurfaceTree();
  int i;
...
ON_BOOL32
ON_NurbsSurface::SetCVRow (int row_index,
...
ON_BOOL32
ON_NurbsSurface::SetCVColumn (int col_index, const ON_3dPoint& point)
{
  DestroySurfaceTree();
  int j;
...
ON_BOOL32
ON_NurbsSurface::SetCVColumn (int col_index,
                              int v_stride,   // v stride
                              const double* v // values (same dim and is_rat as surface)
)
{
...
double
ON_NurbsSurface::GrevilleAbcissa (int dir,   // dir
...
bool
ON_NurbsSurface::GetGrevilleAbcissae ( // see ON_GetGrevilleAbcissae() for details
    int dir,                           // dir
    double* g                          // g[cv1-cv0]
) const
{
...
bool
ON_NurbsSurface::SetClampedGrevilleKnotVector (int dir,        // dir
...
)
{
  ...
  return ON_GetGrevilleKnotVector (
      g_stride, g, false, Order (dir), CVCount (dir), m_knot[dir]);
}

bool
ON_NurbsSurface::SetPeriodicGrevilleKnotVector (int dir,        // dir
                                                int g_stride,   // g_stride
                                                const double* g // g[], Greville abcissa
)
{
  ...
  if (!m_knot[dir] && m_order[dir] >= 2 && m_cv_count[dir] >= m_order[dir])
    ReserveKnotCapacity (dir, KnotCount (dir));
  return ON_GetGrevilleKnotVector (
      g_stride, g, true, Order (dir), CVCount (dir), m_knot[dir]);
}

bool
ON_NurbsSurface::ZeroCVs()
{
  DestroySurfaceTree();
  bool rc = false;
  int i, j;
...
bool
ON_NurbsSurface::IsClamped ( // determine if knot vector is clamped
...
static void
ConvertToCurve (const ON_NurbsSurface& srf, int dir, ON_NurbsCurve& crv)
{
...
static void
ConvertFromCurve (ON_NurbsCurve& crv, int dir, ON_NurbsSurface& srf)
{
  // DO NOT MAKE THIS FUNCTION PUBLIC - IT IS DELICATE AND DEDICATED TO USE IN THIS FILE
  ...
  if (crv.m_cv) {
    if (srf.m_cv && crv.m_cv != srf.m_cv && srf.m_cv_capacity > 0 &&
    ...
    if (srf.m_cv) {
      ...
      memcpy (
          srf.m_cv, crv.m_cv, crv.m_cv_stride * crv.m_cv_count * sizeof (*srf.m_cv));
    }
    else {
...
bool
ON_NurbsSurface::InsertKnot (int dir, // dir 0 = "s", 1 = "t"
                             double knot_value,
...
)
{
  ...
  if ((dir == 0 || dir == 1) && IsValid() && knot_multiplicity > 0 &&
      knot_multiplicity < Order (dir)) {
...
bool
ON_NurbsSurface::ChangeDimension (int desired_dimension //  desired_dimension
)
{
  bool rc = false;
  int i, j, k;
...
bool
ON_NurbsSurface::IncreaseDegree (int dir,           // dir 0 = "s", 1 = "t"
...
ON_BOOL32
ON_NurbsSurface::TensorProduct (const ON_NurbsCurve& nurbscurveA,
                                ...
                                ON_TensorProduct& tensor)
{
  ...
  Create (
      dimC, is_ratC, nurbscurveA.Order(), nurbscurveB.Order(), cv_countA, cv_countB);
  ...
  for (i = 0; i < cv_countA; i++) {
    ...
    for (j = 0; j < cv_countB; j++) {
      ...
      rc = tensor.Evaluate (
          (wA == 0.0) ? 0.0 : 1.0 / wA, cvA, (wB == 0.0) ? 0.0 : 1.0 / wB, cvB, cvC);
      if (!rc)
        return false;
      if (is_ratC) {
        wC = wA * wB;
...
static bool
ON_MakeDegreesCompatible (ON_NurbsCurve& nurbs_curveA, ON_NurbsCurve& nurbs_curveB)
{
...
static bool
ON_MakeDomainsCompatible (ON_NurbsCurve& nurbs_curveA, ON_NurbsCurve& nurbs_curveB)
{
...
static bool
ON_MakeKnotVectorsCompatible (ON_NurbsCurve& nurbs_curveA, ON_NurbsCurve& nurbs_curveB)
{
  ...
  if (nurbs_curveA.IsPeriodic() || nurbs_curveB.IsPeriodic()) {
    ...
    int kiA, kiB;
    for (kiA = nurbs_curveA.m_cv_count, kiB = nurbs_curveB.m_cv_count;
  ...
  while ((ki < nurbs_curveA.m_cv_count - 1 || ki < nurbs_curveB.m_cv_count - 1) &&
         ...
         ki <= nurbs_curveA.m_cv_count - 1 && ki <= nurbs_curveB.m_cv_count - 1) {
    ...
    if (b == nurbs_curveB.m_knot[ki - 1])
      return false;
    multA =
        ON_KnotMultiplicity (order, nurbs_curveA.m_cv_count, nurbs_curveA.m_knot, ki);
    multB =
        ON_KnotMultiplicity (order, nurbs_curveB.m_cv_count, nurbs_curveB.m_knot, ki);

    if (a < b) {
      // insert a in nurbs_curveB
      span.Set (nurbs_curveB.m_knot[ki - 1], nurbs_curveB.m_knot[ki]);
      ktol = ON_SQRT_EPSILON * (span.Length() + fabs (nurbs_curveB.m_knot[ki - 1]) +
                                fabs (nurbs_curveB.m_knot[ki]));
      if (a >= span[1] - ktol) {
      ...
      ktol = ON_SQRT_EPSILON * (span.Length() + fabs (nurbs_curveA.m_knot[ki - 1]) +
                                fabs (nurbs_curveA.m_knot[ki]));
      if (b >= span[1] - ktol) {
...
int
ON_NurbsSurface::CreateRuledSurface (const ON_Curve& curveA,
...
static ON_3dPoint
CornerAt (const ON_Surface& srf, int corner)
{
  double s, t;
  switch (corner) {
  case 0: // sw
...
bool
ON_NurbsSurface::CollapseSide (int side, ON_3dPoint point)
{
...
int
ON_NurbsSurface::CreateConeSurface (ON_3dPoint apex_point,
...
ON_BOOL32
ON_NurbsSurface::ConvertSpanToBezier (int span_index0,
                                      int span_index1,
                                      ON_BezierSurface& bezier_surface) const
{
    ...
    return false;
  {
    ...
    if (m_knot[0][span_index0] != m_knot[0][span_index0 + m_order[0] - 2])
      bClamp = true;
    if (m_knot[0][span_index0 + m_order[0] - 1] !=
        m_knot[0][span_index0 + 2 * m_order[0] - 3])
      bClamp = true;
    if (m_knot[1][span_index1] != m_knot[1][span_index1 + m_order[1] - 2])
      bClamp = true;
    if (m_knot[1][span_index1 + m_order[1] - 1] !=
        m_knot[1][span_index1 + 2 * m_order[1] - 3])
      bClamp = true;
    if (bClamp) {
      memcpy (bispan.m_knot[0],
              m_knot[0] + span_index0,
              bispan.KnotCount (0) * sizeof (bispan.m_knot[0][0]));
      memcpy (bispan.m_knot[1],
              m_knot[1] + span_index1,
              bispan.KnotCount (1) * sizeof (bispan.m_knot[1][0]));
      bispan.ClampEnd (1, 2);
      bispan.ClampEnd (0, 2);
    }
