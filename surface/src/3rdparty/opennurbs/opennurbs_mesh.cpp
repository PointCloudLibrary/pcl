...
ON_Mesh::Write_2(int Vcount, ON_BinaryArchive& file) const
{
  ...
  if (Vcount > 0) {
    ...
    if (rc)
      rc =
          file.WriteCompressedBuffer(Kcount * sizeof(ON_SurfaceCurvature), m_K.Array());
    if (rc)
...
ON_BOOL32
ON_MeshVertexRef::Transform(const ON_Xform&)
{
  return false;
}
...
ON_BOOL32
ON_MeshEdgeRef::Transform(const ON_Xform&)
{
  return false;
}
...
ON_BOOL32
ON_MeshFaceRef::Transform(const ON_Xform&)
{
  return false;
}
...
ON_BOOL32
ON_MeshDoubleVertices::IsValid(ON_TextLog*) const
{
  return true;
}
...
ON__UINT32
ON_MeshDoubleVertices::FloatCRC(const ON_3fPointArray& V)
{
  return V.DataCRC(0);
}

ON__UINT32
ON_MeshDoubleVertices::DoubleCRC() const
{
...
ON_PerObjectMeshParameters::FindOrCreate(const ON_Object* object, bool bCreate)
{
  if (0 == object)
    return 0;
  ON_PerObjectMeshParameters* ud = ON_PerObjectMeshParameters::Cast(object->GetUserData(
...
// virtual ON_Object override
ON_BOOL32
ON_PerObjectMeshParameters::IsValid(ON_TextLog*) const
{
  return true;
}
...
// virtual ON_UserData override
ON_BOOL32
ON_PerObjectMeshParameters::Archive() const
{
  return true;
}
