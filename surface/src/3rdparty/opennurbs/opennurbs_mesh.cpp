...
class /* DO NOT copy, move, or export this class */ ON_MeshDoubleVertices
: public ON_UserData {
...
public:
  ...
  /*
    ...
    a pointer to it.
  */
  static ON_MeshDoubleVertices*
  Get (const ON_Mesh* mesh);
  ...
  */
  static ON_MeshDoubleVertices*
  Attach (const ON_Mesh* mesh);

  // virtual ON_Object overrides
  ON_BOOL32
  IsValid (ON_TextLog* = NULL) const;
  void
...
#undef ON_BOZO_VACCINE_17F24E7521BE4a7b9F3D7F85225247E3
  ...
  DoubleCRC() const;
  static ON__UINT32
  FloatCRC (const ON_3fPointArray&);
...
ON_MeshCurveParameters::ON_MeshCurveParameters() { memset (this, 0, sizeof (*this)); }
...
{
  return (vi[0] >= 0 && vi[0] < mesh_vertex_count && vi[1] >= 0 &&
          vi[1] < mesh_vertex_count && vi[2] >= 0 && vi[2] < mesh_vertex_count &&
          vi[3] >= 0 && vi[3] < mesh_vertex_count && vi[0] != vi[1] && vi[1] != vi[2] &&
          vi[2] != vi[0] && (vi[2] == vi[3] || (vi[0] != vi[3] && vi[1] != vi[3])));
}
...
bool
ON_MeshFace::Repair (int mesh_vertex_count)
{
  ON_MeshFace f;
  int fvi_count = 0;
  ...
  if (vi[3] >= 0 && vi[3] < mesh_vertex_count && f.vi[0] != vi[3] && f.vi[1] != vi[3] &&
      f.vi[2] != vi[3])
    f.vi[fvi_count++] = vi[3];
...
bool
ON_MeshFace::Repair (int mesh_vertex_count, const ON_3fPoint* V)
{
  ON_MeshFace f;
  int fvi_count = 0;
  f.vi[0] = f.vi[1] = f.vi[2] = f.vi[3] = -1;
  ...
  if (vi[3] >= 0 && vi[3] < mesh_vertex_count && f.vi[0] != vi[3] && f.vi[1] != vi[3] &&
      f.vi[2] != vi[3] && V[f.vi[0]] != V[vi[3]] && V[f.vi[1]] != V[vi[3]]) {
...
bool
ON_MeshFace::Repair (int mesh_vertex_count, const ON_3dPoint* V)
{
  ON_MeshFace f;
  int fvi_count = 0;
  f.vi[0] = f.vi[1] = f.vi[2] = f.vi[3] = -1;
...
ON_Mesh::ON_Mesh (int initial_facet_capacity,  // initial facet array capacity
...
ON__UINT32
ON_Mesh::DataCRC (ON__UINT32 current_remainder) const
{
  const ON_3fPoint* p = m_V.Array();
  current_remainder = ON_CRC32 (current_remainder, m_V.Count() * sizeof (p[0]), p);
  current_remainder =
      ON_CRC32 (current_remainder, m_F.Count() * sizeof (ON_MeshFace), m_F.Array());
...
ON_Mesh&
ON_Mesh::operator= (const ON_Mesh& src)
{
  if (this != &src) {
    Destroy();
...
static bool
ON_MeshIsNotValid (bool bSilentError)
{
  return bSilentError ? false : ON_IsNotValid(); // good place for a breakpoint;
}

ON_BOOL32
ON_Mesh::IsValid (ON_TextLog* text_logx) const
{
  ...
  // Greg Arden 9 May 2003. Fixes TRR#10604.  Attempt to detect meshes with non-finite
  // vertices by testing the bounding box.
  int i;
  for (i = 0; i < 3; i++) {
  ...
  if (0 != dV) {
    ...
    for (fi = 0; fi < facet_count; fi++) {
      ...
      //       text_log->Print("ON_Mesh.m_F[%d].vi[] has invalid vertex indices.\n",fi);
      //     else
      //       text_log->Print("ON_Mesh.m_F[%d] has degenerate float precision vertex
      //       locations.\n",fi);
      //   }
      //   return ON_MeshIsNotValid(bSilentError);
      // }
...
void
ON_Mesh::Dump (ON_TextLog& dump) const
{
  ...
  bool bDoubles = vcount > 0 && HasDoublePrecisionVertices() &&
  ...
  dump.Print ("m_packed_tex_domain: (%g,%g)x(%g,%g)\n",
              m_packed_tex_domain[0][0],
              m_packed_tex_domain[0][1],
              m_packed_tex_domain[1][0],
              m_packed_tex_domain[1][1]);
...
bool
ON_Mesh::WriteFaceArray (int vcount, int fcount, ON_BinaryArchive& file) const
{
  unsigned char cvi[4];
...
bool
ON_Mesh::ReadFaceArray (int, int fcount, ON_BinaryArchive& file)
{
  unsigned char cvi[4];
  unsigned short svi[4];
  unsigned int* vi;
...
bool
ON_Mesh::Write_1 (ON_BinaryArchive& file) const
{
  ...
  if (rc)
    rc = file.WriteArray (m_T);
  if (rc)
    rc = file.WriteArray (m_K);
  if (rc)
...
bool
ON_Mesh::Read_1 (ON_BinaryArchive& file)
{
  ...
  if (rc)
    rc = file.ReadArray (m_T);
  if (rc)
    rc = file.ReadArray (m_K);
  if (rc)
...
bool
ON_Mesh::Write_2 (int Vcount, ON_BinaryArchive& file) const
{
  ...
  if (Vcount > 0) {
    ...
    if (rc)
      rc = file.WriteCompressedBuffer (Vcount * sizeof (ON_3fPoint), m_V.Array());
    if (rc)
      rc = file.WriteCompressedBuffer (Ncount * sizeof (ON_3fVector), m_N.Array());
    if (rc)
      rc = file.WriteCompressedBuffer (Tcount * sizeof (ON_2fPoint), m_T.Array());
    if (rc)
      rc = file.WriteCompressedBuffer (Kcount * sizeof (ON_SurfaceCurvature),
                                       m_K.Array());
    if (rc)
      rc = file.WriteCompressedBuffer (Ccount * sizeof (ON_Color), m_C.Array());
    if (e == ON::big_endian) {
...
bool
ON_Mesh::Read_2 (int vcount, ON_BinaryArchive& file)
{
  ...
  if (vcount > 0) {
    ...
    sz = 0;
    if (rc)
      rc = file.ReadCompressedBufferSize (&sz);
    if (rc && sz) {
      if (sz == vcount * sizeof (m_V[0])) {
        m_V.SetCapacity (vcount);
        if (rc)
          rc = file.ReadCompressedBuffer (sz, m_V.Array(), &bFailedCRC);
        if (rc)
          m_V.SetCount (vcount);
      }
      else {
    ...
    if (rc && sz) {
      if (sz == vcount * sizeof (m_N[0])) {
        ...
        if (rc)
          rc = file.ReadCompressedBuffer (sz, m_N.Array(), &bFailedCRC);
        if (rc)
          m_N.SetCount (vcount);
      }
      else {
    ...
    if (rc && sz) {
      if (sz == vcount * sizeof (m_T[0])) {
        ...
        if (rc)
          rc = file.ReadCompressedBuffer (sz, m_T.Array(), &bFailedCRC);
        if (rc)
          m_T.SetCount (vcount);
      }
      else {
        ON_ERROR (
            "ON_Mesh::Read - compressed texture coordinate buffer size is wrong.");
        rc = false; // buffer is wrong size
      }
    }
    ...
    if (rc && sz) {
      if (sz == vcount * sizeof (m_K[0])) {
        ...
        if (rc)
          rc = file.ReadCompressedBuffer (sz, m_K.Array(), &bFailedCRC);
        if (rc)
          m_K.SetCount (vcount);
      }
      else {
    ...
    if (rc && sz) {
      if (sz == vcount * sizeof (m_C[0])) {
        ...
        if (rc)
          rc = file.ReadCompressedBuffer (sz, m_C.Array(), &bFailedCRC);
        if (rc)
          m_C.SetCount (vcount);
      }
      else {
...
ON_BOOL32
ON_Mesh::Write (ON_BinaryArchive& file) const
{
  int i;
  ...
  if (rc)
    rc = file.WriteInt (vcount);
  if (rc)
    rc = file.WriteInt (fcount);
  if (rc)
    rc = file.WriteInterval (m_packed_tex_domain[0]);
  if (rc)
    rc = file.WriteInterval (m_packed_tex_domain[1]);
  if (rc)
    rc = file.WriteInterval (m_srf_domain[0]);
  if (rc)
    rc = file.WriteInterval (m_srf_domain[1]);
  if (rc)
    rc = file.WriteDouble (2, m_srf_scale);
  if (rc)
    rc = file.WriteFloat (6, &m_vbox[0][0]);
  if (rc)
    rc = file.WriteFloat (6, &m_nbox[0][0]);
  if (rc)
    rc = file.WriteFloat (4, &m_tbox[0][0]);
  ...
  if (rc)
    rc = WriteFaceArray (vcount, fcount, file);
  ...
  if (rc)
    rc = file.WriteUuid (m_Ttag.m_mapping_id);
  ...
  if (rc && vcount > 0) {
    ...
    if (rc)
      rc = file.WriteCompressedBuffer (Scount * sizeof (ON_2dPoint), m_S.Array());
    if (e == ON::big_endian) {
      file.ToggleByteOrder (Scount * 2, 8, m_S.Array(), (void*)m_S.Array());
    }
  }
  ...
  if (rc)
    rc = m_Ttag.Write (file);

  // added for minor version 3.5
  if (rc)
    rc = file.WriteChar (m_mesh_is_manifold);
  if (rc)
    rc = file.WriteChar (m_mesh_is_oriented);
  if (rc)
    rc = file.WriteChar (m_mesh_is_solid);
...
static const ON_UUID obsolete_default_srfp_mapping_id = {
    0xb988a6c2, 0x61a6, 0x45a7, {0xaa, 0xee, 0x9a, 0xed, 0x7e, 0xf4, 0xe3, 0x16}};

// overrides virtual ON_Object::Write
ON_BOOL32
ON_TextureMapping::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_TextureMapping::Read (ON_BinaryArchive& file)
{
  Default();
...
GetSurfaceParametersHelper (
    const ON_Mesh& mesh, double tex_x, double tex_y, double* srf_s, double* srf_t)
{
...
ON_BOOL32
ON_Mesh::Read (ON_BinaryArchive& file)
{
  Destroy();
  ...
  if (rc && (1 == major_version || 3 == major_version)) {
    ...
    if (rc)
      rc = file.ReadInt (&vcount);
    if (rc)
      rc = file.ReadInt (&fcount);
    if (rc)
      rc = file.ReadInterval (m_packed_tex_domain[0]);
    if (rc)
      rc = file.ReadInterval (m_packed_tex_domain[1]);
    if (rc)
      rc = file.ReadInterval (m_srf_domain[0]);
    if (rc)
      rc = file.ReadInterval (m_srf_domain[1]);
    if (rc)
      rc = file.ReadDouble (2, m_srf_scale);
    if (rc)
      rc = file.ReadFloat (6, &m_vbox[0][0]);
    if (rc)
      rc = file.ReadFloat (6, &m_nbox[0][0]);
    if (rc)
      rc = file.ReadFloat (4, &m_tbox[0][0]);
    ...
    if (rc) {
      ...
      case 1: // closed;
        SetClosed (1);
        break;
      case 2: // 13 April 2010 Dale Lear - "2" value is obsolete but appears in old
              // files
        SetClosed (1);
        break;
      }
    ...
    if (rc)
      rc = ReadFaceArray (vcount, fcount, file);
    ...
    if (3 == major_version) {
      if (minor_version >= 3) {
        // added for minor version 3.3
        if (rc)
          rc = file.ReadUuid (m_Ttag.m_mapping_id);
          ...
          if (rc && sz) {
            if (sz == vcount * sizeof (ON_2dPoint)) {
              ...
              if (ON::big_endian == file.Endian()) {
                file.ToggleByteOrder (
                    m_S.Count() * 2, 8, m_S.Array(), (void*)m_S.Array());
              }
            }
            else {
              ON_ERROR ("ON_Mesh::Read - surface parameter buffer size is wrong.");
              if (rc && file.ArchiveOpenNURBSVersion() <= 201011049 &&
                  0 == (sz % sizeof (ON_2dPoint)) && sz >= sizeof (ON_2dPoint)) {
        ...
        if (minor_version >= 4 && file.ArchiveOpenNURBSVersion() >= 200606010) {
          ...
          if (minor_version >= 5) {
            if (rc)
              rc = file.ReadChar (&m_mesh_is_manifold);
            if (rc)
              rc = file.ReadChar (&m_mesh_is_oriented);
            if (rc)
              rc = file.ReadChar (&m_mesh_is_solid);
          }
        }
      }
    }

    if (0 == m_S.Count() && m_V.Count() > 0 && HasTextureCoordinates() &&
        m_srf_domain[0].IsIncreasing() && m_srf_domain[1].IsIncreasing() &&
        m_packed_tex_domain[0].IsInterval() && m_packed_tex_domain[1].IsInterval() &&
        0 == m_Ttag.m_mapping_crc && ON_UuidIsNil (m_Ttag.m_mapping_id)) {
...
ON::object_type
ON_Mesh::ObjectType() const
{
  return ON::mesh_object;
}

int
ON_Mesh::Dimension() const
{
  return 3;
}
...
//   C4189: 'breakpoint_here_for_bad_vbox' : local variable is initialized but not
//   referenced
// on the line
...
ON_FloatFloor (double x)
{
  ...
  //
  const double e =
      (x < 0.0) ? (1.0 + 0.5 * ON_FLOAT_EPSILON) : (1.0 - 0.5 * ON_FLOAT_EPSILON);
  float f;
  f = (float)(e * x);
  return f;
}
...
ON_BOOL32
ON_Mesh::GetBBox (  // returns true if successful
    ...
    ON_BOOL32 bGrowBox) const
{
  ...
  if (facet_count >= 1 && vertex_count >= 3) {
    ...
    if (m_vbox[0][0] > m_vbox[1][0]) {
      ...
      float* fbbox[2] = {const_cast<float*> (&m_vbox[0][0]),
      ...
      while (HasDoublePrecisionVertices() && DoublePrecisionVerticesAreValid()) {
        ...
        rc = ON_GetPointListBoundingBox (
            3, 0, vertex_count, 3, &dV[0].x, &dbbox[0][0], &dbbox[1][0], false);
        if (!rc)
          break;
        ...
        if (SinglePrecisionVerticesAreValid()) {
          ON_GetPointListBoundingBox (
              3, 0, vertex_count, 3, &m_V[0].x, &fbbox[0][0], &fbbox[1][0], true);
        }
        break;
      }
      ...
      if (!rc) {
        rc = ON_GetPointListBoundingBox (
            3, 0, vertex_count, 3, &m_V[0].x, fbbox[0], fbbox[1], false);
      }
    }
    else {
      rc = true;
...
bool
ON_Mesh::IsDeformable() const
{
  return true;
}

bool
ON_Mesh::MakeDeformable()
{
  return true;
}

ON_BOOL32
ON_Mesh::Transform (const ON_Xform& xform)
{
  const bool bIsValid_fV = SinglePrecisionVerticesAreValid();
  const bool bIsValid_dV = DoublePrecisionVerticesAreValid();
  const bool bSyncheddV =
      bIsValid_fV && bIsValid_dV && HasSynchronizedDoubleAndSinglePrecisionVertices();
  ...
  if (rc && 0.0 == d) {
    ...
    if (HasVertexNormals()) {
      ...
      rc = ON_TransformVectorList (3, vertex_count, 3, &m_N[0][0], N_xform) ? true
                                                                            : false;
      if (d < 0.0) {
        FlipVertexNormals();
      }
  ...
  if (rc && HasPrincipalCurvatures()) {
    if (fabs (fabs (d) - 1.0) > ON_SQRT_EPSILON) {
      ...
      if (0.0 != scale && 0.0 != d && scale == xform.m_xform[1][1] &&
        ...
        for (int j = 0; j < 4; j++) {
          if (m_kstat[j])
            m_kstat[j]->Set (
                m_kstat[j]->m_style, m_K.Count(), m_K.Array(), m_N.Array());
        }
      }
      else {
...
void
ON_Mesh::DestroyRuntimeCache (bool bDelete)
{
  int i;
...
ON_BOOL32
ON_Mesh::SwapCoordinates (int i, int j // indices of coords to swap
)
{
  if (i == j)
    return true;
  ...
  if (rc) {
    ...
    if (m_vbox[0][0] <= m_vbox[1][0]) {
      x = m_vbox[0][i];
      m_vbox[0][i] = m_vbox[0][j];
      m_vbox[0][j] = x;
      x = m_vbox[1][i];
      m_vbox[1][i] = m_vbox[1][j];
      m_vbox[1][j] = x;
    }
    if (m_nbox[0][0] <= m_nbox[1][0]) {
      x = m_nbox[0][i];
      m_nbox[0][i] = m_nbox[0][j];
      m_nbox[0][j] = x;
      x = m_nbox[1][i];
      m_nbox[1][i] = m_nbox[1][j];
      m_nbox[1][j] = x;
    }
  }
...
void
ON_Mesh::SetSolidOrientation (int solid_orientation)
{
...
static int
ON_MeshIsManifold_CompareV (const void* a, const void* b)
{
...
static int
ON_MeshGetVertexEdges_Compare2dex (const void* a, const void* b)
{
  return ON_Compare2dex ((const ON_2dex*)a, (const ON_2dex*)b);
}

static int
ON_MeshIsManifold_Compare3dex (const void* a, const void* b)
{
...
int
ON_Mesh::GetVertexEdges (int vertex_index_count,
                         ...
                         ON_SimpleArray<ON_2dex>& edges) const
{
  ...
  // 03/12/2007 TimH. The line below appears to be a typo.  Using the following line
  // works better. const int mesh_fcount = m_V.Count();
  const int mesh_fcount = m_F.Count();

  if (vertex_index_count <= 0 || 0 == vertex_index || mesh_fcount <= 0 ||
      mesh_vcount < 3) {
    return 0;
  }
  ...
  if (TopologyExists() && mesh_vcount == m_top.m_topv_map.Count() &&
...
int
ON_Mesh::GetMeshEdges (ON_SimpleArray<ON_2dex>& edges) const
{
  const int edges_count0 = edges.Count();
  int fi, ei, j, fvi;
  const int* f_vi;
...
bool
ON_Mesh::IsSolid() const
{
  return (IsClosed() && IsManifold() && IsOriented());
}

bool
ON_Mesh::IsManifold (bool bTopologicalTest,
                     bool* pbIsOriented,
                     bool* pbHasBoundary) const
{
  ...
  if (vcount > 0 && fcount > 0) {
    ...
    for (i = 0; i < fcount; i++) {
      fvi = (f++)->vi;
      if (fvi[0] >= 0 && fvi[0] < vcount && fvi[1] >= 0 && fvi[1] < vcount &&
          fvi[2] >= 0 && fvi[2] < vcount && fvi[3] >= 0 && fvi[3] < vcount) {
...
static void
ON_hsort_3dex (ON_3dex* e, std::size_t nel)
{
  // dictionary sort e[]
  ...
  for (;;) {
    ...
    while (j <= i_end) {
      if (j < i_end &&
          (e[j].i < e[j + 1].i ||
           (e[j].i == e[j + 1].i &&
            (e[j].j < e[j + 1].j || (e[j].j == e[j + 1].j && e[j].k < e[j + 1].k)))))
        j++;

      if (e_tmp.i < e[j].i ||
          (e_tmp.i == e[j].i &&
           (e_tmp.j < e[j].j || (e_tmp.j == e[j].j && e_tmp.k < e[j].k)))) {
        e[i] = e[j];
        i = j;
        j = (j << 1) + 1;
      }
...
static void
ON_Mesh_SetClosedHelper (bool bClosedOnly,
                         const ON_Mesh& mesh,
                         ...
                         const char& m_mesh_is_oriented)
{
  ...
  for (;;) {
    ...
    int* Vid = mesh.GetVertexLocationIds (
        1, (Vcount * sizeof (*Vid) <= sizeof (Vidbuffer) ? &Vidbuffer[0] : 0), 0);
    if (0 == Vid) {
    ...
    is_manifold = 1;
    i = -1;
    if (!bClosedOnly || 1 == is_closed)
      for (i = 0; i < E_count; /*empty iterator*/) {
        E = E_list[i];
        if (++i >= E_count) {
...
bool
ON_Mesh::SetVertex (int vertex_index, const ON_3dPoint& vertex_location)
{
  bool rc = false;
...
bool
ON_Mesh::SetVertex (int vertex_index, const ON_3fPoint& vertex_location)
{
  bool rc = false;
  int vertex_count = m_V.Count();
  if (vertex_index >= 0 && vertex_index <= vertex_count) {
...
bool
ON_Mesh::SetVertexNormal (int vertex_index, const ON_3dVector& normal)
{
  bool rc = false;
...
bool
ON_Mesh::SetVertexNormal (int vertex_index, const ON_3fVector& normal)
{
...
bool
ON_Mesh::SetTextureCoord (int vertex_index, double s, double t // texture coordinates
)
{
...
bool
ON_Mesh::SetTriangle (int face_index, int a, int b, int c // vertex indices
)
{
...
bool
ON_Mesh::SetQuad (int face_index, int a, int b, int c, int d // vertex indices
)
{
  bool rc = false;
...
int
ON_Mesh::FaceCount() const
{
  return m_F.Count();
}

int
ON_Mesh::QuadCount() const
{
  // number of faces that are quads
  if (m_quad_count < 0 || m_triangle_count < 0 || m_invalid_count < 0 ||
...
int
ON_Mesh::InvalidFaceCount() const
{
...
int
ON_Mesh::VertexCount() const
{
  return m_V.Count();
}

bool
ON_Mesh::HasVertexNormals() const
{
  const int vertex_count = VertexCount();
...
bool
ON_Mesh::HasTextureCoordinates() const
{
  const int vertex_count = VertexCount();
...
bool
ON_Mesh::HasCachedTextureCoordinates() const
{
  const int vertex_count = VertexCount();
  if (vertex_count > 0) {
...
ON_Mesh::CachedTextureCoordinates (const ON_UUID& mapping_id) const
{
  ...
  if (vertex_count > 0) {
    ...
    int tci, tccount = m_TC.Count();
    for (tci = 0; tci < tccount; tci++) {
      if (vertex_count == TC->m_T.Count() && mapping_id == TC->m_tag.m_mapping_id) {
        return TC;
      }
    }
  }
...
bool
ON_Mesh::HasSurfaceParameters() const
{
  const int vertex_count = VertexCount();
...
bool
ON_Mesh::HasPrincipalCurvatures() const
{
  const int vertex_count = VertexCount();
...
void
ON_Mesh::InvalidateBoundingBoxes()
{
  InvalidateVertexBoundingBox();
  InvalidateVertexNormalBoundingBox();
...
void
ON_Mesh::InvalidateVertexBoundingBox()
{
...
void
ON_Mesh::InvalidateVertexNormalBoundingBox()
{
...
void
ON_Mesh::InvalidateTextureCoordinateBoundingBox()
{
...
void
ON_Mesh::InvalidateCurvatureStats()
{
  int i;
  for (i = 0; i < 4; i++) {
    if (m_kstat[i]) {
...
bool
ON_Mesh::GetCurvatureStats ( // returns true if successful
...
int
ON_MeshTopology::WaitUntilReady (int) const
{
  return m_b32IsValid;
}

bool
ON_Mesh::TopologyExists() const
{
  return (1 == m_top.WaitUntilReady (0));
}

const ON_MeshTopology&
ON_Mesh::Topology() const
{
...
bool
ON_MeshFace::ComputeFaceNormal (const ON_3dPoint* dV, ON_3dVector& FN) const
{
  if (0 != dV) {
...
bool
ON_MeshFace::ComputeFaceNormal (const ON_3fPoint* fV, ON_3dVector& FN) const
{
  if (0 != fV) {
...
bool
ON_Mesh::CombineCoincidentVertices (
    const ON_3fVector /*tolerance*/,
    double // cos_normal_angle // = -1.0  // cosine(break angle) -1.0 will merge all
           // coincident vertices
)
{
...
static int
CompareMeshPoint (const void* a, const void* b, void* ptr)
{
  float d;
  const struct tagMESHPOINTS* mp = (const struct tagMESHPOINTS*)ptr;

  // use bogus pointer to convert a,b into vertex indices
  int i = (int)(((const char*)a) -
                mp->p0); // the (int) is for 64 bit std::size_t conversion
...
bool
ON_Mesh::CombineIdenticalVertices (bool bIgnoreVertexNormals,
...
void
ON_Mesh::Append (int mesh_count, const ON_Mesh* const* meshes)
{
  ...
  // Allow the appendage of VertexNormals, TextureCoordinates, PrincipalCurvatures to
  // empty meshes by checking for 0 == vcount0 && 0 == fcount0
  ...
  bool bHasDoubles =
      (0 == vcount0 || HasSynchronizedDoubleAndSinglePrecisionVertices());
...
void
ON_Mesh::Append (const ON_Mesh& m)
{
...
ON_MeshParameters::ON_MeshParameters() { Default(); }

ON_MeshParameters::~ON_MeshParameters() {}

double
ON_MeshParameters::MinEdgeLength (double max_edge_length, double tolerance)
{
...
double
ON_MeshParameters::Tolerance (double density, double actual_size)
{
  ...
  double tol = 0.0;
  double x, e;
  if (ON_IsValid (density) && ON_IsValid (actual_size) && density > 0.0 &&
      actual_size > 0.0) {
    if (density > 1.0)
      density = 1.0;
    ...
    e = (density < 0.5) ? 1.0 + density * (6.0 - 4.0 * density) // 1.0 + 4.0*density
                        : 2.0 + 2.0 * density;
    x = pow (10.0, -e);
...
static ON_MeshParameters
FastRenderMeshParameters ()
{
...
const ON_MeshParameters
    ON_MeshParameters::QualityRenderMesh (QualityRenderMeshParameters());

void
ON_MeshParameters::JaggedAndFasterMeshParameters()
{
  *this = ON_MeshParameters::FastRenderMesh;
}

void
ON_MeshParameters::SmoothAndSlowerMeshParameters()
{
  *this = ON_MeshParameters::QualityRenderMesh;
}

void
ON_MeshParameters::DefaultAnalysisMeshParameters()
{
  Default();
...
bool
ON_MeshParameters::operator== (const ON_MeshParameters& m2) const
{
...
bool
ON_MeshParameters::operator== (const ON_Mesh& mesh) const
{
...
void
ON_MeshParameters::Dump (ON_TextLog& text_log) const
{
  ...
  text_log.Print ("Density = %g (relative tolerance = %g)\n",
                  ...
                  ON_MeshParameters::Tolerance (m_relative_tolerance, 1.0));
...
void
ON_MeshParameters::Set (double density, double min_edge_length)
{
...
int
ON_MeshParameters::Compare (const ON_MeshParameters& src) const
{
...
static int
ON_MeshParameters_CompareDouble (double t0, double t1, double default_value)
{
...
static int
ON_MeshParameters_CompareInt (int i0, int i1)
{
  if (i0 <= 0)
    i0 = 0;
  if (i1 <= 0)
...
static int
ON_MeshParameters_CompareBool (bool b0, bool b1)
{
  const int i0 = b0 ? 1 : 0;
  const int i1 = b1 ? 1 : 0;
  return i1 - i0;
...
int
ON_MeshParameters::CompareGeometrySettings (const ON_MeshParameters& src) const
{
  int rc;
  ...
  rc = ON_MeshParameters_CompareDouble (
      m_relative_tolerance, src.m_relative_tolerance, 0.0);
  if (rc)
    return rc;
  ...
  rc = ON_MeshParameters_CompareDouble (
      m_grid_aspect_ratio, src.m_grid_aspect_ratio, 0.0);
  if (rc)
    return rc;
  ...
  rc = ON_MeshParameters_CompareDouble (
      m_grid_amplification, src.m_grid_amplification, 1.0);
  if (rc)
    return rc;
...
ON__UINT32
ON_MeshParameters::DataCRC (ON__UINT32 current_remainder) const
{
  ON__UINT32 crc =
      ON_CRC32 (current_remainder, sizeof (m_bComputeCurvature), &m_bComputeCurvature);
...
bool
ON_MeshParameters::Write (ON_BinaryArchive& file) const
{
  bool rc = file.Write3dmChunkVersion (1, 4);
  if (rc) {
    if (rc)
      rc = file.WriteInt (m_bComputeCurvature);
    if (rc)
      rc = file.WriteInt (m_bSimplePlanes);
    if (rc)
      rc = file.WriteInt (m_bRefine);
    if (rc)
      rc = file.WriteInt (m_bJaggedSeams);
    if (rc)
      rc = file.WriteInt (0); // obsolete m_bWeld field
    if (rc)
      rc = file.WriteDouble (m_tolerance);
    if (rc)
      rc = file.WriteDouble (m_min_edge_length);
    if (rc)
      rc = file.WriteDouble (m_max_edge_length);
    if (rc)
      rc = file.WriteDouble (m_grid_aspect_ratio);
    if (rc)
      rc = file.WriteInt (m_grid_min_count);
    if (rc)
      rc = file.WriteInt (m_grid_max_count);
    if (rc)
      rc = file.WriteDouble (m_grid_angle);
    if (rc)
      rc = file.WriteDouble (m_grid_amplification);
    if (rc)
      rc = file.WriteDouble (m_refine_angle);
    if (rc)
      rc = file.WriteDouble (5.0 * ON_PI / 180.0); // obsolete m_combine_angle field
    int mft = m_face_type;
    if (mft < 0 || mft > 2) {
    ...
    if (rc)
      rc = file.WriteInt (mft);

    // added for chunk version 1.1
    if (rc)
      rc = file.WriteInt (m_texture_range);

    // added for chunk version 1.2 - 14 October 2005
    if (rc)
      rc = file.WriteBool (m_bCustomSettings);
    if (rc)
      rc = file.WriteDouble (m_relative_tolerance);
    // DO NOT SAVE m_min_tolerance - yet ??

    // added for chunk version 1.3 - 20 February 2006
    if (rc)
      rc = file.WriteChar (m_mesher);

    // added for chunk version 1.4 - 3 March 2011
    if (rc)
      rc = file.WriteBool (m_bCustomSettingsEnabled);
  }
  return rc;
}

bool
ON_MeshParameters::Read (ON_BinaryArchive& file)
{
  Default();
  int major_version = 0;
  int minor_version = 0;
  ...
  if (rc && major_version == 1) {
    ...
    if (rc)
      rc = file.ReadInt (&obsolete_m_bWeld);

    if (rc)
      rc = file.ReadDouble (&m_tolerance);
    if (rc)
      rc = file.ReadDouble (&m_min_edge_length);
    if (rc)
      rc = file.ReadDouble (&m_max_edge_length);
    if (rc)
      rc = file.ReadDouble (&m_grid_aspect_ratio);
    if (rc)
      rc = file.ReadInt (&m_grid_min_count);
    if (rc)
      rc = file.ReadInt (&m_grid_max_count);
    if (rc)
      rc = file.ReadDouble (&m_grid_angle);
    if (rc)
      rc = file.ReadDouble (&m_grid_amplification);
    if (rc)
      rc = file.ReadDouble (&m_refine_angle);
    double obsolete_m_combine_angle;
    if (rc)
      rc = file.ReadDouble (&obsolete_m_combine_angle);
    if (rc)
      rc = file.ReadInt (&m_face_type);
    ...
    if (rc && minor_version >= 1) {
      ...
      if (rc && minor_version >= 2) {
        ...
        if (rc)
          rc = file.ReadDouble (&m_relative_tolerance);
...
void
ON_MeshCurvatureStats::EmergencyDestroy()
{
...
ON_MeshCurvatureStats::operator= (const ON_MeshCurvatureStats& src)
{
  if (this != &src) {
    m_style = src.m_style;
...
bool
ON_MeshCurvatureStats::Write (ON_BinaryArchive& file) const
{
  int i;
  ...
  if (rc) {
    ...
    if (rc)
      rc = file.WriteInt (i);
    if (rc)
      rc = file.WriteDouble (m_infinity);
    if (rc)
      rc = file.WriteInt (m_count_infinite);
    if (rc)
      rc = file.WriteInt (m_count);
    if (rc)
      rc = file.WriteDouble (m_mode);
    if (rc)
      rc = file.WriteDouble (m_average);
    if (rc)
      rc = file.WriteDouble (m_adev);
    if (rc)
      rc = file.WriteInterval (m_range);
  }
  return rc;
}

bool
ON_MeshCurvatureStats::Read (ON_BinaryArchive& file)
{
  int major_version = 0;
  int minor_version = 0;
  Destroy();
  ...
  if (rc && major_version == 1) {
    ...
    if (rc)
      rc = file.ReadInt (&i);
    if (rc)
      m_style = ON::CurvatureStyle (i);
    if (rc)
      rc = file.ReadDouble (&m_infinity);
    if (rc)
      rc = file.ReadInt (&m_count_infinite);
    if (rc)
      rc = file.ReadInt (&m_count);
    if (rc)
      rc = file.ReadDouble (&m_mode);
    if (rc)
      rc = file.ReadDouble (&m_average);
    if (rc)
      rc = file.ReadDouble (&m_adev);
    if (rc)
      rc = file.ReadInterval (m_range);
  }
  return rc;
}
...
bool
ON_Mesh::ConvertTrianglesToQuads (double angle_tol_radians,
    ...
    if (ei.length <= 0.0 || !ON_IsValid (ei.length)) {
      ...
      if (face_flag[ei.fi[0]] || face_flag[ei.fi[1]]) {
        ...
        for (ii = 0; ii < 3 && ei.flag; ii++) {
          for (jj = 0; jj < 3; jj++) {
            if (f0vi[ii] == f1vi[jj] && f0vi[(ii + 1) % 3] == f1vi[(jj + 2) % 3] &&
...
bool
ON_Mesh::NormalizeTextureCoordinates()
{
  ON_2fPoint t0; //, t1;
  int ti;
  ...
  if (rc) {
    ...
    if (!rc) {
      ...
      for (ti = 1; ti < vertex_count; ti++) {
        if (S[ti].x < udom.m_t[0])
          udom.m_t[0] = S[ti].x;
        else if (S[ti].x > udom.m_t[1])
          udom.m_t[1] = S[ti].x;
        if (S[ti].y < vdom.m_t[0])
          vdom.m_t[0] = S[ti].y;
        else if (S[ti].y > vdom.m_t[1])
          vdom.m_t[1] = S[ti].y;
      }
      rc = udom.IsIncreasing() && vdom.IsIncreasing();
    }
...
bool
ON_Mesh::HasPackedTextureRegion() const
{
  return (ON_IsValid (m_srf_scale[0]) && m_srf_scale[0] > 0.0 &&
          ON_IsValid (m_srf_scale[1]) && m_srf_scale[1] > 0.0 &&
          m_packed_tex_domain[0].IsInterval() && m_packed_tex_domain[1].IsInterval() &&
          0.0 <= m_packed_tex_domain[0].Min() && m_packed_tex_domain[0].Max() <= 1.0 &&
          0.0 <= m_packed_tex_domain[1].Min() && m_packed_tex_domain[1].Max() <= 1.0 &&
          (fabs (m_packed_tex_domain[0].Length()) < 1.0 ||
           fabs (m_packed_tex_domain[1].Length()) < 1.0));
}

bool
ON_Mesh::TransposeTextureCoordinates()
{
  if (!HasTextureCoordinates())
    return false;
...
bool
ON_Mesh::ReverseTextureCoordinates (int dir)
{
...
bool
ON_Mesh::ReverseSurfaceParameters (int dir)
{
...
bool
ON_Mesh::EvaluateMeshGeometry (const ON_Surface& srf)
{
  bool rc = false;
  const int vcount = VertexCount();
...
void
ON_Mesh::SetMeshParameters (const ON_MeshParameters& mp)
{
  DeleteMeshParameters();
...
const ON_MeshParameters*
ON_Mesh::MeshParameters() const
{
  return m_mesh_parameters;
}

void
ON_Mesh::DeleteMeshParameters()
{
  if (m_mesh_parameters) {
    delete m_mesh_parameters;
    m_mesh_parameters = 0;
...
static int
compare3fPoint (const ON_3fPoint* a, const ON_3fPoint* b)
{
  if (a->x < b->x)
    return -1;
  if (a->x > b->x)
...
static int
compare3dPoint (const ON_3dPoint* a, const ON_3dPoint* b)
{
  if (a->x < b->x)
    return -1;
  if (a->x > b->x)
  ...
  if (pt_count > 0 && (dV || fV)) {
    ...
    if (dV)
      ON_Sort (ON::quick_sort,
               ...
               pt_count,
               sizeof (*dV),
               (ON_COMPAR_LPVOID_LPVOID)compare3dPoint);
    else
      ON_Sort (ON::quick_sort,
               index,
               fV,
               pt_count,
               sizeof (*fV),
               (ON_COMPAR_LPVOID_LPVOID)compare3fPoint);
    ...
    if (dV) {
      for (vt0 = 0; vt0 < pt_count; vt0 = vt1, max_pt_index++) {
        dp0 = dV[index[vt0]];
        for (vt1 = vt0 + 1;
             vt1 < pt_count && 0 == compare3dPoint (&dp0, dV + index[vt1]);
             vt1++) {
          // empty
        }
        while (vt0 < vt1) {
      ...
      for (vt0 = 0; vt0 < pt_count; vt0 = vt1, max_pt_index++) {
        fp0 = fV[index[vt0]];
        for (vt1 = vt0 + 1;
             vt1 < pt_count && 0 == compare3fPoint (&fp0, fV + index[vt1]);
             vt1++) {
          // empty
        }
        while (vt0 < vt1) {
...
ON_Mesh::CullDegenerateFaces()
{
  ...
  if (fcount > 0) {
    ...
    if (topv_count > 0 && topv_map.Count() == m_V.Count()) {
      ...
      unsigned char* bBadFace =
          (unsigned char*)ws.GetMemory (fcount * sizeof (*bBadFace));
...
double
ON_SurfaceCurvature::MeanCurvature() const
{
  return 0.5 * (k1 + k2);
}

double
ON_SurfaceCurvature::MinimumRadius() const
{
  double k;
  k = (fabs (k1) >= fabs (k2)) ? fabs (k1)
                               : fabs (k2); // k = maximum directional curvature
  k = (k > 1.0e-300) ? 1.0 / k : 1.0e300;   // 1/k = minimum radius of curvature
  return k;
}

double
ON_SurfaceCurvature::MaximumRadius() const
{
  double k;
  if (k1 * k2 <= 0.0) {
...
// double ON_SurfaceCurvature::NormalSectionCurvature( const ON_3dVector&
// section_normal, const ON_3dVector& surface_normal ) const
//{
//   ON_3dVector tangent = ON_CrossProduct( section_normal, surface_normal );
//   if ( fabs(tangent.x) <= ON_SQRT_EPSILON && fabs(tangent.y) <= ON_SQRT_EPSILON &&
//   fabs(tangent.z) <= ON_SQRT_EPSILON )
//     tangent.Zero();
//   else
//     tangent.Unitize();
...
ON_MeshTopology::ON_MeshTopology() : m_mesh (0), m_memchunk (0), m_b32IsValid (0) {}

ON_MeshTopology::~ON_MeshTopology() { Destroy(); }

void
ON_MeshTopology::Destroy()
{
  m_topv_map.Destroy();
  m_topv.Destroy();
  m_tope.Destroy();
...
void
ON_MeshTopology::EmergencyDestroy()
{
  m_mesh = 0;
...
int
ON_MeshTopology::TopVertexCount() const
{
  return m_topv.Count();
}
...
int
ON_MeshTopology::TopEdgeCount() const
{
  return m_tope.Count();
}
...
int
ON_MeshTopology::TopFaceCount() const
{
  return m_topf.Count();
}

ON_3fPoint
ON_MeshTopology::TopVertexPoint (int vtopi) const
{
  return m_mesh->m_V[m_topv[vtopi].m_vi[0]];
}

ON_Line
ON_MeshTopology::TopEdgeLine (int tope_index) const
{
  ...
  if (m_mesh && tope_index >= 0 && tope_index < m_tope.Count()) {
    ...
    if (topvi[0] >= 0 && topvi[0] < m_topv.Count() && topvi[1] >= 0 &&
        topvi[1] < m_topv.Count()) {
...
int
ON_MeshTopology::TopEdge (int vtopi0, int vtopi1) const
{
  int i0, i1, ei, vi0;
  if (vtopi0 > vtopi1) {
    vi0 = vtopi0;
...
bool
ON_MeshTopology::GetTopFaceVertices (int fi, int topvi[4]) const
{
...
bool
ON_MeshTopology::SortVertexEdges() const
{
  bool rc = true;
...
static void
ON_ReverseIntArray (int* e,         // array of ints
...
bool
ON_MeshTopology::SortVertexEdges (int topvi) const
{
    ...
    for (vei = 0; vei < topv.m_tope_count; vei++) {
      ...
      if (topei >= 0 && topei < m_tope.Count()) {
        ...
        if (tope.m_topvi[0] == topvi || tope.m_topvi[1] == topvi) {
          ...
          if (efcnt < 0) {
            ON_ERROR ("ON_MeshTopology::SortVertexEdges(int topvi) - "
                      "m_tope[topei].m_topf_count < 0");
            return false;
          }
          switch (efcnt) {
  ...
  while (vei < topv.m_tope_count) {
    if (next_topei >= 0) {
      ...
      ON_ERROR ("ON_MeshTopology::SortVertexEdges() input vertex had bogus topology "
                "information.");
      return false;
    }
...
int*
ON_MeshTopology::GetIntArray (int length)
{
  int* a = 0;
  if (length > 0) {
    struct memchunk* pm;
...
bool
ON_MeshTopologyFace::IsTriangle() const
{
  return (m_topei[2] == m_topei[3] && m_topei[0] != m_topei[1]) ? true : false;
}

bool
ON_MeshTopologyFace::IsQuad() const
{
  return (m_topei[2] != m_topei[3]) ? true : false;
}

bool
ON_MeshTopologyFace::IsValid() const
{
  return (m_topei[0] != m_topei[1]) ? true : false;
}

bool
ON_MeshTopology::IsValid() const
{
  ON_Workspace ws;
  ...
  for (fi = 0; fi < f_count; fi++) {
    ...
    if (topf.m_topei[0] != 0 || topf.m_topei[1] != 0 || topf.m_topei[2] != 0 ||
        topf.m_topei[3] != 0) {
...
void
ON_MeshTopology::Dump (ON_TextLog& dump) const
{
  ON_3fPoint p;
  int vi, ei, fi, j;
...
ON_MeshTopology::Create()
{
  ...
  while (0 == b32IsValid || -1 == b32IsValid) {
...

    {
      ...
      for (vt0 = 0; vt0 < vcount; vt0 = vt1) {
        ...
        if (topvi != m_topv_map[vindex[vt0]]) {
          ...
          ON_ERROR ("ON_MeshTopology::Create() - topvi != vertex id from "
    ...
    if (topv_count >= 2) {
      ...
      if (fcount > 0 && vcount > 0) {
        // When working on this code be sure to test bug# 9271 and 9254 and file
        // fsv_r4.3dm
        ON_Workspace ws;
        struct ON_MeshFaceSide* e =
            (struct ON_MeshFaceSide*)ws.GetMemory (4 * fcount * sizeof (*e));
        ...
        if (ecnt > 0) {
          ...
          for (ei = 0; ei < ecnt; ei++) {
            ...
            for (efi = 0; efi < tope.m_topf_count; efi++) {
              ...
              // we have topf.m_topei[j] BEGIN at ON_MeshFace.vi[(j+3)%4] and END at
              // ON_MeshFace.vi[j]
              fi = tope.m_topfi[efi];
              f = m_mesh->m_F[fi];
          ...
          for (fi = 0; fi < fcount; fi++) {
            ...
            if (f.m_topei[0] >= 0 && f.m_topei[1] >= 0 && f.m_topei[2] >= 0 &&
                f.m_topei[0] != f.m_topei[1] && f.m_topei[1] != f.m_topei[2] &&
                f.m_topei[2] != f.m_topei[0]) {
              if (m_mesh->m_F[fi].IsTriangle()) {
                bIsGood = true;
                f.m_topei[3] = f.m_topei[2];
              }
              else if (f.m_topei[3] >= 0 && f.m_topei[0] != f.m_topei[3] &&
                       f.m_topei[1] != f.m_topei[3] && f.m_topei[2] != f.m_topei[3]) {
                bIsGood = true;
              }
            }
            if (!bIsGood) {
...
static int
compare_pmark (const int* a, const int* b)
{
  if (*a < *b)
    return -1;
  if (*a > *b)
...
static int
compare_vmap (const void* a, const void* b)
{
  int i = *((int*)a);
  int j = *((int*)b);
  if (i < j)
...
static int
AddToPartition (
    ON_Mesh* mesh, ON_SimpleArray<int>& pmark, int vi, int partition_mark, int fi0)
{
  bool b = true;
...
bool
ON_MeshPartition_IsValid (const ON_MeshPartition& p, const ON_Mesh& mesh)
{
  bool rc = false;
  const int* fvi;
...
static bool
ON_Mesh_CreatePartition_SortFaces (const ON_Mesh& mesh, int* fmap)
{
  ON_RTree rt;
...
const ON_MeshPartition*
ON_Mesh::CreatePartition (
...
)
{
  ...
  if (!m_partition) {
    ...
    if (vertex_count <= partition_max_vertex_count &&
      ...
      //     fvi = m_F[fi].vi;
      //     if ( fvi[2] == fvi[3] ) {
      //       fcenter = 0.333333333333333333f*(m_T[fvi[0]] + m_T[fvi[1]] +
      //       m_T[fvi[2]]);
      //     }
      //     else {
      ...
      //     fvi = m_F[fi].vi;
      //     if ( fvi[2] == fvi[3] ) {
      //       fcenter = 0.333333333333333333f*(m_V[fvi[0]] + m_V[fvi[1]] +
      //       m_V[fvi[2]]);
      //     }
      //     else {
      ...
      fi1 = 0;
      fi0 = 0;
      for (partition_mark = 3, fi0 = 0; fi0 < face_count;
           fi0 = fi1, partition_mark += 2) {
        partition_vertex_count = 0;
        partition_triangle_count = 0;
        for (fi1 = fi0; fi1 < face_count &&
      ...
      ON_Sort (
          ON::quick_sort, pamv, vmap, vertex_count, sizeof (vmap[0]), compare_vmap);
      for (fi = 0; fi < face_count; fi++) {
        fvi = m_F[fi].vi;
        fvi[0] = pamv[fvi[0]];
        fvi[1] = pamv[fvi[1]];
      ...
      int vi0, vi2, vi3;
      for (vi2 = 0; vi2 < vertex_count && pmark[vi2] < 2; vi2++) { /*empty for body*/
      }
      vi3 = vi2;
      for (pi = 0; pi < partition_count; pi++) {
        vi0 = vi2;
        m = 2 * pi + 4;
        for (vi2 = vi3; vi2 < vertex_count && pmark[vi2] < m;
             vi2++) { /*empty for body*/
        }
        for (vi3 = vi2; vi3 < vertex_count && pmark[vi3] <= m;
             vi3++) { /*empty for body*/
        }
...
const ON_MeshPartition*
ON_Mesh::Partition() const
{
  return m_partition;
}

void
ON_Mesh::DestroyPartition()
{
  if (m_partition) {
    delete m_partition;
    m_partition = 0;
...
ON_MeshPartition::~ON_MeshPartition() { m_part.Destroy(); }
...
ON_Mesh::MeshPart (const ON_MeshPart& mesh_part, ON_Mesh* mesh) const
{
  ...
  if (mesh_part.fi[0] < 0 || mesh_part.fi[1] > m_F.Count() ||
      mesh_part.fi[0] > mesh_part.fi[1]) {
    ON_ERROR ("ON_Mesh::MeshPart mesh_part.fi[] is not valid");
    return 0;
  }

  if (mesh_part.vi[0] < 0 || mesh_part.vi[1] > m_V.Count() ||
  ...
  for (int fi = fi0; fi < fi1; fi++) {
    ...
    f.vi[2] -= vi0;
    f.vi[3] -= vi0;
    if (f.vi[0] >= submesh_V_count || f.vi[0] < 0 || f.vi[1] >= submesh_V_count ||
        f.vi[1] < 0 || f.vi[2] >= submesh_V_count || f.vi[2] < 0 ||
...
ON_Mesh::DuplicateFace (int face_index, ON_Mesh* mesh) const
{
  ...
  const ON_3dPoint* dV =
      (HasDoublePrecisionVertices() && DoublePrecisionVerticesAreValid())
          ? DoublePrecisionVertices().Array()
          : 0;
  const ON_3fPoint* fV = (0 == dV) ? m_V.Array() : 0;
...
ON_MeshVertexRef&
ON_MeshVertexRef::operator= (const ON_MeshVertexRef& src)
{
  if (this != &src) {
    ON_Geometry::operator= (src);
    m_mesh = src.m_mesh;
...
ON_BOOL32
ON_MeshVertexRef::IsValid (ON_TextLog* text_log) const
{
  if (0 == m_mesh) {
    if (0 != text_log) {
...
void
ON_MeshVertexRef::Dump (ON_TextLog& text_log) const
{
  text_log.Print (
      "m_mesh=%08x m_mesh_vi=%d m_top_vi=%d\n", m_mesh, m_mesh_vi, m_top_vi);
  ON_3dPoint v = Point();
  if (v.IsValid()) {
    text_log.PushIndent();
...
ON::object_type
ON_MeshVertexRef::ObjectType() const
{
...
int
ON_MeshVertexRef::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_MeshVertexRef::GetBBox (double* boxmin, double* boxmax, ON_BOOL32 bGrowBox) const
{
  bool rc = false;
  ON_3dPoint v = Point();
  if (v.IsValid()) {
    rc = ON_GetPointListBoundingBox (
        3, 0, 1, 3, &v.x, boxmin, boxmax, bGrowBox ? true : false);
  }
  return rc;
}

ON_BOOL32
ON_MeshVertexRef::Transform (const ON_Xform&) { return false; }

const ON_MeshTopology*
ON_MeshVertexRef::MeshTopology() const
{
  return (0 != m_mesh) ? &m_mesh->m_top : 0;
}

ON_3dPoint
ON_MeshVertexRef::Point() const
{
...
ON_MeshVertexRef::MeshTopologyVertex() const
{
...
ON_MeshEdgeRef&
ON_MeshEdgeRef::operator= (const ON_MeshEdgeRef& src)
{
  if (this != &src) {
    ON_Geometry::operator= (src);
    m_mesh = src.m_mesh;
...
ON_BOOL32
ON_MeshEdgeRef::IsValid (ON_TextLog* text_log) const
{
  if (0 == m_mesh) {
    if (0 != text_log) {
      text_log->Print ("m_mesh = NULL\n");
...
void
ON_MeshEdgeRef::Dump (ON_TextLog& text_log) const
{
...
ON::object_type
ON_MeshEdgeRef::ObjectType() const
{
...
// overrides of virtual ON_Geometry functions
int
ON_MeshEdgeRef::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_MeshEdgeRef::GetBBox (double* boxmin, double* boxmax, ON_BOOL32 bGrowBox) const
{
  bool rc = false;
  ON_Line line = Line();
  if (line.from.IsValid() && line.to.IsValid()) {
    rc = ON_GetPointListBoundingBox (
        3, 0, 2, 3, &line.from.x, boxmin, boxmax, bGrowBox ? true : false);
  }
  return rc;
}

ON_BOOL32
ON_MeshEdgeRef::Transform (const ON_Xform&) { return false; }

const ON_MeshTopology*
ON_MeshEdgeRef::MeshTopology() const
{
  return (0 != m_mesh) ? &m_mesh->m_top : 0;
}

ON_Line
ON_MeshEdgeRef::Line() const
{
...
const ON_MeshTopologyEdge*
ON_MeshEdgeRef::MeshTopologyEdge() const
{
...
ON_MeshFaceRef&
ON_MeshFaceRef::operator= (const ON_MeshFaceRef& src)
{
  if (this != &src) {
    ON_Geometry::operator= (src);
    m_mesh = src.m_mesh;
...
ON_BOOL32
ON_MeshFaceRef::IsValid (ON_TextLog* text_log) const
{
  if (0 == m_mesh) {
    if (0 != text_log) {
      text_log->Print ("m_mesh = NULL\n");
...
void
ON_MeshFaceRef::Dump (ON_TextLog& text_log) const
{
  text_log.Print ("m_mesh=%08x, m_mesh_fi=%d\n", m_mesh, m_mesh_fi);
}

unsigned int
ON_MeshFaceRef::SizeOf() const
{
  unsigned int sz = sizeof (*this) - sizeof (ON_Geometry);
  sz += ON_Geometry::SizeOf();
  return sz;
...
ON::object_type
ON_MeshFaceRef::ObjectType() const
{
...
// overrides of virtual ON_Geometry functions
int
ON_MeshFaceRef::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_MeshFaceRef::GetBBox (double* boxmin, double* boxmax, ON_BOOL32 bGrowBox) const
{
  bool rc = false;
  if (0 != m_mesh && m_mesh_fi >= 0 && m_mesh_fi < m_mesh->m_F.Count()) {
    ...
    rc = ON_GetPointListBoundingBox (
        3, 0, 4, 3, &v[0].x, boxmin, boxmax, bGrowBox ? true : false);
  }
  return rc;
}

ON_BOOL32
ON_MeshFaceRef::Transform (const ON_Xform&) { return false; }

const ON_MeshTopology*
ON_MeshFaceRef::MeshTopology() const
{
  return (0 != m_mesh) ? &m_mesh->m_top : 0;
}

const ON_MeshFace*
ON_MeshFaceRef::MeshFace() const
{
...
const ON_MeshTopologyFace*
ON_MeshFaceRef::MeshTopologyFace() const
{
...
ON_MeshVertexRef
ON_Mesh::VertexRef (int mesh_V_index) const
{
  ON_MeshVertexRef vr;
...
ON_MeshVertexRef
ON_Mesh::VertexRef (ON_COMPONENT_INDEX ci) const
{
  ON_MeshVertexRef vr;
...
ON_MeshEdgeRef
ON_Mesh::EdgeRef (int tope_index) const
{
  return m_top.EdgeRef (tope_index);
}

ON_MeshEdgeRef
ON_Mesh::EdgeRef (ON_COMPONENT_INDEX ci) const
{
  return m_top.EdgeRef (ci);
}

ON_MeshFaceRef
ON_Mesh::FaceRef (int mesh_F_index) const
{
  ON_MeshFaceRef fr;
...
ON_MeshFaceRef
ON_Mesh::FaceRef (ON_COMPONENT_INDEX ci) const
{
  ON_MeshFaceRef fr;
...
ON_MeshVertexRef
ON_MeshTopology::VertexRef (ON_COMPONENT_INDEX ci) const
{
  ON_MeshVertexRef vr;
  if (ci.m_index >= 0) {
    switch (ci.m_type) {
...
ON_MeshVertexRef
ON_MeshTopology::VertexRef (int topv_index) const
{
  ON_MeshVertexRef vr;
...
ON_MeshEdgeRef
ON_MeshTopology::EdgeRef (ON_COMPONENT_INDEX ci) const
{
  ON_MeshEdgeRef er;
  if (ON_COMPONENT_INDEX::meshtop_edge == ci.m_type && ci.m_index >= 0 &&
      ci.m_index < m_tope.Count()) {
    er.m_mesh = m_mesh;
...
ON_MeshEdgeRef
ON_MeshTopology::EdgeRef (int tope_index) const
{
  ON_MeshEdgeRef er;
...
ON_MeshFaceRef
ON_MeshTopology::FaceRef (ON_COMPONENT_INDEX ci) const
{
  ON_MeshFaceRef fr;
  if (m_mesh) {
...
ON_MeshFaceRef
ON_MeshTopology::FaceRef (int topf_index) const
{
...
ON_COMPONENT_INDEX
ON_MeshVertexRef::ComponentIndex() const
{
...
ON_COMPONENT_INDEX
ON_MeshEdgeRef::ComponentIndex() const
{
  ON_COMPONENT_INDEX ci (ON_COMPONENT_INDEX::invalid_type, -1);
  if (m_top_ei >= 0) {
...
ON_COMPONENT_INDEX
ON_MeshFaceRef::ComponentIndex() const
{
  ON_COMPONENT_INDEX ci (ON_COMPONENT_INDEX::invalid_type, -1);
  if (m_mesh_fi >= 0) {
...
ON_Geometry*
ON_Mesh::MeshComponent (ON_COMPONENT_INDEX ci) const
{
...
ON_3dVector
ON_TriangleNormal (const ON_3dPoint& A, const ON_3dPoint& B, const ON_3dPoint& C)
{
...
const bool*
ON_Mesh::HiddenVertexArray() const
{
  return ((m_hidden_count > 0 && m_H.Count() == m_V.Count()) ? m_H.Array() : 0);
}

void
ON_Mesh::DestroyHiddenVertexArray()
{
  m_H.Destroy();
  m_hidden_count = 0;
}
...
int
ON_Mesh::HiddenVertexCount() const
{
  return ((m_H.Count() == m_V.Count()) ? m_hidden_count : 0);
}

void
ON_Mesh::SetVertexHiddenFlag (int meshvi, bool bHidden)
{
  const int vcount = m_V.Count();
...
bool
ON_Mesh::VertexIsHidden (int meshvi) const
{
  const int vcount = m_V.Count();
  return (
      (m_hidden_count > 0 && meshvi >= 0 && meshvi < vcount && vcount == m_H.Count())
          ? m_H[meshvi]
          : false);
}

bool
ON_Mesh::FaceIsHidden (int meshfi) const
{
  const bool* bHiddenVertex = HiddenVertexArray();
  if (bHiddenVertex && meshfi >= 0 && meshfi < m_F.Count()) {
    ON_MeshFace f = m_F[meshfi];
    if (bHiddenVertex[f.vi[0]] || bHiddenVertex[f.vi[1]] || bHiddenVertex[f.vi[2]] ||
        bHiddenVertex[f.vi[3]])
      return true;
  }
  return false;
}
...
bool
ON_MeshTopology::TopVertexIsHidden (int topvi) const
{
...
bool
ON_MeshTopology::TopEdgeIsHidden (int topei) const
{
...
bool
ON_MeshTopology::TopFaceIsHidden (int topfi) const
{
  // topology and mesh face indices are the same
  return m_mesh ? m_mesh->FaceIsHidden (topfi) : false;
}

ON_MappingTag::ON_MappingTag() { Default(); }

void
ON_MappingTag::Dump (ON_TextLog& text_log) const
{
...
void
ON_MappingTag::Transform (const ON_Xform& xform)
{
...
void
ON_MappingTag::SetDefaultSurfaceParameterMappingTag()
{
...
void
ON_MappingTag::Set (const ON_TextureMapping& mapping)
{
  Default();
...
bool
ON_MappingTag::IsSet() const
{
  return (0 != m_mapping_crc || !ON_UuidIsNil (m_mapping_id));
}

bool
ON_MappingTag::IsDefaultSurfaceParameterMapping() const
{
...
int
ON_MappingTag::Compare (const ON_MappingTag& other,
                        bool bCompareId,
                        bool bCompareCRC,
                        bool bCompareXform) const
{
  int rc = 0;
  if (!rc && bCompareId) {
...
bool
ON_MappingTag::Write (ON_BinaryArchive& archive) const
{
...
bool
ON_MappingTag::Read (ON_BinaryArchive& archive)
{
  Default();
  int mjv = 0, mnv = 0;
...
ON_TextureCoordinates::ON_TextureCoordinates() { m_dim = 0; }
...
ON_MeshDoubleVertices*
ON_MeshDoubleVertices::Get (const ON_Mesh* mesh)
{
  return ON_MeshDoubleVertices::Cast (mesh->GetUserData (
      ON_MeshDoubleVertices::m_ON_MeshDoubleVertices_class_id.Uuid()));
}

ON_MeshDoubleVertices*
ON_MeshDoubleVertices::Attach (const ON_Mesh* mesh)
{
...
: ON_UserData(), m_fcount (0), m_dcount (0), m_fCRC (0), m_dCRC (0)
{
...
ON_BOOL32
ON_MeshDoubleVertices::IsValid (ON_TextLog*) const { return true; }

void
ON_MeshDoubleVertices::Dump (ON_TextLog& text_log) const
{
...
unsigned int
ON_MeshDoubleVertices::SizeOf() const
{
  return m_dV.SizeOfArray() + ON_UserData::SizeOf();
}

ON__UINT32
ON_MeshDoubleVertices::DataCRC (ON__UINT32 current_remainder) const
{
...
ON_BOOL32
ON_MeshDoubleVertices::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_MeshDoubleVertices::Read (ON_BinaryArchive& file)
{
  m_fcount = 0;
  m_dcount = 0;
  m_fCRC = 0;
...
// virtual ON_UserData overrides
ON_BOOL32
ON_MeshDoubleVertices::GetDescription (ON_wString& description)
{
...
ON_BOOL32
ON_MeshDoubleVertices::Archive() const
{
...
ON_BOOL32
ON_MeshDoubleVertices::Transform (const ON_Xform& xform)
{
  if (!xform.IsIdentity()) {
...
bool
ON_Mesh::HasSynchronizedDoubleAndSinglePrecisionVertices() const
{
  ON_3fPoint P;
...
bool
ON_Mesh::HasDoublePrecisionVertices() const
{
  return (0 != ON_MeshDoubleVertices::Get (this));
}

void
ON_Mesh::DestroyDoublePrecisionVertices()
{
...
void
ON_Mesh::EnableDoublePrecisionVertices (bool bEnableDoublePrecisionVertices)
{
  ON_MeshDoubleVertices* dv = ON_MeshDoubleVertices::Get (this);
...
ON__UINT32
ON_MeshDoubleVertices::FloatCRC (const ON_3fPointArray& V) { return V.DataCRC (0); }

ON__UINT32
ON_MeshDoubleVertices::DoubleCRC() const { return m_dV.DataCRC (0); }

void
ON_Mesh::UpdateSinglePrecisionVertices()
{
  ON_MeshDoubleVertices* dv = ON_MeshDoubleVertices::Get (this);
  if (0 == dv)
    return;
...
void
ON_Mesh::SetSinglePrecisionVerticesAsValid()
{
  ON_MeshDoubleVertices* dv = ON_MeshDoubleVertices::Get (this);
  if (0 == dv)
    return;
...
void
ON_Mesh::SetDoublePrecisionVerticesAsValid()
{
  ON_MeshDoubleVertices* dv = ON_MeshDoubleVertices::Get (this);
  if (0 == dv)
    return;
...
bool
ON_Mesh::SinglePrecisionVerticesAreValid() const
{
  const ON_MeshDoubleVertices* dv = ON_MeshDoubleVertices::Get (this);
  if (0 == dv)
    return true;
  return (dv->m_fcount == m_V.Count() &&
          dv->m_fCRC == ON_MeshDoubleVertices::FloatCRC (m_V));
}

bool
ON_Mesh::DoublePrecisionVerticesAreValid() const
{
  const ON_MeshDoubleVertices* dv = ON_MeshDoubleVertices::Get (this);
  if (0 == dv)
    return true;
...
ON_3dPoint
ON_Mesh::Vertex (int vertex_index) const
{
...
ON_Mesh::DoublePrecisionVertices() const
{
...
const ON_3fPointArray&
ON_Mesh::SinglePrecisionVertices() const
{
  ON_Mesh& mesh = const_cast<ON_Mesh&> (*this);
...
class /*NEVER EXPORT THIS CLASS DEFINITION*/ ON_PerObjectMeshParameters
: public ON_UserData {
...
public:
  ...
  FindOrCreate (const ON_Object*, bool bCreate);
...
ON_PerObjectMeshParameters*
ON_PerObjectMeshParameters::FindOrCreate (const ON_Object* object, bool bCreate)
{
  if (0 == object)
    return 0;
  ON_PerObjectMeshParameters* ud =
      ON_PerObjectMeshParameters::Cast (object->GetUserData (
          ON_PerObjectMeshParameters::m_ON_PerObjectMeshParameters_class_id.Uuid()));
  if (!ud && bCreate) {
...
// virtual ON_Object override
ON_BOOL32
ON_PerObjectMeshParameters::IsValid (ON_TextLog*) const { return true; }

// virtual ON_Object override
unsigned int
ON_PerObjectMeshParameters::SizeOf() const
{
...
// virtual ON_Object override
ON__UINT32
ON_PerObjectMeshParameters::DataCRC (ON__UINT32 current_remainder) const
{
...
// virtual ON_Object override
ON_BOOL32
ON_PerObjectMeshParameters::Write (ON_BinaryArchive& binary_archive) const
{
...
// virtual ON_Object override
ON_BOOL32
ON_PerObjectMeshParameters::Read (ON_BinaryArchive& binary_archive)
{
  m_mp = ON_MeshParameters::FastRenderMesh;
  m_mp.m_bCustomSettings = true;
  m_mp.m_bComputeCurvature = false;
  ...
  int major_version = 0;
  int minor_version = 0;
  if (!binary_archive.BeginRead3dmChunk (
...
// virtual ON_UserData override
ON_BOOL32
ON_PerObjectMeshParameters::Archive() const { return true; }

// virtual ON_UserData override
ON_BOOL32
ON_PerObjectMeshParameters::GetDescription (ON_wString& description)
{
...
const ON_MeshParameters*
ON_3dmObjectAttributes::CustomRenderMeshParameters() const
{
  const ON_PerObjectMeshParameters* ud =
      ON_PerObjectMeshParameters::FindOrCreate (this, false);
  return (0 != ud) ? &ud->m_mp : 0;
}

bool
ON_3dmObjectAttributes::SetCustomRenderMeshParameters (const ON_MeshParameters& mp)
{
  ON_PerObjectMeshParameters* ud =
      ON_PerObjectMeshParameters::FindOrCreate (this, true);
  if (0 == ud)
    return false;
  ud->m_mp = mp;
...
void
ON_3dmObjectAttributes::DeleteCustomRenderMeshParameters()
{
  ON_PerObjectMeshParameters* ud =
      ON_PerObjectMeshParameters::FindOrCreate (this, false);
  if (0 != ud)
...
bool
ON_3dmObjectAttributes::EnableCustomRenderMeshParameters (bool bEnable)
{
  ON_PerObjectMeshParameters* ud =
      ON_PerObjectMeshParameters::FindOrCreate (this, false);
  if (0 != ud)
...
void
ON_Mesh::DestroyTree (bool)
{}

int*
ON_Mesh_GetVidHelper (const int Vcount,
                      const ON_3fPoint* fV,
                      const ON_3dPoint* dV,
                      int first_vid,
                      int* Vid,
                      int* Vindex);
static int
comparefV (const void* a, const void* b)
{
...
static int
comparedV (const void* a, const void* b)
{
...
int*
ON_Mesh::GetVertexLocationIds (int first_vid, int* Vid, int* Vindex) const
{
  const ON_3dPoint* dV = Mesh_dV (*this);
  const ON_3fPoint* fV = (0 == dV) ? m_V.Array() : 0;
...
int
ON_Mesh_GetEidHelper (const int Vcount,
                      const int Fcount,
                      const ON_MeshFace* F,
                      const int* Vid,
...
int
ON_Mesh::GetMeshFaceSideList (const int* Vid, struct ON_MeshFaceSide*& sides) const
{
  const int Vcount = m_V.Count();
  const int Fcount = m_F.Count();
  const ON_MeshFace* F = m_F.Array();
...
static int
ON_SORT_TEMPLATE_COMPARE (ON_SORT_TEMPLATE_TYPE const* side1,
...
void
ON_SortMeshFaceSidesByVertexIndex (int sides_count, struct ON_MeshFaceSide* sides)
{
