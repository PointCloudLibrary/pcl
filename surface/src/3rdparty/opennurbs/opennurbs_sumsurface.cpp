...
ON_SumSurface::~ON_SumSurface() { Destroy(); }

void
ON_SumSurface::Destroy()
{
  // GBA 24-Sept-2004. Since there now could be a surface tree it need to be cleared
  // out.
  DestroyRuntimeCache();
  for (int i = 0; i < 2; i++) {
    if (m_curve[i]) {
...
void
ON_SumSurface::EmergencyDestroy()
{
  m_curve[0] = 0;
  m_curve[1] = 0;
}
...
void
ON_SumSurface::ClearBoundingBox()
{
  m_bbox.Destroy();
}
...
bool
ON_SumSurface::IsDeformable() const
{
  bool rc = true;
  if (m_curve[0])
...
int
ON_SumTensor::DimensionA() const
{
  return dim;
}

int
ON_SumTensor::DimensionB() const
{
  return dim;
}

int
ON_SumTensor::DimensionC() const
{
  return dim;
}
...
int
ON_SumSurface::HasNurbForm() const
