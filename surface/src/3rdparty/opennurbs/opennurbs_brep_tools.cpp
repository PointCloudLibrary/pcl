...
static const ON_BrepEdge*
FindLinearEdge (const ON_Brep& brep, int vi0, int vi1)
{
...
static void
SynchFaceOrientation (ON_Brep& brep, int fi)
{
...
bool
ON_BrepFace::SetMesh(ON::mesh_type mt, ON_Mesh* mesh)
{
...
bool
ON_Brep::SetTrimBoundingBoxes(ON_BOOL32 bLazy)
{
  bool rc = true;
...
bool
ON_Brep::SetTrimBoundingBoxes(ON_BrepFace& face, ON_BOOL32 bLazy)
{
  bool rc = true;
...
bool
ON_Brep::SetTrimBoundingBoxes(ON_BrepLoop& loop, ON_BOOL32 bLazy)
{
...
bool
ON_Brep::SetTrimBoundingBox(ON_BrepTrim& trim, ON_BOOL32 bLazy)
{
...
static bool
CheckForMatchingVertexIndices (int i, int j, int corner_vi[4])
{
...
ON_BrepFace*
ON_Brep::NewFace(ON_Surface* pSurface, int vid[4], int eid[4], ON_BOOL32 bRev3d[4])
{
  m_bbox.Destroy();
  m_is_solid = 0;
...
ON_BrepLoop*
ON_Brep::NewOuterLoop(int face_index, int vid[4], int eid[4], ON_BOOL32 bRev3d[4])
{
  m_is_solid = 0;
  ...
  for (i = 0; i < 4; i++) {
    if (eid[i] != -1) {
      ...
      if (vi0 < 0 || vi1 < 0) {
        ON_ERROR(
            "ON_Brep::NewFace(ON_Surface*,...) error: Bad edge vertex informtion.");
        return 0;
      }
      if (vid[i] == -1)
        vid[i] = vi0;
      else if (vid[i] != vi0) {
        ON_ERROR("ON_Brep::NewFace(ON_Surface*,...) error: Edge and vertex informtion "
                 "do not match.");
        return 0;
      }
      if (vid[(i + 1) % 4] == -1)
  ...
  // if surface has singularities or is closed, make sure vertex and edge information is
  // correct
  for (i = 0; i < 4; i++) {
    if (bIsSingular[i]) {
      if (eid[i] != -1 || bRev3d[i]) {
...
ON_Brep*
ON_BrepBox (const ON_3dPoint* box_corners, ON_Brep* pBrep)
{
  ON_Brep* brep = 0;
  int vi, ei, fi, si, c2i;
  if (box_corners) {
    ...
    */
    ...
    struct {
      int e[4], bRev[4];
    } f[6] = {{{0, 9, 4, 8}, {false, false, true, true}},
    ...
    for (fi = 0; fi < 6; fi++) {
      ...
      si = brep->AddSurface(
          ON_NurbsSurfaceQuadrilateral(v0.point, v1.point, v2.point, v3.point));
      ...
      trim0.m_type =
          (trim0.m_vi[0] != trim0.m_vi[1]) ? ON_BrepTrim::mated : ON_BrepTrim::singular;
      ...
      trim1.m_type =
          (trim1.m_vi[0] != trim1.m_vi[1]) ? ON_BrepTrim::mated : ON_BrepTrim::singular;
      ...
      trim2.m_type =
          (trim2.m_vi[0] != trim2.m_vi[1]) ? ON_BrepTrim::mated : ON_BrepTrim::singular;
      ...
      trim3.m_type =
          (trim3.m_vi[0] != trim3.m_vi[1]) ? ON_BrepTrim::mated : ON_BrepTrim::singular;
...
ON_Brep*
ON_BrepWedge (const ON_3dPoint* corners, ON_Brep* pBrep)
{
  ON_Brep* brep = 0;
    ...
    */
    ...
    for (fi = 0; fi < 5; fi++) {
      ...
      if (f[fi].e[3] >= 0)
        ...
        // 3 sided face
        srf = ON_NurbsSurfaceQuadrilateral(
            v0->point, v1->point, v1->point + (v2->point - v0->point), v2->point);
...
ON_Brep*
ON_BrepSphere (const ON_Sphere& sphere, ON_Brep* pBrep)
{
...
ON_Brep*
ON_BrepTorus (const ON_Torus& torus, ON_Brep* pBrep)
{
...
ON_Brep*
ON_BrepCone (const ON_Cone& cone, ON_BOOL32 bCapBase, ON_Brep* pBrep)
{
  ON_BOOL32 bArcLengthParameterization = true;
  ON_Brep* brep = NULL;
  if (pBrep)
  ...
  if (pRevSurface && pRevSurface->m_curve) {
    ...
    if (!brep->Create(pRevSurface)) {
      ...
      for (int capcount = 0; capcount < 2; capcount++) {
        ...
        ON_NurbsSurface* pCapSurface =
            ON_NurbsSurfaceQuadrilateral(circle.plane.PointAt(-radius, -radius),
...
static ON_BOOL32
AddC3Curve (const ON_Curve* c3, ON_SimpleArray<ON_Curve*>& C3)
{
  int j;
  if (!c3)
    return false;
...
ON_Brep*
ON_BrepTrimmedPlane (const ON_Plane& plane, const ON_Curve& boundary, ON_Brep* pBrep)
{
                 ...
                 ON_Brep* pBrep)
{
  ...
  if (mesh_topology.m_mesh && mesh_topology.IsValid()) {
    ...
    if (mesh_topology.m_mesh->HasDoublePrecisionVertices() &&
        mesh_topology.m_mesh->DoublePrecisionVerticesAreValid())
    ...
    for (fi = 0; fi < face_count; fi++) {
      ...
      srf_2d_corner[0].Set(
          srf_dom[0][0], srf_dom[1][0], 0.0); // SW parameter space corner
      srf_2d_corner[1].Set(
          srf_dom[0][1], srf_dom[1][0], 0.0); // SE parameter space corner
      srf_2d_corner[2].Set(
          srf_dom[0][1], srf_dom[1][1], 0.0); // NE parameter space corner
      srf_2d_corner[3].Set(
          srf_dom[0][0], srf_dom[1][1], 0.0); // NW parameter space corner
      ...
      if (bTriangle && bTrimmedTriangles) {
        ...
        for (lti = 0; lti < 3; lti++) {
          ...
          ON_BrepTrim& trim =
              brep->NewTrim(brep_edge, mesh_face.m_reve[fei] ? true : false, loop, c2i);
          trim.m__legacy_2d_tol = 0.0;
          trim.m__legacy_3d_tol = 0.0;
          trim.m_tolerance[0] = 0.0;
          trim.m_tolerance[1] = 0.0;
          ...
          trim.m_type =
              (mesh_edge.m_topf_count > 1) ? ON_BrepTrim::mated : ON_BrepTrim::boundary;
        }
      }
      else {
        for (lti = 0; lti < 4; lti++) {
          ...
          if (bTriangle && lti == 3) {
            ...
            brep->NewSingularTrim(brep->m_V[fvi[0]], loop, quad_iso[lti], c2i);
          }
          else {
            ...
            ON_BrepTrim& trim = brep->NewTrim(
                brep_edge, mesh_face.m_reve[fei] ? true : false, loop, c2i);
            trim.m__legacy_2d_tol = 0.0;
            trim.m__legacy_3d_tol = 0.0;
            trim.m_tolerance[0] = 0.0;
            trim.m_tolerance[1] = 0.0;
            trim.m_iso = quad_iso[lti];
            trim.m_type = (mesh_edge.m_topf_count > 1) ? ON_BrepTrim::mated
...
static bool
FoundSlitPair (ON_BrepLoop& L, int* t0, int* t1)

{
  ...
  for (i = 0; i < count; i++) {
    ...
    if (T1.m_type != ON_BrepTrim::seam)
      continue;
    if (T0.m_vi[0] != T1.m_vi[1])
      continue;
    if (T0.m_ei != T1.m_ei)
      continue;
    ...
    if (E.m_ti.Count() != 2)
      continue;
    ON_2dPoint P0, P1;
    if (!B->GetTrim2dStart(s0, P0))
      continue;
    if (!B->GetTrim2dEnd(s1, P1))
      continue;
    if (fabs(P0[0] - P1[0]) > utol || fabs(P0[1] - P1[1]) > vtol)
      continue;
    *t0 = s0;
    *t1 = s1;
    return true;
...
bool
ON_Brep::ChangeVertex(int old_vi, int new_vi, bool bClearTolerances)
{
  if (old_vi == new_vi)
    return true;
...
ON_BOOL32
ON_BrepEdge::SetStartPoint(ON_3dPoint)
{
  return false;
}

ON_BOOL32
ON_BrepEdge::SetEndPoint(ON_3dPoint)
{
  return false;
}
...
ON_BOOL32
ON_BrepTrim::SetEndPoint(ON_3dPoint)
{
  return false;
}
...
ON_Brep::CloseTrimGap(ON_BrepTrim& trim0, ON_BrepTrim& trim1)
{
  ...
  if (loop.m_ti.Count() == 1 && trim0.m_trim_index == trim1.m_trim_index) {
    ...
    for (lti = 0; lti < loop.m_ti.Count(); lti++) {
      if (loop.m_ti[lti] == trim0.m_trim_index &&
          loop.m_ti[(lti + 1) % loop.m_ti.Count()] == trim1.m_trim_index)
        break;
    }
  }
...
bool
ON_Brep::CollapseEdge(int edge_index, bool bCloseTrimGap, int vertex_index)
{
...
int
ON_Brep::RemoveWireEdges(bool bDeleteVertices)
{
  int rc = 0;
...
bool
ON_Brep::RemoveNesting(bool bExtractSingleSegments, bool bEdges, bool bTrimCurves)
{
  bool rc = false;
  // TODO
  int i, count;
...
static bool
IsSlitTrim (const ON_BrepTrim& T)
...
static bool
ON_BrepRemoveSlits (ON_BrepLoop& L)
...
static int
sfsort (const srf_face* a, const srf_face* b)
{
  if ((*a)[0] < (*b)[0])
    return -1;
  if ((*b)[0] < (*a)[0])
...
static int
MergeAdjacentEdge (ON_Brep& B, int eid)
