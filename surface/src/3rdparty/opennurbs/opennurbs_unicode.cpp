...
int
ON_IsValidUnicodeCodePoint (ON__UINT32 u)
{
  return (u < 0xD800 || (u >= 0xE000 && u <= 0x10FFFF));
}

int
ON_EncodeUTF8 (ON__UINT32 u, ON__UINT8 sUTF8[6])
{
  ON__UINT32 c;
...
static int
ON_DecodeUTF8Helper (const ON__UINT8* sUTF8,
                     int sUTF8_count,
                     ON__UINT32* value,
...
#define OVERLONG_ENCODING 8
  ...
  if (0xFC == (0xFE & c)) {
    // 6 byte character encoding: 110xxxxx, 10xxxxxx, 10xxxxxx, 10xxxxxx, 10xxxxxx,
    // 10xxxxxx
    if (sUTF8_count < 6) {
      *error_status |= INPUT_BUFFER_TOO_SHORT; // input buffer too short
      return 0;
               ...
               ON__UINT32* unicode_code_point)
{
  ...
  if (i0 > 0 && 0 == error_status &&
      (u0 < 0xD800 || (u0 >= 0xE000 && u0 <= 0x10FFFF))) {
  ...
  if (i0 < sUTF8_count && u0 >= 0xD800 && u0 <= 0xDBFF &&
      (0 == error_status || 8 == error_status) && 0 != (4 & e->m_error_mask)) {
    ...
    if (i1 > 0 && (0 == error_status || 8 == error_status)) {
      ...
      u0 = 0xFFFFFFFF;
      if (2 == ON_ConvertUTF16ToUTF32 (
                   false, sUTF16, 2, &u0, 1, &error_status, 0, 0, 0) &&
          0 == error_status && ON_IsValidUnicodeCodePoint (u0)) {
        *unicode_code_point = u0;
        e->m_error_status |= 4;
        return i0 + i1;
      }
...
int
ON_EncodeUTF16 (ON__UINT32 unicode_code_point, ON__UINT16 sUTF16[2])
{
  ...
  if (unicode_code_point <= 0x10FFFF) {
    ...
    sUTF16[0] =
        (ON__UINT16)(0xD800 + (unicode_code_point /
                               0x400)); // high surrogate value (0xD800 ... 0xDBFF)
    sUTF16[1] =
        (ON__UINT16)(0xDC00 + (unicode_code_point &
                               0x3FF)); // low surrogate value (0xDC00 ... 0xDFFF)
    return 2;
  }
                ...
                ON__UINT32* unicode_code_point)
{
  ...
  if (sUTF16_count >= 2 && sUTF16[0] < 0xDC00 && sUTF16[1] >= 0xDC00 &&
      sUTF16[1] < 0xE000) {
  ...
  if (16 != (16 & e->m_error_mask) ||
      !ON_IsValidUnicodeCodePoint (e->m_error_code_point)) {
    // this error is not masked
    return 0;
  }
  ...
  for (i = 1; i < sUTF16_count; i++) {
    ...
    if (i + 1 < sUTF16_count && sUTF16[i] >= 0xD800 && sUTF16[i] < 0xDC00 &&
                          ...
                          const wchar_t** sNextWideChar)
{
  ...
  case sizeof (ON__UINT16):
    ...
    rc = ON_ConvertUTF16ToUTF8 (bTestByteOrder,
                                (const ON__UINT16*)sWideChar,
                                sWideChar_count,
                                (ON__UINT8*)sUTF8,
                                sUTF8_count,
                                error_status,
  ...
  case sizeof (ON__UINT32):
    ...
    rc = ON_ConvertUTF32ToUTF8 (bTestByteOrder,
                          ...
                          const char** sNextUTF8)
{
  ...
  case sizeof (ON__UINT16):
    // assume wchar_t strings are UTF-16 encoded
    rc = ON_ConvertUTF8ToUTF16 ((const ON__UINT8*)sUTF8,
                                sUTF8_count,
                                (ON__UINT16*)sWideChar,
                                sWideChar_count,
                                error_status,
                                error_mask,
                                error_code_point,
  ...
  case sizeof (ON__UINT32):
    // assume wchar_t strings are UTF-32 encoded
    rc = ON_ConvertUTF8ToUTF32 ((const ON__UINT8*)sUTF8,
                                sUTF8_count,
                                (ON__UINT32*)sWideChar,
                                sWideChar_count,
                                error_status,
                                error_mask,
                                error_code_point,
