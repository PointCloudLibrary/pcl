...
ON_Surface&
ON_Surface::operator= (const ON_Surface& src)
{
  DestroySurfaceTree();
...
ON::object_type
ON_Surface::ObjectType() const
{
  return ON::surface_object;
}

ON_BOOL32
ON_Surface::GetDomain (int dir, double* t0, double* t1) const
{
...
ON_BOOL32
ON_Surface::GetSurfaceSize (double* width, double* height) const
{
  if (width)
    *width = 0.0;
  if (height)
...
bool
ON_Surface::SetDomain (int dir, ON_Interval domain)
{
  return (dir >= 0 && dir <= 1 && domain.IsIncreasing() &&
          SetDomain (dir, domain[0], domain[1]))
             ? true
             : false;
}
...
ON_BOOL32
ON_Surface::GetParameterTolerance ( // returns tminus < tplus: parameters tminus <= s <=
                                    // tplus
    int dir,
...
ON_Surface::IsIsoparametric (const ON_Curve& curve, const ON_Interval* subdomain) const
{
  ...
  if (subdomain) {
    ...
    if (t0 < t1 - ON_SQRT_EPSILON) {
      if ((t0 > ON_SQRT_EPSILON && t0 < 1.0 - ON_SQRT_EPSILON) ||
          (t1 > ON_SQRT_EPSILON && t1 < 1.0 - ON_SQRT_EPSILON)) {
  ...
  if ((dim == 2 || dim == 3) && curve.GetBoundingBox (bbox)) {
    ...
    case E_iso:
      ...
      if (tolerance < ON_ZERO_TOLERANCE &&
          ON_ZERO_TOLERANCE * 1024.0 <= (bbox.m_max.y - bbox.m_min.y)) {
    ...
    case N_iso:
      ...
      if (tolerance < ON_ZERO_TOLERANCE &&
          ON_ZERO_TOLERANCE * 1024.0 <= (bbox.m_max.x - bbox.m_min.x)) {
        // 26 March 2007 Dale Lear
        //    If tolerance is tiny, then use ON_ZERO_TOLERANCE
        //    This fixes cases where iso curves where not getting
        //    the correct flag because tol=1e-16 and the closest
...
ON_BOOL32
ON_Surface::IsPlanar (ON_Plane*, double) const { return false; }
...
ON_BOOL32
ON_Surface::IsPeriodic (int) const { return false; }
            ...
            for (j = (span_index ? 1 : 0); j <= 2 && 1 != *dtype; j++) {
              ...
              if (bTestD1 || bTestT) {
                ...
                if (Evaluate (st0.x, st0.y, 2, 3, &Da[0].x, 1, hinta) &&
                  ...
                  if (bTestD1) {
                    if (!(D1a - D1b).IsTiny (D1b.MaximumCoordinate() *
                                             ON_SQRT_EPSILON)) {
                      if (dtype)
                        *dtype = 1;
                      *t = t1;
...
ON_BOOL32
ON_Surface::IsSingular (int) const { return false; }

bool
ON_Surface::IsSolid() const
{
...
int
ON_Surface::IsAtSeam (double s, double t) const
...
ON_BOOL32
ON_Surface::FrameAt (double u, double v, ON_Plane& frame) const
{
  ON_BOOL32 rc = false;
  ON_3dPoint origin;
...
// virtual
ON_BOOL32
ON_Surface::Trim (int, const ON_Interval&) { return false; }

// virtual
bool
ON_Surface::Extend (int, const ON_Interval&)
{
  return false;
}
...
// virtual
ON_BOOL32
ON_Surface::Split (int, double, ON_Surface*&, ON_Surface*&) const { return false; }

// virtual
int
ON_Surface::GetNurbForm (ON_NurbsSurface&, double) const
{
  return 0;
}
...
ON_SurfaceArray::~ON_SurfaceArray() { Destroy(); }

void
ON_SurfaceArray::Destroy()
{
  int i = m_capacity;
  while (i-- > 0) {
    if (m_a[i]) {
...
ON_BOOL32
ON_SurfaceArray::Duplicate (ON_SurfaceArray& dst) const
{
  dst.Destroy();
...
ON_BOOL32
ON_SurfaceArray::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_SurfaceArray::Read (ON_BinaryArchive& file)
{
  int major_version = 0;
  int minor_version = 0;
  ON__UINT32 tcode = 0;
...
void
ON_Surface::DestroySurfaceTree()
{
  DestroyRuntimeCache (true);
}

ON_SurfaceProperties::ON_SurfaceProperties() { memset (this, 0, sizeof (*this)); }

void
ON_SurfaceProperties::Set (const ON_Surface* surface)
{
  m_surface = surface;
  ...
  if (0 == m_surface) {
    ...
    m_bHasSingularity =
        (m_bIsSingular[0] || m_bIsSingular[1] || m_bIsSingular[2] || m_bIsSingular[3]);
