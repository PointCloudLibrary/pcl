...
ON_LineCurve::ON_LineCurve (const ON_3dPoint& a, const ON_3dPoint& b)
: m_line (a, b), m_dim (3)
{
  double len = m_line.Length();
  if (len <= ON_ZERO_TOLERANCE)
...
ON_LineCurve::ON_LineCurve (const ON_Line& L, double t0, double t1)
: m_line (L), m_t (t0, t1), m_dim (3)
{}

ON_LineCurve::ON_LineCurve (const ON_LineCurve& src) : ON_Curve (src) { *this = src; }

ON_LineCurve::~ON_LineCurve() {}

unsigned int
ON_LineCurve::SizeOf() const
{
...
ON__UINT32
ON_LineCurve::DataCRC (ON__UINT32 current_remainder) const
{
...
ON_LineCurve&
ON_LineCurve::operator= (const ON_LineCurve& src)
{
  if (this != &src) {
...
ON_LineCurve&
ON_LineCurve::operator= (const ON_Line& L)
{
  m_line = L;
  m_t.m_t[0] = 0.0;
  m_t.m_t[1] = L.Length();
...
bool
ON_LineCurve::IsDeformable() const
{
  return true;
}

bool
ON_LineCurve::MakeDeformable()
{
  return true;
}
...
ON_BOOL32
ON_LineCurve::IsValid (ON_TextLog*) const
{
  return (m_t[0] < m_t[1] && m_line.Length() > 0.0) ? true : false;
}

void
ON_LineCurve::Dump (ON_TextLog& dump) const
{
...
ON_BOOL32
ON_LineCurve::Write (ON_BinaryArchive& file // open binary file
) const
{
...
ON_BOOL32
ON_LineCurve::Read (ON_BinaryArchive& file // open binary file
)
{
  int major_version = 0;
  int minor_version = 0;
...
ON_Interval
ON_LineCurve::Domain() const
{
  return m_t;
}

ON_BOOL32
ON_LineCurve::SetDomain (double t0, double t1)
{
  if (t0 < t1) {
    m_t.Set (t0, t1);
    DestroyCurveTree();
...
bool
ON_LineCurve::ChangeDimension (int desired_dimension)
{
...
int
ON_LineCurve::SpanCount() const
{
  return 1;
}

ON_BOOL32
ON_LineCurve::GetSpanVector ( // span "knots"
    double* s) const
{
  s[0] = m_t[0];
  s[1] = m_t[1];
...
int
ON_LineCurve::IsPolyline (ON_SimpleArray<ON_3dPoint>* pline_points,
...
ON_BOOL32
ON_LineCurve::IsPlanar (ON_Plane* plane, // if not NULL and true is returned, then plane
...
ON_BOOL32
ON_LineCurve::IsInPlane (const ON_Plane& plane, // plane to test
                         double tolerance // tolerance to use when checking linearity
) const
{
  ON_BOOL32 rc = false;
...
ON_BOOL32
ON_LineCurve::IsClosed() const { return false; }

ON_BOOL32
ON_LineCurve::IsPeriodic() const { return false; }
...
ON_BOOL32
ON_LineCurve::Evaluate ( // returns false if unable to evaluate
...
ON_BOOL32
ON_LineCurve::SetStartPoint (ON_3dPoint start_point)
{
...
ON_BOOL32
ON_LineCurve::SetEndPoint (ON_3dPoint end_point)
{
  m_line.to = end_point;
  DestroyCurveTree();
  return true;
...
int
ON_LineCurve::GetNurbForm (ON_NurbsCurve& c, double, const ON_Interval* subdomain) const
{
  int rc = 0;
...
int
ON_LineCurve::HasNurbForm() const
...
ON_BOOL32
ON_LineCurve::Trim (const ON_Interval& domain)
{
  ON_BOOL32 rc = false;
...
bool
ON_LineCurve::Extend (const ON_Interval& domain)
...
ON_BOOL32
ON_LineCurve::Split (double t, ON_Curve*& left_side, ON_Curve*& right_side) const
...
ON_BOOL32
ON_LineCurve::GetCurveParameterFromNurbFormParameter (double nurbs_t,
...
ON_BOOL32
ON_LineCurve::GetNurbFormParameterFromCurveParameter (double curve_t,
