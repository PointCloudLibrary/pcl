...
ON::object_type
ON_Light::ObjectType() const
{
  return ON::light_object;
}

int
ON_Light::Dimension() const
{
  return 3;
}
...
ON_BOOL32
ON_Light::IsDirectionalLight() const
{
  ON_BOOL32 rc;
  switch (m_style) {
  // case ON::view_directional_light:
...
ON_BOOL32
ON_Light::IsRectangularLight() const
{
  ON_BOOL32 rc;
  switch (m_style) {
...
double
ON_Light::SpotAngleRadians() const
{
...
double
ON_Light::SpotAngleDegrees() const
{
  return m_spot_angle;
}
...
ON_Light::SetSpotExponent (double e)
{
  ...
  if (e < 0.0 || !ON_IsValid (e))
    ...
    m_spot_exponent = e;
  m_hotspot =
      ON_UNSET_VALUE; // indicates hotspot should be computed from m_spot_exponent
}
...
double
ON_Light::ShadowIntensity() const
{
...
ON_Color
ON_Light::Ambient() const
{
  return m_ambient;
}

ON_Color
ON_Light::Diffuse() const
{
  return m_diffuse;
}

ON_Color
ON_Light::Specular() const
{
  return m_specular;
}

ON::coordinate_system
ON_Light::CoordinateSystem() const // determined by style
{
...
ON_3dPoint
ON_Light::Location() const
{
  return m_location;
}

ON_3dVector
ON_Light::Direction() const
{
  return m_direction;
}

ON_3dVector
ON_Light::PerpindicularDirection() const
{
...
double
ON_Light::PowerWatts() const
{
  return m_watts;
}

double
ON_Light::PowerLumens() const
{
  return m_watts / 683.0;
}

double
ON_Light::PowerCandela() const
{
  return m_watts / 683.0;
}
