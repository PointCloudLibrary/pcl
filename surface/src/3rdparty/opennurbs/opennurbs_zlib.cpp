...
#pragma comment(lib,                                                                   \
                    "\"" OPENNURBS_ZLIB_OUTPUT_ROOT_DIR                                \
                    "/" OPENNURBS_CONFIGURATION_DIR "/" OPENNURBS_ZLIB_FILE_NAME "\"")
...
{
  const std::size_t max_avail =
      0x7FFFFFF0; // See max_avail comment in ON_BinaryArchive::WriteInflate
  ...
  bool bValidCompressedBuffer = false;
  {
    ...
    bValidCompressedBuffer = (m_bad_CRC_count > c0) ? false : rc;
  }
...
bool
ON_BinaryArchive::CompressionInit()
{
...
void
ON_BinaryArchive::CompressionEnd()
{
...
{
  const std::size_t max_avail =
      0x7FFFFFF0; // See max_avail comment in ON_CompressedBuffer::InflateHelper
