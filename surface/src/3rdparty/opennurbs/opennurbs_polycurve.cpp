...
ON_BOOL32
ON_PolyCurve::IsValid (ON_TextLog* text_log) const { return IsValid (false, text_log); }
...
double
ON_PolyCurve::PolyCurveParameter (int segment_index,
                                  double segmentcurve_parameter) const
{
  const ON_Curve* segment_curve = SegmentCurve (segment_index);
  if (!segment_curve)
    return ON_UNSET_VALUE;
...
ON_BOOL32
ON_PolyCurve::Remove() { return Remove (Count() - 1); }
