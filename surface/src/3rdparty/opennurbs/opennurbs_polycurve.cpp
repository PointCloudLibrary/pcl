...
ON_PolyCurve::ON_PolyCurve(int capacity) : m_segment(capacity), m_t(capacity + 1)
{
  m_segment.Zero();
}
...
ON_PolyCurve::~ON_PolyCurve() { Destroy(); }

void
ON_PolyCurve::EmergencyDestroy()
{
...
void
ON_PolyCurve::DestroyRuntimeCache(bool bDelete)
{
...
ON__UINT32
ON_PolyCurve::DataCRC(ON__UINT32 current_remainder) const
{
  int i, count = m_segment.Count();
  for (i = 0; i < count; i++) {
    const ON_Curve* crv = m_segment[i];
...
ON_PolyCurve&
ON_PolyCurve::operator=(const ON_PolyCurve& src)
{
  if (this != &src) {
...
bool
ON_PolyCurve::IsDeformable() const
{
  bool rc = true;
  const int count = Count();
  int segment_index;
...
ON_BOOL32
ON_PolyCurve::IsValid(ON_TextLog* text_log) const
{
  return IsValid(false, text_log);
}

bool
ON_PolyCurve::IsValid(bool bAllowGaps, ON_TextLog* text_log) const
{
  const int count = Count();
  ...
  if (m_t.Count() != count + 1) {
    if (text_log)
      text_log->Print("Polycurve segment count = %d and m_t.Count()=%d (should be "
                      "segment count+1)\n",
                      count,
                      m_t.Count());
    return ON_IsNotValid();
...
void
ON_PolyCurve::Dump(ON_TextLog& dump) const
{
  const int count = Count();
  int i;
...
ON_BOOL32
ON_PolyCurve::Write(ON_BinaryArchive& file // open binary file
) const
{
  ...
  if (rc) {
    ...
    if (rc)
      file.WriteInt(reserved1);
    if (rc)
      file.WriteInt(reserved2);
    if (rc) {
...
ON_BOOL32
ON_PolyCurve::Read(ON_BinaryArchive& file // open binary file
)
{
  // NOTE - if changed, check legacy I/O code
  Destroy();
  ...
  if (rc) {
    ...
    if (rc && m_segment.Count() > 0 && m_segment.Count() == segment_count &&
        m_t.Count() == segment_count + 1) {
...
ON_Curve*
ON_PolyCurve::DuplicateCurve() const
{
...
ON_BOOL32
ON_PolyCurve::SetDomain(double t0, double t1)
{
...
bool
ON_PolyCurve::ChangeDimension(int desired_dimension)
{
  int i, count = m_segment.Count();
  bool rc = (count > 0);
  for (i = 0; i < count; i++) {
...
bool
ON_PolyCurve::SetParameterization(const double* t)
{
  bool rc = false;
...
ON_BOOL32
ON_PolyCurve::ChangeClosedCurveSeam(double t)
{
  ...
  if (rc) {
    ...
    if (old_count == 1) {
      ...
      if (scrv) {
        ...
        double s =
            (old_dom == sdom) ? t : sdom.ParameterAt(old_dom.NormalizedParameterAt(t));
      ...
      if (old_dom.Includes(k, true)) {
        ...
        if (rc && segment_index < old_count) {
          ...
          new_c.Append(sright);
          new_t.Append(k);
          new_c.Append(old_count - segment_index - 1,
                       m_segment.Array() + segment_index + 1);
          ...
          memset(
              m_segment.Array(), 0, m_segment.Capacity() * sizeof(*m_segment.Array()));
...
ON_BOOL32
ON_PolyCurve::GetSpanVector( // span "knots"
...
int
ON_PolyCurve::IsPolyline(ON_SimpleArray<ON_3dPoint>* pline_points,
                         ON_SimpleArray<double>* pline_t) const
{
  ...
  if (seg_count == 1) {
    ...
    for (seg_i = 0; seg_i < seg_count; seg_i++) {
      ...
      seg_t.SetCount(0);
      seg_rc = m_segment[seg_i]->IsPolyline(pline_points ? &seg_points : 0,
                                            pline_t ? &seg_t : 0);
      if (seg_rc < 2) {
        if (pline_points)
          pline_points->SetCount(0);
...
static bool
GetTestPlane (const ON_Curve& curve, ON_Plane& plane)
{
  int i, j;
  ON_3dPoint P, Q, R;
  ON_3dVector X;
...
ON_BOOL32
ON_PolyCurve::IsPlanar(ON_Plane* plane, // if not NULL and true is returned, then plane
...
ON_BOOL32
ON_PolyCurve::IsInPlane(const ON_Plane& plane, // plane to test
                        double tolerance // tolerance to use when checking linearity
) const
{
  ON_BOOL32 rc = false;
...
static bool
GetLineIsoCoordinates (const ON_Line& line, const ON_3dPoint P, ON_3dPoint& C)
{
...
static void
LineLineTieBreaker (const ON_Line& line0,
                    const ON_Line& line1,
                    ON_3dPoint& Q0,
                    ON_3dPoint& Q1)
{
...
static void
SetLineIsoCoords (const ON_Line& line, const ON_3dPoint& P, ON_3dPoint& Q)
{
  ON_3dPoint C;
...
ChangeArcEnd (const ON_ArcCurve* arc, ON_3dPoint P, ON_3dPoint Q, int end_index)
{
  if (P == Q)
    return 0;
...
bool
ON_PolyCurve::CloseGap(int gap_index, int)
{
  ...
  if (0 != arc0 && 0 != arc1) {
    ...
    bool bLine0 =
        (0 == arc0) ? c0->LastSpanIsLinear(min_line_length, is_linear_tolerance, &line0)
                    : false;
    bool bLine1 =
        (0 == arc0)
...
int
ON_PolyCurve::HasGap() const
{
  return FindNextGap(0);
}

bool
ON_PolyCurve::HasGapAt(int segment_index) const
{
  const int count = m_segment.Count();
  ...
  if (false == ON_PointsAreCoincident(3, false, &P0.x, &P1.x)) {
    ...
    const double tol0 =
        arc0 ? (arc0->m_arc.radius * arc0->m_arc.AngleRadians() * 1.0e-10) : 0.0;
    const double tol1 =
        arc1 ? (arc1->m_arc.radius * arc1->m_arc.AngleRadians() * 1.0e-10) : 0.0;
    if (tol < tol0)
      tol = tol0;
    if (tol < tol1)
      tol = tol1;
...
int
ON_PolyCurve::FindNextGap(int segment_index0) const
{
  if (segment_index0 >= 0) {
    const int count = m_segment.Count();
...
bool
ON_PolyCurve::GetNextDiscontinuity(ON::continuity c,
                                   double t0,
                                   double t1,
                                   double* t,
                                   int* hint,
                                   ...
                                   double curvature_tolerance) const
{
  ...
  if (count > 0 && t0 != t1) {
    ...
    int segment_index =
        ON_NurbsSpanIndex(2, count + 1, m_t, t0, (t0 > t1) ? -1 : 1, segment_hint);
...

    {
      ...
      //     of a curve domain.
      double segtol = (fabs(m_t[segment_index]) + fabs(m_t[segment_index + 1]) +
                       fabs(m_t[segment_index + 1] - m_t[segment_index])) *
                      ON_SQRT_EPSILON;
      if (m_t[segment_index] + segtol < m_t[segment_index + 1] - segtol) {
        if (t0 < t1) {
          if (t0 < m_t[segment_index + 1] && t1 > m_t[segment_index + 1] &&
              fabs(t0 - m_t[segment_index + 1]) <= segtol &&
              segment_index + 1 < count) {
            t0 = m_t[segment_index + 1];
            segment_index = ON_NurbsSpanIndex(2, count + 1, m_t, t0, 1, segment_hint);
          }
        }
        else {
          if (t0 > m_t[segment_index] && t1 < m_t[segment_index] &&
              fabs(t0 - m_t[segment_index]) <= segtol && segment_index > 0) {
    ...
    const ON_Curve* crv;
    for (/*empty*/; segment_index >= 0 && segment_index < count &&
      ...
      rc = crv->GetNextDiscontinuity(
          c, s0, s1, &s, &curve_hint, dtype, cos_angle_tolerance, curvature_tolerance);
      if (rc) {
        double kink_t;
        if (sdom == cdom) {
          kink_t = s;
          ...
          if (kink_t <= tmin + t_tol || kink_t >= tmax - t_tol) {
            ...
            if (e < 1.0)
              ...
              e = 1000.0;
            double s_tol =
                (fabs(s0) + fabs(s1) + fabs(s0 - s1)) * ON_ZERO_TOLERANCE * e;
      ...
      case ON::Gsmooth_continuous:
        ...
        crv1->Ev2Der(
            crv1_t, Pp, D1p, D2p, -crv0_side); // corresponding point on next curve
        if (c == ON::C2_continuous) {
          ...
          if (Tm * Tp < cos_angle_tolerance) {
            ...
            if (!bIsCurvatureContinuous) {
              ...
              bool b0 =
                  seg0->LastSpanIsLinear(is_linear_min_length, is_linear_tolerance);
              bool b1 =
                  seg1->FirstSpanIsLinear(is_linear_min_length, is_linear_tolerance);
              if (b0 != b1) {
                rc = true;
                if (dtype)
                  *dtype = 3;
    ...
    if (!rc && input_c != c) {
      ...
      rc = ON_Curve::GetNextDiscontinuity(
          input_c, t0, t1, t, NULL, dtype, cos_angle_tolerance, curvature_tolerance);
    }
  }
  return rc;
}
  ...
  if (count > 0) {
...

    {
      ...
      if (m_t[segment_index] + segtol < m_t[segment_index + 1] - segtol) {
        ...
        else if (fabs(t - m_t[segment_index + 1]) <= segtol &&
                 segment_index + 1 < count) {
    ...
    if (m_t[segment_index] < t && t < m_t[segment_index + 1]) {
      ...
      if (segment_curve) {
        ...
        rc = segment_curve->IsContinuous(desired_continuity,
                                         t,
                                         &curve_hint,
                                         point_tolerance,
                                         d1_tolerance,
      ...
      if (segment_index == 0 && t == m_t[0])
        ...
        rc = ON_Curve::IsContinuous(desired_continuity,
                                    ...
                                    cos_angle_tolerance,
                                    curvature_tolerance);
        if (0 != rc && ON::Gsmooth_continuous == desired_continuity &&
            segment_index >= 0 && segment_index < count) {
...
ON_BOOL32
ON_PolyCurve::Evaluate( // returns false if unable to evaluate
  ...
  if (count > 0 && dim > 0 && dim <= v_stride) {
    ...
    if (c) {
      ...
      if (s0 != s1) {
        ...
        curve_hint = (hint && segment_hint == segment_index) ? ((*hint) >> 14) : 0;
        rc = c->Evaluate(s, der_count, v_stride, v, side, &curve_hint);
...
{
  return (segment_index >= 0 && segment_index < Count()) ? m_segment[segment_index]
                                                         : NULL;
}

double
ON_PolyCurve::SegmentCurveParameter(double polycurve_parameter) const
{
...
double
ON_PolyCurve::PolyCurveParameter(int segment_index, double segmentcurve_parameter) const
{
  const ON_Curve* segment_curve = SegmentCurve(segment_index);
  if (!segment_curve)
    return ON_UNSET_VALUE;
...
ON_BOOL32
ON_PolyCurve::Prepend(ON_Curve* c)
{
  DestroyCurveTree();
  return Insert(0, c);
}
...
ON_BOOL32
ON_PolyCurve::Append(ON_Curve* c)
{
  DestroyCurveTree();
...
ON_BOOL32
ON_PolyCurve::PrependAndMatch(ON_Curve* c)
...
ON_BOOL32
ON_PolyCurve::AppendAndMatch(ON_Curve* c)
...
ON_BOOL32
ON_PolyCurve::Remove(int segment_index)
{
  ON_BOOL32 rc = false;
  const int segment_count = Count();
...
ON_BOOL32
ON_PolyCurve::Insert(int segment_index, ON_Curve* c)
{
  double s0, s1;
  ON_BOOL32 rc = false;
  const int count = Count();
...
ON_BOOL32
ON_PolyCurve::SetStartPoint(ON_3dPoint start_point)
{
  ON_BOOL32 rc = false;
...
ON_BOOL32
ON_PolyCurve::SetEndPoint(ON_3dPoint end_point)
{
  ON_BOOL32 rc = false;
  // just do it // if ( !IsClosed() )
  {
...
int
ON_PolyCurve::HasNurbForm() const
...
int
ON_PolyCurve::SegmentIndex(double curve_t) const
{
  int count = m_segment.Count();
...
int
ON_PolyCurve::SegmentIndex(ON_Interval sub_domain,
                           int* segment_index0,
...
ON_BOOL32
ON_PolyCurve::GetCurveParameterFromNurbFormParameter(double nurbs_t,
...
ON_BOOL32
ON_PolyCurve::GetNurbFormParameterFromCurveParameter(double curve_t,
...
ON_Curve*
ON_PolyCurve::HarvestSegment(int i)
{
...
ON_BOOL32
ON_PolyCurve::Trim(const ON_Interval& domain)
{
  ...
  const double fuzz =
      0.001; // Greg says: anything small and > 1.0e-6 will work about the same
  bool bTrimFirstSegment = (m_t[0] < actual_trim_domain[0] ||
                            (0 == s1 && actual_trim_domain[1] < m_t[s1 + 1]));
    ...
    if (c_dom != s_dom) {
      ...
      if (!trim_c_dom.IsIncreasing()) {
        if (s_dom.NormalizedParameterAt(trim_s_dom[0]) >= 1.0 - fuzz && s1 > 0) {
          ...
          m_t.Remove(0);
          m_segment.Remove(0);
          s1--; // removing a segment, means s1=(index of last valid segment) has to get
                // decremented.
        }
        else
          return false;
  ...
  if (bTrimLastSegment) {
    ...
    if (bTrimLastSegment && c_dom != trim_c_dom) {
      ...
      if (!curve->Trim(trim_c_dom)) {
        ...
        m_t[m_t.Count() - 1] =
            actual_trim_domain[1]; // will be tweaked below when we've finished.
    }
  }
...
bool
ON_PolyCurve::Extend(const ON_Interval& domain)

{
  ...
  if (Domain()[0] > domain[0]) {
    ...
    double a = (sdom == cdom) ? domain[0]
                              : cdom.ParameterAt(sdom.NormalizedParameterAt(domain[0]));
  ...
  if (Domain()[1] < domain[1]) {
    ...
    ON_Interval cdom = seg->Domain();
    double a = (sdom == cdom) ? domain[1]
                              : cdom.ParameterAt(sdom.NormalizedParameterAt(domain[1]));
...
ON_BOOL32
ON_PolyCurve::Split(double split_parameter,
...
) const
{
  ...
  /* 4 April 2003 Greg Arden		Made the following changes:
                                  1.	Use ParameterSearch() to decide if we should snap
     domain boundries to m_t array values.
                                  2.  Make sure resulting polycurves have Domain()
     specified as split parameter.
...
bool
ON_PolyCurve::HasSynchronizedSegmentDomains() const
{
  double t0, t1;
  int i, count = m_segment.Count();
  ...
  for (i = 0; i < count; i++) {
    t0 = -ON_UNSET_VALUE;
    t1 = ON_UNSET_VALUE;
    if (0 != c[i] && c[i]->GetDomain(&t0, &t1) && t0 == t[i] && t1 == t[i + 1]) {
      continue;
    }
    return false;
  }
...
*/
bool
ON_PolyCurve::SynchronizeSegmentDomains()
{
  double t0, t1;
  int i, count = m_segment.Count();
  ...
  for (i = 0; i < count; i++) {
    ...
    t0 = -ON_UNSET_VALUE;
    t1 = ON_UNSET_VALUE;
    if (c[i]->GetDomain(&t0, &t1) && t0 == t[i] && t1 == t[i + 1]) {
      continue;
    }

    if (ON_IsValid(t[i]) && ON_IsValid(t[i + 1]) && t[i] < t[i + 1] &&
...
void
ON_PolyCurve::RemoveNesting()
{
  RemoveNestingEx();
}

bool
ON_PolyCurve::IsNested() const
{
  int i, count = m_segment.Count();
  for (i = 0; i < count; i++) {
...
void
ON_PolyCurve::SetSegment(int i, ON_Curve* crv)
{
  if (i >= 0 && i < Count())
    m_segment[i] = crv;
}
...
bool
ON_PolyCurve::ParameterSearch(double t, int& index, bool bEnableSnap) const
{
  return ON_Curve::ParameterSearch(t, index, bEnableSnap, m_t, ON_SQRT_EPSILON);
}

const ON_CurveArray&
ON_PolyCurve::SegmentCurves() const
{
  return m_segment;
}

const ON_SimpleArray<double>&
ON_PolyCurve::SegmentParameters() const
{
  return m_t;
}
