    ...
    double) const
{
  ...
  if (!rc) {
    if (m_plane.origin.x != ON_UNSET_VALUE && m_plane.xaxis.x != ON_UNSET_VALUE &&
                                           ...
                                           double padding)
{
  ...
  for (int i = 1; i < point_count; i++) {
    ...
    else if (s > plane_domain[0].m_t[1])
      plane_domain[0].m_t[1] = s;
    if (t < plane_domain[1].m_t[0])
      plane_domain[1].m_t[0] = t;
    else if (t > plane_domain[1].m_t[1])
...
ON_ClippingPlane::ON_ClippingPlane() { Default(); }

ON_ClippingPlane::~ON_ClippingPlane() {}

ON_ClippingPlaneInfo
ON_ClippingPlane::ClippingPlaneInfo() const
{
...
void
ON_ClippingPlaneSurface::Default()
{
...
ON::object_type
ON_ClippingPlaneSurface::ObjectType() const
{
  return ON::clipplane_object;
}

ON_ClippingPlaneSurface::ON_ClippingPlaneSurface() { Default(); }
