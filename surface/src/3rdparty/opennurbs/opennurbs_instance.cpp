...
void
ON_InstanceDefinition::Dump (ON_TextLog& text_log) const
{
  ...
  case ON_InstanceDefinition::embedded_def:
    if (m_source_archive.Length() > 0)
      text_log.Print ("OBSOLETE embedded_def with non-empty source archive - should be "
                      "linked_and_embedded_def.");
    else
      text_log.Print (
          "OBSOLETE embedded_def with empty source archive - should be static_def.");
    break;
...
ON_BOOL32
ON_InstanceDefinition::IsValid (ON_TextLog* text_log) const
{
  ...
  case embedded_def: // embedded_def is obsolete -
    if (text_log) {
      text_log->Print (
          "ON_InstanceDefinition.m_idef_update_type = obsolete \"embedded_idef\". Use "
          "\"static_def\" or \"linked_and_embedded_def\".\n");
    }
    return false;
    break;
  ...
  case linked_def:
    ...
    if (m_source_archive.IsEmpty()) {
      if (text_log) {
        text_log->Print ("ON_InstanceDefinition is linked or embedded but "
                         "m_source_archive is empty.\n");
      }
      return false;
    }
    if (!m_source_archive_checksum.IsSet()) {
      if (text_log) {
        text_log->Print ("ON_InstanceDefinition is linked or embedded but "
                         "m_source_archive_checksum is zero.\n");
      }
      return false;
    }
    ...
    if (linked_def == m_idef_update_type) {
      if (1 != m_idef_layer_style && 2 != m_idef_layer_style) {
        if (text_log) {
          text_log->Print ("ON_InstanceDefinition is linked_def but m_idef_layer_style "
                           "is not 1 or 2.\n");
        }
        return false;
      }
      ...
      if (0 != m_idef_layer_style) {
        if (text_log) {
          text_log->Print ("ON_InstanceDefinition is linked_and_embedded_def but "
                           "m_idef_layer_style is not zero.\n");
        }
        return false;
      }
    }
...
unsigned int
ON_InstanceDefinition::SizeOf() const
{
...
ON_BOOL32
ON_InstanceDefinition::Write (ON_BinaryArchive& binary_archive) const
{
...
ON_InstanceDefinition::IDEF_UPDATE_TYPE
ON_InstanceDefinition::IdefUpdateType() const
{
  if (ON_InstanceDefinition::embedded_def == m_idef_update_type) {
    ...
    const_cast<ON_InstanceDefinition*> (this)->m_idef_update_type =
        (m_source_archive.Length() > 0) ? ON_InstanceDefinition::linked_and_embedded_def
...
ON_InstanceDefinition::IDEF_UPDATE_TYPE
ON_InstanceDefinition::IdefUpdateType (int i)
{
  IDEF_UPDATE_TYPE t;
  switch (i) {
  case ON_InstanceDefinition::static_def:
...
ON_BOOL32
ON_InstanceDefinition::Read (ON_BinaryArchive& binary_archive)
{
  int major_version = 0;
  int minor_version = 0;
  ...
  if (rc) {
    ...
    // m_idef_update_type was an unsigned int and got changed to an enum.  Read and
    // write as an unsigned int to support backwards compatibility
    ...
    if (minor_version >= 2) {
      ...
      if (ON::custom_unit_system != m_us.m_unit_system &&
          ON::no_unit_system != m_us.m_unit_system) {
...
ON::object_type
ON_InstanceDefinition::ObjectType() const
{
...
int
ON_InstanceDefinition::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_InstanceDefinition::GetBBox (double* boxmin, double* boxmax, ON_BOOL32) const
{
  if (boxmin) {
...
ON_BOOL32
ON_InstanceDefinition::Transform (const ON_Xform&)
{
...
const wchar_t*
ON_InstanceDefinition::Name() const
{
  return m_name;
}

void
ON_InstanceDefinition::SetName (const wchar_t* name)
{
  ON_wString s (name);
  s.TrimLeftAndRight();
  if (s.IsEmpty())
...
const wchar_t*
ON_InstanceDefinition::URL() const
{
  return m_url;
}

void
ON_InstanceDefinition::SetURL (const wchar_t* url)
{
  ON_wString s (url);
  s.TrimLeftAndRight();
  if (s.IsEmpty())
...
const wchar_t*
ON_InstanceDefinition::URL_Tag() const
{
  return m_url_tag;
}

void
ON_InstanceDefinition::SetURL_Tag (const wchar_t* url_tag)
{
...
const wchar_t*
ON_InstanceDefinition::Description() const
{
  return m_description;
}

void
ON_InstanceDefinition::SetDescription (const wchar_t* description)
{
...
void
ON_InstanceDefinition::SetBoundingBox (ON_BoundingBox bbox)
{
  m_bbox = bbox;
}
...
void
ON_InstanceDefinition::DestroySourceArchive()
{
  m_source_archive.Destroy();
...
const wchar_t*
ON_InstanceDefinition::SourceArchive() const
{
  return m_source_archive;
}

ON_CheckSum
ON_InstanceDefinition::SourceArchiveCheckSum() const
{
  return m_source_archive_checksum;
}

const ON_UnitSystem&
ON_InstanceDefinition::UnitSystem() const
{
  return m_us;
}

void
ON_InstanceDefinition::SetUnitSystem (ON::unit_system us)
{
...
void
ON_InstanceDefinition::SetUnitSystem (const ON_UnitSystem& us)
{
  // make sure we are not getting garbage cast as an ON::unit_system
  if (us.IsValid()) {
    m_us = us;
...
ON_BOOL32
ON_InstanceRef::IsValid (ON_TextLog* text_log) const
{
...
ON_BOOL32
ON_InstanceRef::Write (ON_BinaryArchive& binary_archive) const
{
...
ON_BOOL32
ON_InstanceRef::Read (ON_BinaryArchive& binary_archive)
{
  int major_version = 0;
  int minor_version = 0;
  bool rc = binary_archive.Read3dmChunkVersion (&major_version, &minor_version);
...
ON::object_type
ON_InstanceRef::ObjectType() const
{
...
// virtual ON_Geometry overrides
int
ON_InstanceRef::Dimension() const
{
  return 3;
}

ON_BOOL32
ON_InstanceRef::GetBBox (double* boxmin, double* boxmax, ON_BOOL32 bGrowBox) const
{
  if (!boxmin || !boxmax) {
    bGrowBox = false;
  }
  ...
  if (m_bbox.IsValid()) {
    if (bGrowBox) {
      if (boxmin[0] > m_bbox.m_min.x)
        boxmin[0] = m_bbox.m_min.x;
      if (boxmin[1] > m_bbox.m_min.y)
        boxmin[1] = m_bbox.m_min.y;
      if (boxmin[2] > m_bbox.m_min.z)
        boxmin[2] = m_bbox.m_min.z;

      if (boxmax[0] < m_bbox.m_max.x)
        boxmax[0] = m_bbox.m_max.x;
      if (boxmax[1] < m_bbox.m_max.y)
        boxmax[1] = m_bbox.m_max.y;
      if (boxmax[2] < m_bbox.m_max.z)
        boxmax[2] = m_bbox.m_max.z;
    }
    else {
      if (boxmin) {
...
ON_BOOL32
ON_InstanceRef::Transform (const ON_Xform& xform)
{
...
bool
ON_InstanceRef::IsDeformable() const
{
...
bool
ON_InstanceRef::MakeDeformable()
{
  // 25 Feb 2006 Dale Lear - this seems wrong to me.
  return true;
}

class /*NEVER EXPORT THIS CLASS DEFINITION*/ ON__IDefLayerSettingsUserData
: public ON_UserData {
...
public:
  ...
  ON__IDefLayerSettingsUserData&
  operator= (const ON__IDefLayerSettingsUserData& src);

  static ON__IDefLayerSettingsUserData*
...
private:
  ...
  void
  CopyHelper (const ON__IDefLayerSettingsUserData& src)
  {
    ...
    if (0 != src.m_idef_layer_table_parent_layer) {
      m_idef_layer_table_parent_layer =
          new ON_Layer (*src.m_idef_layer_table_parent_layer);
    }
...
ON__IDefLayerSettingsUserData*
ON__IDefLayerSettingsUserData::FindOrCreate (const ON_InstanceDefinition& idef,
                                             bool bCreate)
{
  ON__IDefLayerSettingsUserData* ud =
      ON__IDefLayerSettingsUserData::Cast (idef.GetUserData (
          ON__IDefLayerSettingsUserData::m_ON__IDefLayerSettingsUserData_class_id
              .Uuid()));
  if (!ud && bCreate) {
...
{
  m_userdata_uuid =
      ON__IDefLayerSettingsUserData::m_ON__IDefLayerSettingsUserData_class_id.Uuid();
  m_application_uuid = ON_opennurbs5_id;
  m_userdata_copycount = 1;
  CreateHelper();
}

ON__IDefLayerSettingsUserData::~ON__IDefLayerSettingsUserData() { DestroyHelper(); }

ON__IDefLayerSettingsUserData::ON__IDefLayerSettingsUserData (
    const ON__IDefLayerSettingsUserData& src)
: ON_UserData (src)
{
  m_userdata_uuid =
      ON__IDefLayerSettingsUserData::m_ON__IDefLayerSettingsUserData_class_id.Uuid();
...
// virtual ON_Object override
ON_BOOL32
ON__IDefLayerSettingsUserData::IsValid (ON_TextLog*) const { return true; }

// virtual ON_Object override
unsigned int
ON__IDefLayerSettingsUserData::SizeOf() const
{
...
// virtual ON_Object override
ON__UINT32
ON__IDefLayerSettingsUserData::DataCRC (ON__UINT32 current_remainder) const
{
...
// virtual ON_Object override
ON_BOOL32
ON__IDefLayerSettingsUserData::Write (ON_BinaryArchive& binary_archive) const
{
...
// virtual ON_Object override
ON_BOOL32
ON__IDefLayerSettingsUserData::Read (ON_BinaryArchive& binary_archive)
{
  DestroyHelper();
...
// virtual ON_UserData override
ON_BOOL32
ON__IDefLayerSettingsUserData::Archive() const
{
...
// virtual ON_UserData override
ON_BOOL32
ON__IDefLayerSettingsUserData::GetDescription (ON_wString& description)
{
...
bool
ON_InstanceDefinition::HasLinkedIdefLayerSettings() const
{
  const ON__IDefLayerSettingsUserData* ud =
      ON__IDefLayerSettingsUserData::FindOrCreate (*this, false);
...
static int
compareLayerPtrId (const void* A, const void* B)
{
  if (0 == A) {
    return 0 == B ? 0 : -1;
  }
...
static int
compareUuidIndexId (const void* a, const void* b)
{
  return ON_UuidIndex::CompareId ((const ON_UuidIndex*)a, (const ON_UuidIndex*)b);
}

void
ON_InstanceDefinition::UpdateLinkedIdefReferenceFileLayerRuntimeId (
...
void
ON_InstanceDefinition::UpdateLinkedIdefParentLayerSettings (
    const ON_Layer* linked_idef_parent_layer)
{
  bool bCreate = (0 != linked_idef_parent_layer);
  ON__IDefLayerSettingsUserData* ud =
      ON__IDefLayerSettingsUserData::FindOrCreate (*this, bCreate);
...
const ON_Layer*
ON_InstanceDefinition::LinkedIdefParentLayerSettings() const
{
  ON__IDefLayerSettingsUserData* ud =
      ON__IDefLayerSettingsUserData::FindOrCreate (*this, false);
...
void
ON_InstanceDefinition::UpdateLinkedIdefReferenceFileLayerSettings (
    unsigned int layer_count, ON_Layer** layer_settings)
{
  ON__IDefLayerSettingsUserData* ud;
  ...
  for (i = 0; i < ud->m_layers.UnsignedCount(); i++) {
    ...
    for (;;) {
      ...
      const ON_UuidIndex* idx0 = (const ON_UuidIndex*)bsearch (
          &idx, iddex, iddex_count, sizeof (iddex[0]), compareUuidIndexId);
      if (0 == idx0)
        break; // this layer is not in the current layer_settings[] list
      layer_settings[idx0->m_i]->SaveSettings (
          settings, false); // saves the layer settings found in linked file
      layer_settings[idx0->m_i]->Set (settings,
                                      *layer); // applies modifications found on idef
      break;
    }
    delete layer;
  }
...
void
ON_InstanceDefinition::UpdateLinkedIdefLayerSettings (
    unsigned int layer_count, const ON_Layer* const* layer_settings)
{
  if (layer_count <= 0 || 0 == layer_settings) {
    // delete linked idef layer settings
    UpdateLinkedIdefReferenceFileLayerSettings (0, 0);
...
class /*NEVER EXPORT THIS CLASS DEFINITION*/ ON__IDefAlternativePathUserData
: public ON_UserData {
...
public:
  ...
  ON__IDefAlternativePathUserData&
  operator= (const ON__IDefAlternativePathUserData& src);

  static ON__IDefAlternativePathUserData*
  FindOrCreate (const ON_InstanceDefinition& idef, bool bCreate);
...
private:
  ...
  void
  CopyHelper (const ON__IDefAlternativePathUserData& src)
  {
...
ON__IDefAlternativePathUserData*
ON__IDefAlternativePathUserData::FindOrCreate (const ON_InstanceDefinition& idef,
                                               bool bCreate)
{
  ON__IDefAlternativePathUserData* ud =
      ON__IDefAlternativePathUserData::Cast (idef.GetUserData (
          ON__IDefAlternativePathUserData::m_ON__IDefAlternativePathUserData_class_id
              .Uuid()));
  if (!ud && bCreate) {
...
ON__IDefAlternativePathUserData::~ON__IDefAlternativePathUserData() { DestroyHelper(); }

ON__IDefAlternativePathUserData::ON__IDefAlternativePathUserData (
    const ON__IDefAlternativePathUserData& src)
: ON_UserData (src)
{
  m_userdata_uuid =
      ON__IDefAlternativePathUserData::m_ON__IDefAlternativePathUserData_class_id
...
// virtual ON_Object override
ON_BOOL32
ON__IDefAlternativePathUserData::IsValid (ON_TextLog*) const
{
...
// virtual ON_Object override
unsigned int
ON__IDefAlternativePathUserData::SizeOf() const
{
...
// virtual ON_Object override
ON__UINT32
ON__IDefAlternativePathUserData::DataCRC (ON__UINT32 current_remainder) const
{
  ON__UINT32 crc =
      ON_CRC32 (current_remainder, sizeof (m_bRelativePath), &m_bRelativePath);
...
// virtual ON_Object override
ON_BOOL32
ON__IDefAlternativePathUserData::Write (ON_BinaryArchive& binary_archive) const
{
...
// virtual ON_Object override
ON_BOOL32
ON__IDefAlternativePathUserData::Read (ON_BinaryArchive& binary_archive)
{
  DestroyHelper();
...
// virtual ON_UserData override
ON_BOOL32
ON__IDefAlternativePathUserData::Archive() const
{
  // don't save empty settings
  return !m_alternate_path.IsEmpty();
}
...
// virtual ON_UserData override
ON_BOOL32
ON__IDefAlternativePathUserData::GetDescription (ON_wString& description)
{
...
void
ON_InstanceDefinition::SetAlternateSourceArchivePath (
    const wchar_t* alternate_source_archive_path, bool bRelativePath)
{
  ON_wString s;
  ...
  ON__IDefAlternativePathUserData* ud = ON__IDefAlternativePathUserData::FindOrCreate (
      *this, 0 != alternate_source_archive_path);
  if (0 != ud) {
    if (0 == alternate_source_archive_path)
      delete ud;
    else {
...
bool
ON_InstanceDefinition::GetAlternateSourceArchivePath (
    ON_wString& alternate_source_archive_path, bool& bRelativePath) const
{
  const ON__IDefAlternativePathUserData* ud =
      ON__IDefAlternativePathUserData::FindOrCreate (*this, false);
