...
static int
ON_WindowsBitmapHelper_PaletteColorCount (int bmiHeader_biClrUsed,
...
ON_WindowsBitmap::Bits (int scan_index)
{
  ...
  unsigned char* bits =
      m_bmi ? (unsigned char*)&m_bmi->bmiColors[PaletteColorCount()] : 0;
...
ON_WindowsBitmap::Bits (int scan_index) const
{
  ...
  const unsigned char* bits =
      m_bmi ? (const unsigned char*)&m_bmi->bmiColors[PaletteColorCount()] : 0;
  if (bits && sizeof_scan && scan_index >= 0 && scan_index < Height()) {
    bits += (sizeof_scan * scan_index);
  }
  else {
...
ON_WindowsBitmap::Pixel (int column_index, const unsigned char* scanbits) const
{
  ...
  if (m_bmi && column_index >= 0 && column_index <= Width() && scanbits &&
      !m_bmi->bmiHeader.biCompression) {
...
ON_EmbeddedBitmap::~ON_EmbeddedBitmap() { Destroy(); }
