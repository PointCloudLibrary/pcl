...
double
ON_DomainTolerance (double a, double b)
{
  if (a == b)
    return 0.0;
...
double
ON_KnotTolerance (int order, int cv_count, const double* knot, int knot_index)
{
  ...
  tol =
      (a == 0.0 && b == 0.0) ? 0.0 : (a + b + fabs(knot[knot_index])) * ON_SQRT_EPSILON;
  return tol;
}
...
double
ON_SpanTolerance (int order, int, const double* knot, int span_index)
{
...
int
ON_KnotCount (int order, int cv_count)
{
...
int
ON_KnotMultiplicity (int order,          // order (>=2)
...
int
ON_KnotVectorSpanCount (int order,         // order (>=2)
...
bool
ON_GetKnotVectorSpanVector (int order,          // order (>=2)
                            int cv_count,       // cv count
                            const double* knot, // knot[] array
                            double* s           // s[] array
)
...
)
{
  ...
  if (hint > 0 && hint < len - 1) {
    while (hint > 0 && knot[hint - 1] == knot[hint])
      hint--;
    if (hint > 0) {
...
int
ON_NextNurbsSpanIndex (int order, int cv_count, const double* knot, int span_index)
...
int
ON_GetSpanIndices (int order, int cv_count, const double* knot, int* span_indices)
...
double
ON_SuperfluousKnot (int order, int cv_count, const double* knot, int end)
{
  double k;
...
bool
ON_IsKnotVectorPeriodic (int order, int cv_count, const double* knot)
...
bool
ON_IsKnotVectorUniform (int order, int cv_count, const double* knot)
{
...
bool
ON_KnotVectorHasBezierSpans (int order,         // order (>=2)
                             int cv_count,      // cv count
                             const double* knot // knot[] array
)
{
  ...
  if (span_count < 1)
    return false;
  if (order >= 2 && cv_count >= order && knot_count == (span_count + 1) * (order - 1) &&
...
ON::knot_style
ON_KnotVectorStyle (int order, int cv_count, const double* knot)
{
  ON::knot_style s = ON::unknown_knot_style;
  if (order >= 2 && cv_count >= order && knot && knot[order - 2] < knot[cv_count - 1]) {
    const int knot_count = order + cv_count - 2;
    const double delta = 0.5 * ((knot[order - 1] - knot[order - 2]) +
                                (knot[cv_count - 1] - knot[cv_count - 2]));
...
bool
ON_SetKnotVectorDomain (int order, int cv_count, double* knot, double t0, double t1)
{
  bool rc = false;
  if (order < 2 || cv_count < order || 0 == knot || t0 >= t1 || !ON_IsValid(t0) ||
      !ON_IsValid(t1)) {
    ON_ERROR("ON_SetKnotVectorDomain - invalid input");
  }
  else if (knot[order - 2] >= knot[cv_count - 1] || !ON_IsValid(knot[order - 2]) ||
...
bool
ON_ReverseKnotVector (int order, int cv_count, double* knot)
{
  if (order < 2 || cv_count < order || knot == NULL)
    return false;
  const int knot_count = (order + cv_count - 2);
...
static bool
ON_KnotVectorIsNotValid (bool bSilentError)
{
...
bool
ON_ClampKnotVector (int order,    // order (>=2)
                    int cv_count, // cv count
...
bool
ON_MakeKnotVectorPeriodic (int order,    // order (>=2)
...
)
{
  ...
  default:
    if (cv_count < 2 * order - 2) {
      ON_ERROR(
          "ON_MakePeriodicKnotVector(): illegal input degree>=3, cv_count<2*degree");
      return false;
    }
    break;
  }
...
ON_DECL
bool
ON_MakeClampedUniformKnotVector (int order, int cv_count, double* knot, double delta)
{
  bool rc = false;
...
ON_DECL
bool
ON_MakePeriodicUniformKnotVector (int order, int cv_count, double* knot, double delta)
{
  bool rc = false;
  if (order >= 2 && cv_count >= order && knot != NULL && delta > 0.0) {
    double k = 0.0;
...
double
ON_GrevilleAbcissa (   // get Greville abcissa
    int order,         // order (>=2)
...
bool
ON_GetGrevilleAbcissae ( // get Greville abcissa from knots
    int order,           // order (>=2)
...
bool
ON_GetGrevilleKnotVector ( // get knots from Greville abcissa
    int g_stride,
  ...
  if (bPeriodic) {
    ...
    for (j = 0; j < g_count + order; j++) {
      ...
      if (half_degree) {
        ...
        if (fabs(k - p[j + half_degree]) <=
            ON_SQRT_EPSILON * (p[j + degree - 1] - p[j]))
          k = p[j + half_degree];
      }
      p[j] = k;
    }
...
)
{
  ...
  if (order < 2 || !knot || knot_value < knot[order - 2] ||
      knot[order - 1] <= knot_value) {
...
)
{
  ...
  int span_index =
      ON_NurbsSpanIndex(order, cv_count, knot, knot_value, 1, hint ? *hint : 0);
  knot += span_index;
  if (cv)
  ...
  if (span_index == cv_count - order) {
    if (knot_value > knot[order - 2] &&
        knot_value >= knot[order - 1] - knot_tolerance) {
  ...
  if (knot_value <= 0.5 * (knot[order - 2] + knot[order - 1]) &&
      fabs(knot_value - knot[order - 2]) <= knot_tolerance) {
  ...
  double* new_knot =
      (double*)onmalloc(((2 * degree + m) + (order + m) * cv_dim) * sizeof(*new_knot));
  if (!new_knot) {
