...
ON_BOOL32
ON_BrepEdge::Write (ON_BinaryArchive& file) const
{
  ON_BOOL32 rc = file.WriteInt (m_edge_index);
  if (rc)
    rc = file.WriteInt (m_c3i);
...
ON_BOOL32
ON_BrepEdge::Read (ON_BinaryArchive& file)
{
  int bReversed = false;
  ON_Interval proxy_domain;
  ON_BOOL32 rc = file.ReadInt (&m_edge_index);
  if (rc)
    rc = file.ReadInt (&m_c3i);
  if (rc)
    rc = file.ReadInt (&bReversed);
  if (rc)
    rc = file.ReadInterval (proxy_domain);
  if (rc)
...
ON_BOOL32
ON_BrepTrim::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_BrepTrim::Read (ON_BinaryArchive& file)
{
  ON_3dPoint P[2];
  int i;
...
ON_BOOL32
ON_BrepLoop::Write (ON_BinaryArchive& file) const
{
  int i;
...
ON_BOOL32
ON_BrepLoop::Read (ON_BinaryArchive& file)
{
  int i;
...
ON_BOOL32
ON_BrepFace::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_BrepFace::Read (ON_BinaryArchive& file)
{
  int i;
...
ON_BOOL32
ON_BrepVertexArray::Read (ON_BinaryArchive& file)
{
  Empty();
  ON__UINT32 tcode = 0;
...
ON_BOOL32
ON_BrepVertexArray::Write (ON_BinaryArchive& file) const
{
  int i;
...
ON_BOOL32
ON_BrepEdgeArray::Read (ON_BinaryArchive& file)
{
  Empty();
  ON__UINT32 tcode = 0;
  ON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
...
ON_BOOL32
ON_BrepEdgeArray::Write (ON_BinaryArchive& file) const
{
  int i;
  ON_BOOL32 rc = file.BeginWrite3dmChunk (TCODE_ANONYMOUS_CHUNK, 0);
  if (rc) {
...
ON_BOOL32
ON_BrepTrimArray::Read (ON_BinaryArchive& file)
{
  Empty();
  ON__UINT32 tcode = 0;
  ON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
...
ON_BOOL32
ON_BrepTrimArray::Write (ON_BinaryArchive& file) const
{
  int i;
  ON_BOOL32 rc = file.BeginWrite3dmChunk (TCODE_ANONYMOUS_CHUNK, 0);
  if (rc) {
...
ON_BOOL32
ON_BrepLoopArray::Read (ON_BinaryArchive& file)
{
  Empty();
  ON__UINT32 tcode = 0;
  ON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
...
ON_BOOL32
ON_BrepLoopArray::Write (ON_BinaryArchive& file) const
{
  int i;
  ON_BOOL32 rc = file.BeginWrite3dmChunk (TCODE_ANONYMOUS_CHUNK, 0);
  if (rc) {
...
ON_BOOL32
ON_BrepFaceArray::Read (ON_BinaryArchive& file)
{
  Empty();
  ON__UINT32 tcode = 0;
  ON__INT64 length_TCODE_ANONYMOUS_CHUNK = 0;
...
ON_BOOL32
ON_BrepFaceArray::Write (ON_BinaryArchive& file) const
{
  int i;
  ON_BOOL32 rc = file.BeginWrite3dmChunk (TCODE_ANONYMOUS_CHUNK, 0);
  if (rc) {
...
ON_BOOL32
ON_Brep::Write (ON_BinaryArchive& file) const
{
  ...
  // 2d curves
  if (rc)
    rc = brep->m_C2.Write (file);

  // 3d curves
  if (rc)
    rc = brep->m_C3.Write (file);

  // untrimmed surfaces
  if (rc)
    rc = brep->m_S.Write (file);

  // vertices
  if (rc)
    rc = brep->m_V.Write (file);

  // edges
  if (rc)
    rc = brep->m_E.Write (file);

  // trims
  if (rc)
    rc = brep->m_T.Write (file);

  // loops
  if (rc)
    rc = brep->m_L.Write (file);

  // faces
  if (rc)
    rc = brep->m_F.Write (file);

  // bounding box
  if (rc)
    rc = file.WritePoint (brep->m_bbox.m_min);
  if (rc)
    rc = file.WritePoint (brep->m_bbox.m_max);
  ...
  if (rc) {
    ...
    if (rc) {
      for (fi = 0; rc && fi < face_count; fi++) {
        const ON_Mesh* mesh =
            file.Save3dmRenderMeshes() ? brep->m_F[fi].m_render_mesh : 0;
        b = mesh ? 1 : 0;
        rc = file.WriteChar (b);
        if (rc && mesh) {
    ...
    if (rc) {
      for (fi = 0; rc && fi < face_count; fi++) {
        const ON_Mesh* mesh =
            file.Save3dmAnalysisMeshes() ? brep->m_F[fi].m_analysis_mesh : 0;
        b = mesh ? 1 : 0;
        rc = file.WriteChar (b);
        if (rc && mesh) {
          rc = file.WriteObject (*mesh);
...
static void
ReadFillInMissingBoxes (ON_Brep& brep)
{
...
ON_BOOL32
ON_Brep::Read (ON_BinaryArchive& file)
{
  int i;
  int C2_count = 0;
  int C3_count = 0;
...
bool
ON_Brep::ReadOld200 (ON_BinaryArchive& file, int minor_version)
{
  bool rc = true;
  ...
  if (rc)
    rc = file.ReadInt (&face_count);
  if (rc)
    rc = file.ReadInt (&edge_count);
  if (rc)
    rc = file.ReadInt (&loop_count);
  if (rc)
    rc = file.ReadInt (&trim_count);

  if (face_count < 1 || edge_count < 1 || loop_count < 1 || trim_count < 1)
    rc = false;

  if (rc)
    rc = file.ReadInt (&outer_flag);
  if (rc)
    rc = file.ReadPoint (m_bbox.m_min);
  if (rc)
    rc = file.ReadPoint (m_bbox.m_max);
  ...
  for (fi = 0; rc && fi < face_count; fi++) {
    ...
    if (rc)
      rc = file.ReadInt (&i); // legacy face index
    if (rc)
      rc = file.ReadInt (&i); // OBSOLETE f.m_material_index
    int k = f.m_bRev;
    if (rc)
      rc = file.ReadInt (&k);
    if (rc)
      f.m_bRev = (k != 0);
    if (rc)
      rc = file.ReadInt (&ftype_flag);
    if (rc)
      rc = file.ReadPoint (f.m_bbox.m_min);
    if (rc)
      rc = file.ReadPoint (f.m_bbox.m_max);
    if (rc)
      rc = file.ReadInt (&fbcnt);
    if (fbcnt < 1)
      rc = false;
    for (fbi = 0; rc && fbi < fbcnt; fbi++) {
      ...
      if (rc)
        rc = file.ReadInt (&i); // legacy loop index
      if (rc)
        rc = file.ReadInt (&btype_flag);
      switch (btype_flag) {
      case 0:
      ...
      if (rc)
        rc = file.ReadDouble (2, &bnd_2d_bbox.m_min.x);
      if (rc)
        rc = file.ReadDouble (2, &bnd_2d_bbox.m_max.x);
      btcnt = 0;
      if (rc)
        rc = file.ReadInt (&btcnt);
      if (btcnt < 1)
        rc = false;
      ...
      for (bti = 0; rc && bti < btcnt; bti++) {
        ...
        if (rc)
          rc = file.ReadInt (&i); // legacy trim index
        ...
        if (rc)
          rc = file.ReadInt (&twin_index);
        te_twin_index.Append (twin_index);
        b = 0;
        if (rc)
          rc = file.ReadChar (&b); // true if legacy trim managed 3d edge
        if (rc)
          rc = file.ReadInt (&trim.m_ei);
        if (b) {
        ...
        if (rc)
          trim.m_bRev3d = (k != 0);
        if (rc)
          rc = file.ReadInt (&gcon_flag);
        if (rc)
          rc = file.ReadInt (&mono_flag);
        if (rc)
          rc = file.ReadDouble (&trim.m__legacy_3d_tol);
        if (rc)
          rc = file.ReadDouble (&trim.m__legacy_2d_tol);
      }
    }
  }
...
bool
ON_Brep::ReadOld100 (ON_BinaryArchive& file)
{
...
bool
ON_Brep::ReadOld101 (ON_BinaryArchive& file)
{
  ON_Object* pO = NULL;
  ON_Curve* pC = NULL;
  ON_Surface* pS = NULL;
...
ON_Curve*
ON_Brep::Read100_BrepCurve (ON_BinaryArchive&) const
{
...
ON_Surface*
ON_Brep::Read100_BrepSurface (ON_BinaryArchive&) const
{
