...
ON_Workspace::~ON_Workspace() { Destroy(); }
...
void*
ON_Workspace::GetMemory (std::size_t size)
{
  void* p = NULL;
  if (size > 0) {
    struct ON_Workspace_MBLK* pBlk =
        (struct ON_Workspace_MBLK*)onmalloc (sizeof (*pBlk));
    if (pBlk) {
...
void*
ON_Workspace::GrowMemory (void* p, std::size_t size)
{
  void* newp = NULL;
  if (!p) {
    newp = GetMemory (size);
...
int
ON_Workspace::KeepMemory (void* p)
{
  int rc = false;
  if (p) {
...
int*
ON_Workspace::GetIntMemory (std::size_t size)
{
...
double*
ON_Workspace::GetDoubleMemory (std::size_t size)
{
...
ON_3dPoint*
ON_Workspace::GetPointMemory (std::size_t size)
{
...
ON_3dVector*
ON_Workspace::GetVectorMemory (std::size_t size)
{
...
int**
ON_Workspace::GetIntMemory (std::size_t row_count, std::size_t col_count)
{
  int** p = 0;
  std::size_t i;
...
double**
ON_Workspace::GetDoubleMemory (std::size_t row_count, std::size_t col_count)
{
  double** p = 0;
  std::size_t i;
  if (row_count > 0 && col_count > 0) {
    // i keeps doubles aligned
    i = (sizeof (*p) < sizeof (**p)) ? (row_count + (row_count % 2)) : row_count;
...
int*
ON_Workspace::GrowIntMemory (int* p, std::size_t size)
{
...
double*
ON_Workspace::GrowDoubleMemory (double* p, std::size_t size)
{
...
ON_3dPoint*
ON_Workspace::GrowPointMemory (ON_3dPoint* p, std::size_t size)
{
...
ON_3dVector*
ON_Workspace::GrowVectorMemory (ON_3dVector* p, std::size_t size)
{
...
FILE*
ON_Workspace::OpenFile (const char* sFileName, const char* sMode)
{
  FILE* pFile = ON::OpenFile (sFileName, sMode);
  if (pFile) {
    struct ON_Workspace_FBLK* pFileBlk =
        (struct ON_Workspace_FBLK*)GetMemory (sizeof (*pFileBlk));
...
FILE*
ON_Workspace::OpenFile (const wchar_t* sFileName, const wchar_t* sMode)
{
  FILE* pFile = ON::OpenFile (sFileName, sMode);
  if (pFile) {
    struct ON_Workspace_FBLK* pFileBlk =
...
int
ON_Workspace::KeepFile (FILE* pFile)
{
  int rc = false;
  if (pFile) {
    struct ON_Workspace_FBLK* pFileBlk = m_pFileBlk;
