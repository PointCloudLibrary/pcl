...
bool
ON_Base32ToString (const ON_SimpleArray<unsigned char>& base32_digits,
                   ON_String& sBase32)
{
...
int
ON_CorrectBase32StringTypos (const char* sBase32, ON_String& sBase32clean)
{
  char* sClean = 0;
...
int
ON_CorrectBase32StringTypos (const wchar_t* sBase32, ON_wString& sBase32clean)
{
...
int
ON_CorrectBase32StringTypos (const char* sBase32, char* sBase32clean)
{
  char c;
  int length = 0;
  if (0 == sBase32clean)
...
int
ON_StringToBase32 (const ON_String& sBase32,
                   ON_SimpleArray<unsigned char>& base32_digits)
{
...
int
ON_StringToBase32 (const char* sBase32, unsigned char* base32_digits)
{
  char c;
  int digit_count = 0;
  if (0 == base32_digits)
...
int
ON_GetBase32Digits (const ON_SimpleArray<unsigned char>& x,
                    ON_SimpleArray<unsigned char>& base32_digits)
{
  int x_count = x.Count();
...
int
ON_GetBase32Digits (const unsigned char* x, int x_count, unsigned char* base32_digits)
{
  ...
  for (i = 0; i < x_count; i++) {
    ...
    for (mask = 128; 0 != mask; mask /= 2) {
      ...
      if (5 == bits_count) {
        base32_digits[base32_digit_count++] =
            16 * bits[0] + 8 * bits[1] + 4 * bits[2] + 2 * bits[3] + bits[4];
        bits_count = 0;
      }
    }
  }
