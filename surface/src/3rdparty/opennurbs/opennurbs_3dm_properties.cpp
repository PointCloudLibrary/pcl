...
ON_3dmRevisionHistory::ON_3dmRevisionHistory() : m_revision_count (0) { Default(); }
...
static int
ON_CompareRevisionHistoryTime (const struct tm* time0, const struct tm* time1)
{
  if (0 == time0 || 0 == time1) {
    if (0 != time0)
      return 1;
...
bool
ON_3dmRevisionHistory::CreateTimeIsSet() const
{
...
*/
bool
ON_3dmRevisionHistory::LastEditedTimeIsSet() const
{
  struct tm jan_1_1970;
  memset (&jan_1_1970, 0, sizeof (jan_1_1970));
  jan_1_1970.tm_mday = 1;  /* day of the month - [1,31] */
...
ON_BOOL32
ON_3dmRevisionHistory::IsValid() const
{
...
ON_BOOL32
ON_3dmRevisionHistory::Write (ON_BinaryArchive& file) const
{
  ON_BOOL32 rc = file.Write3dmChunkVersion (1, 0);
  if (rc)
    rc = file.WriteString (m_sCreatedBy);
  if (rc)
    rc = file.WriteTime (m_create_time);
  if (rc)
    rc = file.WriteString (m_sLastEditedBy);
  if (rc)
    rc = file.WriteTime (m_last_edit_time);
  if (rc)
    rc = file.WriteInt (m_revision_count);
  return rc;
}

ON_BOOL32
ON_3dmRevisionHistory::Read (ON_BinaryArchive& file)
{
  Default();
  int major_version = 0;
  int minor_version = 0;
  ...
  if (rc && major_version == 1) {
    ...
    if (rc)
      rc = file.ReadTime (m_create_time);
    if (rc)
      rc = file.ReadString (m_sLastEditedBy);
    if (rc)
      rc = file.ReadTime (m_last_edit_time);
    if (rc)
      rc = file.ReadInt (&m_revision_count);
  }
  return rc;
}

void
ON_3dmRevisionHistory::Dump (ON_TextLog& dump) const
{
  const wchar_t* ws = m_sCreatedBy;
  if (!ws)
    ws = L"";
  dump.Print ("Created by: %ls\n", ws);
  dump.Print ("Created on: ");
  dump.PrintTime (m_create_time);
  dump.Print ("\n");
  ...
  dump.Print ("Last edited on: ");
  dump.PrintTime (m_last_edit_time);
  dump.Print ("\n");
...
ON_3dmNotes::~ON_3dmNotes() { m_notes.Destroy(); }

ON_3dmNotes&
ON_3dmNotes::operator= (const ON_3dmNotes& src)
{
  if (this != &src) {
    m_notes = src.m_notes;
...
ON_3dmNotes::Read (ON_BinaryArchive& file)
{
  ...
  if (rc && major_version == 1) {
    ...
    if (rc)
      rc = file.ReadString (m_notes);
    if (rc)
      rc = file.ReadInt (&m_bVisible);
    if (rc)
      rc = file.ReadInt (&m_window_left);
    if (rc)
      rc = file.ReadInt (&m_window_top);
    if (rc)
      rc = file.ReadInt (&m_window_right);
    if (rc)
      rc = file.ReadInt (&m_window_bottom);
  }
  return rc;
}

ON_BOOL32
ON_3dmNotes::Write (ON_BinaryArchive& file) const
{
  ON_BOOL32 rc = file.Write3dmChunkVersion (1, 0);
  if (rc)
    rc = file.WriteInt (m_bHTML);
  if (rc)
    rc = file.WriteString (m_notes);
  if (rc)
    rc = file.WriteInt (m_bVisible);
  if (rc)
    rc = file.WriteInt (m_window_left);
  if (rc)
    rc = file.WriteInt (m_window_top);
  if (rc)
    rc = file.WriteInt (m_window_right);
  if (rc)
    rc = file.WriteInt (m_window_bottom);
  return rc;
}

void
ON_3dmNotes::Dump (ON_TextLog& dump) const
{
...
ON_3dmApplication&
ON_3dmApplication::operator= (const ON_3dmApplication& src)
{
  if (this != &src) {
    m_application_name = src.m_application_name;
    m_application_URL = src.m_application_URL;
...
void
ON_3dmApplication::Dump (ON_TextLog& dump) const
{
...
ON_BOOL32
ON_3dmApplication::IsValid() const
{
  return m_application_name.IsEmpty() ? false : true;
}

ON_BOOL32
ON_3dmApplication::Read (ON_BinaryArchive& file)
{
  int major_version = 0;
  int minor_version = 0;
  ON_BOOL32 rc = file.Read3dmChunkVersion (&major_version, &minor_version);
  if (rc)
    rc = file.ReadString (m_application_name);
  if (rc)
    rc = file.ReadString (m_application_URL);
  if (rc)
    rc = file.ReadString (m_application_details);
  return rc;
}

ON_BOOL32
ON_3dmApplication::Write (ON_BinaryArchive& file) const
{
  ON_BOOL32 rc = file.Write3dmChunkVersion (1, 0);
  if (rc)
    rc = file.WriteString (m_application_name);
  if (rc)
    rc = file.WriteString (m_application_URL);
  if (rc)
    rc = file.WriteString (m_application_details);
  return rc;
}
...
ON_3dmProperties::ON_3dmProperties() { Default(); };
...
ON_3dmProperties&
ON_3dmProperties::operator= (const ON_3dmProperties& src)
{
  if (this != &src) {
...
void
ON_SetBinaryArchiveOpenNURBSVersion (ON_BinaryArchive& file, int value)
{
  if (value >= 200012210) {
...
ON_BOOL32
ON_3dmProperties::Read (ON_BinaryArchive& file)
{
  Default();
  ...
  for (;;) {
    ...
    case TCODE_PROPERTIES_OPENNURBS_VERSION: {
      ...
      if (value > 299912319 || (value != 0 && value < 200101010)) {
        ON_ERROR ("ON_3dmProperties::Read - TCODE_PROPERTIES_OPENNURBS_VERSION corrupt "
                  "value");
        rc = false;
      }
      else {
...
ON_BOOL32
ON_3dmProperties::Write (ON_BinaryArchive& file) const
{
  ON_BOOL32 rc = true;

  // This short chunk identifies the version of OpenNURBS that was used to write this
  // file.
  rc = file.BeginWrite3dmChunk (TCODE_PROPERTIES_OPENNURBS_VERSION, ON::Version());
  if (rc)
    rc = file.EndWrite3dmChunk();

  // optional TCODE_PROPERTIES_REVISIONHISTORY chunk - file creation/revision
  // information
...
void
ON_3dmProperties::Dump (ON_TextLog& dump) const
{
