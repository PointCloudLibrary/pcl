...
static bool
ONX_IsValidNameFirstChar (wchar_t c)
{
  if (c > 127)
    return true;
  if (c < '0')
...
static bool
ONX_IsValidNameSecondChar (wchar_t c)
{
                             ...
                             ON_Material& material) const
{
  ...
  case ON::material_from_layer:
    if (attributes.m_layer_index >= 0 &&
        attributes.m_layer_index < m_layer_table.Count())
...
static bool
CheckForCRCErrors (ON_BinaryArchive& archive,
                   ONX_Model& model,
...
public:
  static int
  CompareOldIndex (const ON__CIndexPair* a, const ON__CIndexPair* b);
  static int
  CompareOldAndNewIndex (const ON__CIndexPair* a, const ON__CIndexPair* b);
  int m_old_index; // value in model.m_..._table[m_table_index].m_..._index; (Read from
                   // file)
...
public:
  ...
  */
  int
  RemapLayerIndex (int old_layer_index) const;
  ...
  */
  int
  RemapMaterialIndex (int old_material_index) const;
  ...
  */
  int
  RemapLinetypeIndex (int old_linetype_index) const;
  ...
  */
  int
  RemapGroupIndex (int old_group_index) const;
  ...
  */
  int
  RemapFontIndex (int old_font_index) const;
  ...
  */
  int
  RemapDimstyleIndex (int old_dimstyle_index) const;
  ...
  */
  int
  RemapHatchPatternIndex (int old_hatch_pattern_index) const;
  ...
  */
  int
  RemapGeometryAndObjectAttributes (ONX_Model_Object&);
  ...
  */
  int
  RemapGeometryAttributes (ON_Object*);
  ...
  */
  int
  RemapObjectAttributes (ON_3dmObjectAttributes&);
  ...
  */
  int
  RemapRenderingAttributes (ON_RenderingAttributes& ra);
...
static int
RemapIndexHelper (int old_index,
                  bool bRemapIndex,
                  int count,
                  int default_index,
...
ON__CIndexMaps::RemapRenderingAttributes(ON_RenderingAttributes& ra)
{
  ...
  for (i = ra.m_materials.Count() - 1; i >= 0; i--) {
    ...
    if (-1 == mr.m_material_backface_index ||
      ...
      if (!ON_UuidIsNil(mr.m_material_backface_id) &&
        ...
        mr.m_material_backface_id =
            m_model.m_material_table[mr.m_material_backface_index].m_material_id;
      }
    }
                 ...
                 ON_TextLog* error_log)
{
  ...
  if (0 != version) {
    if (version < 2 || version > ON_BinaryArchive::CurrentArchiveVersion() ||
...
ONX_Model::SetDocumentUserString(const wchar_t* key, const wchar_t* string_value)
{
  ...
  bool rc = false;
  if (0 != key && 0 != key[0]) {
    ON_UUID doc_userstring_id =
        ON_DocumentUserStringList::m_ON_DocumentUserStringList_class_id.Uuid();
...
static int
AuditTextureMappingTableHelper (ONX_Model& model,
                                bool bAttemptRepair,
                                int* repair_count,
                                ON_TextLog* text_log)
{
...
static int
AuditGroupTableHelper (ONX_Model& model,
                       bool bAttemptRepair,
                       int* repair_count,
                       ON_TextLog* text_log)
{
...
static int
AuditFontTableHelper (ONX_Model& model,
                      bool bAttemptRepair,
                      int* repair_count,
                      ON_TextLog* text_log)
{
...
static int
AuditDimStyleTableHelper (ONX_Model& model,
                          bool bAttemptRepair,
                          int* repair_count,
                          ON_TextLog* text_log)
{
...
static int
AuditHatchPatternTableHelper (ONX_Model& model,
                              bool bAttemptRepair,
                              int* repair_count,
                              ON_TextLog* text_log)
{
...
static int
AuditObjectAttributesHelper (ONX_Model& model,
                             ON_3dmObjectAttributes& attributes,
...
static int
AuditLightTableHelper (ONX_Model& model,
                       bool bAttemptRepair,
                       int* repair_count,
                       ON_TextLog* text_log)
{
...
static int
AuditMaterialTableHelper (ONX_Model& model,
                          bool bAttemptRepair,
                          int* repair_count,
                          ON_TextLog* text_log)
{
...
static int
AuditLinetypeTableHelper (ONX_Model& model,
                          bool bAttemptRepair,
                          int* repair_count,
                          ON_TextLog* text_log)
{
...
static int
AuditLayerTableHelper (ONX_Model& model,
                       bool bAttemptRepair,
                       int* repair_count,
                       ON_TextLog* text_log)
{
  ...
  if (count == 0 &&
      (model.m_object_table.Count() > 0 || model.m_light_table.Count() > 0))
    rc = 2;
  for (i = 0; i < count; i++) {
...
static int
AuditIdsHelper (ON_SimpleArray<ON_UuidIndex>& id_list,
...
static int
AuditObjectIdsHelper (ONX_Model& model,
                      bool bAttemptRepair,
                      int* repair_count,
                      ON_TextLog* text_log)
{
...
static int
AuditLightIdsHelper (ONX_Model& model,
                     bool bAttemptRepair,
                     int* repair_count,
                     ON_TextLog* text_log)
{
...
static int
AuditIDefIdsHelper (ONX_Model& model,
                    bool bAttemptRepair,
                    int* repair_count,
                    ON_TextLog* text_log)
{
...
static int
AuditMappingIdsHelper (ONX_Model& model,
                       bool bAttemptRepair,
                       int* repair_count,
                       ON_TextLog* text_log)
{
...
static int
AuditMaterialIdsHelper (ONX_Model& model,
                        bool bAttemptRepair,
                        int* repair_count,
                        ON_TextLog* text_log)
{
...
static int
AuditModelIdsHelper (ONX_Model& model,
                     bool bAttemptRepair,
                     int* repair_count,
                     ON_TextLog* text_log,
...
static int
AuditIDefTableHelper (ONX_Model& model,
                      bool bAttemptRepair,
                      int* repair_count,
                      ON_TextLog* text_log)
{
...
static int
AuditObjectTableHelper (ONX_Model& model,
                        bool bAttemptRepair,
                        int* repair_count,
                        ON_TextLog* text_log)
{
...
static int
AuditHistoryRecordTableHelper (ONX_Model&, bool, int*, ON_TextLog*)
{
...
static const ON_UnknownUserData*
RDKObjectUserDataHelper (const ON_UserData* objectud)
{
