...
ON_BOOL32
ON_Point::IsValid (ON_TextLog* text_log) const
{
...
void
ON_Point::Dump (ON_TextLog& dump) const
{
  dump.Print ("ON_Point: ");
  dump.Print (point);
  dump.Print ("\n");
...
ON_BOOL32
ON_Point::Write (ON_BinaryArchive& file) const
{
...
ON_BOOL32
ON_Point::Read (ON_BinaryArchive& file)
{
  int major_version = 0;
  int minor_version = 0;
...
ON::object_type
ON_Point::ObjectType() const
{
  return ON::point_object;
}

int
ON_Point::Dimension() const
{
  return 3;
}
...
bool
ON_Point::IsDeformable() const
{
  return true;
}

bool
ON_Point::MakeDeformable()
{
  return true;
}

ON_BOOL32
ON_Point::Transform (const ON_Xform& xform)
{
...
ON_BOOL32
ON_Point::SwapCoordinates (int i, int j)
{
...
ON_Point&
ON_Point::operator= (const ON_Point& src)
{
  if (this != &src) {
...
ON_Point&
ON_Point::operator= (const ON_3dPoint& pt)
{
  point = pt;
  return *this;
}

ON_Point::operator double*() { return &point.x; }

ON_Point::operator const double*() const { return &point.x; }

ON_Point::operator ON_3dPoint*() { return &point; }

ON_Point::operator const ON_3dPoint*() const { return &point; }
