...
public:
  ...
  static ON_Value*
  CreateValue (int value_type);

  void
  Report (ON_TextLog& text_log) const;
  ...
  Count () const = 0;
  virtual bool
  ReadHelper (ON_BinaryArchive&) = 0;
  virtual bool
  WriteHelper (ON_BinaryArchive&) const = 0;
  virtual bool
  ReportHelper (ON_TextLog&) const = 0;

  virtual int
  GetBools (const bool*&) const;
  virtual int
  GetInts (const int*&) const;
  virtual int
  GetColors (const ON_Color*&) const;
  virtual int
  GetDoubles (const double*&) const;
  virtual int
  Get3dPoints (const ON_3dPoint*&) const;
  virtual int
  Get3dVectors (const ON_3dVector*&) const;
  virtual int
  GetXforms (const ON_Xform*&) const;
  virtual int
  GetUuids (const ON_UUID*&) const;
  virtual int
  GetObjRefs (ON_ClassArray<ON_ObjRef>&) const;
  virtual int
  GetGeometryPointers (const ON_Geometry* const*&) const;
  virtual int
  GetStrings (ON_ClassArray<ON_wString>&) const;
  virtual int
...
public:
  ...
  Count () const;
  bool
  ReadHelper (ON_BinaryArchive&);
  bool
  WriteHelper (ON_BinaryArchive&) const;
  bool
...
ON_Value*
ON_DummyValue::Duplicate() const
{
  return 0;
}
int
ON_DummyValue::Count() const
{
  return 0;
}
bool
...
public:
  ...
  // virtual
  bool
  ReadHelper (ON_BinaryArchive& archive);

  // virtual
  bool
  WriteHelper (ON_BinaryArchive& archive) const;

  // virtual
  bool
  ReportHelper (ON_TextLog& text_log) const;
...
///////////////////////////////////////////////////////////////////////
//
// ON_PolyEdgeHistoryValue saves geometry values in the ON_HistoryRecord::m_value[]
// array
//
...
// ON_PolyEdgeHistoryValue::ON_PolyEdgeHistoryValue(const ON_PolyEdgeHistoryValue& src)
// : ON_Value(src)
//{
//   *this = src;
// }
//
// ON_PolyEdgeHistoryValue& ON_PolyEdgeHistoryValue::operator=(const
// ON_PolyEdgeHistoryValue& src)
//{
//   if ( this != &src )
//   {
//     int i, count = m_value.Count();
...
// virtual
int
ON_PolyEdgeHistoryValue::Count() const
{
  return m_value.Count();
}
...
static int
CompareValueIdHelper (const ON_Value* a, const ON_Value* b)
{
  if (!a) {
    return b ? -1 : 0;
  }
...
static int
CompareValueId (ON_Value* const* a, ON_Value* const* b)
{
