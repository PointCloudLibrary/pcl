...
static void
ChoosePartition (ON_RTreePartitionVars* a_parVars, int a_minFill);
static void
CountRec (ON_RTreeNode* a_node, int& a_count);
static void
PickSeeds (ON_RTreePartitionVars* a_parVars);
static void
InitParVars (ON_RTreePartitionVars* a_parVars, int a_maxRects, int a_minFill);
static void
GetBranches (ON_RTreeNode* a_node,
             ON_RTreeBranch* a_branch,
             ON_RTreePartitionVars* a_parVars);
static int
PickBranch (ON_RTreeBBox* a_rect, ON_RTreeNode* a_node);
static void
DisconnectBranch (ON_RTreeNode* a_node, int a_index);
static ON_RTreeBBox
NodeCover (ON_RTreeNode* a_node);
static void
InitRect (ON_RTreeBBox* a_rect);
static ON_RTreeBBox
CombineRectHelper (const ON_RTreeBBox* a_rectA, const ON_RTreeBBox* a_rectB);
static double
CalcRectVolumeHelper (const ON_RTreeBBox* a_rect);
static bool
OverlapHelper (const ON_RTreeBBox* a_rectA, const ON_RTreeBBox* a_rectB);
static double
DistanceToCapsuleAxisHelper (const struct ON_RTreeCapsule* a_capsule,
                             const ON_RTreeBBox* a_rect);
static void
ClassifyHelper (int a_index, int a_group, struct ON_RTreePartitionVars* a_parVars);
static bool
SearchHelper (const ON_RTreeNode* a_node,
              ON_RTreeBBox* a_rect,
...
static std::size_t
MemPoolBlkSize (std::size_t leaf_count)
{
...
std::size_t
ON_RTreeMemPool::SizeOf() const
{
  return m_sizeof_heap;
}

std::size_t
ON_RTreeMemPool::SizeOfUnusedBuffer() const
{
...
ON_RTreeIterator::Value() const
{
  return (0 != m_sp) ? &m_sp->m_node->m_branch[m_sp->m_branchIndex] : 0;
}
...
ON_RTree::CreateMeshFaceTree(const ON_Mesh* mesh)
{
  ...
  meshdV =
      mesh->HasDoublePrecisionVertices() ? mesh->DoublePrecisionVertices().Array() : 0;
...
#pragma warning(push)
// Disable warning C4311: 'type cast' : pointer truncation from 'void *' to
// 'ON__INT_PTR'
...
static bool
PairSearchOverlapHelper (const ON_RTreeBBox* a_rectA,
...
static void
PairSearchHelper (const ON_RTreeBranch* a_branchA,
...
static void
PairSearchHelper (const ON_RTreeNode* a_nodeA,
...
static bool
PairSearchHelperBool (const ON_RTreeBranch* a_branchA,
                      const ON_RTreeNode* a_nodeB,
...
static bool
PairSearchHelperBool (const ON_RTreeNode* a_nodeA,
                      const ON_RTreeBranch* a_branchB,
                      ON_RTreePairSearchCallbackResultBool* a_result)
{
  // DO NOT ADD ANYTHING TO THIS FUNCTION
...
std::size_t
ON_RTree::SizeOf() const
{
  return m_mem_pool.SizeOf();
}

static void
NodeCountHelper (const ON_RTreeNode* node,
                 std::size_t& node_count,
                 std::size_t& wasted_branch_count,
  ...
  if (a_node->m_level > a_level) {
    ...
    else // Child was split
    {
      ...
      return AddBranch(&branch, a_node, a_newNode);
           ...
           a_level) // Have reached level for insertion. Add rect, split if necessary
  {
...
#if defined(ON_COMPILER_MSC) && 4 == ON_SIZEOF_POINTER
#pragma warning(push)
// Disable warning C4312: 'type cast' : conversion from 'ON__INT_PTR' to 'ON_RTreeNode
// *' of greater size
...
void
ClassifyHelper (int a_index, int a_group, ON_RTreePartitionVars* a_parVars)
{
...
// static bool OverlapHelper(const struct ON_RTreeSphere* a_sphere, const ON_RTreeBBox*
// a_rect)
//{
//   double d[3], t, r;
//   const double* mn;
...
static double
DistanceToBoxHelper (const double* pt, double r, const ON_RTreeBBox* a_rect)
{
...
static bool
OverlapBoundedPlaneXYZHelper (const double* a_bounded_plane, const ON_RTreeBBox* a_rect)
{
...
static bool
SearchBoundedPlaneXYZHelper (const ON_RTreeNode* a_node,
...
#if defined(ON_COMPILER_MSC) && 4 == ON_SIZEOF_POINTER
#pragma warning(push)
// Disable warning C4312: 'type cast' : conversion from 'const ON__INT_PTR' to 'void *'
// of greater size
#pragma warning(disable : 4312)
#endif
