...
ON_UUID
ON_UserData::UserDataClassUuid() const
{
...
ON_BOOL32
ON_UserData::IsUnknownUserData() const
{
  return (ClassId() == &ON_UnknownUserData::m_ON_UnknownUserData_class_id) ? true
                                                                           : false;
}
...
ON_BOOL32
ON_UnknownUserData::Archive() const
{
...
std::size_t
ON_UnknownUserDataArchive::CurrentPosition() const
{
...
bool
ON_UnknownUserDataArchive::AtEnd() const
{
...
bool
ON_UnknownUserDataArchive::Flush()
{
  // ON_UnknownUserDataArchive does not support Write() and Flush()
  return false;
}
...
ON_UserData*
ON_UnknownUserData::Convert() const
{
  ON_UserData* ud = NULL;
  if (IsValid()) {
...
static int
cmp_hash_2dex_ij (const void* a, const void* b)
{
