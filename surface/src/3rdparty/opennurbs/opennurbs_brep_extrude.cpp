                                ...
                                ON_SimpleArray<int>& side_face_index)
{
  ...
  side_face_index.Reserve (
      side_face_index.Count() +
      loop_trim_count); // index of new face above brep.m_L[loop_index].m_ti[lti]
  ...
  for (lti = 0; lti < loop_trim_count; lti++) {
    ...
    // get side surface for new face
    {
      ...
      if (trim.m_ei >= 0 && trim.m_ei < edge_count0) {
        ...
        sum_srf =
            ON_BrepExtrudeHelper_MakeSumSrf (path_curve, base_edge, trim.m_bRev3d);
      }
    }
    if (!sum_srf)
      continue;
    ...
    if (prev_face_index >= 0) {
      const ON_BrepTrim& prev_west_trim =
          brep.m_T[brep.m_L[brep.m_F[prev_face_index].m_li[0]].m_ti[3]];
    ...
    if (first_face_east_trim_index >= 0 &&
        brep.m_T[first_face_east_trim_index].m_vi[0] == vid[0]) {
...
static bool
ON_BrepExtrudeHelper_CheckPathCurve (const ON_Curve& path_curve,
...
)
{
  ...
  for (lti = 0; lti < loop_trim_count; lti++) {
    if (side_face_index[lti] >= 0) {
      ...
      if (side_face_index[lti_prev] < 0 && side_face_index[lti_next] < 0 &&
          top_vertex_index[lti] < 0 && top_vertex_index[lti_next] < 0) {
        ...
        if (bottom_ti >= 0 && bottom_ti < brep.m_T.Count() && bottom_ti_prev >= 0 &&
            bottom_ti_prev < brep.m_T.Count() && bottom_ti_next >= 0 &&
            bottom_ti_next < brep.m_T.Count()) {
          ...
          if (ON_BrepTrim::seam == bottom_trim_prev.m_type &&
            ...
            if (vi >= 0 && vi < brep.m_V.Count()) {
              ON_BrepVertex& top_vertex =
                  brep.NewVertex (brep.m_V[vi].point + path_vector, 0.0);
  ...
  for (lti = 0; lti < loop_trim_count; lti++) {
    ...
    else if (bottom_trim.m_type != ON_BrepTrim::singular && top_edge_index[lti] >= 0 &&
             top_edge_index[lti] < brep.m_E.Count()) {
      ON_BrepEdge& top_edge = brep.m_E[top_edge_index[lti]];
      top_trim_index =
          brep.NewTrim (top_edge, top_trim_bRev3d[lti], top_loop, top_c2i).m_trim_index;
    }
    else {
...
static bool
ON_BrepExtrudeHelper_CheckLoop (const ON_Brep& brep, int loop_index)
{
  bool rc = false;
  if (loop_index >= 0) {
                              ...
                              const int* side_face_index)
{
  ...
  if (bCap) {
    ...
    if (bottom_loop.m_type == ON_BrepLoop::inner) {
      ...
      // top_loop.m_type = ON_BrepLoop::outer; // done in
      // ON_BrepExtrudeHelper_MakeTopLoop
...
{
  ON_SimpleArray<int>
      side_face_index; // index of new face above brep.m_L[loop_index].m_ti[lti]
  ON_3dVector path_vector;
  ...
  if (brep.m_L[loop_index].m_type != ON_BrepLoop::outer &&
      brep.m_L[loop_index].m_type != ON_BrepLoop::inner)
    bCap = false;
...
ON_BrepExtrudeEdge (ON_Brep& brep, int edge_index, const ON_Curve& path_curve)
{
  ...
  ON_SumSurface* sum_srf =
      ON_BrepExtrudeHelper_MakeSumSrf (path_curve, brep.m_E[edge_index], bRev);
...
bool
ON_BrepExtrude (ON_Brep& brep, const ON_Curve& path_curve, bool bCap)
{
  ON_Workspace ws;
  ...
  if (bCap) {
    for (vi = 0; vi < vcount0; vi++) {
      ...
      ON_BrepVertex& topv =
          brep.NewVertex (bottomv.point + height, bottomv.m_tolerance);
      ...
      if (bSideEdge[ei] && topeimap[ei]) {
        ...
        if (sideveimap[vid[0]] && sideveimap[vid[1]]) {
          ...
          if (!face) {
            ...
            bool bBottomFaceRev =
                brep.m_F[(loop0.m_fi != face->m_face_index) ? loop0.m_fi : loop1.m_fi]
                    .m_bRev;
            bool bSideFaceRev =
                (trim0.m_bRev3d != trim1.m_bRev3d) ? bBottomFaceRev : !bBottomFaceRev;
...
ON_BrepConeEdge (ON_Brep& brep, int edge_index, ON_3dPoint apex_point)
{
  ...
  ON_NurbsSurface* cone_srf =
      ON_BrepExtrudeHelper_MakeConeSrf (apex_point, brep.m_E[edge_index], false);
