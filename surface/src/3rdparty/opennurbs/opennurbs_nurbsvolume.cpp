...
ON_BOOL32
ON_NurbsCage::Read(ON_BinaryArchive& archive)
{
  Destroy();
...
ON_BOOL32
ON_NurbsCage::Write(ON_BinaryArchive& archive) const
{
...
ON_NurbsCage::ON_NurbsCage() : m_dim(0), m_is_rat(0), m_cv_capacity(0), m_cv(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_order[2] = 0;
...
bool
ON_NurbsCage::IsParallelogram(double tolerance) const
{
  int i, j, k;
  double r, s, t;
  double x, y, z, dist;
...
ON_NurbsCage::~ON_NurbsCage() { Destroy(); }

int
ON_NurbsCage::KnotCount(int dir) const
{
  return (dir >= 0 && dir <= 2) ? ON_KnotCount(m_order[dir], m_cv_count[dir]) : 0;
}

double
ON_NurbsCage::GrevilleAbcissa(int dir,   // dir
                              int gindex // index (0 <= index < CVCount(dir)
) const
{
  return (dir >= 0 && dir <= 2) ? ON_GrevilleAbcissa(m_order[dir], m_knot[dir] + gindex)
                                : ON_UNSET_VALUE;
}

bool
ON_NurbsCage::MakeDeformable()
{
  return true;
}

bool
ON_NurbsCage::IsDeformable() const
{
  return true;
}
...
int
ON_NurbsCage::CVCount() const
{
  return CVCount(0) * CVCount(1) * CVCount(2);
}

int
ON_NurbsCage::CVCount(int dir) const
{
  return (dir >= 0 && dir <= 2) ? m_cv_count[dir] : 0;
}

void
ON_NurbsCage::DestroyRuntimeCache(bool bDelete)
{
  ON_Geometry::DestroyRuntimeCache(bDelete);
}

ON::object_type
ON_NurbsCage::ObjectType() const
{
...
unsigned int
ON_NurbsCage::SizeOf() const
{
  unsigned int sz = ON_Geometry::SizeOf();
  sz += (sizeof(*this) - sizeof(ON_Geometry));
  sz += (KnotCount(0) + KnotCount(1) + KnotCount(2) + CVSize() * CVCount()) *
        sizeof(double);
  return sz;
}

ON__UINT32
ON_NurbsCage::DataCRC(ON__UINT32 current_remainder) const
{
  ...
  current_remainder =
      ON_CRC32(current_remainder, 3 * sizeof(m_cv_count[0]), &m_cv_count[0]);
  if (m_cv_count[0] > 0 && m_cv_count[1] > 0 && m_cv_count[2] > 0 &&
      m_cv_stride[0] > 0 && m_cv_stride[1] > 0 && m_cv_stride[2] > 0 && m_cv) {
  ...
  current_remainder =
      ON_CRC32(current_remainder, KnotCount(2) * sizeof(m_knot[2][0]), m_knot[2]);

  return current_remainder;
}

ON_NurbsCage::ON_NurbsCage(const ON_NurbsCage& src)
: ON_Geometry(src), m_dim(0), m_is_rat(0), m_cv_capacity(0), m_cv(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_order[2] = 0;
...
ON_NurbsCage::operator=(const ON_NurbsCage& src)
{
  if (this != &src) {
    ...
    if (Create(src.m_dim,
      ...
      if (m_cv && src.m_cv && m_cv_stride[0] > 0 && m_cv_stride[1] > 0 &&
          m_cv_stride[2] > 0) {
...
ON_BOOL32
ON_NurbsCage::IsValid(ON_TextLog* // text_log
) const
{
  if (0 == m_cv)
    return false;
  ...
  if (m_cv_capacity > 0 &&
      m_cv_capacity < cvdim * m_cv_count[0] * m_cv_count[1] * m_cv_count[2])
    return false;
...
void
ON_NurbsCage::Dump(ON_TextLog& dump) const
{
                     ...
                     int cv_count2)
{
  ...
  if (order0 < 2 || order1 < 2 || order2 < 2) {
    if (0 == dim && 0 == is_rat && 0 == order0 && 0 == order1 && 0 == order2 &&
                     ...
                     int cv_count2)
{
  ...
  if (2 == cv_count0 && 2 == cv_count1 && 2 == cv_count2) {
    ...
    double* g0 =
        (double*)onmalloc(m_cv_count[0] * m_cv_count[1] * m_cv_count[2] * sizeof(*g0));
...
ON_BOOL32
ON_NurbsCage::GetBBox( // returns true if successful
...
) const
{
  int i, j;
  bool rc =
      (0 != m_cv && m_cv_count[0] >= 2 && m_cv_count[1] >= 2 && m_cv_count[2] >= 2 &&
...
ON_BOOL32
ON_NurbsCage::Transform(const ON_Xform& xform)
{
  int i, j;
...
ON_Interval
ON_NurbsCage::Domain(int dir) const
{
  ON_Interval d;
  if (dir < 0 || dir > 2 ||
    ...
    int* hint) const
{
  ...
  const int span_index0 =
      ON_NurbsSpanIndex(m_order[0], m_cv_count[0], m_knot[0], r, side0, hint0);
  const int span_index1 =
      ON_NurbsSpanIndex(m_order[1], m_cv_count[1], m_knot[1], s, side1, hint1);
...
ON_3dPoint
ON_NurbsCage::PointAt(double r, double s, double t) const
{
  ON_3dPoint pt;
  if (m_dim <= 3) {
    pt.x = 0.0;
...
ON_3dPoint
ON_NurbsCage::PointAt(ON_3dPoint rst) const
{
  ON_3dPoint pt;
  if (m_dim <= 3) {
    pt.x = 0.0;
...
ON_NurbsSurface*
ON_NurbsCage::IsoSurface(int dir, double c, ON_NurbsSurface* srf) const
{
  ...
  int span_index =
      ON_NurbsSpanIndex(m_order[dir], m_cv_count[dir], m_knot[dir], c, 0, 0);
  ON_NurbsCurve nurbs_curve;
  nurbs_curve.m_dim =
      cage_cvdim * m_cv_count[0] * m_cv_count[1] * m_cv_count[2] / m_cv_count[dir];
...
bool
ON_NurbsCage::IsRational() const
{
  return m_is_rat ? true : false;
}

int
ON_NurbsCage::CVSize() const
{
  return (m_is_rat && m_dim > 0) ? m_dim + 1 : m_dim;
}

int
ON_NurbsCage::Order(int dir) const
{
  return (dir >= 0 && dir <= 2) ? m_order[dir] : 0;
}

int
ON_NurbsCage::Degree(int dir) const
{
  int order = Order(dir);
...
bool
ON_NurbsCage::IsClosed(int dir) const
{
...
bool
ON_NurbsCage::IsPeriodic(int dir) const
{
...
double*
ON_NurbsCage::CV(int i, int j, int k) const
{
  ...
  if (i < 0 || i >= m_cv_count[0] || j < 0 || j >= m_cv_count[1] || k < 0 ||
      k >= m_cv_count[2]) {
  ...
  return (m_cv) ? (m_cv + i * m_cv_stride[0] + j * m_cv_stride[1] + k * m_cv_stride[2])
                : 0;
}

ON::point_style
ON_NurbsCage::CVStyle() const
{
  return m_is_rat ? ON::homogeneous_rational : ON::not_rational;
}

double
ON_NurbsCage::Weight(int i, int j, int k) const
{
  return (m_cv && m_is_rat) ? m_cv[i * m_cv_stride[0] + j * m_cv_stride[1] +
                                   k * m_cv_stride[2] + +m_dim]
                            : 1.0;
}

bool
ON_NurbsCage::SetWeight(int i, int j, int k, double w)
{
  bool rc = false;
  if (m_is_rat) {
    double* cv = CV(i, j, k);
...
bool
ON_NurbsCage::SetCV(int i, int j, int k, ON::point_style style, const double* Point)
{
  bool rc = true;
  int n;
  double w;
...
bool
ON_NurbsCage::SetCV(int i, int j, int k, const ON_3dPoint& point)
{
  bool rc = false;
  double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_NurbsCage::SetCV(int i, int j, int k, const ON_4dPoint& point)
{
  bool rc = false;
  double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_NurbsCage::GetCV(int i, int j, int k, ON::point_style style, double* Point) const
{
  const double* cv = CV(i, j, k);
  if (!cv)
    return false;
...
bool
ON_NurbsCage::GetCV(int i, int j, int k, ON_3dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_NurbsCage::GetCV(int i, int j, int k, ON_4dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i, j, k);
  if (cv) {
...
bool
ON_NurbsCage::SetKnot(int dir, int knot_index, double knot_value)
{
  bool rc;
  ...
  if (dir >= 0 && dir < 3 && 0 != m_knot[dir] && knot_index >= 0 &&
...
double
ON_NurbsCage::Knot(int dir, int knot_index) const
{
  double knot_value;
  ...
  if (!IsRational()) {
    ...
    if (m_cv_count[0] > 0 && m_cv_count[1] > 0 && m_cv_count[2] > 0 && dim > 0) {
      ...
      if (m_cv_stride[0] <= dim || m_cv_stride[1] <= dim || m_cv_stride[2] <= dim) {
        // there's not room for the weight in the existing m_cv array - adjust the
        // strides
        double* new_cv = (double*)onmalloc(m_cv_count[0] * m_cv_count[1] *
                                           m_cv_count[2] * (dim + 1) * sizeof(*new_cv));
        double* cv1 = new_cv;
...
bool
ON_NurbsCage::ReserveCVCapacity(int capacity // number of doubles to reserve
)
{
...
bool
ON_NurbsCage::ReserveKnotCapacity(int dir, int knot_capacity)
{
  bool rc = false;
    ...
    if (m_knot_capacity[dir] < knot_capacity) {
      if (m_knot[dir]) {
        if (m_knot_capacity[dir]) {
          m_knot[dir] =
              (double*)onrealloc(m_knot[dir], knot_capacity * sizeof(m_knot[dir][0]));
...
bool
ON_NurbsCage::IsSingular( // true if surface side is collapsed to a point
...
ON_GetCageXform (const ON_NurbsCage& cage, ON_Xform& cage_xform)
{
  ...
  if (cage.IsValid()) {
    ...
    ON_Xform x1;
    x1.Rotation(P000, X0, Y0, Z0, ON_origin, ON_xaxis, ON_yaxis, ON_zaxis);
...
ON_CageMorph::ON_CageMorph() { m_control = 0; }

ON_CageMorph::~ON_CageMorph() { m_control = 0; }

bool
ON_MorphControl::IsIdentity(const ON_BoundingBox& bbox) const
{
...
bool
ON_CageMorph::IsIdentity(const ON_BoundingBox& bbox) const
{
  return m_control ? m_control->IsIdentity(bbox) : true;
}

ON_MorphControl::ON_MorphControl() : m_varient(0), m_nurbs_cage0(1.0)
{
...
void
ON_MorphControl::MemoryRelocate()
{
...
ON_BOOL32
ON_MorphControl::IsValid(ON_TextLog* text_log) const
{
  ON_BOOL32 rc = false;
  switch (m_varient) {
  case 1:
...
void
ON_MorphControl::Dump(ON_TextLog& text_log) const
{
...
unsigned int
ON_MorphControl::SizeOf() const
{
  unsigned int sz = sizeof(*this) - 2 * sizeof(ON_NurbsCurve) -
                    2 * sizeof(ON_NurbsSurface) - sizeof(m_nurbs_cage);
...
ON::object_type
ON_MorphControl::ObjectType() const
{
  return ON::morph_control_object;
}

void
ON_MorphControl::DestroyRuntimeCache(bool bDelete)
{
...
int
ON_MorphControl::Dimension() const
{
  int dim = 0;
  switch (m_varient) {
  case 1:
...
ON_BOOL32
ON_MorphControl::GetBBox(double* boxmin, double* boxmax, int bGrowBox) const
{
  ON_BOOL32 rc = false;
  switch (m_varient) {
  case 1:
...
void
ON_MorphControl::ClearBoundingBox()
{}

ON_BOOL32
ON_MorphControl::Transform(const ON_Xform& xform)
{
  ON_BOOL32 rc = false;
...
ON_BOOL32
ON_MorphControl::HasBrepForm() const
{
  ON_BOOL32 rc = false;
...
ON_Brep*
ON_MorphControl::BrepForm(ON_Brep* brep) const
{
  switch (m_varient) {
  case 1:
...
bool
ON_MorphControl::IsRational() const
{
  bool rc = false;
  switch (m_varient) {
  case 1:
...
bool
ON_MorphControl::MakeNonRational()
{
  bool rc = false;
  switch (m_varient) {
  case 1:
...
int
ON_MorphControl::CVCount(int dir) const
{
  int rc = 0;
  switch (m_varient) {
  case 1:
...
int
ON_MorphControl::Order(int dir) const
{
  int rc = 0;
  switch (m_varient) {
  case 1:
...
ON_3dex
ON_MorphControl::MaxCVIndex() const
{
  ON_3dex maxdex;
...
const double*
ON_MorphControl::Knot(int dir) const
{
  const double* knot = 0;
...
const double*
ON_MorphControl::CV(ON_3dex ijk) const
{
  const double* cv = 0;
  switch (m_varient) {
  case 1:
...
double
ON_MorphControl::Weight(ON_3dex ijk) const
{
  double w = 1.0;
...
bool
ON_MorphControl::GetCageMorph(ON_CageMorph& cage_morph) const
{
...
ON_BOOL32
ON_MorphControl::Read(ON_BinaryArchive& archive)
{
  Destroy();
  int major_version = 0;
  int minor_version = 0;
...
ON_BOOL32
ON_MorphControl::Write(ON_BinaryArchive& archive) const
{
...
bool
ON_MorphControl::AddControlLocalizer(double support_distance, double falloff_distance)
{
...
bool
ON_MorphControl::AddConvexPolygonLocalizer(const ON_SimpleArray<ON_Plane>& planes,
                                           double support_distance,
                                           double falloff_distance)
{
