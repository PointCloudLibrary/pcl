...
ON_Mesh::SwapEdge_Helper (int topei, bool bTestOnly)
{
  ...
  if (tope.m_topf_count != 2 || tope.m_topvi[0] == tope.m_topvi[1] ||
      tope.m_topvi[0] < 0 || tope.m_topvi[1] < 0 || tope.m_topvi[0] >= topv_count ||
...
static int
CompareNEWVI (const ON__NEWVI* a, const ON__NEWVI* b)
{
...
ON_Mesh::CollapseEdge (int topei)
{
  ...
  if (tope.m_topf_count < 1 || tope.m_topvi[0] == tope.m_topvi[1] ||
      tope.m_topvi[0] < 0 || tope.m_topvi[1] < 0 || tope.m_topvi[0] >= topv_count ||
      tope.m_topvi[1] >= topv_count) {
    return false;
  }
      ...
      for (efi = 0; efi < e.m_topf_count; efi++) {
        ...
        if (bChangedFace) {
          if (!f.IsValid (V_count)) {
            ...
            if (f.vi[0] == f.vi[1] || f.vi[1] == f.vi[2] || f.vi[2] == f.vi[0] ||
                f.vi[2] != f.vi[3]) {
...
// static int NewIndex( int old_index, int missing_index_count, const int*
// missing_index_list )
//{
                       ...
                       ON_Mesh* input_mesh)
{
  ...
  if (bCleanMesh) {
    ...
    if (badfi.Count() > 0) {
      ...
      // Added crash protection. At this point mesh is NULL if it contained all bad
      // faces.
      if (mesh)
...
// mesh components
static bool
IsUnweldedEdge (int edgeidx, const ON_MeshTopology& Top)
{
  ...
  if (1 == Top.m_topv[edge.m_topvi[0]].m_v_count ||
      1 == Top.m_topv[edge.m_topvi[1]].m_v_count) {
    // Both ends of the edge have to have more than one mesh vertex or they are for sure
    // welded. However having more than 1 vertex at both ends does not necessarily mean
    // it is unwelded.
    return false;
  }
                   ...
                   bool bTopologicalConnections)
{
  int fi, vi, ei, facecount = FacesToCheck.Count(),
                  totalcount = SortedFaceArray.Count();
  DupFaceArray.Zero();
                                 ...
                                 ON_SimpleArray<ON_Mesh*>* components) const
{
  ...
  for (i = 1; compct >= i; i++) {
    ...
    ON_3dPointArray& pNewMesh_D =
        (0 != pMesh_D) ? pNewMesh->DoublePrecisionVertices() : bogus_D;
