...
bool
ON_BezierCurve::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                    int bGrowBox,
                                    const ON_Xform* xform) const
{
  ...
  return ON_GetPointListBoundingBox(
      m_dim, m_is_rat, m_order, m_cv_stride, m_cv, tight_bbox, bGrowBox, xform);
}
...
ON_PolynomialCurve::~ON_PolynomialCurve() { Destroy(); }
...
ON_BOOL32
ON_PolynomialCurve::Create(int dim, ON_BOOL32 is_rat, int order)
{
  ON_BOOL32 rc = true;
  if (dim > 0)
    m_dim = dim;
...
ON_PolynomialCurve&
ON_PolynomialCurve::operator=(const ON_PolynomialCurve& src)
{
  if (this != &src) {
    m_dim = src.m_dim;
...
ON_PolynomialCurve&
ON_PolynomialCurve::operator=(const ON_BezierCurve& src)
{
  int i;
  double d;
  m_dim = src.m_dim;
...
ON_BOOL32
ON_PolynomialCurve::Evaluate( // returns false if unable to evaluate
...
ON_PolynomialSurface::~ON_PolynomialSurface() { Destroy(); }
...
ON_PolynomialSurface::operator=(const ON_PolynomialSurface& src)
{
  if (this != &src) {
...
ON_PolynomialSurface&
ON_PolynomialSurface::operator=(const ON_BezierSurface& src)
{
  if (Create(src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1])) {
    // m_domain[0] = src.m_domain[0];
    // m_domain[1] = src.m_domain[1];
...
ON_BOOL32
ON_PolynomialSurface::Create(int dim, int is_rat, int order0, int order1)
{
  ON_BOOL32 rc = true;
  if (dim > 0)
    m_dim = dim;
...
ON_BOOL32
ON_PolynomialSurface::Evaluate( // returns false if unable to evaluate
    double,
...
: m_dim(0), m_is_rat(0), m_order(0), m_cv_stride(0), m_cv(0), m_cv_capacity(0)
{
...
ON_BezierCurve::~ON_BezierCurve() { Destroy(); }
...
ON_BezierCurve&
ON_BezierCurve::operator=(const ON_2dPointArray& cv)
{
...
ON_BezierCurve&
ON_BezierCurve::operator=(const ON_3dPointArray& cv)
{
...
ON_BezierCurve&
ON_BezierCurve::operator=(const ON_4dPointArray& cv)
{
...
ON_BezierCurve&
ON_BezierCurve::operator=(const ON_BezierCurve& src)
{
  int i;
  if (this != &src) {
    // m_domain = src.m_domain;
...
ON_BezierCurve&
ON_BezierCurve::operator=(const ON_PolynomialCurve& src)
{
  // m_domain = src.m_domain;
...
bool
ON_BezierCurve::Loft(const ON_3dPointArray& pt)
{
...
bool
ON_BezierCurve::Loft(int pt_dim,
                     int pt_count,
                     int pt_stride,
                     const double* pt,
                     int t_stride,
...
{
  bool rc = false;
  if (pt_dim >= 1 && pt_count >= 2 && pt_stride >= pt_dim && pt != NULL &&
      (t_stride >= 1 || t == 0)) {
    int i, j;
    ...
    M.BackSolve(ON_EPSILON, m_dim, m_order, m_cv_stride, m_cv, m_cv_stride, m_cv);
    if (rank == m_order)
      rc = true;
  }
  return rc;
...
void
ON_BezierCurve::Dump(ON_TextLog& dump) const
{
  ...
  if (!m_cv) {
    ...
    dump.PrintPointList(m_dim, m_is_rat, m_order, m_cv_stride, m_cv, "  CV");
  }
}

int
ON_BezierCurve::Dimension() const
{
  return m_dim;
}

bool
ON_BezierCurve::Create(int dim, int is_rat, int order)
{
  m_dim = (dim > 0) ? dim : 0;
  m_is_rat = is_rat ? 1 : 0;
...
void
ON_BezierCurve::EmergencyDestroy()
{
  m_cv = 0;
}

bool
ON_BezierCurve::GetBBox( // returns true if successful
    double* boxmin,      // minimum
    double* boxmax,      // maximum
    int bGrowBox) const
{
  return ON_GetPointListBoundingBox(
      m_dim, m_is_rat, m_order, m_cv_stride, m_cv, boxmin, boxmax, bGrowBox);
}

bool
ON_BezierCurve::GetBoundingBox( // returns true if successful
    ON_BoundingBox& bbox,
...
ON_BoundingBox
ON_BezierCurve::BoundingBox() const
{
  ON_BoundingBox bbox;
  GetBoundingBox(bbox);
  return bbox;
...
bool
ON_Line::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                             int bGrowBox,
                             const ON_Xform* xform) const
{
...
bool
ON_Arc::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                            int bGrowBox,
                            const ON_Xform* xform) const
{
  ...
  if (GetNurbForm(nurbs_arc)) {
    ...
    int i;
    for (i = nurbs_arc.m_order - 2; i < nurbs_arc.m_cv_count - 1;
         i++, bez_arc.m_cv += bez_arc.m_cv_stride) {
...
bool
ON_Circle::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                               int bGrowBox,
                               const ON_Xform* xform) const
{
...
bool
ON_ArcCurve::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                 int bGrowBox,
                                 const ON_Xform* xform) const
{
...
bool
ON_LineCurve::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                  int bGrowBox,
                                  const ON_Xform* xform) const
{
...
bool
ON_PolylineCurve::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                      int bGrowBox,
                                      const ON_Xform* xform) const
{
...
bool
ON_PolyCurve::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                  int bGrowBox,
                                  const ON_Xform* xform) const
{
...
bool
ON_CurveArray::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                   int bGrowBox,
                                   const ON_Xform* xform) const
{
  if (1 == m_count && m_a[0]) {
...
bool
ON_3dPointArray::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                     int bGrowBox,
                                     const ON_Xform* xform) const
{
  if (bGrowBox && !tight_bbox.IsValid()) {
  ...
  if (m_count > 0) {
    ...
    if (xform && !xform->IsIdentity()) {
      ...
      for (i = 1; i < m_count; i++) {
        P = (*xform) * m_a[i];
        if (P.x < points_bbox.m_min.x)
          points_bbox.m_min.x = P.x;
        else if (P.x > points_bbox.m_max.x)
          points_bbox.m_max.x = P.x;
        if (P.y < points_bbox.m_min.y)
          points_bbox.m_min.y = P.y;
        else if (P.y > points_bbox.m_max.y)
          points_bbox.m_max.y = P.y;
        if (P.z < points_bbox.m_min.z)
          points_bbox.m_min.z = P.z;
        else if (P.z > points_bbox.m_max.z)
          points_bbox.m_max.z = P.z;
      }
    }
    else {
...
bool
ON_PointCloud::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                                   int bGrowBox,
                                   const ON_Xform* xform) const
{
  if (bGrowBox && !tight_bbox.IsValid()) {
...
bool
ON_Mesh::GetTightBoundingBox(ON_BoundingBox& tight_bbox,
                             int bGrowBox,
                             const ON_Xform* xform) const
{
  if (bGrowBox && !tight_bbox.IsValid()) {
  ...
  if (xform && !xform->IsIdentity()) {
    ...
    if (vcount > 0) {
      ...
      for (i = 1; i < vcount; i++) {
        P = m_V[i];
        P = (*xform) * P;
        if (P.x < mesh_bbox.m_min.x)
          mesh_bbox.m_min.x = P.x;
        else if (P.x > mesh_bbox.m_max.x)
          mesh_bbox.m_max.x = P.x;
        if (P.y < mesh_bbox.m_min.y)
          mesh_bbox.m_min.y = P.y;
        else if (P.y > mesh_bbox.m_max.y)
          mesh_bbox.m_max.y = P.y;
        if (P.z < mesh_bbox.m_min.z)
          mesh_bbox.m_min.z = P.z;
        else if (P.z > mesh_bbox.m_max.z)
          mesh_bbox.m_max.z = P.z;
      }
    }
  }
...
bool
ON_BezierCurve::Transform(const ON_Xform& xform)
{
  if (0 == m_is_rat) {
    if (xform.m_xform[3][0] != 0.0 || xform.m_xform[3][1] != 0.0 ||
...
bool
ON_BezierCurve::Rotate(double sin_angle,        // sin(angle)
...
bool
ON_BezierCurve::Rotate(double angle,            // angle in radians
                       const ON_3dVector& axis, // axis of rotation
                       const ON_3dPoint& center // center of rotation
)
{
...
bool
ON_BezierCurve::Translate(const ON_3dVector& delta)
{
  ON_Xform tr;
...
bool
ON_BezierCurve::Reverse()
{
  bool rc =
      ON_ReversePointList(m_dim, m_is_rat, m_order, m_cv_stride, m_cv) ? true : false;
  // if ( rc )
  //   m_domain.Reverse();
  return rc;
}
...
bool
ON_BezierCurve::Evaluate( // returns false if unable to evaluate
    double t,             // evaluation parameter
    int der_count,        // number of derivatives (>=0)
    int v_stride,         // array stride (>=Dimension())
    double* v             // array of length stride*(ndir+1)
...
{
  return ON_EvaluateBezier(m_dim,
                           m_is_rat,
                           m_order,
                           m_cv_stride,
                           m_cv,
                           0.0,
                           1.0, // m_domain[0], m_domain[1],
                           der_count,
                           t,
                           v_stride,
                           v)
...
bool
ON_BezierCurve::GetNurbForm(ON_NurbsCurve& n) const
{
  bool rc = false;
...
bool
ON_BezierCurve::IsRational() const
{
  return m_is_rat ? true : false;
}

int
ON_BezierCurve::CVSize() const
{
  return (m_dim > 0 && m_is_rat) ? m_dim + 1 : m_dim;
}

int
ON_BezierCurve::Order() const
{
  return m_order;
}

int
ON_BezierCurve::Degree() const
{
  return m_order >= 2 ? m_order - 1 : 0;
}

double*
ON_BezierCurve::CV(int i) const
{
  return m_cv ? m_cv + (i * m_cv_stride) : 0;
}

ON::point_style
ON_BezierCurve::CVStyle() const
{
  return m_is_rat ? ON::homogeneous_rational : ON::not_rational;
}

double
ON_BezierCurve::Weight(int i) const
{
  return (m_is_rat && m_cv) ? CV(i)[m_dim] : 1.0;
}

bool
ON_BezierCurve::SetWeight(int i, double w)
{
  bool rc = false;
  if (m_is_rat) {
    double* cv = CV(i);
...
bool
ON_BezierCurve::SetCV(int i, ON::point_style style, const double* Point)
{
  bool rc = true;
  int k;
  double w;
...
bool
ON_BezierCurve::SetCV(int i, const ON_3dPoint& point)
{
  bool rc = false;
  double* cv = CV(i);
  if (cv) {
...
bool
ON_BezierCurve::SetCV(int i, const ON_4dPoint& point)
{
  bool rc = false;
  double* cv = CV(i);
  if (cv) {
...
bool
ON_BezierCurve::GetCV(int i, ON::point_style style, double* Point) const
{
...
bool
ON_BezierCurve::GetCV(int i, ON_3dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i);
  if (cv) {
...
int
ON_BezierCurve::CVCount() const
{
  return Order();
}

bool
ON_BezierCurve::MakeRational()
{
  if (!IsRational()) {
...
bool
ON_BezierCurve::MakeNonRational()
{
  if (IsRational()) {
    const int dim = Dimension();
    const int cv_count = CVCount();
...
bool
ON_BezierCurve::IncreaseDegree(int desired_degree)
{
  bool rc = false;
  if (desired_degree > 0) {
    if (desired_degree == m_order - 1)
      ...
      while (m_order <= desired_degree) {
        rc = ON_IncreaseBezierDegree(m_dim, m_is_rat, m_order, m_cv_stride, m_cv)
                 ? true
                 : false;
        if (!rc)
          break;
...
bool
ON_BezierCurve::ReserveCVCapacity(int desired_capacity)
{
  bool rc = false;
...
bool
ON_BezierCurve::ChangeDimension(int dim)
{
  ON_NurbsCurve c;
  c.m_dim = m_dim;
  c.m_is_rat = m_is_rat;
...
double
ON_BezierCurve::ControlPolygonLength() const
{
  double length = 0.0;
...
bool
ON_BezierCurve::Trim(const ON_Interval& n)
{
...
) const
{
  ...
  if (rc) {
    ...
    if (this != &left_bez) {
      if (0 == left_bez.m_cv ||
          (0 < left_bez.m_cv_capacity && left_bez.m_cv_capacity < cvdim * m_order)) {
        left_bez.Create(m_dim, m_is_rat, m_order);
      }
      else if (left_bez.m_dim != m_dim || left_bez.m_is_rat != m_is_rat ||
               left_bez.m_order != m_order || left_bez.m_cv_stride < cvdim) {
    ...
    if (this != &right_bez) {
      if (!right_bez.m_cv ||
          (0 < right_bez.m_cv_capacity && right_bez.m_cv_capacity < cvdim * m_order)) {
        right_bez.Create(m_dim, m_is_rat, m_order);
      }
      else if (right_bez.m_dim != m_dim || right_bez.m_is_rat != m_is_rat ||
               right_bez.m_order != m_order || right_bez.m_cv_stride < cvdim) {
...
ON_BezierSurface::ON_BezierSurface() : m_dim(0), m_is_rat(0), m_cv(0), m_cv_capacity(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_cv_stride[0] = 0;
...
: m_dim(0), m_is_rat(0), m_cv(0), m_cv_capacity(0)
{
  m_order[0] = 0;
  m_order[1] = 0;
  m_cv_stride[0] = 0;
...
ON_BezierSurface::~ON_BezierSurface() { Destroy(); }
...
ON_BezierSurface&
ON_BezierSurface::operator=(const ON_BezierSurface& src)
{
  if (this != &src) {
    if (Create(src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1])) {
...
ON_BezierSurface&
ON_BezierSurface::operator=(const ON_PolynomialSurface& src)
{
  if (Create(src.m_dim, src.m_is_rat, src.m_order[0], src.m_order[1])) {
...
bool
ON_BezierSurface::IsValid() const
{
  if (m_dim <= 0)
    return false;
  if (m_is_rat != 0 && m_is_rat != 1)
...
void
ON_BezierSurface::Dump(ON_TextLog& dump) const
{
  ...
  if (!m_cv) {
    ...
    for (i = 0; i < m_order[0]; i++) {
      ...
      dump.PrintPointList(
          m_dim, m_is_rat, m_order[1], m_cv_stride[1], CV(i, 0), sPreamble);
    }
  }
}

int
ON_BezierSurface::Dimension() const
{
  return m_dim;
}

bool
ON_BezierSurface::Create(int dim, ON_BOOL32 is_rat, int order0, int order1)
{
  if (m_cv_capacity < 1)
    m_cv = 0;
  m_dim = (dim > 0) ? dim : 0;
...
void
ON_BezierSurface::EmergencyDestroy()
{
  m_cv = 0;
}

bool
ON_BezierSurface::Loft(const ON_ClassArray<ON_BezierCurve>& curve_list)
{
  int i;
...
bool
ON_BezierSurface::Loft(int curve_count, const ON_BezierCurve* const* curve_list)
{
  ...
  if (curve_count >= 2 && 0 != curve_list && 0 != curve_list[0]) {
    ...
    for (i = 0; i < curve_count; i++) {
      if (curve_list[i]->m_order < 2 || curve_list[i]->m_dim < 1 ||
          0 == curve_list[i]->m_cv)
        return false;
    ...
    for (i = 0; i < curve_count; i++) {
      ...
      if (shape->m_order != shape_order || shape->m_is_rat != shape_is_rat ||
        ...
        if (temp_shape->m_dim != shape_dim || temp_shape->m_is_rat != shape_is_rat ||
    ...
    if (meta_point.Count() == curve_count * shape_cv_stride * shape_order) {
      ...
      rc = bez.Loft(shape_cv_stride * shape_dim,
                    curve_count,
                    shape_cv_stride * shape_dim,
                    meta_point.Array(),
                    1,
                    t.Array())
               ? true
...
bool
ON_BezierSurface::GetBBox( // returns true if successful
    double* boxmin,        // minimum
    double* boxmax,        // maximum
    int bGrowBox           // true means grow box
) const
...
bool
ON_BezierSurface::GetBoundingBox( // returns true if successful
    ON_BoundingBox& bbox,
    int bGrowBox // true means grow box
) const
{
...
ON_BoundingBox
ON_BezierSurface::BoundingBox() const
{
  ON_BoundingBox bbox;
...
bool
ON_BezierSurface::Transform(const ON_Xform& xform)
{
  int i;
  bool rc = (m_order[0] > 0 && m_order[1] > 0) ? true : false;
  if (rc) {
    ...
    for (i = 0; rc && i < m_order[0]; i++) {
      rc = ON_TransformPointList(
          m_dim, m_is_rat, m_order[1], m_cv_stride[1], CV(i, 0), xform);
    }
  }
  return rc;
}
...
bool
ON_BezierSurface::Rotate(double sin_angle,        // sin(angle)
                         double cos_angle,        // cos(angle)
                         const ON_3dVector& axis, // axis of rotation
                         const ON_3dPoint& center // center of rotation
)
...
bool
ON_BezierSurface::Rotate(double angle,            // angle in radians
                         const ON_3dVector& axis, // axis of rotation
                         const ON_3dPoint& center // center of rotation
)
{
...
bool
ON_BezierSurface::Translate(const ON_3dVector& delta)
{
  ON_Xform tr;
  tr.Translation(delta);
  return Transform(tr);
...
bool
ON_BezierSurface::Scale(double x)
{
  ON_Xform s;
  s.Scale(x, x, x);
  return Transform(s);
...
bool
ON_BezierSurface::Reverse(int dir)
{
  int i;
  bool rc = (m_order[0] > 0 && m_order[1] > 0) ? true : false;
  if (dir > 0) {
...
bool
ON_BezierSurface::Evaluate( // returns false if unable to evaluate
    double s,
    double t,      // evaluation parameter
    int der_count, // number of derivatives (>=0)
    int v_stride,  // array stride (>=Dimension())
...
ON_3dPoint
ON_BezierSurface::PointAt(double s, double t) const
{
  ON_3dPoint P;
...
ON_BezierCurve*
ON_BezierSurface::IsoCurve(int dir, double t, ON_BezierCurve* pCrv) const
{
  if (pCrv == NULL) {
    pCrv = new ON_BezierCurve(m_dim, m_is_rat, m_order[dir]);
  }
  else if (pCrv->m_dim != m_dim || pCrv->m_is_rat != m_is_rat ||
           pCrv->m_order != m_order[dir]) {
  ...
  ON_EvaluateBezier(
      bigdim, 0, m_order[1 - dir], stride, cv, 0.0, 1.0, 0, t, bigdim, pCrv->m_cv);
...
bool
ON_BezierSurface::GetNurbForm(ON_NurbsSurface& n) const
{
  bool rc = false;
...
bool
ON_BezierSurface::IsRational() const
{
  return m_is_rat ? true : false;
}

int
ON_BezierSurface::CVSize() const
{
  return (m_is_rat && m_dim > 0) ? m_dim + 1 : m_dim;
}

int
ON_BezierSurface::Order(int dir) const
{
  return (dir >= 0 && dir <= 1) ? m_order[dir] : 0;
}

int
ON_BezierSurface::Degree(int dir) const
{
  int order = Order(dir);
...
double*
ON_BezierSurface::CV(int i, int j) const
{
  return (m_cv) ? (m_cv + i * m_cv_stride[0] + j * m_cv_stride[1]) : NULL;
}

ON::point_style
ON_BezierSurface::CVStyle() const
{
  return m_is_rat ? ON::homogeneous_rational : ON::not_rational;
}

double
ON_BezierSurface::Weight(int i, int j) const
{
  return (m_cv && m_is_rat) ? m_cv[i * m_cv_stride[0] + j * m_cv_stride[1] + m_dim]
                            : 1.0;
}

bool
ON_BezierSurface::SetWeight(int i, int j, double w)
{
  bool rc = false;
  if (m_is_rat) {
    double* cv = CV(i, j);
...
bool
ON_BezierSurface::SetCV(int i, int j, ON::point_style style, const double* Point)
{
  bool rc = true;
  int k;
  double w;
...
bool
ON_BezierSurface::SetCV(int i, int j, const ON_3dPoint& point)
{
  bool rc = false;
  double* cv = CV(i, j);
  if (cv) {
...
bool
ON_BezierSurface::SetCV(int i, int j, const ON_4dPoint& point)
{
  bool rc = false;
  double* cv = CV(i, j);
  if (cv) {
...
bool
ON_BezierSurface::GetCV(int i, int j, ON::point_style style, double* Point) const
{
...
bool
ON_BezierSurface::GetCV(int i, int j, ON_3dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i, j);
  if (cv) {
...
bool
ON_BezierSurface::GetCV(int i, int j, ON_4dPoint& point) const
{
  bool rc = false;
  const double* cv = CV(i, j);
  if (cv) {
...
bool
ON_BezierSurface::MakeNonRational()
{
  if (IsRational()) {
    const int dim = Dimension();
    if (m_order[0] > 0 && m_order[1] > 0 && dim > 0) {
...
// Tools for managing CV and knot memory
bool
ON_BezierSurface::ReserveCVCapacity(int capacity // number of doubles to reserve
)
{
...
bool
ON_BezierSurface::Trim(int dir, const ON_Interval& domain)
{
  bool rc = false;
  ON_BezierCurve crv;
  double* cv;
...
) const
{
  ...
  if (0.0 < t && t < 1.0) {
    ...
    if (rc) {
      ...
      left_bez.m_cv_stride[dir] =
          leftcrv.m_cv_stride; // 3 March 2005 - Dale Lear changed crvdim to m_cv_stride
...
bool
ON_BezierSurface::IsSingular( // true if surface side is collapsed to a point
...
bool
ON_BezierCurve::Reparametrize(double c)
{
  return Reparameterize(c);
}

bool
ON_BezierCurve::Reparameterize(double c)
{
  if (!ON_IsValid(c) || 0.0 == c)
    return false;
...
bool
ON_BezierCurve::ScaleConrolPoints(int i, double w)
{
...
bool
ON_BezierCurve::ChangeWeights(int i0, double w0, int i1, double w1)
{
  // double r, s, v0, v1;
  double v0, v1;
  int i;
  ...
  MakeRational();
  return ON_ChangeRationalBezierCurveWeights(
      m_dim, m_order, m_cv_stride, m_cv, i0, w0, i1, w1);
...
ON_3dPoint
ON_BezierCurve::PointAt(double t) const
{
...
ON_3dVector
ON_BezierCurve::DerivativeAt(double t) const
{
  ON_3dPoint p(0.0, 0.0, 0.0);
...
ON_3dVector
ON_BezierCurve::TangentAt(double t) const
{
  ON_3dPoint point;
  ON_3dVector tangent;
...
ON_3dVector
ON_BezierCurve::CurvatureAt(double t) const
{
  ON_3dPoint point;
...
bool
ON_BezierCurve::EvTangent(double t, ON_3dPoint& point, ON_3dVector& tangent) const
{
  ON_3dVector D1, D2; //, K;
  tangent.Zero();
    ...
    if (Ev2Der(t, point, D1, D2)) {
      ...
      if (rc) {
        ...
        if (domain.IsIncreasing() &&
            ON_GetParameterTolerance(domain[0], domain[1], t, &tminus, &tplus)) {
          ON_3dPoint p;
          ON_3dVector d1, d2;
          double eps = 0.0;
...
bool
ON_BezierCurve::EvCurvature(double t,
                            ON_3dPoint& point,
                            ON_3dVector& tangent,
...
bool
ON_BezierCurve::EvPoint( // returns false if unable to evaluate
    double t,            // evaluation parameter
...
bool
ON_BezierCurve::Ev1Der( // returns false if unable to evaluate
    double t,           // evaluation parameter
    ON_3dPoint& point,
    ON_3dVector& derivative) const
{
  bool rc = false;
  const int dim = Dimension();
  double ws[2 * 64];
...
bool
ON_BezierCurve::Ev2Der( // returns false if unable to evaluate
    double t,           // evaluation parameter
    ON_3dPoint& point,
    ON_3dVector& firstDervative,
    ON_3dVector& secondDervative) const
{
  bool rc = false;
  const int dim = Dimension();
  double ws[3 * 64];
