...
double
ON_ArrayDistance (int dim, const double* A, const double* B)
{
...
double
ON_ArrayDistanceSquared (int dim, const double* A, const double* B)
{
  // returns (A-B)o(A-B)
...
double
ON_ArrayMagnitude (int dim, const double* A)
{
  double a, b, c, len;
  switch (dim) {
  case 1:
...
double
ON_ArrayMagnitudeSquared (int dim, const double* A)
{
  double x, len_sq = 0.0;
  while (dim--) {
    x = *A++;
...
ON_BOOL32
ON_EvJacobian (double ds_o_ds, double ds_o_dt, double dt_o_dt, double* det_addr)
...
)
{
  ...
  if (rc) {
    if (t < t0)
      ...
      t = t1;
    double dt =
        (t1 - t0) * 8.0 * ON_SQRT_EPSILON + (fabs (t0) + fabs (t1)) * ON_EPSILON;
    if (dt >= t1 - t0)
      dt = 0.5 * (t1 - t0);
...
bool
ON_EvTangent (const ON_3dVector& D1, // first derivative
...
ON_BOOL32
ON_EvCurvature (const ON_3dVector& D1, // first derivative
                const ON_3dVector& D2, // second derivative
                ON_3dVector& T,        // Unit tangent returned here
...

{
  ...
  i0 = 0;
  i1 = length;
  while (array[i0] == array[i0 + 1])
    i0++;
  while (array[i1] == array[i1 - 1])
    i1--;
  /* From now on we have
...
ON_BOOL32
ON_IsValidPointList (int dim, int is_rat, int count, int stride, const float* p)
{
  return (dim > 0 && stride >= (is_rat ? (dim + 1) : dim) && count >= 0 && p != NULL)
             ? true
             : false;
}

ON_BOOL32
ON_IsValidPointList (int dim, int is_rat, int count, int stride, const double* p)
{
  return (dim > 0 && stride >= (is_rat ? (dim + 1) : dim) && count >= 0 && p != NULL)
             ? true
             : false;
...
bool
ON_ReversePointList (int dim, int is_rat, int count, int stride, double* p)
{
...
bool
ON_SwapPointListCoordinates (int count, int stride, float* p, int i, int j)
{
  float t;
  int k;
...
bool
ON_SwapPointListCoordinates (int count, int stride, double* p, int i, int j)
{
  double t;
  int k;
  if (!ON_IsValidPointList (stride, 0, count, stride, p))
    ...
    int dim, int is_rat, int count, int stride, float* point, const ON_Xform& xform)
{
  ...
  if (is_rat) {
    ...
    case 2:
      while (count--) {
        x = xform.m_xform[0][0] * point[0] + xform.m_xform[0][1] * point[1] +
            xform.m_xform[0][3] * point[2];
        y = xform.m_xform[1][0] * point[0] + xform.m_xform[1][1] * point[1] +
            xform.m_xform[1][3] * point[2];
        w = xform.m_xform[3][0] * point[0] + xform.m_xform[3][1] * point[1] +
            xform.m_xform[3][3] * point[2];
        point[0] = (float)x;
        point[1] = (float)y;
        point[2] = (float)w;
        point += stride;
    ...
    default: // dim >= 3
      while (count--) {
        x = xform.m_xform[0][0] * point[0] + xform.m_xform[0][1] * point[1] +
            xform.m_xform[0][2] * point[2] + xform.m_xform[0][3] * point[dim];
        y = xform.m_xform[1][0] * point[0] + xform.m_xform[1][1] * point[1] +
            xform.m_xform[1][2] * point[2] + xform.m_xform[1][3] * point[dim];
        z = xform.m_xform[2][0] * point[0] + xform.m_xform[2][1] * point[1] +
            xform.m_xform[2][2] * point[2] + xform.m_xform[2][3] * point[dim];
        w = xform.m_xform[3][0] * point[0] + xform.m_xform[3][1] * point[1] +
            xform.m_xform[3][2] * point[2] + xform.m_xform[3][3] * point[dim];
        point[0] = (float)x;
        point[1] = (float)y;
        point[2] = (float)z;
        point[dim] = (float)w;
    ...
    default: // dim = 3
      while (count--) {
        w = xform.m_xform[3][0] * point[0] + xform.m_xform[3][1] * point[1] +
            xform.m_xform[3][2] * point[2] + xform.m_xform[3][3];
        if (w == 0.0) {
          rc = false;
          w = 1.0;
        }
          ...
          w = 1.0 / w;
        x = xform.m_xform[0][0] * point[0] + xform.m_xform[0][1] * point[1] +
            xform.m_xform[0][2] * point[2] + xform.m_xform[0][3];
        y = xform.m_xform[1][0] * point[0] + xform.m_xform[1][1] * point[1] +
            xform.m_xform[1][2] * point[2] + xform.m_xform[1][3];
        z = xform.m_xform[2][0] * point[0] + xform.m_xform[2][1] * point[1] +
            xform.m_xform[2][2] * point[2] + xform.m_xform[2][3];
        point[0] = (float)(w * x);
        point[1] = (float)(w * y);
        point[2] = (float)(w * z);
        point += stride;
    ...
    int dim, int count, int stride, float* vector, const ON_Xform& xform)
{
  ...
  default: // dim >= 3
    while (count--) {
      x = xform.m_xform[0][0] * vector[0] + xform.m_xform[0][1] * vector[1] +
          xform.m_xform[0][2] * vector[2];
      y = xform.m_xform[1][0] * vector[0] + xform.m_xform[1][1] * vector[1] +
          xform.m_xform[1][2] * vector[2];
      z = xform.m_xform[2][0] * vector[0] + xform.m_xform[2][1] * vector[1] +
          xform.m_xform[2][2] * vector[2];
      vector[0] = (float)x;
      vector[1] = (float)y;
      vector[2] = (float)z;
      vector += stride;
...
bool
ON_PointsAreCoincident (int dim, int is_rat, const double* pointA, const double* pointB)
{
  double d, a, b, wa, wb;
    ...
    const double* pointB)
{
  int i, rc = 0, rc1 = 0;
  bool bDoSecondCheck =
      (1 == is_rat && dim <= 3 && point_count > 0 && ON_IsValid (pointA[dim]) &&
       0.0 != pointA[dim] && ON_IsValid (pointB[dim]) && 0.0 != pointB[dim]);
  ...
  for (i = 0; i < point_count && !rc; i++) {
    ...
    if (rc && bDoSecondCheck && fabs (wA - pointA[dim]) <= wAtol &&
...
ON_BOOL32
ON_IsPointListClosed (int dim, int is_rat, int count, int stride, const double* p)
{
  ON_BOOL32 rc = false;
                      ...
                      int dir)
{
  ...
  if (point_count0 > 0 && point_count1 > 0 && p != NULL) {
    ...
    rc = (0 == ON_ComparePointList (dim, is_rat, count, stride, p0, stride, p1))
             ? true
             : false;
  }
  return rc;
...
int
ON_SolveQuadraticEquation (double a, double b, double c, double* r0, double* r1)
 ...
 */
{
  ...
  if (x0 <= y && y <= x1) {
    ...
    y0 = (a * x0 + b) * x0 + c;
    y1 = (a * x1 + b) * x1 + c;
    if (fabs (y) <= fabs (y0) || fabs (y) <= fabs (y1) || (a > 0.0 && y > 0.0) ||
        (a < 0.0 && y < 0.0)) {
      *r0 = *r1 = -0.5 * b / a;
      return 1;
    }
             ...
             double* pivot_ratio)
{
  ...
  *pivot_ratio = *x_addr = *y_addr = *z_addr = *w_addr = 0.0;
  ...
  case 1: /* swap rows 0 and 1 */
    ...
    p0[4] = d1;
    memcpy (p1, row0, sizeof_row);
    p1[4] = d0;
    memcpy (p2, row2, sizeof_row);
    p2[4] = d2;
    memcpy (p3, row3, sizeof_row);
    p3[4] = d3;
    break;
  case 2: /* swap rows 0 and 2 */
    memcpy (p0, row2, sizeof_row);
    p0[4] = d2;
    memcpy (p1, row1, sizeof_row);
    p1[4] = d1;
    memcpy (p2, row0, sizeof_row);
    p2[4] = d0;
    memcpy (p3, row3, sizeof_row);
    p3[4] = d3;
    break;
  case 3: /* swap rows 0 and 3 */
    memcpy (p0, row3, sizeof_row);
    p0[4] = d3;
    memcpy (p1, row1, sizeof_row);
    p1[4] = d1;
    memcpy (p2, row2, sizeof_row);
    p2[4] = d2;
    memcpy (p3, row0, sizeof_row);
    p3[4] = d0;
    break;
  default:
...
static int
qicompar2 (void* p, const void* a, const void* b)
{
  return ((struct tagON_SORT_CONTEXT*)p)
      ->compar2 (((struct tagON_SORT_CONTEXT*)p)->qdata + *((unsigned int*)a),
                 ((struct tagON_SORT_CONTEXT*)p)->qdata + *((unsigned int*)b));
}

static int
qicompar3 (void* p, const void* a, const void* b)
{
  return ((struct tagON_SORT_CONTEXT*)p)
      ->compar3 (((struct tagON_SORT_CONTEXT*)p)->qdata + *((unsigned int*)a),
                 ((struct tagON_SORT_CONTEXT*)p)->qdata + *((unsigned int*)b),
...
*/
{
  ...
  if (ON::quick_sort == method) {
    ...
    for (;;) {
      ...
      while (j <= icount) {
        if (j < icount &&
            context.compar2 (context.qdata + idx[j], context.qdata + idx[j + 1]) < 0) {
          j++;
        }
...
static void
ON_hsort_str (char** e, std::size_t nel)
{
  std::size_t i_end, k;
  char* e_tmp;
...
static int
compar_string (const void* pa, const void* pb)
{
...
void
ON_SortStringArray (ON::sort_algorithm method,
...
{
  double F, Fr, Fs, Ft, wr, ws, wt, wrr, wrs, wrt, wss, wst, wtt, *f, *x;
  int i, j, k, n;
...
)
{
  ...
  // return ON_WRONG_EvPrincipalCurvatures( Ds, Dt, Dss, Dst, Dtt, N, gauss, mean,
  // kappa1, kappa2, K1, K2 );
...

  {
    ...
    x = trace * trace;
    double tol =
        fabs (det) * 1.0e-12; // 31 March 2008 Dale Lear - added tol to fix 32091
    if (x < 4.0 * det - tol) {
      if (det <= ON_EPSILON) {
        k1 = k2 = 0.0;
        if (gauss)
      ...
      if (fabs (k1 - k2) > 1.0e-6 * (fabs (k1) + fabs (k2))) {
        ...
        for (ki = 0; ki < 2; ki++) {
          ...
          if (fabs (x) > 1.0e-8) {
            if (0 == ki) {
              ON_EPC_WARNING (
                  "ON_EvPrincipalCurvatures() |det(shape op - [k1,k1])| > 1.0e-8");
            }
            else {
              ON_EPC_WARNING (
                  "ON_EvPrincipalCurvatures() |det(shape op - [k2,k2])| > 1.0e-8");
            }
          }
#endif
          ...
          if (fabs (x) > 1.0e-8 || fabs (y) > 1.0e-8) {
            if (0 == ki) {
              ON_EPC_WARNING (
                  "ON_EvPrincipalCurvatures() (shape op k1 eigenvector is noisy).");
            }
            else {
              ON_EPC_WARNING (
                  "ON_EvPrincipalCurvatures() (shape op k2 eigenvector is noisy).");
            }
          }
#endif
...
OUTPUT:
   The return value is the normal curvature vector for the surface in the direction of
UnitTangent Normal curvature vector ((anti)parallel to UnitNormal). The scalar normal
curvature is equal to NormalCurvature o UnitNormal.
*****************************************************************************/
{
...
bool
ON_Evaluator::FiniteDomain() const
{
  return ((m_parameter_count == m_domain.Count() && m_parameter_count > 0) ? true
                                                                           : false);
}

bool
ON_Evaluator::Periodic (int parameter_index) const
{
...
ON_Interval
ON_Evaluator::Domain (int parameter_index) const
{
