...
public:
   ...
   * However, if the order and/or number of non-skip fields is different, the skip
   * fields are dropped and non-skip fields copied selectively. This function returns an
   * error if
   *   * the total number of non-skip fields is different
   *   * the non-skip field names are named differently (excluding "rbg{a}") in serial
   * order
