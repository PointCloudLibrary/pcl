...
template<template<class > class Distance, typename PointInT, typename FeatureT>
  ...
  pcl::rec_3d_framework::LocalRecognitionPipeline<Distance, PointInT, FeatureT>::recognize ()
  {
    ...
    transforms_.reset (new std::vector<Eigen::Matrix4f>);
...
#pragma omp parallel for num_threads(omp_get_num_procs())
    ...
    if (hv_algorithm_)
    {
      ...
      boost::shared_ptr < std::vector<Eigen::Matrix4f> > transforms_temp;

      models_temp.reset (new std::vector<ModelT>);
      transforms_temp.reset (new std::vector<Eigen::Matrix4f>);
...
template<template<class > class Distance, typename PointInT, typename FeatureT>
  ...
  pcl::rec_3d_framework::LocalRecognitionPipeline<Distance, PointInT, FeatureT>::getPose (ModelT & model, int view_id, Eigen::Matrix4f & pose_matrix)
  {
    ...
    if (use_cache_)
    {
      ...
      std::map<std::pair<std::string, int>, Eigen::Matrix4f, std::less<std::pair<std::string, int> >, Eigen::aligned_allocator<Eigen::Matrix4f> >::iterator
                                                                                                                                                            it =
                                                                                                                                                                poses_cache_.find (
                                                                                                                                                                                   pair_model_view);
