      ...
      if (strs[0] == "descriptor") {
        ...
        if (use_cache_) {
          std::string path = source_->getModelDescriptorDir (
              models->at (i), training_dir_, descr_name_);
      ...
      for (std::size_t v = 0; v < models->at (i).views_->size(); v++) {
        ...
        if (success) {
          ...
          if (v < models->at (i).self_occlusions_->size()) {
            ...
            PersistenceUtils::writeFloatToFile (
                path_entropy, models->at (i).self_occlusions_->at (v));
          }
    ...
    recognize()
{
  ...
  std::map<std::string, ObjectHypothesis> object_hypotheses;
  {
    for (std::size_t idx = 0; idx < signatures->size(); idx++) {
      ...
      if ((it_map =
               object_hypotheses.find (flann_models_.at (indices[0][0]).model.id_)) !=
          object_hypotheses.end()) {
