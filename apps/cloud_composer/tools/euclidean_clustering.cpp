    ...
    if (cloud_item->isSanitized()) {
      ...
      for (const auto& cluster : cluster_indices) {
        ...
        pcl::PointIndices::ConstPtr indices_ptr =
            pcl::make_shared<pcl::PointIndices>(cluster);
        filter.setIndices(indices_ptr);
        extracted_indices->insert(
            extracted_indices->end(), cluster.indices.begin(), cluster.indices.end());
      ...
      // We copy input cloud over for special case that no clusters found, since
      // ExtractIndices doesn't work for 0 length vectors
