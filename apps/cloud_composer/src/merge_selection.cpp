                                                   ...
                                                   PointTypeFlags::PointType type)
{
  ...
  foreach (const CloudItem* input_cloud_item, selected_item_index_map_.keys()) {
    ...
    if (!input_data.contains(input_cloud_item)) {
      ...
      pcl::PCLPointCloud2::Ptr original_minus_indices =
          pcl::make_shared<pcl::PCLPointCloud2>();
      ...
      if (!pose_found) {
        source_origin =
            input_cloud_item->data(ItemDataRole::ORIGIN).value<Eigen::Vector4f>();
        source_orientation = input_cloud_item->data(ItemDataRole::ORIENTATION)
