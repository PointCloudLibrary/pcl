...
pcl::cloud_composer::PropertiesModel::PropertiesModel(CloudComposerItem* parent_item,
                                                      QObject* parent)
: QStandardItemModel(parent), parent_item_(parent_item)
{
  setHorizontalHeaderItem(0, new QStandardItem("Name"));
  setHorizontalHeaderItem(1, new QStandardItem("Value"));
                                                  ...
                                                  const QString& category)
{
  ...
  if (category.size() > 0) {
    ...
    if (items.empty())
      qWarning() << "No category named " << prop_name << " found in "
                 << parent_item_->text() << " adding to root";
...
pcl::cloud_composer::PropertiesModel::getProperty(const QString& prop_name) const
{
  ...
  QList<QStandardItem*> items =
      findItems(prop_name, Qt::MatchExactly | Qt::MatchRecursive, 0);
  if (items.empty()) {
  ...
  // qDebug () << "Prop name="<<prop_name<<" row="<<property->row ()<<"
  // col="<<property->column();
