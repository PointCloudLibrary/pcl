...
, is_sanitized_ (false)
{
  ...
  // qDebug () << "Cloud size before passthrough :
  // "<<cloud_ptr->width<<"x"<<cloud_ptr->height;

  //  qDebug () << "Cloud size after passthrough :
  ...
  color_handler_.reset (
      new pcl::visualization::PointCloudColorHandlerRGBField<pcl::PCLPointCloud2> (
          cloud_ptr));
  this->setData (QVariant::fromValue (color_handler_), ItemDataRole::COLOR_HANDLER);
  geometry_handler_.reset (
      new pcl::visualization::PointCloudGeometryHandlerXYZ<pcl::PCLPointCloud2> (
          cloud_ptr));
  this->setData (QVariant::fromValue (geometry_handler_),
...
void
pcl::cloud_composer::CloudItem::removeFromView (
    pcl::visualization::PCLVisualizer::Ptr vis) const
{
...
{
  // Check if we're trying to get something which is template dependent, if so, create
  // the template if it hasn't been set
  if ((role == ItemDataRole::CLOUD_TEMPLATED || role == ItemDataRole::KD_TREE_SEARCH) &&
      !template_cloud_set_) {
...
pcl::cloud_composer::CloudItem::setTemplateCloudFromBlob()
{
  if (!template_cloud_set_) {
    ...
    point_type_ = PointTypeFlags::NONE;
    if (field_names.contains ("x") && field_names.contains ("y") &&
        field_names.contains ("z"))
    ...
    if (field_names.contains ("normal_x") && field_names.contains ("normal_y") &&
        field_names.contains ("normal_z")) {
      if (field_names.contains ("curvature"))
        point_type_ |= PointTypeFlags::NORMAL;
      else
        point_type_ |= PointTypeFlags::AXIS;
    }
    if (field_names.contains ("h") && field_names.contains ("s") &&
    ...
    case (PointTypeFlags::XYZ): {
      ...
      pcl::search::KdTree<PointXYZ>::Ptr kd_search =
          pcl::make_shared<search::KdTree<PointXYZ>>();
    ...
    case (PointTypeFlags::XYZ | PointTypeFlags::RGB): {
      ...
      pcl::fromPCLPointCloud2 (*cloud_blob_ptr_, *cloud_ptr);
      cloud_pointer_variant = QVariant::fromValue (cloud_ptr);
      pcl::search::KdTree<PointXYZRGB>::Ptr kd_search =
          pcl::make_shared<search::KdTree<PointXYZRGB>>();
      kd_search->setInputCloud (cloud_ptr);
      kd_tree_variant = QVariant::fromValue (kd_search);
      break;
    }
    case (PointTypeFlags::XYZ | PointTypeFlags::RGBA): {
      ...
      pcl::fromPCLPointCloud2 (*cloud_blob_ptr_, *cloud_ptr);
      cloud_pointer_variant = QVariant::fromValue (cloud_ptr);
      pcl::search::KdTree<PointXYZRGBA>::Ptr kd_search =
          pcl::make_shared<search::KdTree<PointXYZRGBA>>();
      kd_search->setInputCloud (cloud_ptr);
      kd_tree_variant = QVariant::fromValue (kd_search);
      break;
    }
