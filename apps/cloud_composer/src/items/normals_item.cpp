...
pcl::cloud_composer::NormalsItem::NormalsItem(
    QString name, const pcl::PointCloud<pcl::Normal>::Ptr& normals_ptr, double radius)
: CloudComposerItem(std::move(name)), normals_ptr_(normals_ptr)
...
{
  pcl::PointCloud<pcl::Normal>::Ptr normals_copy(
      new pcl::PointCloud<pcl::Normal>(*normals_ptr_));
...
void
pcl::cloud_composer::NormalsItem::paintView(
    pcl::visualization::PCLVisualizer::Ptr vis) const
{
  ...
  if (parent()->type() == CLOUD_ITEM) {
    ...
    pcl::PCLPointCloud2::ConstPtr cloud_blob =
        cloud_ptr.value<pcl::PCLPointCloud2::ConstPtr>();
    ...
    qDebug() << QString("Adding point cloud normals, level=%1, scale=%2")
                    .arg(level)
                    .arg(scale);
    vis->addPointCloudNormals<pcl::PointXYZ, pcl::Normal>(
        cloud, normals_ptr_, level, scale, getId().toStdString());
    std::cout << (*cloud)[0] << std::endl;
    std::cout << (*normals_ptr_)[0] << std::endl;
  }
  else
    qWarning()
        << "Normal item inserted, but parent not a cloud. Don't know how to draw that!";
}

void
pcl::cloud_composer::NormalsItem::removeFromView(
    pcl::visualization::PCLVisualizer::Ptr vis) const
{
