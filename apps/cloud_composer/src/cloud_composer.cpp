...
pcl::cloud_composer::ComposerMainWindow::connectEditActions()
{
  ...
  action_rectangular_frustum_select_->setData (
      QVariant::fromValue (interactor_styles::RECTANGULAR_FRUSTUM));
  action_manipulate_selected_->setData (
      QVariant::fromValue (interactor_styles::SELECTED_TRACKBALL));
  action_manipulate_clicked_->setData (
      QVariant::fromValue (interactor_styles::CLICK_TRACKBALL));
...
void
pcl::cloud_composer::ComposerMainWindow::setMouseStyleAction (
...
pcl::cloud_composer::ComposerMainWindow::initializeToolBox()
{
  ...
  connect (action_run_tool_, SIGNAL (clicked()), tool_box_model_, SLOT (toolAction()));
...
pcl::cloud_composer::ComposerMainWindow::initializePlugins()
{
  ...
  qDebug() << plugin_dir.path() << "   "
           << QDir::cleanPath ("../lib/cloud_composer_plugins");
#ifdef _WIN32
