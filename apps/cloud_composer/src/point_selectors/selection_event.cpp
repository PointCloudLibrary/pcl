    ...
    CloudItem* cloud_item, const pcl::PointIndices::Ptr& indices)
{
  ...
  // Go through every point in the selected data set and find the matching index in this
  // cloud pcl::search::KdTree<pcl::PointXYZ>::Ptr search = cloud_item->data
  // (KD_TREE_SEARCH).value <pcl::search::Search<pcl::PointXYZ>::Ptr> ();
  //  double dblpts[3];
  //  PointXYZ pt;
  ...
  /*for (int i = 0; i < points->GetNumberOfPoints (); ++i)
  {
    ...
    else if (indices.size () > 1)
    {
      ...
      if (idx < indices.size ())
      {
        ...
        qCritical () << "More found points in selection than are possible from cloud
  items!";
    }
