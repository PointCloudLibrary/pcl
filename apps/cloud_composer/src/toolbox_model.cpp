...
void
pcl::cloud_composer::ToolBoxModel::setSelectionModel (
...
void
pcl::cloud_composer::ToolBoxModel::activeProjectChanged (ProjectModel* new_model,
                                                         ProjectModel*)
{
...
void
pcl::cloud_composer::ToolBoxModel::selectedToolChanged (const QModelIndex& current,
                                                        const QModelIndex&)
{
...
void
pcl::cloud_composer::ToolBoxModel::selectedItemChanged (const QItemSelection& selected,
...
void
pcl::cloud_composer::ToolBoxModel::updateEnabledTools (
    const QItemSelection& current_selection)
{
    ...
    // Check if selection includes at least one item with correct input type
      ...
      disabled_tools.insert (
          ...
          tr ("Tool Requires item type %1 selected")
              .arg (ITEM_TYPES_STRINGS.value (input_type -
                                              CloudComposerItem::CLOUD_COMPOSER_ITEM)));
    }
    // Check if any of selected items have required children
    else if (!required_children_types.empty()) {
      QList<QStandardItem*> matching_selected_items =
          type_items_map.values (input_type);
      ...
      foreach (QStandardItem* item, matching_selected_items) {
        ...
        for (int i = 0; i < item->rowCount(); ++i)
          found_children_types.append (
              static_cast<CloudComposerItem::ItemType> (item->child (i)->type()));
      ...
      if (!found_valid_items) {
        ...
        foreach (CloudComposerItem::ItemType type, missing_children)
          missing_children_string.append (
              " " +
              ITEM_TYPES_STRINGS.value (type - CloudComposerItem::CLOUD_COMPOSER_ITEM));
        disabled_tools.insert (tool_item,
