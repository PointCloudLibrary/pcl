...
pcl::cloud_composer::CloudView::CloudView(QWidget* parent) : QWidget(parent)
{
  ...
  vis_->setupInteractor(
      getInteractorCompat(*qvtk_), getRenderWindowCompat(*qvtk_), style_switch_);
...
: QWidget(), vis_(to_copy.vis_), model_(to_copy.model_), qvtk_(to_copy.qvtk_)
{}
                                                     ...
                                                     int end)
{
  ...
  if (!parent.isValid())
    ...
    parent_item = model_->itemFromIndex(parent);
  // qDebug () << "Rows about to be removed, parent = "<<parent_item->text ()<<"
  // start="<<start<<" end="<<end;
  for (int row = start; row <= end; ++row) {
...
void
pcl::cloud_composer::CloudView::selectedItemChanged(const QItemSelection& selected,
                                                    const QItemSelection& deselected)
{
  ...
  foreach (QModelIndex previous, previous_indices) {
    if (previous.isValid()) {
      ...
      if (previous_item->type() == CloudComposerItem::CLOUD_ITEM ||
          previous_item->type() == CloudComposerItem::NORMALS_ITEM) {
        vis_->setPointCloudSelected(
            false, previous_item->data(ItemDataRole::ITEM_ID).toString().toStdString());
      }
    }
  }
  foreach (QModelIndex current, current_indices) {
    if (current.isValid()) {
      ...
      if (current_item->type() == CloudComposerItem::CLOUD_ITEM ||
          current_item->type() == CloudComposerItem::NORMALS_ITEM) {
        vis_->setPointCloudSelected(
            true, current_item->data(ItemDataRole::ITEM_ID).toString().toStdString());
      }
    }
  }
  refresh();
...
void
pcl::cloud_composer::CloudView::setInteractorStyle(
...
pcl::cloud_composer::CloudView::manipulationCompleted(vtkObject*,
                                                      ...
                                                      void* call_data)
{
  std::shared_ptr<ManipulationEvent> manip_event(
