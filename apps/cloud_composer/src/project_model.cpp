...
void
pcl::cloud_composer::ProjectModel::setPointSelection (
    const std::shared_ptr<SelectionEvent>& selected_event)
{
  ...
  foreach (CloudItem* cloud_item, project_clouds) {
    ...
    if (!found_indices->indices.empty()) {
      qDebug() << "Found " << found_indices->indices.size() << " points in "
               << cloud_item->text();
...
void
pcl::cloud_composer::ProjectModel::manipulateClouds (
    const std::shared_ptr<ManipulationEvent>& manip_event)
{
...
pcl::cloud_composer::ProjectModel::insertNewCloudFromFile()
{
  ...
  pcl::PCDReader pcd;
  if (pcd.read (filename.toStdString(), *cloud_blob, origin, orientation, version) <
      0) {
  ...
  CloudItem* new_item =
      new CloudItem (short_filename, cloud_blob, origin, orientation, true);
...
pcl::cloud_composer::ProjectModel::insertNewCloudFromRGBandDepth()
{
  ...
  if (depth_info_list.size() > 1) {
    qWarning() << "Found more than one file which matches depth naming format, using "
                  "first one!";
  }
  ...
  vtkImageReader2* rgb_reader =
      reader_factory->CreateImageReader2 (rgb_filename.toStdString().c_str());
  ...
  vtkImageReader2* depth_reader =
      reader_factory->CreateImageReader2 (depth_filename.toStdString().c_str());
  ...
  depth_pixel = static_cast<unsigned short*> (
      depth_image->GetScalarPointer (depth_dims[0] - 1, depth_dims[1] - 1, 0));
  color_pixel = static_cast<unsigned char*> (
      rgb_image->GetScalarPointer (depth_dims[0] - 1, depth_dims[1] - 1, 0));

  for (std::uint32_t y = 0; y < cloud->height; ++y) {
    for (std::uint32_t x = 0; x < cloud->width; ++x, --depth_pixel, color_pixel -= 3) {
      PointXYZRGB new_point;
      //  std::uint8_t* p_i = &(cloud_blob->data[y * cloud_blob->row_step + x *
      //  cloud_blob->point_step]);
      ...
      if (depth == 0.0f) {
        ...
        new_point.y = ((float)(centerY - y)) * depth *
      ...
      // qDebug() << "r ="<<color_pixel[0]<<" g="<<color_pixel[1]<<"
      // b="<<color_pixel[2];
    }
  }
  ...
  CloudItem* new_item =
      CloudItem::createCloudItemFromTemplate<PointXYZRGB> (short_filename, cloud);
...
pcl::cloud_composer::ProjectModel::saveSelectedCloudToFile()
{
  ...
  pcl::PCLPointCloud2::ConstPtr cloud = cloud_to_save->data (ItemDataRole::CLOUD_BLOB)
                                            .value<pcl::PCLPointCloud2::ConstPtr>();
  Eigen::Vector4f origin =
      cloud_to_save->data (ItemDataRole::ORIGIN).value<Eigen::Vector4f>();
  Eigen::Quaternionf orientation =
      cloud_to_save->data (ItemDataRole::ORIENTATION).value<Eigen::Quaternionf>();
...
{
  qDebug() << "Enqueuing tool action " << tool->getToolName() << " in project model "
           << this->getName();
...
{
  // We set the project model here - this wasn't done earlier so model is never exposed
  // to plugins
...
void
pcl::cloud_composer::ProjectModel::insertNewCloudComposerItem (
    CloudComposerItem* new_item, QStandardItem* parent_item)
{
...
pcl::cloud_composer::ProjectModel::deleteSelectedItems()
{
  ...
  if (selected_indexes.empty()) {
    QMessageBox::warning (qobject_cast<QWidget*> (this->parent()),
                          "No Items Selected",
                          "Cannot execute delete command, no item is selected in the "
                          "browser or cloud view");
    return;
  }
...
{
  interactor_styles::INTERACTOR_STYLES selected_style =
      new_style_action->data().value<interactor_styles::INTERACTOR_STYLES>();
...
pcl::cloud_composer::ProjectModel::createNewCloudFromSelection()
{
  ...
  if (!onlyCloudItemsSelected()) {
    qCritical() << "Only Clouds Selected = False -- Cannot create a new cloud from "
                   "non-cloud selection";
    return;
  }
  ...
  // We don't call the enqueueToolAction function since that would abort if we only have
  // a green selection Move the tool object to the work queue thread
...
void
pcl::cloud_composer::ProjectModel::itemSelectionChanged (const QItemSelection&,
                                                         const QItemSelection&)
{
  // qDebug () << "Item selection changed!";
  // Set all point selected cloud items back to green text, since if they are selected
  // they get changed to white
  foreach (CloudItem* selected_item, selected_item_index_map_.keys()) {
...
void
pcl::cloud_composer::ProjectModel::setSelectedStyle (
    interactor_styles::INTERACTOR_STYLES style)
{
