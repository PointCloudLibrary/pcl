...
if(Qhull_FOUND)
  ...
  if(PCL_QHULL_REQUIRED_TYPE MATCHES "DONTCARE")
    if(PCL_SHARED_LIBS AND TARGET Qhull::qhull_r)
      if(TARGET Qhull::qhull_r)
        ExtractProperties(Qhull::qhull_r)
      endif()
      set(QHULL_LIBRARY_TYPE SHARED)
    elseif(TARGET Qhull::qhullstatic_r)
      ExtractProperties(Qhull::qhullstatic_r)
      set(QHULL_LIBRARY_TYPE STATIC)
    endif()
  elseif(PCL_QHULL_REQUIRED_TYPE MATCHES "SHARED" AND TARGET Qhull::qhull_r)
    if(TARGET Qhull::qhull_r)
      ExtractProperties(Qhull::qhull_r)
    endif()
    set(QHULL_LIBRARY_TYPE SHARED)
  elseif(PCL_QHULL_REQUIRED_TYPE MATCHES "STATIC" AND TARGET Qhull::qhullstatic_r)
    ExtractProperties(Qhull::qhullstatic_r)
    set(QHULL_LIBRARY_TYPE STATIC)
  else()
     if(PCL_SHARED_LIBS AND TARGET Qhull::qhull_r)
  ...
  set_target_properties(QHULL::QHULL PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${QHULL_INTERFACE_INCLUDE_DIRECTORIES}") 
  set_target_properties(QHULL::QHULL PROPERTIES IMPORTED_CONFIGURATIONS "${QHULL_IMPORTED_CONFIGURATIONS}")
  if(msvc)
  ...
  foreach(config ${QHULL_IMPORTED_CONFIGURATIONS})
    if(EXISTS "${QHULL_IMPORTED_LOCATION_${config}}")
      set_target_properties(QHULL::QHULL PROPERTIES IMPORTED_LOCATION_${config} "${QHULL_IMPORTED_LOCATION_${config}}")
    endif()
    if(EXISTS "${QHULL_IMPORTED_IMPLIB_${config}}")
      set_target_properties(QHULL::QHULL PROPERTIES IMPORTED_IMPLIB_${config} "${QHULL_IMPORTED_IMPLIB_${config}}")
    endif()
  endforeach()
