...
macro(PCL_SUBSYS_OPTION _var _name _desc _default)
  ...
  if(NOT (subsys_status STREQUAL "AUTO_OFF"))
    ...
    # _opt_name expands to ie. BUILD_apps which expands to true or false
...
macro(PCL_SUBSYS_DEPEND _var _name)
  ...
  cmake_parse_arguments(PSUBSYS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(PSUBSYS_DEPS)
    SET_IN_GLOBAL_MAP(PCL_SUBSYS_DEPS ${_name} "${PSUBSYS_DEPS}")
  endif()
  if(PSUBSYS_EXT_DEPS)
    SET_IN_GLOBAL_MAP(PCL_SUBSYS_EXT_DEPS ${_name} "${PSUBSYS_EXT_DEPS}")
  endif()
  if(PSUBSYS_OPT_DEPS)
    SET_IN_GLOBAL_MAP(PCL_SUBSYS_OPT_DEPS ${_name} "${PSUBSYS_OPT_DEPS}")
  endif()
  GET_IN_MAP(subsys_status PCL_SUBSYS_HYPERSTATUS ${_name})

  #_var expands to "build" which is true or false
  if(${_var} AND (NOT (subsys_status STREQUAL "AUTO_OFF")))
    if(PSUBSYS_DEPS)
      foreach(_dep ${PSUBSYS_DEPS})
...
macro(PCL_SUBSUBSYS_DEPEND _var _parent _name)
  ...
  set(oneValueArgs)
  set(multiValueArgs DEPS EXT_DEPS OPT_DEPS)
  cmake_parse_arguments(PSUBSUBSYS "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

  if(PSUBSUBSYS_DEPS)
    SET_IN_GLOBAL_MAP(PCL_SUBSYS_DEPS ${_parent}_${_name} "${PSUBSUBSYS_DEPS}")
  endif()
  if(PSUBSUBSYS_EXT_DEPS)
    SET_IN_GLOBAL_MAP(PCL_SUBSYS_EXT_DEPS ${_parent}_${_name} "${PSUBSUBSYS_EXT_DEPS}")
  endif()
  if(PSUBSUBSYS_OPT_DEPS)
    SET_IN_GLOBAL_MAP(PCL_SUBSYS_OPT_DEPS ${_parent}_${_name} "${PSUBSUBSYS_OPT_DEPS}")
  endif()
  ...
  if(${_var} AND (NOT (subsys_status STREQUAL "AUTO_OFF")))
    ...
    if(PSUBSUBSYS_EXT_DEPS)
      foreach(_dep ${PSUBSUBSYS_EXT_DEPS})
        string(TOUPPER "${_dep}_found" EXT_DEP_FOUND)
        #Variable EXT_DEP_FOUND expands to ie. QHULL_FOUND which in turn is then used to see if the EXT_DEPS is found.
        if(NOT ${EXT_DEP_FOUND})
          set(${_var} FALSE)
...
macro(PCL_GET_SUBSYS_INCLUDE_DIR _var _name)
  ...
  # _var expands to _include_dir which expands again to ie. cuda/common
  if(NOT ${_var})
    set(${_var} ${_name})
  endif()
endmacro()
...
macro(collect_subproject_directory_names dirname filename names dirs)
  ...
  #ARGC has to be manually expanded in macro's, see: https://cmake.org/cmake/help/latest/command/macro.html
  if(${ARGC} GREATER 4)
...
macro(PCL_DISABLE_DEPENDIES _subsys)
  ...
  #PCL_SUBSYS_DEPENDIES expands to ie. PCL_COMMON_DEPENDIES which again expands to 2d;apps;examples;features...
