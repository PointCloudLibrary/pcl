...
SumReduce::SumReduce(int width, int height, int levels) : levels_(levels),
                                                          width_(width),
                                                          height_(height)
{
  ...
  if (!sum_program_->add_shader_file("sum_score.vert", gllib::VERTEX))
  {
  ...
  // TODO: to remove file dependency include the shader source in the binary
  if (!sum_program_->add_shader_file("sum_score.frag", gllib::FRAGMENT))
  {
...
void
SumReduce::sum(GLuint input_array, float* output_array)
{
  ...
  // use program
  sum_program_->use();
  glUniform1i(sum_program_->get_uniform_location("ArraySampler"), 0);
