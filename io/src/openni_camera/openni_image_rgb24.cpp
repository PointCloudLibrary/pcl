                          ...
                          unsigned gray_line_step) const
{
  if (width > image_md_->XRes() || height > image_md_->YRes())
    THROW_OPENNI_EXCEPTION("Up-sampling not supported. Request was %d x %d -> %d x %d.",
                           image_md_->XRes(),
                           image_md_->YRes(),
                           width,
                           height);
  ...
  if (image_md_->XRes() % width == 0 && image_md_->YRes() % height == 0) {
    ...
    for (unsigned yIdx = 0; yIdx < height;
         ++yIdx, src_line += src_skip, dst_line += dst_skip) {
      for (unsigned xIdx = 0; xIdx < width; ++xIdx, src_line += src_step, dst_line++) {
        *dst_line =
            static_cast<unsigned char>((static_cast<int>(src_line->nRed) * 299 +
                                        static_cast<int>(src_line->nGreen) * 587 +
                                        static_cast<int>(src_line->nBlue) * 114) *
                                       0.001);
      }
    }
  }
  else {
    THROW_OPENNI_EXCEPTION("Down-sampling only possible for integer scale. Request was "
                           "%d x %d -> %d x %d.",
                           image_md_->XRes(),
                           image_md_->YRes(),
                           width,
                    ...
                    unsigned rgb_line_step) const
{
  ...
  if (width == image_md_->XRes() && height == image_md_->YRes()) {
    ...
    else // line by line
    {
      ...
      for (unsigned yIdx = 0; yIdx < height;
           ++yIdx, rgb_line += rgb_line_step, src_line += line_size) {
        std::copy(src_line, src_line + line_size, rgb_line);
      }
    }
  }
  else if (image_md_->XRes() % width == 0 &&
           image_md_->YRes() % height == 0) // downsamplig
  {
    unsigned src_step = image_md_->XRes() / width;
    unsigned src_skip = (image_md_->YRes() / height - 1) * image_md_->XRes();
    ...
    for (unsigned yIdx = 0; yIdx < height; ++yIdx, src_line += src_skip) {
      ...
      if (dst_skip != 0) {
        // use bytes to skip rather than XnRGB24Pixel's, since line_step does not need
        // to be multiple of 3
