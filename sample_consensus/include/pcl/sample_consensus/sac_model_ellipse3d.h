 ...
 * "Semi-autonomous Prosthesis Control Using Minimal Depth Information and Vibrotactile
 * Feedback", Miguel N. Castro & Strahinja Dosen. IEEE Transactions on Human-Machine
 * Systems [under review]. arXiv:2210.00541.
...
public:
  ...
  /** \brief Check whether the given index samples can form a valid 3D ellipse model,
   * compute the model coefficients from these samples and store them in
   * model_coefficients. The ellipse coefficients are: x, y, R. \param[in] samples the
   ...
   * \param[in] model_coefficients the coefficients of a 3D ellipse model that we need
   * to compute distances to \param[out] distances the resultant estimated distances
   */
  void
  ...
  /** \brief Compute all distances from the cloud data to a given 3D ellipse model.
   * \param[in] model_coefficients the coefficients of a 3D ellipse model that we need
   * to compute distances to \param[in] threshold a maximum admissible distance
  ...
  /** \brief Recompute the 3d ellipse coefficients using the given inlier set and return
   * them to the user.
   * @note: these are the coefficients of the 3d ellipse model after refinement (e.g.
  ...
  /** \brief Verify whether a subset of indices verifies the given 3d ellipse model
   * coefficients. \param[in] indices the data indices that need to be tested against
   * the 3d ellipse model \param[in] model_coefficients the 3d ellipse model
   * coefficients \param[in] threshold a maximum admissible distance threshold for
...
private:
    ...
    operator()(const Eigen::VectorXd& x, Eigen::VectorXd& fvec) const
    {
      ...
      for (int i = 0; i < values(); ++i) {
        ...
        const Eigen::Vector3f p =
            (*model_->input_)[indices_[i]].getVector3fMap().template cast<float>();
  ...
  static float
  golden_section_search (const Eigen::VectorXf& par,
                         float u,
                         float v,
                         float th_min,
                         float th_max,
