 ...
 * \note The idea is to offer a simple visualizer that stores and display the last X
 * values as a curve. \note The class is based on PCLHistogramVisualizer and
 * pcl::VFHSignature308 for display. \note Therefore, the number of values is limited to
 * [2-308]. \author Raphael Favier \ingroup visualization
 */
...
public:
   ...
   * \note The point here is to change the background to orange when the latest value is
   * under a threshold. \param[in] value if true, automatic mode is enabled. Else,
   * background will be white \param[in] threshold value that triggers the background to
   * turn orange if the latest value is lower \note This functionality does not work yet
   * at time of commit (see http://dev.pointclouds.org/issues/829)
   */
  void
  setAutomaticBackgroundColorControl (const bool value = true,
   ...
   * \note If set to false, the Y scale is automatically adjusted to the current values
   * (default). \param[in] value boolean that enable or disable this policy
   */
  void
...
private:
  ...
  /** \brief pushes the values contained inside the buffer to the cloud used for
   * visualization. */
  void
  updateValuesToDisplay ()
  {
  ...
  /** \brief boolean used to know if we need to change the background color in case of
   * low values. */
  ...
  /** \brief boolean used to know if we need to change the background color in case of
   * low values. True means we do it ourselves. */
