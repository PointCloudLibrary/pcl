...
static void
BM_passthrough(benchmark::State& state, const typename PointCloud<PointT>::Ptr cloud)
{
            ...
            const benchmark::TimeUnit unit)
{
  ...
  benchmark::RegisterBenchmark(bm_name.c_str(), &BM_passthrough<PointT, T>, dataset)
      ->Unit(unit);
};
